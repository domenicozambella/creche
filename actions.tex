...
\def\medrel#1{\parbox[t]{4.5ex}{$\displaystyle\hfil #1$}}
\def\ceq#1#2#3{\parbox[t]{4.5ex}{$\displaystyle #1$}\medrel{#2}{$\displaystyle #3$}}
...
\begin{definition}\label{def_G0}\noindent\vspace*{0.1ex}
  ...
  \ceq{\hfill{\gr\Z^0}}{=}{\bigcap\big\{\grW\le\grZ\ :\ \textrm{ stabilizer of a }\LDelta(\grZ)\textrm{-definable set with finite $G$-orbit}\big\}}.\smallskip
...
Note also that, as $\grZ\le G$, every $\grW\le\grZ$ on the r.h.s.\@ is the stabilizer of a set with finite $\grZ$-orbit, then it has finite index in $\grZ$.
