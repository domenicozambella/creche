...
We want to compare the notions of $G$-syndetic and $G$-wide when relative to $\mrX$ or to some $\mrY\subseteq\mrX$.
...
\begin{proof}
  ...
  Note that $\cap\,G{\cdot}\mrY$ is a $G$-invariant $\BDelta(\grZ)$-type-definable set.
  We claim that it is nonempty.
  By the transitivity of the action, boundedly many $G$-translates of $\mrY$ cover $\mrX$.
  Then, by Proposition~\ref{prop_HgenGgen}, we obtain that $p({\mr x})$ is $G$-wide relative to any $G$-translate of $\mrY$.
  Then $p({\mr x})$ entails $g{\cdot}\mrY$ for every $g\in G$.
...
\begin{theorem}\label{thm_gammaK}
  Write $G^1$ for $\Stab(Q_G)$.
  Then any finite conjunction of formulas in $\gamma_{G^1}({\mr x})$ is $G$-syndetic.
...
Unfortunately, we cannot conclude that the intersection of $G^1$-syndetic sets is $G^1$-syndetic.
...
\begin{definition}\label{def_G00}
  Let $\Phi^0$ be the collection of $\BDelta(\grZ)$-definable with finite $G$-orbit.
  Let $\Phi^{00}$ be the collection of $\BDelta(\grZ)$-type-definable with bounded $G$-orbit.
  Define\medskip
