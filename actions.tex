...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item[2.] $\varnothing\ \neq\ \mrX\,\cap\,\big(\cap\, C{\cdot}\neg{\mr\D}\big)$ for every finite $C\subseteq G$.\qedhere
  \end{itemize} 
\end{proof}
...
\begin{corollary}\label{fact_fip2}
  ...
  \begin{itemize}
    ...
    \item[2.] $\mrD\cap\mrC\neq\varnothing$ for every $G$-syndetic set $\mrC$.
  \end{itemize}
\end{corollary}

\begin{proof}
  \ssf1$\IMP$\ssf2. 
  If $\mrD$ is $G$-thick and $\mrD\subseteq\neg\mrC$ implies that $\neq\mrC$ is also $G$-thick, then $\mrC$ cannot be $G$-syndetic.
...
\ceq{\hfill\emph{$\gamma_H({\mr x})$}}{=}{\{\theta({\mr x})\in \BDelta({\grZ})\ :\ \theta({\mr x})\textrm{ is }G\textrm{-syndetic}\}}
...
\begin{proof}
  ...
  Pick $C\subseteq G$ be finite such that $\mrX\subseteq\cup\,C{\cdot}\mrD$.
...
\begin{corollary}\label{corol_def_mu}
  If an $G$-thick type $p({\mr x})\in S_\Delta(\grZ)$ exists, then for any $\mrD\in\BDelta({\grZ})$
  \begin{itemize}
...
A set $\mrD$ is \emph{$G$-wide\/} if every finite cover of $\mrD$ by $\BDelta({\grZ})$-sets contains a $G$-thick set.
...
\begin{proof}
  ...
  \ssf3$\IMP$\ssf2.
  Let $p({\mr x})$ be maximal among the $\BDelta({\grZ})$-types that are finitely satisfiable in $\mrX\cap\mrD$ and are $G$-wide.
...
We say that the type $q({\mr x})$ is \emph{$G$-stationary\/} if there exists exactly one $G$-thick type $p({\mr x})\in S_\Delta(\grZ)$ consistent with $q({\mr x})$.
