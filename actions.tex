...
As there is little risk of confusion, these relations symbols are also denoted by $\phi({\mr x}\,;{\gr z})$, and we identify $\LDelta$ with a subset of $L$. We write \emph{$G$\/} for $\Aut(\UDelta)$.
\end{assumption}
...
An immediate consequence of the invariance of $\phi(\mrX\,;\grZ)$ is that any $H$-translate of a $\BDelta({\grZ})$-definable set is again $\BDelta({\grZ})$-definable.
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  Let $\mrD$ be a $H$-generic $\BDelta(\grZ)$-definable set.
  ...
  By completeness $p({\mr x})\proves{\mr x}\in(\neg\mrD\cap g{\cdot}\mrD)$ for some $g\in H$ and some $\BDelta(\grZ)$-definable set $\mrD$.
...
A set $\mrD$ is \emph{$H$-wide\/} if every finite cover of $\mrD$ by $\BDelta({\grZ})$-definable sets contains a $H$-persistent set.
...
\begin{proof}
  ...
  By maximality there is some formula $\psi({\mr x})$, a conjunction of formulas in $p({\mr x})$, and some $\BDelta({\grZ})$-definable sets ${\mr\C_1},\dots,{\mr\C_n}$ that cover both $\psi(\U^{\mr x})\cap\theta(\U^{\mr x})$ and $\psi(\U^{\mr x})\smallsetminus\theta(\U^{\mr x})$ and such that no ${\mr\C_i}$ is $H$-persistent.
...
\begin{corollary}\label{corol_intersectionGwide}
  Let $\mrD$ be a $H$-wide $\BDelta(\grZ)$-definable set.
...
\begin{remark}
As $\gamma_G({\mr x})$ may be inconsistent, the following notion is relevant \smallskip

\ceq{\hfill Q}{=}{\big\{q({\mr x})\subseteq\gamma_G({\mr x})\;:\textrm{ maximally consistent set}\big\}}. \smallskip

It is easy to see that $Q$ is closed under the action of $G$.
Write for brevity\smallskip

\ceq{\hfill H}{=}{\Stab(Q)}\medrel{=}$\displaystyle\bigcap_{q\in Q}\Stab(q)$,

where $\Stab(q)$ is the stabilized of $q({\mr x})$.
...
\begin{definition}
  ...
  \ceq{\hfill\Phi}{=}{\big\{\phi({\mr x})\in\BDelta(\grZ)\ :\ \textrm{ with finite }G\textrm{-orbit}\big\}}

  and 

  \ceq{\hfill P}{=}{\big\{p({\mr x})\subseteq\Phi\ :\ \textrm{ maximally consistent (in $\mrX$)}\big\}}.\smallskip

  It is easy to see that $P$ is closed under the action of $G$.
  For every formula $\phi({\mr x})\in\Phi$, say with orbit $\{\phi_1({\mr x}),\dots,\phi_n({\mr x})\}$, define 

  \ceq{\hfill\epsilon_\phi({\mr x},{\mr y})}{=}{\bigwedge_{i=1}^n\big[\phi_i({\mr x})\iff\phi_i({\mr y})\big]}

  which is clearly a finite equivalence relation (in $\mrX$).
  Finally let\smallskip

  \ceq{\hfill E}{=}{\big\{\epsilon_\phi({\mr x},{\mr y})\ :\  \phi({\mr x})\in\Phi\big\}.}
\end{definition}

% \def\DLascar{L\kern-.6ex\raisebox{1.1ex}{\tiny$\Delta$}\kern-.1ex ascar}
It is easy to see that equivalence classes of relations in $E$ are $\Phi$-definable and that all $\Phi$-definable sets are union of such equivalence classes.
  Therefore, for every ${\mr a}\in\mrX$ the type $\big\{\epsilon({\mr x}\,;{\mr a})\ :\ \epsilon({\mr x}\,;{\mr y})\in E\big\}$  is equivalent to a type $p({\mr x})\in P$.

\begin{remark}\label{lem_trans_action}
  When $G$ acts transitively on $\mrX$ all $\Phi$-definable sets are $G$-generic and every $p({\mr x})$ extends to some $q({\mr x})\in Q$.
  Then $\Stab(P)\le\Stab(Q)$,  where $\Stab(P)$ be the stabilizer of all $p\in P$.
\end{remark}

Ideally, we would like $\Stab(P)=\Stab(Q)$.
...
\begin{example}\label{ex_H_neq_acleq}
  ...
  Let $\X=\Z=\U$ and $\Delta=\{x<z\}$.
  There are plenty of $G$-generic formulas but there is no nontrivial $\BDelta(\Z)$-formula with finite $G$-orbit.
\end{example}
...
\begin{proof}
  ...
  \ceq{\hfill\cup\,F{\cdot}\mrB}{\supseteq}{\cup\,F\big[\big(\cap\, F{\cdot}{\mr\C'}\big)\ \cap\ {\mr\D'}\big]}
...
\ceq{\hfill\emph{${}^{\rm s}\kern-.2ex\gamma_G({\mr x})$}}{=}{\{\theta({\mr x})\in \BDelta({\grZ})\ :\ \theta({\mr x})\textrm{ is strongly }H\textrm{-generic}\}}

\begin{corollary}\label{corol_str_gen}
  ${}^{\rm s}\kern-.2ex\gamma_G({\mr x})$ is finitely consistent, strongly $H$-generic, and $H$-invariant.
\end{corollary}
...
\begin{theorem}\label{thm_coalesce}
  Assume 
  \begin{itemize}
    \item[1.] $H$-persistent $\BDelta({\grZ})$-definable sets are $H$-wide.
  \end{itemize}\smallskip

  Then the following hold
  \begin{itemize}
    \item[2.] $H$-generic $\BDelta({\grZ})$-definable sets are closed under intersection 
    \item[3.] $H$-generic $\BDelta({\grZ})$-definable sets are strongly $H$-generic
    \item[4.] weakly $H$-persistent $\BDelta({\grZ})$-definable sets are $H$-persistent
    \item[5.] $H$-generic $\BDelta({\grZ})$-definable sets are $H$-wide.
  \end{itemize}
\end{theorem}
...
\begin{proof}
  ...
  \ssf1$\IMP$\ssf2.
  Let $\mrC$ and $\mrD$ be $H$-generic $\BDelta({\grZ})$-definable sets.
  ...
  % Let $\mrD$ be a $H$-persistent $\BDelta({\grZ})$-definable set.
...
\begin{definition}\label{def_G0}\noindent\smallskip
  Recall that $\Phi$ is defined in Section~\ref{normalsubgroups}

  \ceq{\hfill{\gr\Z^0}}{=}{\bigcap\big\{\grW\le\grZ\ :\ \textrm{ stabilizer of a }\Phi\textrm{-definable set}\big\}}.
