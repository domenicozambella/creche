...
%We write \emph{$G$\/} for some fixed subgroup of $\Aut(\UDelta)$ and $H$ is some arbitrary subgroup of $G$.
\end{assumption}
...
Clearly, $\Stab(Q_G)\trianglelefteq G$.

\begin{proposition}\label{prop_gammaK}
  Write $K$ for $\Stab(Q_G)$. 
  For every $q({\mr x})\in Q_G$ we have $q({\mr x})\proves\gamma_K({\mr x})$.
  In particular, $\gamma_K({\mr x})$ is finitely consistent.
  % Moreover $\gamma_K({\mr x})$ is closed under conjunction.
\end{proposition}

\begin{proof}
  % As the types in $Q_G$ are closed under conjunction, the second claim in the proposition follows from the first.
  Let $\mrD\in\gamma_K({\mr x})$.
  Then $q({\mr x})\proves\cup\, C{\cdot}\mrD$ for some finite $C\subseteq K$.
  As $k{\cdot}\mrD$ is $G$-generic for every $k\in C$, by maximality $q({\mr x})\proves{\mr x}\in k{\cdot}\mrD$ for some $k\in C$
...
\begin{proposition}\label{prop_Gsyndetic_Hthick1}
  ...
  In particular all types in $Q_G$ are $\Stab(Q_G)$-wide.
\end{proposition}
...
Let $K=\Stab(Q_G)$ be the group introduced in Section~\ref{G0}.
...
\begin{question}
  ...
  In particular, does strongly $G$-syndetic implies $\Stab(Q_G)$-syndetic?
\end{question}
...
Under suitable assumptions~--~e.g.\@ if $H=\Stab(Q_G)$ for some $G\le\Aut(\UDelta)$~--~some notions introduced in this chapter coalesce, and we are left with a tamer landscape.
