...
\begin{theorem}\label{thm_generic_invariant2}
  ...
  Then the following are equivalent 
  \begin{itemize}
    \item[1.] there exists a $G$-persistent type $p({\mr x})\in S_\Delta(\grZ)$ containing ${\mr x}\in\mrD$
...
We say that $q({\mr x})$ is \emph{$G$-prime\/} if for every $\BDelta(\grZ)$-definable set $\mrD$ and every $g\in G$ if $q({\mr x})\proves{\mr x}\in(\mrD\cup g\,\mrD)$ then $q({\mr x})\proves{\mr x}\in\mrD$.

\begin{proposition}\label{prop_gammaG_prime}
  %aaaaa
  The type $\gamma_G({\mr x})$ is $G$-prime.
\end{proposition}

\begin{proof}
  Assume that $\gamma_G({\mr x})\proves{\mr x}\in(\mrD\cup g\,\mrD)$.
  ...
  As $p({\mr x})$ is arbitrary, $\gamma_G({\mr x})\proves{\mr x}\in\mrD$ follows from Corollary~\ref{corol_gammaG_invaqriancer}.
\end{proof}
...
The following fact is an immediate consequence of normality.

\begin{fact}
  For every $\mrD\subseteq\U^{\mr x}$ and every $g\in G$
  \begin{itemize}
    \item [1.] if $\mrD$ is $H$-persistent then also $g\cdot\mrD$ is $H$-persistent
    \item [2.] if $\mrD$ is $H$-generic then also $g\cdot\mrD$ is $H$-generic.
  \end{itemize}
\end{fact}

By the fact, we can easili adapt the proof of Proposition~\ref{prop_gammaG_prime} to obtail the following.

\begin{proposition}
  The type $\gamma_H({\mr x})$ is $G$-invariant and $G$-prime.
\end{proposition}

% \begin{proof}
%   By the above fact  guarantees that if $\mrD$ is $H$-generic then $g\cdot\mrD$ is also $H$-generic, for any $g\in G$.
%   By normality, if $p({\mr x})\in S_\Delta(\grZ)$ is $H$-persistent then $g\cdot p({\mr x})$ is also $H$-persistent for every $g\in G$.
% \end{proof}
