...
The notions of invariance introduceted in Chapter~\ref{invariant} extend seamlessly to the action of a group $G$ on $\U$.
...
In this chapter the types in $S_\Delta(\grZ)$ are required to be finitely consistent in $\mrX$~--~a piece of information that is not displayed in the notation as there is little risk of ambiguity.

A set $\mrD\subseteq\mrX$ is (relatively) \emph{$\qfDelta({\grZ})$-definable\/} if $\mrD=\phi(\mrX)$ for some $\phi({\mr x})\in\qfDelta({\grZ})$.
...
\ceq{\ssf{it.}\hfill\theta({\mr x}\,;{\gr\bar a})\in p}{\IFF}{\theta({\mr x}\,;g\,{\gr\bar a})\in p}\hfill for every $g\in G$ and every $\theta({\mr x}\,;{\gr\bar a})\in\qfDelta(\grZ)$.
...
\ceq{\hfill\emph{$\gamma_G({\mr x})$}}{=}{\{\theta({\mr x})\in \qfDelta({\grZ})\ :\ \theta({\mr x})\textrm{ is }G\textrm{-generic}\}}
...
\begin{proof}
  ...
  % Then, by completeness, $p({\mr x})\proves\phi({\mr x}\,;{\gr b})\wedge\neg\phi({\mr x}\,;g\,{\gr b})$ for some $g\in G$ and $\phi({\mr x}\,;{\gr b})\in\qfDelta(\grZ)$.
...
The theorem yields a necessary condition for the existence of $G$-persistent global $\qfDelta({\grZ})$-types.
...
% Let $q({\mr x})\subseteq\qfDelta(\grZ)$.
% We say that $q({\mr x})$ is \emph{$G$-prime\/} if for every $\qfDelta(\grZ)$-definable set $\mrD$ and every $g_1,g_2\in G$ if $q({\mr x})\proves{\mr x}\in(g_1{\cdot}\mrD\cup g_2{\cdot}\mrD)$ then $q({\mr x})\proves{\mr x}\in g_i{\cdot}\mrD$ for some $i$.
% The following fact is immediate.

% \begin{fact}
%   The following are equivalent for every $\qfDelta(\grZ)$-type $q({\mr x})$
%   \begin{itemize}
%     \item [1.] $q({\mr x})$ is $G$-prime and $G$-invariant
%     \item [2.] if $\mrD$ is $\qfDelta(\grZ)$-definable, $g\in G$, and $q({\mr x})\proves{\mr x}\in(\mrD\cup g{\cdot}\mrD)$, then $q({\mr x})\proves{\mr x}\in\mrD$.
%   \end{itemize}
% \end{fact}

% \begin{proposition}
%   For every type $q({\mr x})\subseteq\qfDelta(\grZ)$ the following are equivalent
%   \begin{itemize}
...
\ceq{\hfill Q}{=}{\big\{q({\mr x})\subseteq\gamma_G({\mr x})\;:\textrm{ maximally consistent}\big\}}.
...
Below $\qfDelta(\grZ)$ may also denote $\LDelta_{{\rm\,qf},\,{\mr x},{\mr y}}(\grZ)$.
In this section a $\qfDelta(\grZ)$-definable finite equivalence relation is a formula $\epsilon({\mr x}\,;{\mr y})\in\qfDelta(\grZ)$ such that $\epsilon(\mrX\,;\mrX)$ is an equivalence relation with finitely many classes.
...
% \begin{remark}\label{rem_trans_action}
  ...
  \ceq{\hfill e({\mr x}\,;{\mr y})}{=}{\big\{\epsilon({\mr x}\,;{\mr y})\in\qfDelta(\grZ)\ :\ G\textrm{-invariant, finite equivalence relation}\big\}}.

  The $e({\mr x}\,;{\mr y})$-equivalence classes are those defined by the types
  
  \ceq{\hfill q({\mr x})}{=}{\big\{\epsilon({\mr x}\,;{\mr a})\in\qfDelta(\grZ)\ :\ G\textrm{-invariant, finite equivalence relation}\big\}.}

% \end{remark}

\begin{fact}
  Assume that $G$ acts transitively on $\mrX$.
  Let $\mrD$ be a $\qfDelta(\grZ)$-definable set that intersects $\mrX$ and has finite $G$-orbit.
...
\begin{lemma}\label{lem_trans_action}
  ...
  Let $p({\mr x})$ be a complete $\GDelta(A)$-type.
  Finally, let 
  
  \ceq{\hfill Q}{=}{\big\{q({\mr x})\subseteq\GDelta(\acl^\eq\!A)\ :\ q({\mr x})\proves p({\mr x})\textrm{ and is complete}\big\}}.
...
\begin{corollary}
  ...
  Then for every $\qfDelta(\grZ)$-definable set $\mrD$ that intersects $\mrX$ the following are equivalent
  \begin{itemize}
...
\ceq{\hfill\emph{${}^{\rm s}\kern-.2ex\gamma_G({\mr x})$}}{=}{\{\theta({\mr x})\in \qfDelta({\grZ})\ :\ \theta({\mr x})\textrm{ is strongly }G\textrm{-generic}\}}
...
\begin{proof}
  ...
  % Let $\mrD$ be a $G$-persistent $\qfDelta({\grZ})$-definable set.
...
% \begin{fact}
%   For every  $\qfDelta(\grZ)$-type-definable set $\mrD$ the following are 
%   \begin{itemize}
...
Let $\mrD\subseteq\mrX$ be definable by some $p({\mr x})\subseteq\qfDelta(M)$.
We say that $\mrD$ has 
...
%   \ceq{\ssf2.\hfill{\gr\Z^0}}{=}{\bigcap\big\{\grW\le\grZ\ :\ \LDelta(\mrX\,;\grZ)\textrm{-definable subgroup with finite index}\big\}}.\smallskip

%   % \ceq{\ssf3.\hfill{\gr\Z^0}}{=}{\bigcap\big\{\grW\le\grZ\ :\ \textrm{ stabilizer of a }\LDelta(\mrX,\grZ)\textrm{-definable $\grZ$-generic set}\big\}}.\smallskip
% \end{fact}
...
\begin{proposition}\label{prop_Z0_inclusione1}
  ...
  % Let ${\gr g}$ be such that $\ $ for every $\grZ$-generic $\qfDelta(\grZ)$-definable set $\mrD$.
  % Then ${\gr g}\in{\gr\Z^0}$.
\end{proposition}

\begin{proof}
  
  % We apply Proposition~\ref{prop_Ggeneric_Hpersistent} with $H=\gr\Z^0$, $G=\grZ$, and $\Delta=\LDelta_{\;{\mr x},{\gr z}}$.
...
\begin{proof}
 ...
 \begin{definition}\label{def_G00}\noindent\vspace*{0.1ex}
  ...
  \ceq{\hfill{\gr\Z^{00}}}{=}{\bigcap\big\{\grW\le\grZ\ :\textrm{ stabilizer of a }\LDelta(\grZ)\textrm{-type-definable set with small $G$-orbit}\big\}}.\smallskip
