...
\begin{proof}
  ...
  \begin{itemize}
    \item[1$'$.] there are no finite $C\subseteq G$ such that $\mrX\ \subseteq\ \cup\, C{\cdot}\mrD$.
    \item[2$'$.] $\varnothing\ \neq\ \cap\, C{\cdot}\neg\mrD$ for every finite $C\subseteq G$.
  \end{itemize} 
...
\begin{proposition}\label{prop_coalese}\ 
  ...
  \ceq{\ssf5.\hfill\Sigma_G({\mr x})}{=}{\bigcap\Big\{p({\mr x})\in S(\grZ)\ :\ p({\mr x})\text{ is }G\text{-invariant}\Big\}}
\end{proposition}

Note that $\subseteq$ is always true, therefore (5K) amounts to claiming that the type on the r.h.s.\@ is $G$-syndetic.
\begin{proof} 
...
\begin{proof}
  ...
  1$\Rightarrow$2.
  Assume \ssf1, then $\Sigma_G({\mr x})$ is finitely consistent.
  Then $G$-syndetic $ L(\grZ)$-sets are $G$-thick.
  Finally, they are $G$-wide by \ssf1 in Theorem~\ref{thm_coalesce}.
...
This says that the action of $\Aut(\U/A)$ on $\grZ$ is isomorphic to the action $\Aut(\U/A)$ on itself by left multiplication.
Therefore we may safely identify $\grZ$ with $\Aut(\U/A)$ and never mention $\pi$ again.
% We write ${\gr g}{\cdot}{\gr h}$ for the group operation on $\grZ$ induced by composition of automorphisms.
...
\begin{example}
  This example shows that the above assumption is not unnatural.
  Let $\U=\<\mrX\,;\grZ\>$ where $\grZ$ is a group that acts on $\mrX$.
  The group operations and the group action are assumed definable in some signature $L'$.
  Let $L$ be the reduct of $L'$ that has a relation symbol for each formula of the form $\psi({\gr z^{-1}}\!\cdot{\mr x})$ for some $\psi({\mr x})\in L$.
  To each ${\gr h}\in\grZ$ we associate the $L$-automorphism $\pi({\gr h}):\<{\mr a}\,;{\gr g}\>\mapsto\<{\gr h}{\cdot}{\mr a}\,;{\gr h}{\cdot}{\gr g}\>$.
  Therefore $\grZ$ embeds into $L$-$\Aut(\U)$.
  It is easy to see that this embedding is onto.
\end{example}

Let $M\preceq\U$ be a model containing $A$.
We write $X$ and $Z$ for $\mrX\cap M^{\mr x}$ and $\grZ\cap M^{\mr z}$ respectively.
...
If $\mrD\subseteq\mrX$ we write \emph{${\rm cl}_M(\mrD)$\/} for the closure of $\mrD$ in the topology generated by the subsets of $\mrX$ that are $L(M)$-definable.
Explicitely,
...
\begin{fact}
  Every minimal $\grZ$-${\diamond}$-invariant subset of $\mrX$ is of the form $\grZ\diamond {\mr a}$ for some ${\mr a}\in\mrX$ so, in particular, it is type-definable over $M$.
...
\begin{theorem}
  ...
  \begin{itemize}
    ...
    \item [2.] ${\mr a}\subseteq\mrD$ for some minimal ${\mr a}\in\mrD$.
  \end{itemize}
\end{theorem}
