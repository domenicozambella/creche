...
\ceq{\hfill\emph{$\Sigma_{G,\Y}({\mr x})$}}{=}{\big\{\theta({\mr x})\in \BDelta({\grZ})\ :\ \theta({\mr x})\textrm{ is }G\textrm{-syndetic relatve to }\mrY\big\}.}
...
\begin{theorem}\label{thm_syndetic_invariant}
  ...
  \begin{itemize}
    ...
    %\item[2$'$.] $p({\mr x})\proves\Sigma_{G}({\mr x})$ and is finitely consistent in $\mrY$
...
%   Then there is a type $p({\mr x})\in S_\Delta(\grZ)$ finitely consistent with $\Sigma_G({\mr x})\cup\{{\mr x}\notin\mrD\}$.
...
%   \begin{itemize}
%     \item[1.] $\Sigma_{G,\Y}({\mr x})\cup q({\mr x})$ is finitely consistent in $\mrY$
...
\begin{proposition}\label{prop_StabQ}\ \smallskip
  ...
  \ceq{\hfill G^1}{=}{\big\{g\in G\ :\ \mrD\cap g{\cdot}\mrD\in\Sigma_G({\mr x})\textrm{ whenever }\mrD\in\Sigma_G({\mr x})\big\}.}\smallskip
\end{proposition}
...
%   \begin{itemize}
%     \item [1.] $\Sigma_{G^1}({\mr x})$ is closed under conjunction
...
%   % \ceq{\hfill G^1}{\subseteq}{\big\{g\in G\ :\ \mrD\cap g{\cdot}\mrD\in\Sigma_G({\mr x})\textrm{ whenever }\mrD\in\Sigma_G({\mr x})\big\}}\smallskip

%   % \ceq{}{\subseteq}{\big\{g\in G\ :\ \mrD\cap g{\cdot}\mrD\neq\varnothing\textrm{ whenever }\mrD\in\Sigma_G({\mr x})\big\}}\smallskip
...
\ceq{\hfill\emph{${\mathstrut}^{\rm s}\kern-.1ex\Sigma_{G,\Y}({\mr x})$}}{=}{\{\theta({\mr x})\in \BDelta({\grZ})\ :\ \theta({\mr x})\textrm{ is strongly }G\textrm{-syndetic relative to }\mrY\}.}\smallskip
...
\begin{corollary}\label{corol_str_gen}
  ...
  \ceq{\hfill{\mathstrut}^{\rm s}\kern-.1ex\Sigma_{G,\Y}({\mr x})}{=}{\bigcap_{q\in Q_{G,\Y}}q({\mr x})}.\smallskip
...
%     \item [2.]$\mrD$ is syndetic relative to some $G$-invariant $\BDelta(\grZ)$-type-definable set $\mrC$.
%   \end{itemize}
% \end{proposition}
...
When $\Sigma_{H,\Y}({\mr x})$ is consistent, by \ssf1 in Theorem~\ref{thm_syndetic_invariant2} this is equivalent to requiring that $\mrD$ and $\neg\mrD$ are not both wide relative to $\mrY$.
