...
Throughout this chapter \emph{$\mrX$\/} $\subseteq\U^{\mr x}$ and \emph{$\grZ$\/} $\subseteq\U^{\gr z}$ are two given sets that are type-definable over some set $A$.
...
Let \emph{$G$\/} $\le\Aut(\U/A)$.
We view $G$ as a group acting on $\mrX$ and $\grZ$ and write ${\cdot}$ for such action.
We write $\cdot$ also for the operation of $G$, i.e.\@ composition of automorphisms.
If $\phi({\mr x}\,;{\gr z})\in L(A)$ and $\mrD=\varphi(\mrX\,;{\gr b})$ then $g{\cdot}\mrD=\varphi(\mrX\,;g{\cdot}{\gr b})$.
A formula is $G$-invariant if the set it defines is $G$-invariant (i.e.\@ fixed by the action of $G$).

We say that a set $\mrD\subseteq\mrX$ is \emph{syndetic\/} under the action of $G$, or \emph{$G$-syndetic\/} for short, if finitely many $G$-translates of $\mrD$ cover $\mrX$; we say \emph{$n$-$G$-syndetic\/} if $\le n$ translates suffices.
Dually, we say that $\mrD$ is \emph{thick\/} under the action of $G$, or \emph{$G$-thick\/} for short, if the intersection of any finitely many $G$-translates of $\mrD$ is consistent; we say \emph{$n$-$G$-thick\/} when the request is limited to $\le n$ translates.
...
Let $q({\mr x})\subseteq L(\grZ)$.
We say that $\mrD$ is $G$-syndetic \emph{relative\/} to $q({\mr x})$ if $q({\mr x})$ entails $\cup\,F{\cdot}\mrD$ for some finite $F\subseteq G$.
Dually, $\mrD$ is $G$-thick \emph{relative\/} to $q({\mr x})$ if $G{\cdot}\mrD$ is finitely consistent with $q({\mr x})$.
...
\begin{fact}\label{fact_fip}
  ...
  \begin{itemize}
    ...
    \item[2.] $\neg\mrD$ is $G$-thick (throughout the chapter \emph{$\neg\mrD$\/} denotes that complement in $\mrX$).
...
\begin{proof}
  ...
  \begin{itemize}
    \item[1$'$.] there are no finite $C\subseteq G$ such that $\mrX\ \subseteq\ \cup\, C{\cdot}{\mr\D}$.
    \item[2$'$.] $\varnothing\ \neq\ \cap\, C{\cdot}\neg{\mr\D}$ for every finite $C\subseteq G$.
  \end{itemize} 
...
\ceq{\hfill\emph{$\Sigma_G({\mr x})$}}{=}{\big\{\theta({\mr x})\in  L(\grZ)\ :\ \theta({\mr x})\textrm{ is }G\textrm{-syndetic }\big\}.}

% We remark that the consistency of $\Sigma_G({\mr x})$ is far from guaranteed.

\begin{corollary}\label{corol_q_pers}
  Let $\mrD\in L(\grZ)$ and assume that $\Sigma_G({\mr x})\cup\{{\mr x}\in\mrD\}$ is finitely consistent.
  Then $\mrD$ is $G$-thick.
\end{corollary}

\begin{proof}
  As $\Sigma_G({\mr x})\cup\{{\mr x}\in\mrD\}$ is finitely consistent, it cannot be that $\neg\mrD$ is $G$-syndetic.
...
\begin{theorem}\label{thm_syndetic_invariant}
  For every $p({\mr x})\in S(\grZ)$ the following are equivalent
  \begin{itemize}
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  Let $\mrD\in L(\grZ)$ be a $G$-syndetic.
  Pick $C\subseteq G$ be finite such that $\cup\,C{\cdot}\mrD$ covers $\mrX$.
  Then $p({\mr x})$ is finitely consistent with ${\mr x}\in\cup\,C{\cdot}\mrD$.
  Therefore, by completeness, $p({\mr x})\proves {\mr x}\in h\,\mrD$ for some $h\in C$.
  ...
  % Then 
  Negate \ssf1.
  Let $\mrD\in L(\grZ)$ and $g\in G$ such that $p({\mr x})\proves{\mr x}\in\mrD$ and $p({\mr x})\notproves g{\cdot}\mrD$.
...
%   The following are equivalent for every $ L(\grZ)$-definable set $\mrD$
%   \begin{itemize}
%     \item [1.] $\Sigma_G({\mr x})\proves{\mr x}\in\mrD$
%     \item [2.] $p({\mr x})\proves{\mr x}\in\mrD$ for every $G$-thick $p({\mr x})\in S(\grZ)$.
%   \end{itemize}
% \end{corollary}
...
%   Then there is a type $p({\mr x})\in S(\grZ)$ finitely consistent with $\Sigma_G({\mr x})\cup\{{\mr x}\notin\mrD\}$.
%   By Corollary~\ref{corol_q_pers} $p({\mr x})$ is $G$-thick.
%   Then $\neg\ssf2$.
% \end{proof}

The theorem yields a necessary condition for the existence of global $ L(\grZ)$-types that are $G$-thick.

\begin{corollary}\label{corol_def_mu}
  If a type a $G$-thick $p({\mr x})\in S(\grZ)$ exists, then for any $\mrD\in L(\grZ)$
  \begin{itemize}
    \item[1.] $\mrD$ and $\neg\mrD$ are not both $G$-syndetic
    \item[2.] if $\mrD$ is $G$-syndetic then it is $G$-thick
    \item[3.] $\Sigma_G({\mr x})$ is finitely consistent.\smallskip
  \end{itemize}
\end{corollary}
...
The following theorem gives a necessary and sufficient condition for the  existence of global $G$-invariant $ L(\grZ)$-type.
Ideally, we would like that every $G$-thick $ L(\grZ)$-type extends to a global thick type.
Unfortunately this is not true in general (it is a strong assumption, see Section~\ref{tame_landscape}).

A set $\mrD$ is \emph{$G$-wide\/} if every finite cover of $\mrD$ by $L(\grZ)$-sets contains a set that is $G$-thick.
...
\begin{theorem}\label{thm_syndetic_invariant2}
  For every $\mrD\in L(\grZ)$ the following are equivalent 
  \begin{itemize}
    \item[1.] $\Sigma_G({\mr x})\cup\{{\mr x}\in\mrD\}$ is finitely consistent
    \item[2.] there is a $G$-thick type $p({\mr x})\in S(\grZ)$ that entails ${\mr x}\in\mrD$
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  By Corollary~\ref{corol_q_pers}, it suffices to pick any $p({\mr x})\in S(\grZ)$ that entails $\Sigma_G({\mr x})\cup\{{\mr x}\in\mrD\}$.

  \ssf2$\IMP$\ssf1.
  By Theorem~\ref{thm_syndetic_invariant}.

  \ssf2$\IMP$\ssf3.
  Let ${\mr\C_1},\dots,{\mr\C_n}$ be $ L(\grZ)$-sets that cover $\mrD$.
  Pick $p({\mr x})$ as in \ssf2.
  By completeness, $p({\mr x})\proves {\mr x}\in{\mr\C_i}$ for some $i$.
  Therefore, ${\mr\C_i}$ is $G$-thick.

  \ssf3$\IMP$\ssf2.
  Let $p({\mr x})$ be maximal among the $L(\grZ)$-types containing $\mrD$ that are $G$-wide.
  We claim that $p({\mr x})$ is a complete, $G$-thick $L(\grZ)$-type.
  We prove completeness.
  Suppose for a contradiction that $\theta({\mr x}),\neg\theta({\mr x})\notin p$.
  By maximality there is some formula $\psi({\mr x})$, a conjunction of formulas in $p({\mr x})$, and some $L(\grZ)$-definable sets ${\mr\C_1},\dots,{\mr\C_n}$ that cover both $\psi(\mrX)\cap\theta(\mrX)$ and $\psi(\mrX)\smallsetminus\theta(\mrX)$ and such that no ${\mr\C_i}$ is $G$-thick.
...
\begin{corollary}\label{corol_intersectionGwide}
  Let $\mrD\in L(\grZ)$ be $G$-wide.
  Then $\mrD\cap g{\cdot}\mrD$ is $G$-wide for every $g\in G$.
\end{corollary}

\begin{proof}
  Let $p({\mr x})\in S(\grZ)$ be a $G$-thick type such that $p({\mr x})\proves{\mr x}\in\mrD$.
  By $G$-invariance $p({\mr x})\proves{\mr x}\in g{\cdot}\mrD$.
\end{proof}

% Let $q({\mr x})\subseteq L(\grZ)$.
% We say that $q({\mr x})$ is \emph{$G$-prime\/} if for every $ L(\grZ)$-definable set $\mrD$ and every $g_1,g_2\in G$ if $q({\mr x})\proves{\mr x}\in(g_1{\cdot}\mrD\cup g_2{\cdot}\mrD)$ then $q({\mr x})\proves{\mr x}\in g_i{\cdot}\mrD$ for some $i$.
% The following fact is immediate.

% \begin{fact}
%   The following are equivalent for every $ L(\grZ)$-type $q({\mr x})$
%   \begin{itemize}
%     \item [1.] $q({\mr x})$ is $G$-prime and $G$-invariant
%     \item [2.] if $\mrD$ is $ L(\grZ)$-definable, $g\in G$, and $q({\mr x})\proves{\mr x}\in(\mrD\cup g{\cdot}\mrD)$, then $q({\mr x})\proves{\mr x}\in\mrD$.
%   \end{itemize}
% \end{fact}

% \begin{proposition}
%   For every type $q({\mr x})\subseteq L(\grZ)$ the following are equivalent
%   \begin{itemize}
...
%   Let $p({\mr x})\in S(\grZ)$ extend $q({\mr x})$.
...
\begin{exercise}
  ...
  Prove that if $p(x)\in S(\U)$ is finitely satisfiable in every $M\supseteq A$ then it is thick under the action of $G=\Autf(\UDelta)$.
...
\begin{exercise}
  ...
  \begin{itemize}
    \item[1.] $\D$ is $G$-thick
    \item[2.] there is a type $p(x)\in S(\Z)$ containing $\{x\in g{\cdot}\D\;:\;g\in G\}$.
  \end{itemize}
\end{exercise}
...
\begin{exercise}
  ...
  Prove that the following are equivalent 
  \begin{itemize}
    \item[1.] there is a $G$-invariant type $p(x)\in S(\Z)$ such that $p(x)\proves x\in\D$.
...
% \begin{exercize}
%   Let $\mrD$ be a $ L(\grZ)$-definable set.
%   Prove that the following are equivalent
%   \begin{itemize}
...
We write $\Stab(q)$ for the stabilizer of $q({\mr x})\subseteq L(\grZ)$ in $G$, that is, the subgroup $\{g\in G:g{\cdot}q({\mr x})=q({\mr x})\}$.
...
\begin{proposition}\label{prop_StabQ}\ \smallskip
  ...
  \ceq{\hfill G^1}{=}{\big\{g\in G\ :\ \mrD\cap g{\cdot}\mrD\in\Sigma_G\textrm{ whenever }\mrD\in\Sigma_G\big\}.}\smallskip
\end{proposition}
...
% In particular, every $q({\mr x})\in Q_G$ has an extension to a $G^1$-invariant $p({\mr x})\in S(\grZ)$.
...
\def\ceq#1#2#3{\parbox[t]{20ex}{$\displaystyle #1$}\medrel{#2}{$\displaystyle #3$}}
  ...
  % It also follows that for any $p({\mr x})\in S(\grZ)$
  % \begin{itemize}
...
\begin{proposition}\label{prop_Gsyndetic_Hthick1}
  ...
  Then every $G$-syndetic $\mrD\in L(\grZ)$ is $H$-wide.
  In particular all types in $Q_G$ are $G^1$-wide.
\end{proposition}

\begin{proof}
  Let $p({\mr x})\in S(\grZ)$ be finitely consistent with $\Sigma_H({\mr x})$.
...
% \begin{theorem}
%   Every $G^1$-thick $ L(\grZ)$-set is $G^1$-wide.
% \end{theorem}
...
We revisit the notions introduced in Section~\ref{eq_algebraic} with the only difference that we apply them to $ L(\grZ)$, not to the full language.%$ L(\mrX,\grZ)$.

\begin{definition}\label{def_G0}
  Write \emph{$\Phi^0$\/} for the set of $ L(\grZ)$-formulas/sets that have finite $G$-orbit and define%\vskip-1ex
...
\begin{definition}\label{def_G00}
  Let $\Phi^0$ be the collection of $L(\grZ)$-definable sets with finite orbits.
  that are equivalence classes of a bounded $G$-invariant $L(\grZ)$-type-definable equivalence relation.
  Let $\Phi^{00}$ be the collection of sets that are $L(\grZ)$-type-definable and have a bounded $G$-orbit.
  Define\smallskip
...
\begin{proof}
  ...
  Let $\mrY\in\Phi^{00}$ and $\mrY\subseteq\mrD\in L(\grZ)$.
  Then a small numaber of translations of $\mrD$ cover $\mrX$.
  ...
  By $G^1$-invariance $q({\mr x})$ entails also $k{\cdot}\mrY$ for every $k\in G^1$.
  Now, let $\mrY$ be minimal in $\Phi^{00}$, then $k$ is in the stabilizer of $\mrY$.
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%
\section{Examples: random graph}

In this section $\U$ is a saturated model of the of theory of the random graph.
Let $\mrX$ adn $\grZ$ are two copies of $\U$ and let $G={\rm Aut}(\U)$.
...
\ceq{\hfill\emph{${\mathstrut}^{\rm s}\kern-.1ex\Sigma_G({\mr x})$}}{=}{\{\theta({\mr x})\in  L(\grZ)\ :\ \theta({\mr x})\textrm{ is strongly }G\textrm{-syndetic relative to }\mrY\}.}\smallskip
...
\begin{corollary}\label{corol_str_gen}
  ...
  \ceq{\hfill{\mathstrut}^{\rm s}\kern-.1ex\Sigma_G({\mr x})}{=}{\bigcap_{q\in Q_G}q({\mr x})}.\smallskip

  Where $Q_G$ is as in Definition~\ref{def_Q} but relativized to $\mrY$.  
\end{corollary}
...
% Assume that every $\mrD\in L(\grZ)$ has a small $G$-orbit.
...
% \begin{proposition}
%   Assume that every $\mrD\in L(\grZ)$ has a small $G$-orbit.
%   Then the following are equivalent for every $\mrD\in L(\grZ)$
%   \begin{itemize}
%     \item [1.] $\mrD$ is weakly $G$-thick
%     \item [2.]$\mrD$ is syndetic relative to some $G$-invariant $ L(\grZ)$-type-definable set $\mrC$.
%   \end{itemize}
% \end{proposition}
...
We work under Assumption~\ref{notation_GXphi} and also assume that $H\le\Aut(\U/A)$ acts transitively on $\mrX$.
...
\begin{theorem}\label{thm_coalesce}
  ...
  The following are equivalent
  \begin{itemize}
    \item[1.] $H$-thick $ L(\grZ)$-sets are $H$-wide
    \item[2.] $H$-syndetic $ L(\grZ)$-sets are closed under intersection 
    \item[3.] $H$-syndetic $ L(\grZ)$-sets are strongly $H$-syndetic
    \item[4.] weakly $H$-thick $ L(\grZ)$-sets are $H$-thick.
  \end{itemize}
\end{theorem}
...
\begin{proof}
  ...
  \ssf1$\IMP$\ssf2.
  Let $\mrC$ and $\mrD$ be $H$-syndetic $ L(\grZ)$-sets.
  Suppose for a contradiction that $\mrC\cap\mrD$ is not $H$-syndetic.
  Then $\neg(\mrC\cap\mrD)$ is $H$-thick.
  By \ssf1 and Theorem~\ref{thm_syndetic_invariant2} there is a $H$-invariant type $p({\mr x})\in S(\grZ)$ that is finitely consistent and such that $p({\mr x})\proves{\mr x}\notin\mrC\cap\mrD$.
  ...
  % \ssf2$\IMP$\ssf1.
  % By \ssf2, $\Sigma_H({\mr x})$ is consistent.
  % Then $\mrC$ is $H$-syndetic if and only if $\Sigma_H({\mr x})\proves{\mr x}\in\mrC$.
...
\begin{proposition}\label{prop_coalese}\ 
  ...
  \ceq{5.\hfill\Sigma_G({\mr x})}{=}{\bigcap\Big\{p({\mr x})\in S(\grZ)\ :\ p({\mr x})\text{ is }G\text{-invariant}\Big\}}
\end{proposition}
...
\begin{proof} 
  ...
  Therefore $x\in\mrD$ does not belong to the type on the r.h.s.
...
Let $q({\mr x})$ be a $ L(\grZ)$-type.
We say that $q({\mr x})$ is \emph{$G$-stationary\/} if it extends to a unique $G$-in\-var\-iant $p({\mr x})\in S(\grZ)$.
...
\begin{fact}\label{fact_stationarity}
  Let $q({\mr x})$ be $G$-stationary.
  Assume that the equivalent conditions in Theorem~\ref{thm_coalesce} hold relative to $q({\mr x})$.
  Then the following are equivalent
  \begin{itemize}
    \item [1.] $\mrD$ is $G$-syndetic relative to $q({\mr x})$
    \item [2.] $\mrD$ is $G$-wide/thick relative to $q({\mr x})$.
  \end{itemize}
\end{fact}

\begin{proof}
  Work relatively to $q({\mr x})$.

  1$\Rightarrow$2.
  Assume (1), then $\Sigma_G({\mr x})$ is finitely consistent.
  Then $G$-syndetic $ L(\grZ)$-sets are $G$-thick.
...
In this section we assume that there is a bijection $\pi:\grZ\to\Aut(\U/A)$ such that $\pi\big(\pi({\gr g}){\cdot}{\gr h}\big)=\pi({\gr g}){\cdot}\pi({\gr h})$.
Therefore we may safely identify $\grZ$ with $\Aut(\U/A)$,in fact the action of $\Aut(\U/A)$ on $\grZ$ corresponds to the action $\Aut(\U/A)$ on itself by left multiplication.
It is worth stressing that, bar trivial cases, neither the group operation $\grZ$ nor the group action $\grZ$ on $\mrX$ are definable in $L$.
% Note that ${\gr g}\cdot\varphi(\mrX\,;{\gr h})=\varphi(\mrX\,;{\gr g}{\cdot}{\gr h})$ for all ${\gr g},{\gr h}\in\grZ$ and $\varphi({\mr x}\,;{\gr z})\in L(A)$.

Let $M\preceq\U$ be a model containing $A$.
We write ${\mr X}$ and ${\gr Z}$ for $\mrX\cap M^{\mr x}$ and $\grZ\cap M^{\mr z}$ respectively.

When ${\gr g}\in\grZ$ and ${\mr a}\in\mrX$ we write ${\gr g}\nonfork{\mr a}$ if for every $\varphi({\mr x}\,;{\gr z})\in L(A)$ such that $\varphi({\mr a}\,;{\gr g})$ we have that $\varphi({\mr a}\,;{\gr Z})\neq\varnothing$.
When $\grC\subseteq\grZ$ and $\mrD\subseteq\mrX$ we define

\ceq{\hfill\grC\circ\mrD}{=}{\Big\{{\gr g}{\cdot}{\mr a}\ :\ {\gr g}\in\grC,\ {\mr a}\in\mrD,\ {\gr g}\nonfork {\mr a}\Big\}}

% If $G\le{\rm Aut}(\UDelta)$ and $a\in\mrX$, we write $G{\cdot}a$ for the orbit of $a$ under the action of $G$.
When $\mrD$ is the orbit of ${\mr a}\in\mrX$ under the action of ${\rm Aut}(\U/M)$, we write $\grC\circ {\mr a}$ for $\grC\circ\mrD$.

If $\mrD\subseteq\mrX$ we write ${\rm cl}(\mrD)$ for the closure of $\mrD$ in the topology generated by the subsets of $\mrX$ that are $L(M)$-definable.
Explicitely,

\ceq{1.\hfill{\rm cl}(\mrD)}{=}{\bigcap\Big\{\varphi(\mrX)\ :\ \varphi({\mr x})\in L(M)\text{ such that }\mrD\subseteq\varphi(\mrX)\Big\}}

\ceq{2.}{=}{\Big\{{\mr b}\ :\ \varphi(\mrD)\neq\varnothing\text{ for every }\varphi({\mr x})\in L(M)\text{ that is satisfied by }{\mr b}\Big\}}


% \begin{fact}
%   For every $\mrD\subseteq\mrX$\smallskip

%   \ceq{\hfill{\rm cl}(\cup\,Z{\cdot}\mrD)}{=}{\grZ\circ\mrD}
% \end{fact}

% \begin{proof}
%   $\supseteq$. 
%   Let $b\in\grZ\circ\mrD$, say $b=g{\cdot}a$ for some $g\in\grZ$ and $a\in\mrD$ such that $g\nonfork a$.
%   Let $\varphi(x)\in L(M)$ be such that $Z{\cdot}\mrD\subseteq\varphi(\mrX)$.
%   Then $g\nonfork a$ implies $\varphi(g{\cdot}a)$.
%   This proves that $b\in{\rm cl}(\cup\,Z{\cdot}\mrD)$.
  
%   $\subseteq$. 
%   Let $b\in{\rm cl}(\cup\,Z{\cdot}\mrD)$ be given and define $p(x)=\LDelta\mbox{-tp}(b/M)$.
%   It suffices to prove that for every $a\in\mrD$ the type $z\nonfork a$ is consistent with $p(z{\cdot}a)$.
%   In fact, if $g\nonfork a$, then $g{\cdot}a\in\grZ\circ a$.
%   And, as $\grZ\circ a$ is $M$-invariant, if $p(g{\cdot}a)$ then $b\in\grZ\circ a$.
...
\begin{fact}
  ...
  \ceq{\hfill{\rm cl}({\gr Z}{\cdot}{\mr a})}{=}{\grZ\circ {\mr a}}
\end{fact}

\begin{proof}
  $\supseteq$. 
  Let ${\mr b}\in\grZ\circ {\mr a}$, say ${\mr b}=g{\cdot}{\mr a}$ for some $g\nonfork {\mr a}$.
  Let $\varphi({\mr x})\in L(M)$ be such that ${\gr Z}{\cdot}{\mr a}\subseteq\varphi(\mrX)$.
  Then $g\nonfork {\mr a}$ implies $\varphi(g{\cdot}{\mr a})$.
  This proves that ${\mr b}\in{\rm cl}({\gr Z}{\cdot}{\mr a})$.
  
  $\subseteq$. 
  Let ${\mr b}\in{\rm cl}({\gr Z}{\cdot}{\mr a})$ be given and define $p({\mr x})=\LDelta\mbox{-tp}({\mr b}/M)$.
  It suffices to prove that $z\nonfork {\mr a}$ is consistent with $p(z{\cdot}{\mr a})$.
  In fact, if $g\nonfork {\mr a}$, then $g{\cdot}{\mr a}\in\grZ\circ {\mr a}$.
  And, as $\grZ\circ {\mr a}$ is $M$-invariant, if $p(g{\cdot}{\mr a})$ then ${\mr b}\in\grZ\circ {\mr a}$.
  Now, to prove concistency, assume for a contradiction that $z\nonfork {\mr a}\rightarrow\neg\varphi(z{\cdot}{\mr a})$ for some $\varphi({\mr x})\in p$.
  As any $m\in {\gr Z}$ realizes $z\nonfork {\mr a}$, no element of ${\gr Z}{\cdot}{\mr a}$ satisfies $\varphi({\mr x})$.
  This is a contradiction by (2) above.
\end{proof}

% A subset $A\subseteq\mrX$ is dense if $\psi(A)\neq\varnothing$ for every $\psi(x)\in\Psi$.

Let $\D\subseteq\mrX$ be an $M$-definable set.
As ${\gr Z}$ is small, $\D$ is ${\gr Z}$-thick if and only if $\cap\,{\gr Z}{\cdot}\D$ is consistent.

\begin{fact}\label{fact_thick_circ}
  Let $\D\subseteq\mrX$ be definable over $M$.
  Then the following are equivalent
  \begin{itemize}
    \item [1.] $\D$ is ${\gr Z}$-thick
    \item [2.] $\grZ\circ {\mr a}\subseteq\D$ for some ${\mr a}\in\D$.
  \end{itemize}
\end{fact}

\begin{proof}
  1$\Rightarrow$2. \ 
  Pick ${\mr a}\in\cap\,{\gr Z}{\cdot}\D$.
  Suppose for a contradiction that $g{\cdot}{\mr a}\notin\D$ for some $g\in\grZ$ such that $g\nonfork_M a$.
  Then $m{\cdot}{\mr a}\notin\D$ for some $m\in {\gr Z}$.
  This contradicts the choice of ${\mr a}$.  

  2$\Rightarrow$1. \ 
  It suffices to note that $m\cdot\grZ\circ {\mr a}=\grZ\circ {\mr a}$ for every $m\in {\gr Z}$.
\end{proof}
...
\begin{fact}
  ...
  Then the following are equivalent
  \begin{itemize}
    \item [1.] $\D$ is ${\gr Z}$-syndetic
    \item [2.] $(\grZ\circ {\mr a})\cap\D\neq\varnothing$ for every ${\mr a}\in\mrX$.
  \end{itemize}
\end{fact}

We say that $\grC\subseteq\mrX$ is $\grZ{\circ}$-invariant if $\grZ\circ\grC\subseteq\C$.
Note that, when $\grZ=\mrX$, this says that $\grC$ is a left ideal.
The following fact is easy.

\begin{fact}
  Every minimal $\grZ{\circ}$-invariant subset of $\mrX$ is of the form $\grZ\circ {\mr a}$ for some ${\mr a}\in\mrX$ so, in particular, it is type-definable over $M$.
  % We also have that $\grZ\circ a=\M\circ a$ for $\M$ any left ideal of $\grZ$.
\end{fact}

We say that ${\mr a}\in\mrX$ is \emph{minimal\/} if $\grZ\circ {\mr a}$ is a minimal $\grZ{\circ}$-invariant subset of $\mrX$.

\begin{theorem}
  Let $\D\subseteq\mrX$ be definable over $M$.
  Then the following are equivalent
  \begin{itemize}
    \item [1.] $\D$ is weakly ${\gr Z}$-thick
    \item [2.] ${\mr a}\subseteq\D$ for some minimal ${\mr a}\in\D$.
  \end{itemize}
\end{theorem}

\begin{proof}
  2$\Rightarrow$1. \ 
  Let ${\mr b}\in\grZ\circ {\mr a}$.
  Then ${\mr b}\in\grZ\circ\D$.
  Therefore ${\mr b}\in m{\cdot}\D$ for some $m\in {\gr Z}$.
  This proves that $\grZ\circ {\mr a}\subseteq\cup\,{\gr Z}{\cdot}\D$.
  By compactness $\grZ\circ {\mr a}\subseteq\cup\,C{\cdot}\D$ for some finite $C\subseteq {\gr Z}$.
  By Fact~\ref{fact_thick_circ}, $\D$ is weakly ${\gr Z}$-thick.

  1$\Rightarrow$2. \ 
  Let $C\subseteq {\gr Z}$ be a finite set such that $\cup\,C{\cdot}\D$ is ${\gr Z}$-thick.
  By Fact~\ref{fact_thick_circ}, there is ${\mr a}\in\cup\,C{\cdot}\D$ such that $\grZ\circ {\mr a}\subseteq\cup\,C{\cdot}\D$.
  Let ${\mr a'}\in\grZ\circ {\mr a}$ be minimal.
  As ${\mr a'}\in m{\cdot}\D$ for some $m\in C$, we conclude that $m^{-1}\!\cdot {\mr a'}\in\D$.
  Then $m^{-1}\!\cdot {\mr a'}$ is the minimal element required by (2).
\end{proof}
...
% \begin{fact}
%   For every  $ L(\grZ)$-type-definable set $\mrD$ the following are 
%   \begin{itemize}
