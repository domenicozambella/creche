...
Furthermore, we assume that $\grZ$ and $\mrX$ are definable over some set of parameters $A$ and that the group operations and the group action are definable over $A$.
...
\begin{proof}
  We spell out the proof for genericity, for persistency the proof is similar.

  \ssf1$\IMP$\ssf2.
  As \ssf1 implies that $\phi({\mr x}\,;{\gr 1})$ is $\grZ$-generic, \ssf2 follows by elementarity.

  \ssf2$\IMP$\ssf1.
  Assume \ssf2.
  By elementarity there is some ${\gr g'}\in\grZ\cap M^{\gr z}$ such $\phi({\mr x}\,;{\gr g'})$ is generic under the action of $\grZ\cap M^{\gr z}$ and, a fortiori, under the action of $\grZ$.
  Then \ssf2 follows.
\end{proof}
...
Consider the 2-sorted model $\U'=\big\<\mrX\,;\grZ\big\>$ whose signature $L'$ contains only a relation symbol for every formula $\phi({\mr x}\,;{\gr z})\in\Delta$ which we continue to indicate with $\phi({\mr x}\,;{\gr z})$.
As $\mrX$ and $\grZ$ are assumed to be type-definable, $\U'$ is a saturated $L'$-structure.
