...
\begin{theorem}\label{thm_generic_invariant2}
  ...
  \begin{itemize}
    ...
    \item[3.] $\mrD$ is hereditarily $G$-persistent.
  \end{itemize}
\end{theorem}
...
\begin{proof}
  ...
  \ssf2$\IMP$\ssf3.
  Suppose ${\mr\C_1},\dots,{\mr\C_n}$ cover $\mrD$ and pick $p({\mr x})$ as in \ssf2.
...
\begin{exercise}
  %aaaaa
  Let $\D$ be a $\BDelta({\Z})$-definable set.
  Prove that the following are equivalent 
  \begin{itemize}
...
%
  ...
  \begin{itemize}
    \item [1.] \ceq{\hfill\mrD\textrm{ is }H\textrm{-persistent}}{\IFF}{g\cdot\mrD\textrm{ is }H\textrm{-persistent}}
    \item [2.] \ceq{\hfill\mrD\textrm{ is }H\textrm{-generic}}{\IFF}{g\cdot\mrD\textrm{ is }H\textrm{-generic.}}
  \end{itemize}
...
From what have seen in Section~\ref{dual_perspective}, if $\gamma_H({\mr x})$ is consistent, then $H$-generic sets are hereditarily $H$-persistent. 
...
\begin{proposition}\label{prop_Ggeneric_Hpersistent}
  ...
  Then every $G$-generic set is hereditarily $H$-persistent.
\end{proposition}

\begin{proof}  
  If $\phi({\mr x}\,;{\gr b})$ is $G$-generic, then, by completeness, $p({\mr x})\proves\phi({\mr x}\,;g\,{\gr b})$ for some $g\in G$.
  Equivalently, $\phi({\mr x}\,;{\gr g})\in g^{-1}p$.
  By Remark~\ref{rem_invariance_normalsubg} ${\gr h^{-1}}p({\mr x})$ is $H$-persistent and the proposition follows from Theorem~\ref{thm_generic_invariant2}.
\end{proof}
...
\begin{proposition}
  The type $\gamma_H({\mr x})$ is $G$-prime.
\end{proposition}

\begin{proof}
  Assume that $\gamma_H({\mr x})\proves{\mr x}\in(\mrD\cup g\,\mrD)$.
  Assume $\gamma_H({\mr x})$ otherwise the claim is trivial.
  Let $p({\mr x})\in S_\Delta(\grZ)$ be $H$-persistent.
  By completeness $p({\mr x})\proves{\mr x}\in\mrD$ or $p({\mr x})\proves{\mr x}\in g\,\mrD$.
  If $p({\mr x})\proves{\mr x}\in\mrD$ we are done.
  Otherwise $g^{-1}\cdot p({\mr x})\proves{\mr x}\in\mrD$.
  As $g^{-1}\cdot p({\mr x})$ is an arbitrary global $H$-persistent type, $\gamma_G({\mr x})\proves{\mr x}\in\mrD$ follows from Corollary~\ref{corol_gammaG_invaqriancer}.
\end{proof}
...
It is easy to verify that is a normal subgroup.
...
\begin{proof}
  ...
  As, a fortiori, $\phi({\mr x}\,;{\gr g})$ is $G$-generic \ssf2 follows from Proposition~\ref{prop_Ggeneric_Hpersistent}.
\end{proof}
