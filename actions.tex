...
\begin{proof}
  ...
  \begin{itemize}
    \item[1$'$.] there is no finite $C\subseteq G$ such that $\mrX\ \subseteq\ \cup\, C{\cdot}\mrD$.
...
It is important to note that the consistency of $\Sigma_G({\mr x})$ is far from guaranteed.
Let $\mrD\in L(\grZ)$.
Note that if $\Sigma_G({\mr x})\cup\{{\mr x}\in\mrD\}$ is finitely consistent, then  $\neg\mrD$ is not $G$-syndetic.
Then $\mrD$ is $G$-thick.
...
\begin{theorem}\label{thm_syndetic_invariant2}
  ...
  \begin{itemize}
    ...
    \item[2.] $p({\mr x})\proves {\mr x}\in\mrD$ for some $G$-thick type $p({\mr x})\in S(\grZ)$
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  Pick any $p({\mr x})\in S(\grZ)$ that extends $\Sigma_G({\mr x})\cup\{{\mr x}\in\mrD\}$.
...
\begin{exercise}
  Prove that the following are equivalent for every $\D\in L(\Z)$.
  \begin{itemize}
    \item[1.] $\D$ is $G$-thick
...
\begin{exercise}
  Let $\D$ be a $L(\Z)$-type-definable set.
  Prove that the following are equivalent 
  \begin{itemize}
...
\begin{exercise}\label{ex_syndetic_type_vs_formulas}
  Let $\D\subseteq\X$ be a $G$-syndetic $L(\Z)$-type-definable set.
  Prove that $\D$ is definable by an $L(\Z)$-type containing only $G$-syndetic formulas.
\end{exercise}
...
\begin{proof}
  ...
  In particular $\mrX\subseteq\cup\,F{\cdot}\mr\D'$ for some finite $F\subseteq G$.
  Note that
