...
Throughout this chapter \emph{$\mrX$\/} $\subseteq\U^{\mr x}$ and \emph{$\grZ$\/} $\subseteq\U^{\gr z}$ are two abitrary sets (unless otherwise is explicitely stated).
...
Let \emph{$G$\/} be a group that acts on $\grZ$.
...
We say that a set $\mrD\subseteq\mrX$ is \emph{$\Delta(\grZ)$-definable\/} if it is of the form $\theta(\mrX\,;{\gr\bar c})$ for some $\theta({\mr x}\,;{\gr\bar z})\in\Delta$ and ${\gr\bar c}\in{\gr\Z^n}$.
If $g\in G$, we write $g{\cdot}\mrD$ for $\theta(\mrX\,;g{\cdot}{\gr\bar c})$.
In this chapter the symbol $\mrD$ (and occasionally $\mrC$) always denotes a $\Delta(\grZ)$-definable subset of $\mrX$ also when we do not explicitly mention this.
...
We say that a set $\mrA\subseteq\mrX$ is \emph{syndetic\/} under the action of $G$, or \emph{$G$-syndetic\/} for short, if finitely many $G$-translates of $\mrA$ cover $\mrX$; we say \emph{$n$-$G$-syndetic\/} if $\le n$ translates suffices.
Dually, we say that $\mrA$ is \emph{thick\/} under the action of $G$, or \emph{$G$-thick\/} for short, if the intersection of any finitely many $G$-translates of $\mrA$ is consistent; we say \emph{$n$-$G$-thick\/} when the request is limited to $\le n$ translates.
...
Notation: for $\mrA\subseteq\mrX$ and $C\subseteq G$ we write \emph{$C{\cdot}\mrA$\/} for $\{h{\cdot}\mrA: h\in C\}$.
...
\begin{proof}
  ...
  \begin{itemize}
    \item[1$'$.] there is no finite $C\subseteq G$ such that $\mrX\ \subseteq\ \cup\, C{\cdot}\mrA$.
    \item[2$'$.] $\varnothing\ \neq\ \cap\, C{\cdot}\neg\mrA$ for every finite $C\subseteq G$.
  \end{itemize} 
...
\begin{corollary}\label{fact_fip2}
  The following are equivalent
  \begin{itemize}
    \item[1.] $\mrA$ is $G$-thick
    \item[2.] $\mrA\cap\mrB\neq\varnothing$ for every set $\mrB$ that is $G$-syndetic.
  \end{itemize}
\end{corollary}
...
Note that when $\Sigma_G({\mr x})\cup\{{\mr x}\in\mrD\}$ is finitely consistent in $\mrX$, then $\neg\mrD$ is not $G$-syndetic.
Then $\mrD$ is $G$-thick.
The converse implication holds for complete types.
We write $S_{\Delta,\X}(\grZ)$ for the set of $\Delta(\grZ)$-types that are maximal among those that are finitely consistent in $\mrX$.

\begin{theorem}\label{thm_syndetic_invariant}
  For every $p({\mr x})\in S_{\Delta,\X}(\grZ)$ the following are equivalent
  \begin{itemize}
...
%     \item [2.] $p({\mr x})\proves{\mr x}\in\mrD$ for every $G$-thick $p({\mr x})\in S_{\Delta,\X}(\grZ)$.
%   \end{itemize}
% \end{corollary}
...
%   Then there is a type $p({\mr x})\in S_{\Delta,\X}(\grZ)$ finitely consistent with $\Sigma_G({\mr x})\cup\{{\mr x}\notin\mrD\}$.
%   By Corollary~\ref{corol_q_pers} $p({\mr x})$ is $G$-thick.
%   Then $\neg\ssf2$.
% \end{proof}

From the theorem we derive some necessary conditions for the existence of types in $S_{\Delta,\X}(\grZ)$ that are $G$-thick.

\begin{corollary}\label{corol_def_mu}
  If a type a $G$-thick $p({\mr x})\in S_{\Delta,\X}(\grZ)$ exists, then for any $\mrD$
  \begin{itemize}
    \item[1.] $\mrD$ and $\neg\mrD$ are not both $G$-syndetic
    \item[2.] if $\mrD$ is $G$-syndetic then it is $G$-thick
    \item[3.] $\Sigma_G({\mr x})$ is finitely consistent in $\mrX$.\smallskip
  \end{itemize}
\end{corollary}
...
\begin{theorem}\label{thm_syndetic_invariant2}
  ...
  \begin{itemize}
    ...
    \item[2.] $p({\mr x})\proves {\mr x}\in\mrD$ for some $G$-thick type $p({\mr x})\in S_{\Delta,\X}(\grZ)$
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  Pick any $p({\mr x})\in S_{\Delta,\X}(\grZ)$ that extends $\Sigma_G({\mr x})\cup\{{\mr x}\in\mrD\}$.
...
\begin{proof}
  Let $p({\mr x})\in S_{\Delta,\X}(\grZ)$ be a $G$-thick type such that $p({\mr x})\proves{\mr x}\in\mrD$.
...
%   Let $p({\mr x})\in S_{\Delta,\X}(\grZ)$ extend $q({\mr x})$.
...
\begin{exercise}
  Let $\X=\Z=\U$.
  Prove that if $p(x)\in S_{\Delta,\X}(\U)$ is finitely satisfiable in every $M\supseteq A$ then it is thick under the action of $G=\Autf(\U)$.
...
\begin{exercise}
  ...
  \begin{itemize}
    \item[1.] $\D$ is $G$-thick
    \item[2.] there is a type $p(x)\in S_{\Delta,\X}(\Z)$ containing $\{x\in g{\cdot}\D\;:\;g\in G\}$.
  \end{itemize}
\end{exercise}

\begin{exercise}
  Let $p(x)\in S_{\Delta,\X}(\Z)$.
...
\begin{exercise}
  Prove that the following are equivalent 
  \begin{itemize}
    \item[1.] there is a $G$-invariant type $p(x)\in S_{\Delta,\X}(\Z)$ such that $p(x)\proves x\in\D$.
...
% In particular, every $q({\mr x})\in Q_G$ has an extension to a $G^1$-invariant $p({\mr x})\in S_{\Delta,\X}(\grZ)$.
...
\def\ceq#1#2#3{\parbox[t]{20ex}{$\displaystyle #1$}\medrel{#2}{$\displaystyle #3$}}
  ...
  % It also follows that for any $p({\mr x})\in S_{\Delta,\X}(\grZ)$
  % \begin{itemize}
  %   \item [3.] \ceq{\hfill p({\mr x})\textrm{ is }H\textrm{-invariant}}{\IFF}{g\cdot p({\mr x})\textrm{ is }H\textrm{-invariant}.}
  % \end{itemize}
\end{remark}

Recall that when $\Sigma_H({\mr x})$ is finitely consistent in $\mrX$ then $H$-syndetic sets are $H$-wide, see Theorem~\ref{thm_syndetic_invariant2}.
...
\begin{proposition}\label{prop_Gsyndetic_Hthick1}
  Assume $H\trianglelefteq G$ and that $\Sigma_H({\mr x})$ is finitely consistent in $\mrX$.
  Then every $G$-syndetic $\mrD$ is $H$-wide.
  In particular all types in $Q_G$ are $G^1$-wide.
\end{proposition}

\begin{proof}
  Let $p({\mr x})\in S_{\Delta,\X}(\grZ)$ be finitely consistent with $\Sigma_H({\mr x})$.
...
A set $\mrY\subseteq\mrX$ is \emph{$G$-minimal\/} if for every $g\in G$ either $g{\cdot}\mrY=\mrY$ or $\mrY\cap g{\cdot}\mrY=\varnothing$.

\begin{definition}\label{def_G00}
  Let $\Phi^0$ be the collection of $\Delta(\grZ)$-definable, $G$-minimal sets that have finite orbits.
  Let $\Phi^{00}$ be the collection of $\Delta(\grZ)$-type-definable, $G$-minimal sets that have a bounded $G$-orbit.
  Define\smallskip
...
\begin{proof}
  ...
  Let $\mrY\in\Phi^{00}$.
  If $\mrD$ is any $\Delta(\grZ)$-definable set containing $\mrY$, a small number of translations of $\mrD$ cover $\mrX$.
...
\begin{theorem}\label{thm_coalesce}
  The following are equivalent (set is a short for $\Delta(\grZ)$-definable set)
  \begin{itemize}
...
\begin{proof}
  ...
  By \ssf1 and Theorem~\ref{thm_syndetic_invariant2} there is a $G$-invariant type $p({\mr x})\in S_{\Delta,\X}(\grZ)$ such that $p({\mr x})\proves{\mr x}\notin\mrC\cap\mrD$.
...
\begin{proposition}\label{prop_coalese}\ 
  ...
  \ceq{\ssf5.\hfill\Sigma_G({\mr x})}{=}{\bigcap\Big\{p({\mr x})\in S_{\Delta,\X}(\grZ)\ :\ p({\mr x})\text{ is }G\text{-invariant}\Big\}.}
\end{proposition}
...
We say that $\mrX$ is \emph{$G$-stationary\/} if there is a unique $G$-in\-var\-iant $p({\mr x})\in S_{\Delta,\X}(\grZ)$.
...
Let $\mrX$ and $\grZ$ be two copies of $\U$ and let $G={\rm Aut}(\U)$.
