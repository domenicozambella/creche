...
In this chapter $\Delta\subseteq L_{{\mr x}\,{\gr z}}(\U)$, ${\mrX}\subseteq\U^{\mr x}$, and ${\grZ}\subseteq\U^{\gr z}$ are some arbitrary nonempty sets (for some arguments we require that $\mrX$ and $\grZ$ are type-definable).

Let ${\gr\Aa}\subseteq\U^{\gr z}$.
We write \emph{$\pmDelta({\gr\Aa})$\/} for the set of formulas of the form $\phi({\mr x}\,;{\gr b})$ or $\neg\phi({\mr x}\,;{\gr b})$ for some $\phi({\mr x}\,;{\gr z})\in\Delta$ and some ${\gr b}\in{\gr\Aa}$.
We write $S_\Delta({\gr\Aa})$ for the set of complete $\pmDelta({\gr\Aa})$-types~--~needless to say, these are equivalent to complete $\BDelta({\gr\Aa})$-types.

We write \emph{$\BDelta({\gr\Aa})$} for the set of Boolean combinations of formulas in $\pmDelta({\gr\Aa})$.
% A $\BDelta(\Aa)$-definable set is a set of the form $\theta(\U^{\mr x})$ for some $\BDelta(\Aa)$-formula $\theta({\mr x})$.
% Subsets of $\BDelta(\Aa)$ are called $\BDelta(\Aa)$-types.
...
\begin{proof}
  ...
  By \ssf1 and Theorem~\ref{thm_generic_invariant2} there is a invariant global $\BDelta({\grZ})$-type $p({\mr x})$ containing ${\mr x}\in\neg\mrC\cup\neg\mrD$.
