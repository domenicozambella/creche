...
We write $\Stab(q)$ for the  stabilized of $q({\mr x})\subseteq\BDelta(\gr Z)$, that is, $\{g\in G:g{\cdot}q({\mr x})=q({\mr x})\}$.
Note that $\Stab(q)\le G$. 
We write

\ceq{\hfill\Stab(Q)}{=}{\displaystyle\bigcap_{q\in Q}\Stab(q),}

Then $\Stab(Q)\trianglelefteq G$.
It is easy to verify that $\gamma_{\Stab(Q)}({\mr x})\subseteq q({\mr x})$ for every $q\in Q$.
In particular, $\gamma_{\Stab(Q)}({\mr x})$ is consistent.
In fact, if $\mrD$ is $\Stab(Q)$-generic, then $q({\mr x})\proves{\mr x}\in g{\cdot}\mrD$ for some $g\in\Stab(Q)$.
...
\begin{lemma}\label{lem_trans_action0}
  ...
  \ceq{\hfill\Phi}{=}{\big\{\phi({\mr x})\in\BDelta(\grZ)\ :\ \textrm{ with finite }G\textrm{-orbit}\big\},}\medskip

  \ceq{\hfill P}{=}{\big\{p({\mr x})\subseteq\Phi\ :\ \textrm{ maximally consistent (in $\mrX$)}\big\}.}\smallskip

  Then $P$ is closed under the action of $G$ and, when $G$ acts transitively on $\mrX$, it works transitively also on $P$. Moreover, let
  
  \ceq{\hfill\epsilon_\phi({\mr x},{\mr y})}{=}{\bigwedge_{i=1}^n\big[\phi_i({\mr x})\iff\phi_i({\mr y})\big],}\smallskip
  
  \ceq{\hfill E}{=}{\big\{\epsilon_\phi({\mr x},{\mr y})\ :\  \phi({\mr x})\in\Phi\big\}.}\smallskip
  
  Then, for every ${\mr a}\in\mrX$, the type $\big\{\epsilon({\mr x}\,;{\mr a})\ :\ \epsilon({\mr x}\,;{\mr y})\in E\big\}$  is equivalent to a type $p({\mr x})\in P$.
\end{lemma}
...
\begin{remark}\label{lem_trans_action}
  When $G$ acts transitively on $\mrX$ all $\Phi$-definable sets are $G$-generic and every $p({\mr x})\in P$ extends to some $q({\mr x})\in Q$.
