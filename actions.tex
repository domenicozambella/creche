...
We omit reference of $\mrY$ when this is $\mrX$, or clear from the context.
...
\ceq{\hfill\emph{$\gamma_{G,\Y}({\mr x})$}}{=}{\{\theta({\mr x})\in \BDelta({\grZ})\ :\ \theta({\mr x})\textrm{ is }G\textrm{-syndetic relatve to }\mrY\}}
...
\begin{corollary}\label{corol_q_pers}
  (Relative to $\mrY$.)
  Let $\mrD\in\BDelta(\grZ)$ be such that $\gamma_{G,\Y}({\mr x})\cup\{{\mr x}\in\mrD\}$ is finitely consistent in $\mrY$.
  Then $\mrD$ is $G$-thick.
\end{corollary}
...
\begin{theorem}\label{thm_syndetic_invariant}
  For every $p({\mr x})\in S_\Delta(\grZ)$ the following are equivalent
  \begin{itemize}
    \item[1.] $p({\mr x})$ is $G$-invariant and finitely consistent in $\mrY$
    \item[2.] $p({\mr x})\proves\gamma_{G,\Y}({\mr x})$
    \item[2$'$.] $p({\mr x})\proves\gamma_{G}({\mr x})$ and is finitely consistent in $\mrY$
    \item[3.] $p({\mr x})$ is $G$-thick relative to $\mrY$
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  Let $\mrD\in\BDelta(\grZ)$ be a $G$-syndetic relative to $\mrY$.
  ...
  % Then 
  Negate \ssf1.
  Assume that $p({\mr x})$ finitely consistent in $\mrY$, otherwise $\neg$\ssf2 is immediate.
...
\begin{corollary}\label{corol_def_mu}
  (Relative to $\mrY$.)
  If a type $p({\mr x})\in S_\Delta(\grZ)$ exists that is $G$-thick, then for any $\mrD\in\BDelta({\grZ})$
  \begin{itemize}
...
\begin{theorem}\label{thm_syndetic_invariant2}
  ...
  \begin{itemize}
    ...
    \item[2.] there exists a $G$-thick type $p({\mr x})\in S_\Delta(\grZ)$ entailing ${\mr x}\in\mrD$
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  By Corollary~\ref{corol_q_pers}, it suffices to pick any $p({\mr x})\in S_\Delta(\grZ)$ finitely consistent in $\mrY$ with $\gamma_G({\mr x})\cup\{{\mr x}\in\mrD\}$.
...
\begin{corollary}
  ...
  \begin{itemize}
    \item[1.] $\gamma_G({\mr x})\cup q({\mr x})$ is finitely consistent
    \item[2.] there exists an $G$-thick type $p({\mr x})\in S_\Delta(\grZ)$ consistent with $q({\mr x})$
...
\begin{proof}
  Let $p({\mr x})\in S_\Delta(\grZ)$ be a $G$-thick type such that $p({\mr x})\proves{\mr x}\in\mrD$.
...
A subset $\mrY\subseteq\mrX$) is \emph{$H$-stationary\/} if there is a unique $H$-thick $p({\mr x})\in S_\Delta(\grZ)$ finitely consistent in $\mrY\subseteq\mrX$.
...
\begin{fact}\label{fact_starionarity_thick_syndetic}
  ...
  Suppose that the set $\mrX$ is $H$-stationary.
  Then the following are equivalent
  \begin{itemize}
...
Stationarity is a very strong condition when required for the whole set $\mrX$.
It may be easy to realize realize in a smaller subset. 
To adapt the proof of Fact~\ref{fact_starionarity_thick_syndetic} with some $\mrY\subseteq\mrX$ for $\mrX$, we need to relate the notions of thick, syndetic, etc.\@ when relative to different sets.
...
\begin{proposition}
  Assume that $\mrX$ is type-definable.
  Let $\mrY_i\subseteq\mrX$ for $i<\lambda$ be a cover of $\mrX$ of small cardinalty.
  Then the following are equivalent for every $\mrD\in\BDelta(\grZ)$ 
  \begin{itemize}
    \item [1.] $\mrD$ is $G$-thick relative to $\mrX$
    \item [2.] $\mrD$ is $G$-thick relative to some $\mrY_i$.
  \end{itemize}\smallskip
  The same holds for weakly $G$-thick.
\end{proposition}

\begin{proof}
  Only \ssf1$\IMP$\ssf2.
  Negate \ssf2.
  Then for every $i<\lambda$ there is a finite set $C_i\subseteq G$ such that $\cap\,C_i{\cdot}\mrD$ does not intersect ${\mr\Y_i}$.
  Let $C$ be the union of all these $C_i$.
  As $C$ has small cardinality and $\cap\,C{\cdot}\mrD$ is empty, by compactyess (recall that $\mrX$ is type definable) there is a finite subset of $C$ that contradicts \ssf1.
\end{proof}
