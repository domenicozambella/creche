...
In particular $\gamma_G({\mr x})$ is not a $G$-generic type, and it may even be inconsistent.
Then following notion is relevant

\begin{definition}\label{def_Q}\ 

  \ceq{\hfill\emph{$Q$\/}}{=}{\big\{q({\mr x})\subseteq\gamma_G({\mr x})\;:\ q({\mr x})\textrm{ maximally generic}\big\}.}\smallskip
...
\begin{proof}
  ...
  By maximality, $\psi({\mr x})\wedge g{\cdot}\phi({\mr x})$ is not $G$-generic for some $\psi({\mr x})\in q$.
...
As it happens, under the assumption of normality also $G$-generic sets are $H$-wide.


\begin{proposition}\label{prop_Ggeneric_Hpersistent1}
...
\begin{proof}
  ...
  As $p({\mr x})$ is $H$-persistent, by Remark~\ref{rem_invariance_normalsubg} also $g^{-1}\!\cdot p({\mr x})$ is $H$-persistent.
  Then the proposition follows from Theorem~\ref{thm_generic_invariant2}.
\end{proof}

% \begin{theorem}
%   Every $\Stab(Q)$-persistent $\BDelta(\grZ)$-set is $\Stab(Q)$-wide.
% \end{theorem}
...
For instance, when we say that $\mrD$ is $H$-generic relative $\mrY$, we actually mean $\mrD\cap\mrY$.
...
\begin{proposition}
  ...
  Then every $\mrD\in\BDelta(\grZ)$ that is $H$-generic relative to $\mrY$ is $G$-generic.
  % Then the following are equivalent for every $\mrD\in\BDelta(\grZ)$
  % \begin{itemize}
  %   \item [1.] $\mrD$ is $G$-generic
  %   \item [2.] $\mrD$ is $H$-generic relative to $\mrY$.
...
%
%   \ssf2$\IMP$\ssf1.
  By normality, $g{\cdot}\mrD$ is $H$-generic relative to $g{\cdot}\mrY$ for any $g\in G$.
...
The group $G^0$ defined below is a good candidate. %~--~we will see that under the assumption of stability it indeed coincides with $\Stab(Q)$. 
First we need to revisit the notions introduced in Section~\ref{eq_algebraic} as we want to apply them to $\BDelta(\grZ)$, not to the full language.%$\LDelta(\mrX,\grZ)$.
...
\begin{remark}\label{lem_trans_action}
  ...
  Then the sets in $\Phi^0$ are $G$-generic.
...
%   % \ceq{\hfill \Stab(Q)}{\subseteq}{\big\{g\in G\ :\ \mrD\cap g{\cdot}\mrD\textrm{ is }G\textrm{-generic for all }G\textrm{-generic }\mrD\in\BDelta(\grZ)\big\}}\smallskip

%   % \ceq{}{\subseteq}{\big\{g\in G\ :\ \mrD\cap g{\cdot}\mrD\neq\varnothing\textrm{ for any }G\textrm{-generic }\mrD\in\BDelta(\grZ)\big\}}\smallskip
...
\begin{proof}
  ...
  Let ${\mr\D}$ and ${\mr\C}$ be strongly $H$-generic and let $C\subseteq H$ be an arbitrary finite set.
  It suffices to prove that $\mrB=\cap\, C{\cdot}({\mr\C}\cap{\mr\D})$ is $H$-generic. 
...
\begin{proof}
  ...
  The finite consistency is a consequence of genericity. % , see Exercise~\ref{ex_gen_sat}.
  Finally, $H$-invariance is clear because any translate of a strongly $H$-generic formula is also strongly $H$-generic.
\end{proof}
