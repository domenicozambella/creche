...
\begin{remark}\label{rem_H2}
  The formulas $\zeta_n({\gr z_1},\dots,{\gr z_n},{\gr z_1'},\dots,{\gr z_n'})$ ensure that for every automorphism in $f\in G$ and finite set $B\subseteq\grZ$, there is an automorphism in ${\gr g}\in \grZ$ that coincides with $f$ on $B$.
  By saturation this is true also for any small set $B$.
  We conclude that for formulas in $\LDelta(\grZ)$ the notions of generic and persistent under the action of $G$ and of $\grZ$ coincide.
\end{remark}
...
\begin{definition}\label{def_G0}\noindent\vspace*{0.1ex}
  ...
  \ceq{\hfill{\gr\Z^0}}{=}{\bigcap\big\{\grW\le\grZ\ :\ \grW\textrm{ is }\BDelta(\grZ)\textrm{-definable and has finite index in }\grZ\big\}}.
\end{definition}

Note that we can replace \textit{definable\/} with \textit{type-definable\/} (essentially, by Exercise~\ref{ex_generic_type_vs_formulas}) and/or require that $\grW$ is normal.

\begin{proposition}
  Assume that the action of $\grZ$ is regular. Then
