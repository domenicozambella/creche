...
The technical reason that induced Shalah to introduce imaginaries will only be clear later, see Section~\hyperref[stable_teories]{\ref*{external}.\ref*{stable_teories}}, but the theory is of independent interest.
...
% N.B.\@ it is important to note the difference between $\acl{\vl\Aa}$ and $\acl\{{\vl\Aa}\}$. In the first, the parameter are the elements of ${\vl\Aa}$. In the latter, the set ${\vl\Aa}$ is the (second-order) paramenter.
...
\ceq{\hfill\textstyle\acl^\eq\{{\vl\Aa}\}}{=}{\textstyle\acl^\eq\big(\acl\{{\vl\Aa}\}\big)}
