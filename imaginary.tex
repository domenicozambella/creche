...
%(beware that this is an improper use of the word). 
% However $\U^\eq$ differs from the second-order structures mentioned in the previous section in two respects. 
% Firstly, in $\U^\eq$ all sets are definable, secondly $\U^\eq$ the has a distict sort for each formula $\sigma({\mr x}\,;{\gr z})$.
...
% {\color{red} It is important to note right away that this expansion of $\U$ is a mild one: the definable subsets of the home sort of $\U^\eq$ are the same as those of $\U$.
% In particular, iterating the expansion gives nothing new.}
