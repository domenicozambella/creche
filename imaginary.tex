...
The description of first-order definability is simplified if we allow definable sets to be used as second-order parameters in formulas. This leads to the theory of (elimination of) \textit{imaginaries}. The theory may be appreciated in itself though the reasons that induced Shalah to introduce imaginaries will only be clear later, see Sections~\hyperref[stable_teories]{\ref*{external}.\ref*{stable_teories}} and~\hyperref[canonical_bases]{\ref*{external}.\ref*{canonical_bases}}.
...
Let $\phi({\mr u}\,;{\vl\X})\in L^\eq$ where ${\mr u}\,$ is a tuple of first-order variables and  ${\vl\X}$ a tuple of variables of sort $\sigma_i({\mr x_i}\,;{\gr z_i})$ for $i=1,\dots,n$. Then there is a formula $\phi'({\mr u}\,;{\gr z})\in L$ such that 

\hfil$\displaystyle\A {\vl\X},{\gr z}\ \bigg[\bigwedge^n_{i=1}{\vl\X_i}=\big\{{\mr x_i}:\sigma_i({\mr x_i}\,;{\gr z_i})\big\}\imp\A {\mr u}\ \big[\phi({\mr u}\,;{\vl\X})\ \iff\ \phi'({\mr u}\,;{\gr z})\big]\bigg]$

where ${\vl\X}$ stand for the tuple ${\vl\X_1},\dots,{\vl\X_n}$ and ${\gr z}$ for the concatenation of ${\gr z_1},\dots,{\gr z_n}$.% For simplicity we are assuming that the tuples ${\gr z_i}$ do not have variables in common.
\end{proposition}

When $n=0$ the proposition asserts that $L_{{\mr u}}^\eq$ and $L_{{\mr u}}$ have the same expressive power.

\begin{proof}
By induction on syntax. When $\phi$ is atomic $\phi'=\phi$ unless it is of the form ${\mr t}\in {\vl\X}$ for some tuple of terms ${\mr t}$ or it has the form ${\vl\X_1}={\vl\X_2}$. In the first case $\phi'$ is the formula $\sigma({\mr t}\,;{\gr z})$. In the second case it is the formula $\A {\mr x}\,\big[\sigma_1({\mr x}\,;{\gr z_1}) \iff\sigma_2({\mr x}\,;{\gr z_2})\big]$. In the latter case we assume that the variables ${\mr x_1}$,${\mr x_2}$ in the sorts of  ${\vl\X_1}$,${\vl\X_2}$ have equal length otherwise we may replace ${\vl\X_1}={\vl\X_2}$ by $\bot$.

As for connectives they stay unchanged but for the quantifiers $\E {\vl\X}$ where ${\vl\X}$ is a second-order variable, say of sort $\sigma({\mr x}\,;{\gr z})$. These quantifiers are replaced with $\E{\gr z}$.
\end{proof}
...
\end{theorem}
\begin{proof}
Let $\phi({\vl\X})\in L^\eq(A)$ be an algebraic formula that has ${\vl\Aa}$ among its solutions and define
...
We write \emph{$a\stackrel{\smash{\scalebox{.5}{\rm\bf Sh}}}{\equiv}_Ab$} for ${\mr a}\,\equiv_{\acl^\eq\! A}{\mr b}$. In words we say that ${\mr a}$ and ${\mr b}$ have the same \emph{Shelah strong-type over $A$}.\QED
\end{definition}
...
Let $A\subseteq\U$, let ${\vl\Aa}$ be a definable set with finite orbit over $A$. Without using $\U^\eq$ prove that ${\vl\Aa}$ is union of classes of a finite equivalence relation definable over $A$.\QED
\end{exercise}
