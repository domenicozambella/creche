...
Then for every formula $\phi({\mr u}\,;{\vl\bar \X})\in L^\eq$ there is a formula $\phi'({\mr u}\,;{\gr\bar z})\in L$, where ${\gr\bar z}={\gr z_1},\dots,{\gr z_n}$ is a tuple variables of length $|{\gr z}|$, such that the following holds in $\U^\eq$

\ceq{\hfill \phi({\mr u}\,;{\vl\bar\Aa})}{\iff}{\phi'({\mr u}\,;{\gr\bar b})}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The eq-algebraic closure}\label{eq_algebraic}

The following is the natural generalization of the notion introduced in Section~\ref{acl}.

\begin{definition}
  Let $A\subseteq\U^\eq$ and $a\in\U^\eq$.
  We say that $a$ is \emph{algebraic\/}  over $A$ if there is a formula $\phi(v)\in L^\eq(A)$ such that $\phi(a)\wedge\E^{=k}v\,\phi(v)$ holds for some and some positive integer $k$.
...
It is worthwhile to spell out the equivalence \ssf2$\IFF$\ssf3 of the theorem above when $a$ is a definable set ${\vl\Aa}$.
...
\end{theorem}
\begin{proof} \ssf{2}$\IMP$\ssf{1}\quad If  $\epsilon({\mr x}\,;{\mr y})$ has $m$ classes, then $\Orbit(\Aa/A)$ contains at most $\displaystyle\binom{m}{n}$ sets.
...
\ceq{\hfill\S({\mr a}/A)}{=}{\Orbit({\mr a}/\acl^\eq A)}

\ceq{}{=}{q(\U^{\mr x})}\hfill where  $q({\mr x})=\tp({\mr a}/\acl^\eq\!A)$

\ceq{}{=}{\bigcap\big\{\epsilon(\U^{\mr x};{\mr a})\hfill :\hfill\epsilon({\mr x}\,;{\mr y})\in L(A)\ \textrm{ a finite equivalence relation}\big\}}

\ceq{}{=}{\bigcap\big\{\mrA\ :\ {\mr a}\in\mrA\in\acl^\eq\!A\big\}.}

\begin{proposition}\label{prop_trans_action}
  Let $p({\mr x})\in S(A)$ be given.
  Let \smallskip
  
  \ceq{\hfill Q}{=}{\big\{q({\mr x})\in S(\acl^\eq\!A)\ :\  p({\mr x})\subseteq q({\mr x})\big\}}.\smallskip
...
\begin{proof}
  ...
  \ceq{\hfill Q'}{=}{\big\{\S({\mr a}/A)\ :\  {\mr a})\models p({\mr x})\big\}.}
  ...
  % Then $p({\mr x})\imp\neg\psi({\mr x})$.
  % But $\psi({\mr x})$ is invariant over $A$.
  % Therefore $\psi({\mr x})\in L(A)$.
...
\begin{proof}
  \ssf{1}$\IMP$\ssf{2} 
  Assume \ssf{1} and let $\phi({\mr x}\,;{\gr z})$ be the formula in \ssf{wei}.
  Let ${\gr\B}$ be the set of solutions of the formula

\ceq{\hfill\A{\mr x}\ \bigg[{\mr x}\in{\vl\Aa}}{\iff}{\phi({\mr x}\,;{\gr z})\bigg].}

Hence ${\gr\B}$ is finite and ${\gr\B}\in\dcl^\eq\{{\vl\Aa}\}$.
We also have ${\vl\Aa}\in\dcl^\eq\{{\gr\B}\}$ because ${\vl\Aa}$ is definable by the formula $\E{\gr z}\,\big[{\gr z}\in {\gr\B}\wedge \phi({\mr x}\,;{\gr z})\big]$.
...
By the assumption there is a formula $\psi({\mr x}\,;{\gr\B})\in L^\eq$ that defines ${\vl\Aa}$.
Then ${\vl\Aa}$ is also defined by a formula $\psi'({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ with parameters in $\acl\{{\vl\Aa}\}$.

\ssf{3}$\IMP$\ssf{1} Assume \ssf{3}.
As \ssf{wei} holds trivially for all $\varnothing$-definable sets, we may assume ${\vl\Aa}\neq\varnothing$.
...
There are notions of elimination of imaginaries that are weaker than weak elimination.
For instance, we say that $T$ has \emph{geometric elimination of imaginaries\/} if for every $A\subseteq\U$

\ceq{\hfill {\vl\Aa} }{\in}{{\acl}^\eq(\acl\{{\vl\Aa}\})}
...
\begin{theorem}\label{elimimf} 
  ...
  \item[3.] every definable set ${\vl\Aa}\in\dcl^\eq(\dcl\{{\vl\Aa}\})$.
  \end{itemize}
\end{theorem}
...
\begin{example}
  ...
  \begin{itemize}
  \item[1.] $T$ has weak elimination of imaginaries
  \item[2.] for every $\Aa\in \U^\eq$ there exists the least algebraically closed set $A\subseteq\U$ such that $\Aa$ is definable over $A$.
  \end{itemize}
\end{example}
...
\begin{lemma}\label{lem_suff_wei}
  ...
  $\natural$\qquad $\mrD\subseteq\U^{\mr x}$ is definable both over $A$ and over $B$, then it is definable over $A\cap B$.
\end{lemma}

\begin{proof}
  Let $\mrD\subseteq\U^{\mr x}$ be a definable set, and pick a finite set $A\subseteq\U$ of minimal size over which $\mrD$ is definable. 
  Now, let $f$ be an automorphism that fixes $\mrD$. Then $\mrD$ is definable over $A\cap f[A]$, so by minimality of $|A|$ we have $A= f[A]$. 
  Now let ${\gr a}$ be an enumeration of $A$.
  By the above, ${\gr a}$ has a finite orbit under $\Aut(\U/\{\mrD\})$.
  Hence ${\gr a}\in\acl\{\mrD\}$.
  Therefore $\mrD\in\dcl^\eq\big(\acl\{\mrD\}\big)$
