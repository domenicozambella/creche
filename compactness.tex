...
In the last equivalence, $\PMI$ holds because $q({\gr z})$ is complete. (Using a term that will be introduced only in Section~\hyperref[morphisms]{\ref*{types}.\ref*{morphisms}}, this shows that  $h:M\to N$ is an elementary embedding.) 

By the remarks after Definition~\ref{isomorfismodef}, $h:M\to N$ is an embedding. Hence the equivalence above prove that $h[M]\preceq N$. So the theorem follows by identifying $M$ with $h[M]$.
\end{proof}
