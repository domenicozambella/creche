...
\begin{void}[Compactness Theorem]\label{thmcompattezza}
...
We write \emph{$M\models p(a)$} if $M\models\phi(a)$ for every $\phi(x)\in p$.
...
% The theory $\Th(M/M)$ is called the \emph{elementary diagram of $M$.}
Before that, we recall an important particular case of Proposition~\ref{oss_Delta-morfismi}. 
% It is more convenient to represent the elementary diagram with the type $q(z)=\tp(c)$, where $c$ is any tuple that enumerates $M$.
% The following lemma introduces a useful technique.

\begin{lemma}[ (The elementary diagram method)]\label{lem_el_diag}
...
\end{lemma}
 ...
 We call $q(z)$ the \emph{elementary diagram of $M$} though, stricticly speaking, the elementary diagram of $M$ is theory $\Th(M/M)=q(c)$.
...
\begin{void}[Compactness Theorem for types]\label{thm_compattezzatipi}
    Let $M$ be an infinite model.
...
\begin{void}[Upward L\"owenheim-Skolem Theorem]
...
\begin{exercise}\label{ex_fin_cons_in_M}
...
\item[d.] \ceq{\hfill a{\doteq}b,\ \phi[x/a]\in T}{\IMP}{\phi[x/b]\in T}.
\end{itemize}
\end{definition}
...
\begin{void}[Compactness Theorem]
...
We can extend $T$ to a theory $S$ that meets Definition~\ref{def_Henkin}, up to condition~\ssf{4}, by adding systematically all required formulas. The procedure is effective, hence Zorn's lemma is not required. 

It is interesting to consider the case when $T$ is finite.
...
% \ceq{\hfill T_{\infty}}{=}{\big\{\E^{\ge n}x\;(x=x)\ :\ n\in\omega\big\}}

% is not finitely axiomatizable.
% Every infinite set is a model of $T_{\infty}$ and, vice versa, every model of $T_{\infty}$ is an infinite set.
% Then $\ccl(T_{\infty})$ is the theory of infinite sets. Suppose for a contradiction that $T_{\infty}$ is finitely axiomatizable. By Proposition~\ref{finaxsub}, $\E^{\ge n}x\, (x{=}x)\proves T_{\infty}$ for some $n$. Any set of cardinality $n+1$ proves that this is not the case.
% \end{example}
...
% \def\ceq#1#2#3{\parbox{4ex}{$\displaystyle #1$}\parbox{4ex}{\hfil$#2$}$\displaystyle #3$.}
...
% To prove that $\K$ is not finitely axiomatizable note that no finite $S\subseteq T$ axiomatizes $\K$ and apply Proposition~\ref{finaxsub}.
% \end{example}
