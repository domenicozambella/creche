...
As for successor ordinals, let $S$ be a maximally finitely consistent set of $L_i$-formulas containing $T_i$. (He we appleal to Zorn's lemma, but see the remark below.) It is immediate that $S$ satisfies all requirements in Definition~\ref{def_Henkin} but possibly for \ssf{4}.
...
\begin{remark}
To keep the proof above short, we appealed to Zorn's lemma. This is not strictly necessary. In fact, if we are given a finitely consistent theory $T$. We can extend $T$ to a theory $S$ that meets Definition~\ref{def_Henkin}, up to condition~\ssf{4}, by adding systematically all required formulas. The procedure is effective and there is no need of Zorn's lemma. 

It is interesting to note that we only need to add to $S$  formulas that are subformulas of formulas in $T$, or alphabetic variants of these (i.e\@ renaming of terms). In particular, when $T$ is finite, $S$ is also finite (assume for simplicity that the language is finite and contains no functions other then constants). Then the construction in Proposition~\ref{prop_Henkin} is an effective procedure to construct in $\omega$ steps a model of $T$.

Now suppose instead that we start with an inconsistent $T$. The procedure above has to come to an halt at same (finite) stage. When it halts, we end up with a finite sequence of finite theories $T_0,\dots, T_n$ where $T_0=T$ and $T_n$ contains some blatant inconsistency (i.e.\@ $\phi$ and $\neg\phi$). Many have interpreted  $T_0,\dots, T_n$ as a formal \textit{proof\/} of the inconsinstency of $T$.

All this has little or no interest to model theory.\QED
\end{remark}
