...
We give two proofs of the compactness theorem. The first is syntactic, the second uses ultrapowers.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Compactness from syntax.}

We are going to prove the compactness theorem using the so-called Henkin method. First we observe that when all existential statements witnessed by closed terms (called \textit{Henkin constants\/}), models can be constructed using these witnesses as elements. Then we show that we can always add all the required Henkin constants.
...
\begin{proposition}If every finite subset of $T$ has a model then there is a Henkin theory $T'$ containing $T$. (The theory $T'$ may be in a language $L'$ that expands the language $L$ of $T$.)
\end{proposition}
...
At stage $\lambda$, as all possible counterexamples to \ssf{3} have been ruled out, $T_\lambda$ is a Henkin theory.
\end{proof}

The compactness theorem is an immediate corollary of the proposition above.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Compactness from ultraproducts}\label{compactness}
\def\medrel#1{\parbox[t]{6ex}{$\displaystyle\hfil #1$}}
\def\ceq#1#2#3{\parbox{25ex}{$\displaystyle #1$}\medrel{#2}$\displaystyle  #3$}
