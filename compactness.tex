...
It is interesting to note that we only need to add to $S$  formulas that are subformulas of formulas in $T$, or alphabetic variants of these (i.e\@ renaming of terms). In particular, when $T$ is finite, $S$ is also finite (assume for simplicity that the language is finite and contains no functions other then constants). Then the construction in Proposition~\ref{prop_Henkin} is an effective procedure that produces in $\omega$ steps a model of $T$.
...
But all this has little or no interest to model theory.\QED
\end{remark}
