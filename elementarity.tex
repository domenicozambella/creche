...
\begin{itemize}
\item[] $M\models T\ \ \IMP\ \ M\models\phi$\qquad for every $M$.
\end{itemize}

In words, we say that $\phi$ is a \emph{logical consequence\/} of $T$ or that $\phi$ \emph{follows from\/} $T$. If $S$ is a theory \emph{$T\proves S$\/} has a similar meaning. When $T\subseteq S$ and $T\proves S$ we say that $T$ \emph{axiomatises\/} $S$. If $T\proves S$ and $S\proves T$ we say that $T$ and $S$ are \emph{logically equivalent}.

The \emph{closure of $T$ by logical consequence\/} is the set $\ccl(T)$ which is defined as follows:  

\begin{itemize}
\item[] \emph{$\textrm{ccl}(T)$}$\ \ =\ \ \Big\{\phi\in L\ :\ \textrm{ sentence such that } T\proves\phi\Big\}$
\end{itemize}
...
Let $L$ be the language multiplicative groups. Let $T_{\rm g}$ be the set containing the universal closure of following three formulas
\begin{itemize}
\item[ass.]\ \  $(x\cdot y)\cdot z\ =\ x\cdot(y\cdot z)$;
\item[inv.]\ \  $ x\cdot x^{-1}\ =\ x^{-1}\cdot x\ =\ 1$;
\item[neu.]\ \  $x\cdot 1\ =\ 1\cdot x\ =x$.
\end{itemize}
Every model of $T_{\rm g}$ is a group and, vice versa, every group is a structure that models $T_{\rm g}$. Let $\phi$ be the universal closure of the following formula
\begin{itemize}
\item[can.]\ \  $\ z\cdot x\ =\  z\cdot y\ \imp\ x\ =\ y$. 
\end{itemize}
As $\psi$ formalizes the cancellation property then $T_{\rm g} \proves\phi$, that is, $\phi$ is a logical consequence of $T_{\rm g}$. Now consider the sentence $\psi$ which is the universal closure of
\begin{itemize}
...
ation of vector spaces is less straightforward.
...
The \emph{theory of $M$\/} is the set of sentences that hold in $M$ and is denoted by \emph{$\Th(M)$}. More generally, if $\K$ is a class of structures, \emph{$\Th(\K)$} is the set of sentences that hold in every model in $\K$. For instance if $\K$ is the class of all groups,  $\Th(\K)$ is the theory $\ccl(T_{\rm g})$. The class of all models of $T$ is denoted by \emph{$\textrm{Mod}(T)$}. We say that $\K$ is \emph{axiomatizable\/} if $\Mod(T)=\K$ for some theory $T$. If we can also require that $T$ is finite we say that $\K$ is \emph{finitely axiomatizable}. To sum up

\hspace*{14ex}\llap{$\Th(M)$}\ \ =\ \ $\Big\{\phi\ :\ M\models\phi\Big\}$

\hspace*{14ex}\llap{$\Th(\K)$}\ \ =\ \ $\Big\{\phi\ :\ M\models\phi \textrm{ for all }M\in\K\Big\}$

\hspace*{14ex}\llap{$\Mod(T)$}\ \ =\ \ $\Big\{M\ :\ M\models T\Big\}$
...
The following is one of the most fundamental notions in model theory.
...
\end{itemize}
Note that \ssf{c'} extends to all definable sets what Definition~\ref{substructuredef} requires for a few basic definable sets.
...
\begin{exercise}
Let $M\preceq N$ and let $\phi(x)\in L(M)$. Prove that if $\phi(N)$ is finite then $\phi(N)=\phi(M)$.\QED
\end{exercise}
...
\begin{itemize}
\item[1.]\ceq{$a\in r^M$}{$\IFF$}{$ha\in r^N$} for every $r\in L_{\rm rel}$ and $a\in M^{n_r}$
\item[2.]\ceq{$h\,f^M(a)$}{$=$}{$f^N(h\,a)$} for every $f\in L_{\rm fun}$ and $a\in M^{n_f}$.
\end{itemize}
...
\begin{itemize}
\item[1'.]\ceq{$M\models r(a)$}{$\IFF$}{$N\models r(ha)$} for every $r\in L_{\rm rel}\cup\{=\}$ and every $a\in M^{n_r}$.
\end{itemize}

Note also that, by straightforward induction on syntax, from \ssf{2} we obtain

\begin{itemize}
\item[2']\ceq{$h\,t^M(a)$}{$=$}{$t^N(h\,a)$} for every term $t(x)$ and every  $a\in M^{|x|}$.
\end{itemize}

Combining these two properties, and by straightforward induction on the syntax, we obtain 
\begin{itemize}
\item[3.]\ceq{$M\models\phi(a)$}{$\IFF$}{$N\models\phi(ha)$} for every $\phi(x)\in L_{\rm qf}$ and every $a\in M^{|x|}$.
\end{itemize}
...
\begin{itemize}
\item[3']\ceq{$M\models \phi(a)$}{$\IFF$}{$N\models \phi(a)$} for every $\phi(x)\in L_{\rm qf}$ and for every $a\in M^{|x|}$.
\end{itemize}
...
\begin{itemize}
\item[\#]\ceq{$M\models \phi(a)$}{$\IFF$}{$N\models \phi(ha)$} for every $a\in M^{|x|}$
\end{itemize}
...
\hspace*{18ex}\llap{$M\ \models\ \E y\;\phi(a,y)$}\parbox{7ex}{\hfil$\IFF$}\parbox{14ex}{$M\models\phi(a,b)$} for some $b\in M$

\hspace*{18ex}\parbox{7ex}{\hfil$\IFF$}\parbox{14ex}{$N\models\phi(ha,hb)$} for some $b\in M$\hfill by induction hypothesis

\hspace*{18ex}\parbox{7ex}{\hfil$\IFF$}\parbox{14ex}{$N\models\phi(ha,c)$} for some $c\in N$\hfill $\PMI$ requires surjectivity
...
\begin{corollary}
If $h:M\imp N$ is an $A$-isomorphism then $h\big[\phi(M)\big]=\phi(N)$ for every $\phi(x)\in L(A)$.\QED
\end{corollary}

We can now give a few very simple examples of elementarily equivalent structures.

\begin{example}\label{expl_ordini_elem}
Let $L$ be the language of strict orders. The intervals $[0,1]$ and $[0,2]$ are isomorphic, hence $[0,1]\equiv[0,2]$ follows from Theorem~\ref{isomorfoeleq}. Clearly, $[0,1]$ is a substructure of $[0,2]$. However $[0,1]\npreceq [0,2]$, in fact the formula $\A  x\,(x\mathord\le1)$ holds in $[0,1]$ and is false in $[0,2]$. 

The example above shows that $M\subseteq N$ and $M\equiv N$ does not imply $M\preceq M$.

Now we prove that $(0,1)\preceq (0,2)$. By Exercise~\ref{finitanaturaeqel}, it suffices to verify that $(0,1)\equiv_B  (0,2)$ for every finite $B\subseteq(0,1)$. This follows again by Theorem~\ref{isomorfoeleq} as $(0,1)$ and $(0,2)$ are $B$-isomorphic for every finite $B\subseteq(0,1)$.\QED
\end{example}
...
Prove that if $\phi(x)$ is an existential formula and $h:M\to M$ is an embedding
\begin{itemize}
...
\end{proposition}
\begin{proof}
To prove \ssf{a}$\IMP$\ssf{b}, assume \ssf{a}. As $T$ is consistent, there is $M\models T$. Then $T\subseteq\Th (M)$. As $T$ is maximal consistent $T=\Th(M)$. Implication \ssf{b}$\IMP$\ssf{c} is immediate.  As for \ssf{c}$\IMP$\ssf{a} note that if $T\cup\{\phi\}$ is consistent then $\neg\phi\not\in T$ therefore $\phi\in T$ follows from \ssf{c}.
\end{proof}
...
\end{itemize}
Hint: consider the theory containing all sentences where the symbol $\neg$ occours an even number of times. This theory is not consistent as it contains $\bot$.\QED
\end{exercise}

\begin{exercise}
Prove that if $T$ has exacly $2$ maximal consistent extension $T_1$ and $T_2$ then there is a sentence $\phi$ such that $T,\phi\proves T_1$ and $T,\neg\phi\proves T_2$.\QED
\end{exercise} 
...
If $\phi(x)$ is atomic the claim follows from $M\subseteq N$ and Lemma~\ref{immersioniqfeq}. The case of Boolean connectives is straightforward, so only the existential quantifier requires a proof
...
The following theorem was proved by L\"owenheim at the beginning of last century. Skolem gave a simpler proof immediately after. At that time the result was perceived as paradoxical. Qualche anno prima i lavori di Zermelo e di Fraenkel avevano convinto la comunit\`a scientifica che l'intera theory degli insiemi potesse essere formalizzata con un linguaggio del prim'ordine. Il teorema  L\"owenheim-Skolem all'ingi\`u implica l'esistenza di un modello numerabile della theory degli insiemi: questo is chiamato il \emph{paradosso di Skolem}. \`E paradossale perch\'e $M$ rende vero, tra l'altro, l'sentence che formalizza l'assioma delle parti, quindi $M$ contiene un elemento $b$ che, dal punto di vista di $M$, is l'insieme dei sottoinsiemi dei numeri naturali. Ma l'insieme degli elementi di $b$ is un sottoinsieme di $M$ and is quindi numerabile. Non is una contraddizione, semplicemente l'espressione: \textit{tutti\/} i sottoinsiemi dei naturali, non ha in $M$ lo stesso significato che ha nel mondo reale. Anche la nozione di cardinali\`a assume un significato diverso. Nel linguaggio della theory degli insiemi is possibile formalizzare al prim'ordine: $b$ non is numerabile. Basta dire che non esiste nessuna biiezione tra $b$ e i numeri naturali. Quindi la biiezione tra gli elementi di $b$ and i numeri naturali (che esiste nel mondo reale) non appartiene ad $M$. Il concetto di equipotente in $M$ ha un altro significato che nella realt\`a~--~anche se chi vive dentro $M$ non se ne pu\`o rendere conto.

\begin{theorem}[downward L\"owenheim-Skolem.]\label{lowenheimskolemallingiu}
Let $N$ be an infinite structure of signature $L$ and let $A\subseteq N$. Then there is a structure $M$ of cardinality $\le|L(A)|$ such that $A\subseteq M\preceq N$. 
\end{theorem}
...
We will need to adapt the construction above to meet more requirements on the model $M$. To better control the elements that enters $M$ it is convenient to add one element at the time (above we add $\lambda$ elements at each stage). We need to enumerate formulas with some care if we want to complete the construction by stage $\lambda$.

\begin{void}[Second proof of Theorem~\ref{lowenheimskolemallingiu}.]\label{diym_II_lowenheimskolemallingiu}
...
We use Tarski-Vaught test to prove $M\preceq N$. Let $\phi(x)\in L(M)$ be consistent in $N$. As $\pi$ is a bijection, there is an $i$ such that $\phi(x)$ is the $\pi_2(i)$-th formula enumerated in $L(A_{\pi_1(i)})$. Hence a witness of $\phi(x)$ is enumerated in $M$ at stage $i$.
\end{void}
...
Assume $L$ is countable and let $M\subseteq N$ and $A\subseteq N$ be both countable. Prove that there is a countable model $K$ such that $A\subseteq K\preceq N$ and $K\cap M\preceq N$ (in particular, $K\cap M$ is a model).  Hint: adapt the construction used to prove the downward L\"owenheim-Skolem.\QED
\end{exercise}
...
\begin{exercise}
Give an alternative answer to Exercise~\ref{ex_LS} using the L\"owenheim-Skolem Theorem (instead of its proof). Hint: construct two countable chains $\<K_i:i<\omega\>$ and  $\<M_i:i<\omega\>$ of countable models such that $K_i\cap M\subseteq M_i\preceq M$ and $A\cup M_i\subseteq K_{i+1}\preceq M$. The required model is $K=\bigcup_{i\in\omega}K_i$ as $K\cap M=\bigcup_{i\in\omega}M_i$.\QED
\end{exercise}