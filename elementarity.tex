...
The class of all models of $T$ is denoted by \emph{$\textrm{Mod}(T)$}. We say that $\K$ is \emph{axiomatizable\/} if $\Mod(T)=\K$ for some theory $T$. If we can also require that $T$ is finite we say that $\K$ is \emph{finitely axiomatizable}. To sum up
