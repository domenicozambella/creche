...
\item[d.]  \parbox{27ex}{$\phi(M^{\mr x}) \cap A^x\ =\ \phi(N) \cap A^x$} for every $\phi(x)\in L$.
\end{itemize}
\end{proposition}
...
\begin{itemize}
\item[a'.] $M\preceq N$
\item[d'.] \parbox{26ex}{$\phi(M^{\mr x})\ =\ \phi(N) \cap M^x$} for every $\phi(x)\in L$.
\end{itemize}
...
Prove that $\phi(M^{\mr x})$ is finite if and only if $\phi(N)$ is finite and in this case $\phi(N)=\phi(M^{\mr x})$.
\end{exercise}
...
\ceq{\hfill h\big[\phi(M^{\mr x})\big]}{=}{\phi(N)}
\end{corollary}
...
Prove that there is no existential formula $\phi(x)$ such that $\phi(M^{\mr x})$ is the set of odd integers.
