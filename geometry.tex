% !TEX root = creche.tex
\chapter{Strongly minimal theories}\label{geometria}
...
\end{itemize}
\end{proposition}
\begin{proof} Only \ssf{1}$\IMP$\ssf{2} requires a proof.
Let $\{a_1,\dots,a_n\}=p(\U)$.
Then 

\ceq{\hfill p(x)}{\imp}{\bigvee^n_{i=1}a_i=x}

and \ssf{2} follows by compactness.
\end{proof}
...
\begin{proof} 
Only \ssf{2}$\IMP$\ssf{1} requires a proof.
...
Then $a\notin f^{-1}[M]$, so $f^{-1}[M]$ is a model that contradicts \ssf{3}.
\end{proof}
...
Properties \ssf{1}-\ssf{4} say that $\acl(\mbox{-})$ is a closure operator with finite character.

\begin{proof}
Properties \ssf{1}-\ssf{3} are obvious, \ssf{4} follows from \ssf{5} which in turn follows from Theorem~\ref{thm_fmgalois}.
\end{proof}
...
\begin{exercise}
  ...
  Prove that $\Aut(\U/A)$ acts on $Q$ transistively, i.e.\@ any two types in $Q$ are conjugated.
\end{exercise}
...
\end{proposition}
\begin{proof}
Implications \ssf{1}$\IMP$\ssf{2}$\IMP$\ssf{3} are immediate, we prove \ssf{3}$\IMP$\ssf{1}.
Let $\phi({\mr x}\,;{\gr z})\in L$ and let $N$ be the elementary extension given by \ssf{3}.
...
\ceq{\hfill M}
{\models}
{\A {\gr z}\ \Big[\E^{\le n}{\mr x}\;\phi({\mr x}\,;{\gr z})\ \vee\ \E^{\le n}{\mr x}\;\neg\phi({\mr x}\,;{\gr z})\Big].}
...
\ceq{\hfill {\mr b}\in\acl (B,{\mr a})}{\IFF}{{\mr a}\in\acl(B,{\mr b})}
...
\ceq{\hfill\phi({\mr a},{\mr b})}{\wedge}{\E^{\le n}{\mr x}\,\phi({\mr x},{\mr b}).}

As ${\mr b}\notin\acl(B,{\mr a})$, the formula

\ceq{\hfill\psi({\mr a},{\mr y})\medrel{=}\phi({\mr a},{\mr y})}{\wedge}{\E^{\le n}{\mr x}\,\phi({\mr x},{\mr y}).}
...
\begin{proof}
  ...
  As for \ssf{2}, assume for a contradiction that $A,B\subseteq C$ are two bases of $C$ and that $|A|<|B|$.
  ...
  \ceq{\hfill D}{=}{\bigcup_{{\mr a}\in A}D_{\mr a}.}
...
\begin{proof}
  \ssf{2}$\IMP$\ssf{1}.
  Assume \ssf{2} and let $k:M\imp N$ be an elementary map of cardinality $<|N|$ and let ${\mr b}\in M$.
...
%\begin{exercise}
%Siano $a\equiv_A c$ due tuple e siano $b\notin\acl(A,a)$ e $d\notin\acl(A, c)$ due elementi.
%Si dimostri che $a,b\equiv_A c,d$.
%\end{exercise}
% 
% \begin{exercise} 
...
\p
We cannot use $\UU^\eq$ nor canonical bases yet so, to keep the proofs concise, we work under the assumption that $\acl\varnothing$ is infinite (we want that $T$ weakly eliminates imaginary, see Chapter~\ref{eq}). Necessarily, we define linearity without referring to canonical bases but the notion is the usual one.\STOP
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\PARAGRAPH{A lemma.}%
\Proposition Let $T$ be linear (and recall that in this section we always assume $\acl\varnothing$ infinite). Then every planar curve over $A$ is similar to a planar curve that is over some tuple $\aa\sbs\acl A$ of dimension $\le1$.
...
\Proof Let $\psi(\cc\,{\mr x}\,y)$ be a planar curve, where $\cc$ are all the parameters occurring in the formula. Fi{\mr x} some $\bb\equiv\cc$ such that $\bb\nonfork_\varnothing E$. Let $\phi(\ee\,\aa\,{\mr x}\,y)$ be a planar curve similar to $\psi(\bb\,{\mr x}\,y)$, say equal up to $n$ points, for some $\ee\sbs E$ and some $\aa$ such that $\dim_{\ee}\aa\le 1$. There is a first-order formula $\theta(\ee\,\bb)$ saying that $\phi(\ee\,\zz\,{\mr x}\,y)$ is equal to $\psi(\bb\,{\mr x}\,y)$ up to $n$ points for some $\zz$ such that $\dim_{\ee}\zz\le 1$. By Proposition~\ref{Independence and finite satisfiability} above, there is $\ee'\sbs\acl\varnothing$ such that $\theta(\ee'\,\bb)$. Let $\aa'$ witness $\E \zz$ in  $\theta(\ee'\,\bb)$. Then $\phi(\ee'\,\aa'\,{\mr x}\,y)$ is a formula over $\aa'$, where $\dim\aa'\le1$, and is similar to $\psi(\bb\,{\mr x}\,y)$. Finally, by homogeneity, we map automorphically $\cc$ to $\bb$. The image $\aa''$ of $\aa'$ under this automorphism gives the required planar curve $\phi(\ee'\,\aa'\,{\mr x}\,y)$.
...
\Proof  We prove first a slightly different claim: for every non-negative integer $k$ there is a parameter-free formula $\psi(\zz)$ such that $\psi(\aa)$ holds if and only if $\phi(\aa\,\bar\UU)$ has dimension $k$ over $\aa$. By what observed above, for positive $k$ we can replace $\aa$ with $\varnothing$.
\p
