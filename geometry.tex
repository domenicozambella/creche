%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\chapter{Geometry and dimension}\label{geometria}% OMITTING
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In this chapter we fix a signature $L$, a complete theory $T$ without finite models, and a saturated model $\U$ of inaccessible cardinality $\kappa$ larger than $|L|$. Notation and implicit assumptions are as presented in Section~\ref{saturation}.\ref{monster}.
...
For every $A\subseteq\U$ and every type $p(u)\subseteq L(A)$, where $u$ is a tuple of variables finite length, the following are equivalent
\begin{itemize} 
\item[1] $\E^{\le n}u\; p(u)$;
\item[2] $\E^{\le n}u\;\phi(u)$ for some $\phi(u)$ which is conjunction of formulas $p(u)$.
\end{itemize}
\end{proposition}
\begin{proof} The non trivial implication is \ssf{1}$\IMP$\ssf{2}. Assume \ssf{1}, then the following type is inconsistent
...
\begin{itemize}
\item[1.] $a$ is solution of some algebraic formula  $\phi(u)\in L(A)$, with $|u|=n$;
...
\item[2.] there is no consistent formula $\psi(z_1,\dots,z_n)\in L(A)$ such that \smash{$\displaystyle\psi(z_1,\dots,z_n)\imp\bigwedge^n_{i=1}\phi(z_i)$}.
\end{itemize}
Hint: for some $A\subseteq N$, let $c=\<c_i:i<\lambda\>$ be an enumeration of $N^{|x|}$. Let $p(z)=\tp(c/A)$.  Prove that \ssf{2} implies the consistency of the type $p(z)\,\cup\, \big\{\neg\phi(z_i)\, :\, i<\lambda\big\}$ and deduce the existence of the required $M$.\QED
\end{exercise}
...
\end{proposition}
\begin{proof}
Implications \ssf{1}$\IMP$\ssf{2}$\IMP$\ssf{3} are immediate, we prove \ssf{3}$\IMP$\ssf{1}.  Fix a formula $\phi({\mr x}\,;{\gr z})\in L$ and an elementary extension $N$ as required by \ssf{2}. Let $p({\gr z})\subseteq L$ be the following type

\hfil$p({\gr z})\ \ \deq\ \ \Big\{ \E^{> n}{\mr x}\;\phi({\mr x}\,;{\gr z})\ \wedge\ \E^{> n}{\mr x}\;\neg\phi({\mr x}\,;{\gr z}) \ \ :\ \ n\in\omega\Big\}$.
