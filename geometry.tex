...
\begin{proof}
  ...
  As $b\in\acl(A)$ but $b\notin\acl(B\sm b)$ then $A\nsubseteq\acl(B\sm b)$.
  Then there is an $a\in A$ such that $a\notin\acl(B\sm b)$.
  By Lemma~\ref{indipendenza+1} $B\sm b, a$ is an independent set that, by what proved above, is contained in a base $B'$.
  As $|A|<|B'|$, we can iterate the procedure.
  After $|A|+1$ iterations we reach a contradiction.
\end{proof}
