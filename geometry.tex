...
In this chapter we fix a signature $L$, a complete theory $T$ without finite models, and a saturated model $\U$ of inaccessible cardinality $\kappa$ larger than $|L|$. The notation and implicit assumptions are as in Section~\hyperref[monster]{\ref*{saturation}.\ref*{monster}}.
...
We write \emph{acl$(\Aa)$} for the \emph{algebraic closure of $\Aa$}, that is, the set of all the elements that are algebraic over $\Aa$. If $\Aa=\acl(\Aa)$, we say that $\Aa$ is \emph{algebraically closed}. The \emph{definable closure of $\Aa$\/} is defined similarly and is denoted by \emph{dcl$(\Aa)$}. 

Let $x$ be a finite tuple of variables. Formulas $\phi(x)\in L(A)$, or types $p(x)\subseteq L(A)$, with finitely many solutions are called \emph{algebraic}.
...
\end{itemize}
\end{proposition}
\begin{proof} The non trivial implication is \ssf{1}$\IMP$\ssf{2}. Let $\{a_i,\dots,a_n\}$ be all the solutions of $p(x)$. Then 
...
Any $\bar a\models p(\bar x)$ enumerates a model $A$-isomorphic to $N$.\QED%la cui consistenza dev'essere verificata. Si pu\`o usare l'esercizio~\ref{pofu2} oppure mostrare che il tipo qui sopra \`e chiuso per congiunzione e poi applicare l'esercizio~\ref{pofu} (che \`e pi\`u semplice).
\end{exercise}
...
\end{itemize}
Hint: let $\bar c$ be an enumeration of $N^{|x|}$, where $N$ is any model containing $A$. Let $p(\bar z)=\tp(\bar c/A)$.  Prove that \ssf{2} implies the consistency of $p(\bar z)\,\cup\, \big\{\neg\phi(z_i)\, :\, i<|\bar z|\big\}$ and deduce the existence of the required $M$.\QED
\end{exercise}
...
When ${\mr a}\notin\acl B$ we say that ${\mr a}$ is \emph{algebraically independent from\/} $B$. We say that $B$ is an \emph{algebraically independent set\/} if every ${\mr a}\in B$ is independent from $B\sm\big\{{\mr a}\big\}$. Below we shall abbreviate $B\cup\big\{{\mr a}\big\}$ by \emph{$B,{\mr a}$} and  $B\sm\big\{{\mr a}\big\}$ by \emph{$B\sm a$}. 
...
($T$ strongly minimal.) Let $k$ be an elementary map. Then $k\cup\big\{\<{\mr b},{\mr c}\>\big\}$ is also an elementary map for every ${\mr b}\notin\acl(\dom k)$ and ${\mr c}\notin\acl(\range k)$.
\end{proposition}
\begin{proof}
