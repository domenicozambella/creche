...
\begin{exercise}\label{pofu}
  Let $\phi(\bar x)\in L(A)$ be a consistent formula and let

  \ceq{\hfill\psi(y)}{=}{\A\bar x\ \Big[\phi(\bar x)\imp\bigvee_{i<|\bar x|} x_i=y\Big]}

  Prove that, if consistent, $\psi(y)$ is algebraic. 
  Give both a direct proof and one using the exercise above.\QED
\end{exercise}
...
\begin{exercise}\label{ex_acl_intersezione_2_modelli_iso}
  ...
  \ :\ 
  b\in N{\sm}\acl(A),\ \psi(y,x){\in} L(A),\ n{<}\omega\Big\}$.\QED
\end{exercise}
...
\ceq{\hfill\psi(z_1,\dots,z_n)}{\imp}{\bigvee^n_{i=1}\phi(z_i).}
\end{itemize}
...
This proves \ssf{1} and it is immediate that the argument can be generalized as required in \ssf{3}.
