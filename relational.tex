...
In the first section we prove that theory of \textit{dense linear orders without endpoints\/} is \textit{$\omega\jj$cate\-gorical}.
...
In the second section we apply the same technique to prove that the theory of the \textit{random graph\/} is $\omega\mbox{-}$categorical.
...
It is easy to check that setting $h=k\cup\big\{\<b,c\>\big\}$ we obtain the required extension.
\end{proof}
...
In many other contexts, as soon as we have an extension lemma, we can apply verbatim the same construction (cfr.\@ Theorem~\ref{thm_riccozigzag}).

\begin{proof}
Fix some enumerations of $M$ and $N$, say $\<a_i:i<\omega\>$, respectively $\<b_i:i<\omega\>$.
...
We begin by setting $g_0=k$.
The inductive step consists of two half-steps that we may call the \textit{forth-step} and \textit{back-step}.
...
\begin{proof}
Let $M$ and $N$ be any two models of $T_{\rm dlo}$.
...
Let $L$ be the language of strict orders augmented with countably many constants $\big\{c_i: i\in\omega\big\}$.
...
For $N$ a rightly chosen model of $T$, prove the statement in Lemma~\ref{lem_ordinericco}, with $M$ any model of $T$.\QED
\end{exercise}
...
The scheme of axioms \ssf{r$_n$} plays the same role of density in the previous section.
...
Then there is a partial isomorphism $h:M\imp N$ that extends $k$ and is defined in $b$.
\end{lemma}
\begin{proof} The schema of the proof is the same as in Lemma~\ref{lem_ordinericco}, so we use the same notation.
...
Let $N\models r(n,m)$ hold if the $n\jj$th prime number divides $m$ or vice versa $m\jj$th prime number divides $n$.
\end{proof}
...
Hint: given a random graph $N$ and a set $P$ of cardinality $n+1$ show that you can add edges to $M\sqcup P$ and make it satisfy axiom \ssf{r$_n$} but not \ssf{r$_{n+1}$}.\QED
\end{exercise}
...
\end{itemize}
Hint: for \ssf{2}$\IMP$\ssf{1} first show that if $r(a,M)=\0$ for some $a\in M$ then the graph is null and if $\{b\}\cup r(b,M)=M$ for some $b$, then the graph is complete.
