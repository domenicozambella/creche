...
\ceq{\hfill X}{=}{\big\{x\in\dom(k)\;:\;x<b\big\};}

\ceq{\hfill Y}{=}{\big\{y\in\dom(k)\;:\;b<y\big\}.}
...
Let $L$ be the language of strict orders augmented with countably many constants $\big\{c_i: i\in\omega\big\}$. Let $T$ be the theory that extends $T_{\rm dlo}$ with the axioms $c_i<c_{i+1}$ for all $i$. Prove that $T$ is complete. Exhibit three non isomorphic models of this theory. For $N$ a rightly chosen model of $T$, prove the statement in Lemma~\ref{lem_ordinericco}, with $M$ any model of $T$.\QED
\end{exercise}
...
The theory of graphs is denoted by \emph{$T_{\textrm{gph}}$} and the theory of random graphs is denoted by \emph{$T_{\textrm{rg}}$}. The scheme of axioms \ssf{r$_n$} plays the same role of density in the previous section.  It says that given two non empty disjoint sets $X$ and $Y$ of cardinality $\le n$ there is a vertex $z$ that is adjacent to all vertices in $X$ and to no vertex in $Y$. Note that it might well be that $z\in Y$, however $z\in X$ would contradict \ssf{1}.
...
As $k[A]\cup k[B]=\range k$, it is immediate to verify that $h=k\cup\big\{\<b,c\>\big\}$ is the require extension.\QED
\end{proof}
...
\begin{exercise}
Let $a,b,c\in N\models T_{\rm rg}$. Prove that if $r(a,N)=r(b,N)\cap r(c,N)$ then $a=b=c$.\QED
\end{exercise}
...
\begin{exercise}
Prove that for every $b\in N\models T_{\rm rg}$ the set $r(b,N)$ is a random graph. Is every random  graph $M\subseteq N$ of the form $r(b,N)$ for some $b\in N$~?\QED
\end{exercise}
...
\begin{exercise}
Prove that $T_{\rm rg}$ is not finitely axiomatizable. Hint. Given a random graph $N$ and a set $P$ of cardinality $n+1$ show that you can add arcs to $M\sqcup P$ and make it satisfy axiom \ssf{r$_n$} but not \ssf{r$_{n+1}$}.\QED
\end{exercise}
...
\begin{exercise}
Let $M$ be a graph which is neither null nor complete. Prove that the following are equivalent
\begin{itemize}
\item[1.] $M$ is a random graph;
\item[2.] if $M_1,M_2\subseteq M$ are such that $M_1\cup M_2=M$, then $M_1\simeq M$ or $M_2\simeq M$.\QED
\end{itemize}
%Hint. For \ssf{2}$\IMP$\sff{1} suppose for a contradiction \ssf{r$_n$} fails for some $n$ and obtain a contradiction reasoning by induction. Suppose \ssf{r$_n$} and \ssf{r$_{n+1}$} fails. Let   
\end{exercise}
