...
\end{itemize}
Hint. For \ssf{2}$\IMP$\ssf{1} first show that if $r(a,M)=\0$ for some $a\in M$ then the graph is null and if $r(b,M)=M\sm\{b\}$ for some $b$, then the graph is complete. Also note that \ssf{2} implies that any finite partition of $M$ contain an element isomorphic to $M$. It follows that if \ssf{2} and there is a finite $A$ such that $\bigcap_{a\in A}r(a, M)=\0$ then the graph is null and if there is a finite $B$ such that $B\cup\bigcup_{b\in B}r(b,M)=M$ then the graph is complete. 
