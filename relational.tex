...
Every finite partial isomorphism $k:M\imp N$ between countable models of $T_{\rm dlo}$ extends to an isomorphism $g:M\imp N$.
\end{theorem}
...
\{b\}\cup r(b,M)=M$ for some $b$ then the graph is complete. Also note that \ssf{2} implies that a similar property holds for any finite partition of $M$, not just bi-partitions. It follows that if \ssf{2} and there is a finite $A$ such that $\bigcap_{a\in A}r(a, M)=\0$ then the graph is null and if there is a finite $B$ such that $B\cup\bigcup_{b\in B}r(b,M)=M$ then the graph is complete. 

Suppose $M$ is not a random graph. Fix some finite, disjoint $A$ and $B$ such that no $c$ satisfies both $r(A,c)= A$ and $r(B,c)=\0$. Let $M_1=\{c:r(A,c)\neq A\}\sm B$ and $M_2=B\cup\{c:r(B,c)\neq\0\}\sm M_1$. This is a partition of $M$. Now note that 
