...
Prove that for every graph $M$ the following are equivalent
\begin{itemize}
...
\end{itemize}
Hint. For \ssf{2}$\IMP$\ssf{1} first show that if $r(a,M)=\0$ for some $a\in M$ then the graph is null and if $\
\{b\}\cup r(b,M)=M$ for some $b$ then the graph is complete. Also note that \ssf{2} implies that a similar property holds for any finite partition of $M$, not just bi-partitions. It follows that if \ssf{2} and there is a finite $A$ such that $\bigwedge_{a\in A}r(a, M)=\0$ then the graph is null and if there is a finite $B$ such that $B\cup\bigvee_{b\in B}r(b,M)=M$ then the graph is complete. 
...
%Hint. For \ssf{2}$\IMP$\ssf{1} first show that if there is a point jointed to all the other the the graph is complete and that if there is an isolate node then the graph is empty. If neither is the case prove \ssf{r$_n$} by induction. Suppose \ssf{r$_n$} and \ssf{r$_{n+1}$} fails. Let $A$ and $B$ a counter example to \ssf{r$_{n+1}$}. Then Let $M_1$ be the union of $B$ and the set of point correctly jointed to $A$. Let $M_2$ be the union of $A$ and set of those correctly jointed to $B$. Check that these is a partition of $M$ and that both $M_1$ and $M_2$ falsify \ssf{r$_n$}.\QED
\end{exercise}
