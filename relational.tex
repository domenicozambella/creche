...
\begin{proof}
Let $M$ and $N$ be any two models of $T_{\rm dlo}$. Applying the upward and/or downward L\"owenheim-Skolem theorem, we may assume they both have cardinality $\lambda$. Note that here we use that $N$ and $N$ are both infinite that $\lambda\ge|L|$. Hence $M\simeq N$ and in particular $M\equiv N$.
\end{proof}
...
Strictly speaking, the axioms \ssf{r$_n$} do not mention the cases when $A^+$ or $A^-$ are empty. But as it is evident that random graphs are infinite, we can deal with them by adding redundant elements.
