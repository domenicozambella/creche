...
The terminology below non-standard.
In the literature some distinguished authors write \textit{quasi-non-dividing\/} for \textit{persistent}.
...
A set $\mrD\subseteq\U^{\mr x}$ is \emph{strongly generic\/} if the intersection of any initely many translations of $\mrD$ is generic.
...
\begin{theorem}\label{thm_generic_invariant}
  Let $G$,  $\mrX$, and $\phi({\mr x}\,;{\gr z})\in L$ be given.
  Let $\psi({\mr x})$ be a generic formula.
  Then the type containing $\psi({\mr x})$ and all strongly generic formulas $\phi({\mr x})\in L_\phi(\U)$ is finitely consistent in $\mrX$.
\end{theorem}
