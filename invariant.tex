...
One may be tempted to require that all formula in the type define an invariant set~--~but this is to strong a requirement.
Alternatively, one could require that the set defined by the type is invariant.
This is closer in spirit to the definition below but, as stated, it is an empty requirement for types that are not realized in $\U$.
...
To obtain closure under intersection, we need to press the concept to the next level of complexity.
...
We may abbreviate this by writing \emph{$A$-invariant.}
