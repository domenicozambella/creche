...
% Let $p({\mr x})\in L(\U)$. Let $\V$ be a saturated elementary extension of $\U$. Prove that the following are equivalent:
% \begin{itemize}
% \item[1.] the type $p({\mr x})$ is quasi-invariant over $A$;
...
We say that a type $p({\mr x})$ is  \emph{finitely satisfiable\/} in $A$ if $\phi({\mr\U})\cap A^{|{\mr x}|}\neq\0$ for every $\phi({\mr x})$ that is a conjunction of formulas in $p({\mr x})$.
...
\begin{proof}
Clearly, the very same ${\mr a}\in A^{|{\mr x}|}$ that satisfies $\phi({\mr x})$ satisfies also all $\Aut(\U/A)$-translates of $\phi({\mr x})$.
\end{proof}
...
We need to prove that $\phi({\gr a},\,{\mr b_1})\iff\phi({\gr a},\,{\mr b_2})$ for every  $\phi({\gr z},\,{\mr x})\in L(A)$. Let $\psi({\mr x})\in L(A)$ be such that $\phi({\gr a},\,{\mr \U})=_A\psi({\mr \U})$. 
...
is also called a \emph{$k$-coloring\/} of $X^{[n]}$.
...
\section{Hindmann theorem from coheir sequences}
...
Suppose that for every ${\mr a},{\mr b}\in\U$ the type ${\mr a}\nonforkc_G{\mr x}\equiv_G{\mr b}$ is complete.
...
We will prove that $q({\mr x})=\tp(a/G)$ in three steps.
...
Note that, by extension, see Lemma~\ref{lem_coheir_independence}, we can choose $c'$ such that ${\mr a},{\mr b},{\mr b'}\nonforkc_G{\mr c'}$.
