...
The properties above may be attributed to formulas (as these are identified with the set they define).
...
\begin{proof}
  ...
  \ssf3$\IMP$\ssf1. Let $p({\mr x})$ be maximal among the types that contain ${\mr x}\in\mrD$ and are such that $\theta({\mr\U})$ satisfies \ssf3 for every $\theta({\mr x})\in p$.
