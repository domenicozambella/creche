...
\ceq{\ssf{it2.}\hfill  {\gr a}\equiv_A{\gr b}}{\IMP}{\Big(\phi({\mr x}\,;{\gr a})\in p\,\IFF\,\phi({\mr x}\,;{\gr b})\in p\Big)}

% In words we say that $p({\mr x})\subseteq L(\U)$ \emph{does not split\/} over $A$.

If $p({\mr x})$ is a global $\phi$-types types \ssf{it2} is equivalent to requiring that for every ${\gr a},{\gr b}\in\U^{\gr z}$
...
\begin{proof}
  Suppose not.
  By \ssf{it3} there are ${\gr a}\equiv_A{\gr b}$ and $\phi({\mr x}\,;{\gr z})\in L$ such that $p({\mr x})\proves\phi({\mr x}\,;{\gr a})\niff\phi({\mr x}\,;{\gr b})$.
...
\begin{exercise}\label{ex_heir_type}
  Show that for every $b\in\U^z$ there is a type $p(x\,;z)\subseteq L(M)$ such that $a\cnonfork b'$ if and only if $a,b'\models p(x\,;z)$.
\end{exercise}
