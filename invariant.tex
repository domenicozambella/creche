...
\begin{proof}
  ...
  Let $p(z)=\tp(c/M,{\gr b})$.
  We can take $\V$ to be the structure enumerated by the realization of the following type 

  \ceq{\hfill p(z)}{\cup}{\Big\{\neg\phi({\mr a},z)\ :\ \phi({\mr x},z)\in L(M),\ p(z)\imp\phi(M,z)=\0\Big\}.}

  We only need to prove consistency.
  If inconsistent, then 

  \ceq{\hfill\theta(z)}{\imp}{\bigvee_{i=1}^n\phi_i({\mr a},z)}
  
for some $\theta(z)\in p$ and some $\phi_i({\mr x},z)$ such that $p(z)\imp\phi_i(M,z)=\0$.
