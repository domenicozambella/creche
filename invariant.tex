...
Their terminology has good motivations, but it is a mouthful in this context.
...
\begin{corollary}\label{corol_str_gen}
  ...
  Then $q({\mr x})$ is finitely consistent in $\mrX$, strongly generic, and invariant.
\end{corollary}

\begin{proof}
  Strong genericity is an immediate consequence of Lemma~\ref{lem_strongly_generic}.
  Finite concistency follows from genericity.
  As for invariance, note that any translate of a strongly generic formula is also strongly generic.
\end{proof}

\begin{remark}
  Any finitely consistent type $p({\mr x})\subseteq L_\phi(\U)$ containing $q({\mr x})$ of Corollary~\ref{corol_str_gen} is weakly persistent.
  In fact, concistency implies that for every $\theta({\mr x})\in p$ the formula $\neg\theta({\mr x})$ is not strongly generic.
  Then $\theta({\mr x})$ is weakly persistent.
\end{remark}

\begin{exercise}
  Let $G$, $\X$, and $\phi(x,;z)\in L$ be given.
  Let $p(x)\subseteq L_\phi(\U)$ be generic, invariant and closed under conjunctions.
  Prove that $p(x)$ is strongly generic.
\end{exercise}
