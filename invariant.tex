...
\ceq{\ssf{it2.}\hfill  {\gr a}\equiv_A{\gr b}}{\IMP}{\Big(\phi({\mr x}\,;{\gr a})\in p\,\IFF\,\phi({\mr x}\,;{\gr b})\in p\Big)}\hfill for every $\phi({\mr x}\,;{\gr z})\in L$ and ${\gr a},{\gr b}\in\U^{\gr z}$.

% In words we say that $p({\mr x})\subseteq L(\U)$ \emph{does not split\/} over $A$.

If $p(x)$ is a global $\phi$-types types \ssf{it2} is equivalent to
