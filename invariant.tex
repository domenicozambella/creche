...
\ceq{\hfill\emph{${\gr a}\nonforkc_A{\mr x}$}}{=}{\Big\{\phi({\gr a}\,;{\mr x})\ :\ \phi({\gr z}\,;{\mr x})\in L(A)\textrm{ and }A^{|{\mr x}|}\subseteq\phi({\gr a}\,;{\mr \U})\Big\}}

and we will use the symbol \emph{${\gr a}\nonforkc_A{\mr x}\equiv_A{\mr b}$} for the union of the types ${\gr a}\nonforkc_A{\mr x}$ and $\tp({\mr b}/A)$.\QED
\end{definition}
...
\ssf{2}$\IMP$\ssf{1}.
Let $q({\mr x})=\{\phi({\mr x})\in L({\mr\bar c})\,:\, \phi({\mr c_n}) \textrm{ holds for cofinitely many } n\}$. From  \ssf{2} it follows that $q({\mr x})$ is finitely satisfied in $A$. Then ${\mr\bar c}$ a coheir sequence of any global type that extends $q({\mr x})$.
\end{proof}
