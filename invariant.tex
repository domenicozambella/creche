...
% Suppose that $\grD=\phi({\mr a}\,;\U^{\gr z})$ for some $\phi({\mr x}\,;{\gr z})\in L$ and that $\phi(\U^{\mr x};\U^{\gr z})$ is invariant (this is always the case when $G$ acts by automorphisms).
...
Unfortunately this is not true -- we need a stronger property, see \ssf2 of Theorem~\ref{thm_generic_invariant2}.
...
\begin{proof}
  ...
  \ssf3$\IMP$\ssf1.
  Let $p({\mr x})$ be maximal among the $\phi$-types that contain ${\mr x}\in\mrD$ and are such that $\theta(\U^{\mr x})$ satisfies \ssf3 for every $\theta({\mr x})$ that is conjunction of formulas in $p({\mr x})$.
  ...
  This is a contradiction because ${\mr\C_1},\dots,{\mr\C_n}$ cover $\psi(\U^{\mr x})$ which satisfies \ssf3.
...
Note that by homogeneity \ssf{is1} in Section~\ref{invariant_sets} can  be restated as

\ceq{\ssf{is2.}\hfill{\gr a}\equiv_A{\gr b}}{\IMP}{{\gr a}\in\grD\iff{\gr b}\in\grD}\hfill for all ${\gr a},{\gr b}\in\U^{\gr z}$.
