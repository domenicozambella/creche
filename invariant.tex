...
%
In general it is not symmetric, that is ${\mr b}\cnonfork_A{\gr a}$ is not 
equivalent to ${\gr a}\cnonfork_A{\mr b}$ 
...
such that ${\mr a_i}\cnonfork_M{\gr b}$ and ${\mr a_1}\equiv_M{\mr a_2}$.
...
Stationarity of $\cnonfork_A$ over every set $A$, or just over every model, is equivalent to the stability of $T$, see Section~\hyperref[stable_teories]{\ref*{external}.\ref*{stable_teories}}.
However, there are many more theories where the assumption holds for some particular set $A$.
For example, if every subset of $A^n$ is $A$-definable, $\cnonfork_A$ is clearly $n$-stationary.
