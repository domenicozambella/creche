...
We say that ${\mr\bar c}=\<{\mr c_i}:i<\alpha\>$ is a \emph{Morley sequence\/} of $p({\mr x})$ over $M$ if for every $i<\alpha$

\ceq{\ssf{Ms.}\hfill {\mr c_i}}{\models}{p_{\restriction  M,\,{\mr c_{\restriction i}}}({\mr x})}.

We usually require that $p({\mr x})$ is invariant over $M$.
In particular, when $p({\mr x})$ is finitely satisfiable in $M$ we say that ${\mr\bar c}$ is a \emph{coheir sequence\/} of $p({\mr x})$ over $M$. 

When we say that ${\mr\bar c}$ is a coheir sequence over $M$ we mean that there is a type $p({\mr x})$ that is finitely satisfiable in $M$ such that ${\mr\bar c}$ is a \emph{coheir sequence\/} of $p({\mr x})$.
