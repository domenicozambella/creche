...
% -- the generality is only required in Section~\ref{}.
% The terms invariant, orbit, and the few other indroduced below all refer to the action of $G$ on $\U$.
Let $\mrX\subseteq\U^{\mr x}$ be a set that is type-definable over some small set of parameters and $G$-invariant.
...
When the properties above are attributed to a type $p({\mr x})$, it is understood that it holds for every $\phi({\mr x})$ that is a conjunction of formulas in $p({\mr x})$.
...
A \emph{$\phi$-formula\/} is a formula the form $\theta({\mr\U}\,;{\gr\bar b})$ for some ${\gr\bar b}\in\U^{\gr\bar z}$ and $\theta({\mr x}\,;{\gr\bar z})$ a Boolean combination of formulas $\phi({\mr x}\,;{\gr z_i})$, where ${\gr\bar z}={\gr z_1},\dots,{\gr z_n}$.
A set is \emph{$\phi$-definable\/} if it is definable by a $\phi$-formula.
We write \emph{$L_\phi(\U)$\/} for the set of $\phi$-formulas.
Recall that we are working under the assumption that the action of $G$ preserves the truth of $\phi({\mr x}\,;{\gr z})$.
This ensures that the action of $G$ maps $\phi$-definable sets into  $\phi$-definable sets.
...
\begin{proof}
  ...
  By \ssf3 $p({\mr x})$ is finitely consistent in   $\neg\mrD$, a contradiction..
