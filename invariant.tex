...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Invariance sets and types}\label{invariant_sets}

Let $G$ be a group that acts on $\U$.
...
% Suppose that $\grD=\phi({\mr a}\,;{\gr\U})$ for some $\phi({\mr x}\,;{\gr z})\in L$ and that $\phi({\mr\U}\,;{\gr\U})$ is invariant (this is always the case when $G$ acts by automorphisms).
...
We may sometimes identify $S_\phi(\Aa)$ with a subset of $\Aa^{\gr z}$.
...
\end{proposition}
\begin{proof}
By \ssf{is3}, sets that are invariant over $A$ are unions of equivalence classes of the relation $\equiv_A$, that is, unions of sets of the form $p({\gr\U})$ where $p({\gr z})\in S\/(A)$.
...
Here examine invariance using the definable sets that are implied by $p({\mr x})$.

Let $G$ be a group that acts on $\U$.
Let $\phi({\mr x}\,;{\gr z})\in L$ be such that $\phi({\mr\U}\,;{\gr\U})$ is invariant under the action of $G$.
(This assumpion is trivial when $G$ acts by automorphisms.)
...
The same properties may be attributed to formulas (a formula has the property of the set it defines).
When attributed to a type, it is understood that every conjunction of formulas in the type has the property.

\noindent\llap{\textcolor{red}{\Large\warning}\kern1.5ex}\ignorespaces
The above terminology is non-standard.
...
A set is \emph{$\phi$-definable\/} if it is of the form $\theta(\mrU\,;{\gr\bar b})$ for some $\phi$-formula $\theta({\mr x}\,;{\gr\bar z})$ and ${\gr\bar b}={\gr b_1},\dots,{\gr b_n}$.

Our assumptions on $\phi$ ensure that $g[\theta(\mrU\,;{\gr\bar b})]=\theta(\mrU\,;g{\gr\bar b})$ for every $g\in G$.
...
\begin{proof}
  \ssf1$\IMP$\ssf2. Let ${\mr\D_1},\dots,{\mr\D_n}$ be translations of $\mrD$ that cover $\mrX$.
  ...
  \ssf3$\IMP$\ssf4. Let $\mrD$ be defined by a conjunction of formulas in $p({\mr x})$.
  ...
  \ssf5$\IMP$\ssf2.  Trivial.

  \ssf2$\IMP$\ssf5.  By the same argument as in \ssf3$\IMP$\ssf4.
\end{proof}
...
\begin{proof}
  ...
  \ssf3$\IMP$\ssf1. Let $p({\mr x})$ be maximal among the types that contain ${\mr x}\in\mrD$ and are such that $\theta(\mrU)$ satisfies \ssf3 for every $\theta({\mr x})\in p$.
  We claim that $p$ is a complete type.
  Suppose for a contradiction that $\theta({\mr x}),\neg\theta({\mr x})\notin p$.
  By maximality there is some $\psi({\mr x})\in p$ and some ${\mr\C_1},\dots,{\mr\C_n}$ that cover both $\psi(\mrU)\cap\theta(\mrU)$ and $\psi(\mrU)\smallsetminus\theta(\mrU)$ and are such that no ${\mr\C_i}$ is persistent.
  This is a contradiction because ${\mr\C_1},\dots,{\mr\C_n}$ cover $\psi(\mrU)$ which satisfies \ssf3.
...
%
In fact, its completeness is a strong property.
...
If for every $\phi({\mr x})\in L(\U)$ there is a formula $\psi({\mr x})\in L(M)$ such that $\phi({\mr M})=\psi({\mr M})$ then $\cnonfork_M$ is $n$-stationary.
\end{proposition}
...
We need to prove that $\phi({\mr a_1}\,;{\gr b})\iff\phi({\mr a_2}\,;{\gr b})$ 
for every  $\phi({\mr x}\,;{\gr z})\in L(M)$.
