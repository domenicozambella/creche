...
We say that $\grD$ is \emph{invariant\/} tout court if it is invariant over some (small) set $A$. As we require $\kappa$ to be inaccessible, there are exactly $\kappa$ invariant sets.
...
Let $p({\mr x})\subseteq L(\U)$ be a consistent type (at a first reading the reader may assume that $p({\mr x})$ is a global types). We say that $p({\mr x})$ is  \emph{invariant over $A$}, or \emph{$A\jj$invariant}, if for every formula $\phi({\mr x}\,;{\gr z})\in L$, 

\ceq{\ssf{it1.}\hfill \phi({\mr x}\,;{\gr a})\in p}{\IFF}{\phi({\mr x}\,;f{\gr a})\in p}\hfill for every ${\gr a}\in\U^{|{\gr z}|}$ and every $f\in\Aut(\U/A)$.

A global $\phi$-type can be identified with $\D_{p,\phi}$ and a global type can be identified with the collection of all these sets. The notions of invariance for types and sets coincide.
...
Note that \ssf{3} applies to global types $p({\mr x})\in S(\U)$ but not to $\phi$-types.
...
The easiest way to obtain quasi-invariant types is via types that are finitely satisfiable. We say that a type \emph{$p({\mr x})$ is finitely satisfiable in $A$\/} if $\phi({\mr\U})\cap A^{|{\mr x}|}\neq\0$ for every $\phi({\mr x})$ that is a conjunction of formulas in $p({\mr x})$.
...
\begin{proof}
  ...
  \ceq{\hfill q(x)}{\cup}{\Big\{\phi(x_{\restriction I_0})\iff\phi(x_{\restriction J_0})\ :\ \phi(x_{\restriction n})\in L(A),\ \ I_0\in[I]^n,\ \ J_0\in[J]^n,\ \ n<\omega\Big\}.}
  ...
  \ceq{\ssf{\#}}{~}{\Big\{\phi_i(x_{\restriction J_0})\iff\phi_i(x_{\restriction I_0})\ :\  I_0\in[I]^n\textrm{ and } i<m\Big\}.}
