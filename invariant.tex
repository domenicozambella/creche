...
\begin{definition} 
  ...
  \ceq{\hfill\grD}{=}{\Big\{{\gr b}\in\Aa^{|{\gr z}|}\ :\ \phi({\mr x}\,;{\gr b})\in p\Big\}.}\QED
\end{definition}
...
The following is another important equivalent characterization of invariance over $A$ of a global type $p(x)\in S(\U)$ that follows easily  from \ssf{it2}
...
We say that the type $p({\mr x})\subseteq L(\U)$, typically a global type, is \emph{quasi-invariant\/} over $M$ if $\phi({\mr\U})$ is quasi-invariant over $M$  for every  conjunction $\phi({\mr x})$ of formulas in $p({\mr x})$.
...
However the correct generalization involves the notion of strong automorphism developed in Chapter~\ref{invariant}.
...
\begin{exercise}
We say that $\mrB\subseteq\U^{|{\mr x}|}$ \textit{strongly\/} quasi-invariant if for every finite cover of $\U^{|{\mr x}|}$ by finitely many definable sets $\mr\D_i$, for $i=1,\dots,n$, one set $\mrB\cap\mrD_i$ is quasi-invariant.
...
Hint: it may help to prove that if $\mrB$ is strongly quasi-invariant then for every finite definable cover $\mr\D_i$, for $i=1,\dots,n$,  some $\mrB\cap\mr\D_i$ is strongly quasi-invariant.
...
\begin{proof} 
Let $p({\mr x})\subseteq L(\U)$ be maximal among the types containing $q({\mr x})$ and finitely satisfiable in $M$.
...
%
Hence, from \ssf{2} we infer ${\mr c_m}\cnonfork_{M,\,{\mr c_{\restriction n}}}{\mr c_n}$ and conclude that $\phi({\mr x}\,;{\mr c_{\restriction n}})$ is satisfied in $M$.
