...
\begin{definition}\label{def_coheir_idepencence}
  ...
  \textrm{ and } \phi(M^{\mr x};{\gr b})=\varnothing\Big\}.}\smallskip
...
\begin{remark}\label{rem_heir_type}
  In general there is no type that says ${\mr x}\cnonfork_M{\gr z}$, see Exercise~\ref{ex_heir_coheir_counterexample}.
  However, for every ${\gr b}\in\U^{\gr z}$ there is a type that says ${\mr x}\cnonfork_M{\gr z}\equiv_M{\gr b}$, namely\smallskip

  \ceq{\hfill p({\mr x}\,;{\mr z})}{\IFF}{ \big\{\neg\phi({\mr x}\,;{\gr z})\ :\ \phi(M^{\mr x}\,;{\gr b})=\varnothing, \ \ \phi({\mr x}\,;{\gr z})\in L(M)\big\}\ \cup\ q({\gr z})}\smallskip

  where $q({\gr z})=\tp({\gr b}/M)$.
\end{remark}
...
\begin{proof}
  ...
  \ceq{\hfill\Big\{\neg\phi({\mr a},w)}{:}{\phi({\mr x},w)\in L(M),\ \ \phi(M^{\mr x},c)=\varnothing\Big\}.}

  We only need to prove finite consistency.
  If inconsistent, for some $\theta(w,{\gr z})\in p$ and some $\phi_i({\mr x},w,{\gr z})$ in the above set
