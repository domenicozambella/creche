...
In this chapter we work with $\Delta$-types where $\Delta$ may be either $L(\Aa)$ or the set of Boolean combinations of $\phi({\mr x}\,;{\gr b})$ for ${\gr b}\in\Aa^{|{\gr z}|}$ and $\phi({\mr x}\,;{\gr z})\in L$ some fixed formula. In the latter case a  $\Delta$-type is also called a \emph{$\phi$-type}. Typically $\Aa$ is either the whole of $\U$ or some small set $A\subseteq\U$. 

We denote by \emph{$S_{\phi}(A)$\/} the set of complete $\phi$-types with parameters in $A$. Types in \emph{$S_{\phi}(\U)$\/} are called \emph{global $\phi$-types}.
...
A global $\phi$-type can be identified with $\D_{p,\phi}$. A global type can be identified with the collection of all these sets. The notions of invariance coincide.
