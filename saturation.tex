...
Recall that a type $p({\mr x})\subseteq L(M)$ is \emph{finitely consistent in $M$\/} if every $\phi({\mr x})$, conjunction of formulas in $p({\mr x})$, has a solution in $M$. When $A\subseteq M$ we write $S_{\mr x}(A)$ for the set of types that are complete and finitely consistent in $M$. We never display $M$ in the notation as it will always be clear from the context. When $A$ is empty it usual to write $S_{\mr x}(T)$ for $S_{\mr x}(A)$ where $T=\Th(M)$. We write $S(A)$ for the union of $S_{\mr x}(A)$ as ${\mr x}$ ranges over all tuples of variables. Similarly for $S(T)$.
...
Assume $|L|<\lambda$ where $\lambda$ is be such that $\lambda^{<\lambda}=\lambda$. Then every structure $M$ of cardinality $\le\lambda$ has a saturated elementary extension of cardinality $\lambda$.
\end{theorem}
...
The following easy proposition is very useful. Its proof is left to the reader. Note that it would not hold without saturation: for instance, consider $\RR$ as a structure in the language of strict orders and let $q(x,y)=\tp(-1, 0\,/\,A)$, where $A=\{1/n: 0<n\}$. By quantifier elimination, $-1\equiv_{A}0$. But $0\notmodels\E y\;q(x,y)$.
