...
\item[\emph{truth}] we say that $\phi(x)$ holds if $\U\models\A x\,\phi(x)$ and similarly for other expressions such as $p(x)\imp\neg q(x)$ or $\E y\,p(x,y)$ etc., which are not first-order formulas (not even types);
\item[\emph{consistency}] we say that $\phi(x)$ is consistent if $\U\models\E x\,\phi(x)$, similarly for a type $p(x)$ or expressions as those above;
