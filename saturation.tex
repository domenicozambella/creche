...
\begin{exercise}\label{ex_Hilbert_basis}
  Prove the model theoretic version of Hilbert's Basis Theorem~\ref{Hilbert_basis} from Lemma~\ref{lem_basisthm}. Hint: use Remark~\ref{rem_normality}.\ssf{a}.\QED
\end{exercise}
...
Let $p(x)\subseteq L(A)$, with $|x|<\omega$. Prove that if $p(\U)$ is infinite then it has cardinality $\kappa$. Show that this may not be true if $x$ is an infinite tuple (i.e., $p(\U)$ may be infinite and of cardinality $<\kappa^{|x|}$).\QED 
\end{exercise}
