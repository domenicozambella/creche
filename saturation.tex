...
We shall see that some theories have saturated models of small size.
...
\begin{remark}
The reader who did not read Section~\ref{rich} may replace \ssf{2} in the theorem below with the following (and forget about $\M$).
\begin{itemize}
...
By \ssf{2}, there is an elementary embedding $h:M\hookrightarrow N$ that extends $\id_A$.
...
Moreover, all countable models of $T_{\rm dlo}$ and $T_{\rm rg}$ and all uncountable models of $T_{\rm dag}$ or $T_{\rm acf}$ are saturated. 
\end{corollary}
\begin{proof}
...
Assume $|L|\le\lambda$. Consider the category that consists of models of some theory $T_0$ and partial embeddings. Suppose $\lambda\mbox{-}$rich models exist and denote by $T_1$ their theory. Then the following are equivalent
\begin{itemize}
...
\begin{proof}\ssf{2}$\IMP$\ssf{1}.
  Let $N\models T_1$ be $\lambda\mbox{-}$saturated.
  Let $k:M\to N$ be a partial embedding of cardinality $<\lambda$
  Pick ${\mr b}\in M$.
  By elimination of quantifiers, $k:M\to N$ is an elementary map.
  Then reasoning as in the proof of Theorem~\ref{saturo->ricco} there is some ${\mr c}\in N$ such that $k\cup \{\<{\mr b},{\mr c}\>\}:M\to N$ is elementary so, in particular, a partial embedding.

  \ssf{1}$\IMP$\ssf{2}. Let $k:M\to N$ be a finite partial embedding between models of $T$. We claim that it is an elementary map. Let $M'\succeq M$ and $N'\succeq N$ are $\lambda$-saturated models. As these are $\lambda$-rich, $k:M'\to N'$ is elementary by Theorem~\ref{thm_morphism_rich_elementary}.
\end{proof}
...
However, one often omits to specify \textit{elementary}. 
We repeat Definition~\ref{def_omogenea_universale} in this specific case. 
...
We say \emph{universal\/} if it is $\lambda$-universal and of cardinality $\lambda$.\smallskip

A model $N$ is \emph{(elementarily) $\lambda$-homogeneous\/} if every elementary map $k:N\to N$ of cardinality $<\lambda$ extends to an automorphism. 
...
Let ${\mr a}$ be a tuple of elements of $N$. The \emph{orbit of ${\mr a}$ over $A$ in $N$} is the set

\ceq{\hfill\emph{$O_N({\mr a}/A)$}}{=}{\Big\{f{\mr a}\;:\; f\in\Aut(N/A)\Big\}.}
...
\begin{definition}\label{def_w_saturation}
  We say that $N$ is \emph{weakly $\lambda\mbox{-}$saturated\/} if $N$ realizes every type $p({\mr x})\subseteq L$, where $|{\mr x}|\le\lambda$, that is finitely consistent in $N$.
\end{definition}
...
Let $M$ be an arbitrary structure of cardinality $\lambda\ge|L|$, a regular cardinal.
Prove that $M$ has an homogeneous elementary extension of the same cardinality. 
\end{exercise}
...
\item[d.] if $\displaystyle p(x)\imp\bigvee_{\psi\in\Psi}\psi(x)$, where $|\Psi|<\kappa$,  then \smash{$\displaystyle p(x)\imp\bigvee^{n}_{i=1}\psi_i(x)$} for some $\psi_i\in\Psi$.
\end{itemize}
\end{remark}
...
\begin{remark}\label{rem_image_def_set}
  ...
  \ceq{\hfill f\big[\phi(\U^{\mr x};{\gr b})\big]}{=}{\big\{f{\mr a}\ :\ \phi({\mr a}\,;{\gr b}),\phantom{ff}\ {\mr a}\in\U^{{\mr x}}\big\}}

  \ceq{ }{=}{\big\{f{\mr a}\ :\ \phi(f{\mr a}\,;f{\gr b}),\ {\mr a}\in\U^{{\mr x}}\big\}}

  \ceq{ }{=}{\phi(\U^{\mr x};f{\gr b}).}

  Hence automorphisms act on definable sets in a very natural way. 
  Their action on type-definable sets is similar.
\end{remark}

We say that a set ${\mr\D}\subseteq\U^{{\mr x}}$ is \emph{invariant over $A$\/} if $f[{\mr\D}]={\mr\D}$ for every $f\in\Aut(\U/A)$.
...
\ssf{2}$\IMP$\ssf{1}\quad It suffices to show that for every formula $\psi({\mr x})\in p({\mr x})$ there is a formula $\theta({\mr x})\in L(A)$ such that $p({\mr x})\imp\theta({\mr x})\imp\psi({\mr x})$. Fix $\psi({\mr x})\in p({\mr x})$. By invariance, any $q({\mr x})\in S(A)$ consistent with $p({\mr x})$ implies $p({\mr x})$, hence
...
There is a type $q({\mr x})\subseteq L(A)$ such that\smallskip

\hfil$\A {\mr x}\,\big[\E {\gr z}\,p({\mr x}\,;{\gr z})\ \iff\ q({\mr x})\big].$\smallskip
...
\ceq{\hfill A}{=}{\Big\{1+\frac1{n+1}\ :\ n\in\omega\Big\}.} 
...
\begin{proof}[Second proof of Proposition~\ref{prop_inv_types}] \ssf{2}$\IMP$\ssf{1}\quad  Write $p({\mr x})$ as the type $q({\mr x}\,;{\gr b})$ for some $q({\mr x}\,;{\gr z})\subseteq L$ and some ${\gr b}\in\U^{\gr z}$.
Let $s({\gr z})=\tp({\gr b}/A)$.
