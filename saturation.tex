...
The following easy proposition is very useful. Its proof is left to the reader. Note that it would not hold without saturation. For a counter example consider $\RR$ as a structure in the language of strict orders and let $q(x,y)=\tp_\RR(0,1/A)$, where $A=\{1+1/n: n>1\}$.
...
\begin{exercise}
  Let $\phi(x)\in L(\U)$ and suppose that $\phi(a)\iff\phi(b)$ for every $a,b\in\U\sm A$ such that $a\equiv_Bb$. Prove that there is a formula $\psi(x)\in L(B)$ and a finite $A_0\subseteq A$ such that $\phi(\U)\sm A_0=\psi(\U)\sm A_0$.\QED
\end{exercise}
