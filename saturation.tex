...
\begin{remark}
The reader who did not read Section~\hyperref[rich]{\ref*{fraisse}.\ref*{rich}} may replace \ssf{2} in the theorem below with the following (and forget about $\M$).
\begin{itemize}
...
When the morphisms of the underlying category are the elementary maps, we say \emph{weakly $\lambda\jj$saturated} for weakly $\lambda\jj$universal (cfr.\@ Definition~\ref{def_weakly_universal}). This is not the standard definition (that is, \ssf{2} of the proposition below) but the reader can easily verify that it is equivalent by reasoning as in the proof of theorem~\ref{saturo->ricco}.
...
\item[\emph{parameters}] are always in $\U$; the symbols $A$, $B$, $C$, etc.\@ denote sets of parameters of small cardinality; calligraphic letters as $\Aa$, $\B$, $\C$, etc.\@ are used for sets of parameters of arbitrary cardinality;
...
Let $x$ be a tuple of variables. For any fixed $A\subseteq \U$ we introduce a topology on $\U^{|x|}$ that we call the \emph{topology induced by $A$} or, for short, \emph{$A\jj$topology}. 
(This is non standard terminology, not to be confused with the \textit{logic\/} $A\jj$topology in Section~\hyperref[invariant]{\ref*{invariant}.\ref*{KPtypes}}.)
The closed sets of the $A\jj$topology are those of the form $p(\U)$ where $p(x)\subseteq L(A)$ is a type over $A$.
...
Then there are a conjuction $\phi(x)$ of formulas in $p(x)$ and a conjuction $\psi(x)$ of formulas in $q(x)$ such that $\phi(x)\imp\neg\psi(x)$.
\end{proposition}
...
\item[d.] if $\displaystyle p(x)\imp\bigvee_{\psi\in\Psi}\psi(x)$, where $|\Psi|<\kappa$,  then \smash{$\displaystyle p(x)\imp\bigvee^{n}_{i=1}\psi_i(x)$} for some $\psi_i\in\Psi$.\QED
\end{itemize}
\end{remark}
...
However, in any sufficiently saturated elementary extension of $\RR$, we have $1\notmodels\E y\;q(x,y)$.
