...
\ceq{\hfill (a_1,a_2)<(b_1,b_2)}{\IFF}{a_1<b_1\ \ \textrm{or}\ \ a_1=b_1\ \textrm{e}\ a_2<b_2}
...
\ceq{ }{=}{\big\{\phantom{f}{\mr a}\ :\ \phi({\mr a}\,;f{\gr b}),\phantom{f}\ {\mr a}\in\U^{|{\mr x}|}\big\}}

Hence automorphisms act on definable sets in a very natural way. Their action of type definable sets is similar.

We say that a set, or a type-definable set $p(\U)$, is \emph{invariant over $A$\/} if $f[p(\U)]=p(\U)$ for every $f\in\Aut(\U/A)$. Equivalently, if $\orbit({\mr a}/A)\subseteq p(\U)$ for every ${\mr a}\models p({\mr x})$. By homogeneity this is equivalent to requiring that $q({\mr x})\imp p({\mr x})$ for every ${\mr a}\models p({\mr x})$ and $q({\mr x})=\tp({\mr a}/A)$. Proposition~\ref{prop_inv_types} below an important fact about invariant type-definable sets. It may clarify the proof to consider first the particular case of definable sets.
...
\ceq{\hfill p({\gr z})\wedge p({\gr z'})}{\imp}{\A {\mr x}\ \big[\phi({\mr x}\,;{\gr z})\iff\phi({\mr x}\,;{\gr z'})\big]}

By compactness there is a formula $\theta({\gr z})\in L(A)$ such that 

\ceq{\hfill \theta({\gr z})\wedge \theta({\gr z'})}{\imp}{\A {\mr x}\ \big[\phi({\mr x}\,;{\gr z})\iff\phi({\mr x}\,;{\gr z'})\big]}
...
\ssf{2}$\IMP$\ssf{1}\quad It suffices to show that for every formula $\psi(x)\in p(x)$ there is a formula $\phi(x)\in L(A)$ such that $p(x)\imp\phi(x)\imp\psi(x)$. Fix $\psi(x)\in p(x)$. By invariance, any $q(x)\in S(A)$ consistent with $p(x)$ implies $p(x)$, hence
...
\begin{proof}[Second proof of Proposition~\ref{prop_inv_types}] \ssf{2}$\IMP$\ssf{1}\quad  Write $p({\mr x})$ as the type $q({\mr x}\,;{\gr b})$ for some $q({\mr x}\,;{\gr z})\subseteq L$ and some $a\in\U^{|{\gr z}|}$. Let $s({\gr z})=\tp({\gr b}/A)$. By invariance and homogeneity the types $p({\mr x}\,;f{\gr b})$ for $f\in\Aut(\U/A)$ are all equivalent. Therefore
