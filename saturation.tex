...
\begin{exercise}\label{ex_qe_in_U}
  ...
  \begin{exercise}\label{ex_inv_def}
    Let $\phi(x)\in L(\U)$.
    Let $\D\subseteq\U^x$ be type-definable over $A$.
    Prove that the following are equivalent
    \begin{itemize}
    \item[1.] $\phi(\D)=\psi(\D)$ for some formula $\psi(x)\in L(A)$
    \item[2.] $\phi(\D)$ is invariant over $A$.
    \end{itemize}
  \end{exercise}
