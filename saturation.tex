...
\item[3] $N$ realizes all types $p({\vl x})\subseteq L(A)$, with $|{\vl x}|\le\lambda$ and $|A|<\lambda$, finitely consistent in $N$.
\end{itemize}
\end{theorem}
...
\ssf{2}$\IMP$\ssf{3}. Let $p({\vl x})$ be as in \ssf{3}. By the compactness theorem $N\preceq K\models p({\vl b})$ for some model $K$ and ${\vl b}\in K^{|{\vl x}|}$. By the downward  L\"owenheim-Skolem theorem there is a model $A,{\vl b}\subseteq M\preceq K$ of cardinality $\le\lambda$. (Here we use $|L|,|A|,|{\vl x|}\le\lambda$.) By \ssf{2}, there is an elementary embedding $h:M\hookrightarrow N$ that extends $\id_A$. Finally, as $M\models p({\vl b})$,  elementarity yields $N\models p(h{\vl b})$.
