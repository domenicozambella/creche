...
\begin{proposition}\label{prop_Lascar_indiscernibles}
  Let $p({\mr x})\in S(\U)$ be a global type and ${\gr\bar c}=\<{\gr c_i}:i<\omega\>$ be an arbtrary $A$-indiscernible sequence. Then the following are equivalent
  \begin{itemize}
    \item[1.] $p({\mr x})$ is Lascar invariant over $A$;
    \item[2.] ${\gr\bar c}$ is indiscernible over $A,{\mr a}$ for every ${\mr a}\models p_{\restriction A,{\gr\bar c}}({\mr x})$;
    \item[3.] ${\gr\bar c}$ is indiscernible over any ${\mr a}\models p_{\restriction{\gr\bar c}}({\mr x})$;
  \end{itemize}
