...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Lascar graphm}\label{lascar_graph}
...
% 
% \begin{corollary}
% Let $p({\gr z})\in S(A,{\gr a})$, then $p(\U^{\gr z})\subseteq\Ll({\gr a}/A)$.
% \end{corollary}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The diameter of the Lascar graph}\label{newelski}

Section~\ref{dual_perspective}

Let $G$ be a subgroup of $\Aut(\U)$.
Assume $G$ is the union of a countable chain $\<G_n:n\in\omega\>$ of symmetric subsets of $G$.
A subset of $G$ is symmetric if it contains the unit element and it is closed under inverse.
Let $\mrX\subseteq^{\mr x}$ be $G$-invariant.
We assume that the relations $\{\<{\mr a},{\mr b}\>\in{\mr\X^2}\ :\ {\mr a}\in G_n{\mr b}\}$ are $G$-invariant and type-definable.
That is, they equal $p_n({\mr\X},{\mr\X})$ for some types $p_n({\mr x},{\mr y})$.
