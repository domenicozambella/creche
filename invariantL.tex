...
\begin{proposition}\label{prop_exisntence_coheirs_over_sets}
  ...
  \begin{itemize}
    ...
    \item[2.] if $\psi({\mr x})\in L(\U)$ is almost statisfied in $A$, then for all $\phi({\mr x})\in L(\U)$ either $\psi({\mr x})\wedge\phi({\mr x})$ or $\psi({\mr x})\wedge\neg\phi({\mr x})$ is almost satisfied in $A$. 
  \end{itemize}  
\end{proposition}
  
\begin{proof} 
  \ssf1$\IMP$\ssf2 Assume \ssf1. If $p({\mr x})\in S(\U)$ is almost satisfiable in $A$, then 
  
  
  $\phi({\mr x})\vee\psi({\mr x})$ is almost satisfiable in $A$, then it is contained in a global type $p({\mr x})$ that is almost satisfiable in $A$. 
  Then by the completeness of $p({\mr x})$ either $\phi({\mr x})$ or $\psi({\mr x})$ is in $p(({\mr x})$ and, consequently, is almost satisfied in $A$.
