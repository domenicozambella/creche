...
Given $\grD\subseteq\U^{|{\gr z}|}$, occasionally we will find it convenient to expand the language $L$ with a predicate for ${\gr z}\in\grD$. Generally, we denote by $\<\U,\grD\>$ the corresponding expansion of $\U$. We write \emph{$L(\X)$} for the expanded language but, if the intended interpretation of ${\gr\X}$ only going to be $\grD$, we may write \emph{$L(\grD)$} and abbreviate $\<\U\,;\grD\>\models\phi({\gr\X})$ as $\phi(\grD)$. 
...
What said above is adapted to define the expansion  \emph{$L(\X_i:i<\lambda)$}, where $\X_i$ are predicates of arity $|{\gr z_i}|$. Again, when the sets ${\gr\D_i}\subseteq\U^{|{\gr z_i}|}$ are the only intended interpretation of  $\X_i$, we may write \emph{$L(\D_i:i<\lambda)$}.
...
\begin{remark}
The definitions above are straightforeward when ${\gr z}$ finite tuple. When ${\gr z}$ is an infinite tuple the intuition stays the same but a more involved definition is required. In fact, first-order logic does not allow infinitary predicates. We think of $L({\gr\X})$ for a two sorted language. Call one sort the \textit{home sort\/} and the ${\gr z}\jj$\textit{sort}. The expansion $\<\U,\grD\>$ has domain $\U$ for the home sort, and $\U^{|{\gr z}|}$ for the ${\gr z}\jj$sort. Besides the symbols of $L$, there is a function symbol $\pi_i$ for every $i<|{\gr z}|$ which is interpreted as the projection to the $i\jj$coordinate. These functions have arity $\<{\gr z},0\>$ (see Section~\hyperref[many-sorted]{\ref*{imaginary}.\ref*{many-sorted}} for the notation). There is also a predicate of sort $\<{\gr z}\>$ interpreted as $\grD$.\QED
\end{remark}
...
\ceq{\hfill\emph{$\oorbit(\D/A)$}}{\deq}{\Big\{f[\grD]\ :\ f\in\Aut(\U/A)\Big\}}
...
\begin{proof}
  The implication \ssf{1}$\IMP$\ssf{2} is clear because all sets in $o(\grD/A)$ are Lascar invariant over $A$. When $|M|\le|L(A)|$ implication \ssf{2}$\IMP$\ssf{3} follows from the bounds discussed in Section~\ref{invariant}. So, we may temporary add this assumption on $M$ as, once the proof of the proposition is finished, it is easily seen to be redundant. Implication \ssf{3}$\IMP$\ssf{4} holds because $\kappa$ is strong limit.

  We now prove \ssf{4}$\IMP$\ssf{5}. Assume $\neg$\ssf{5}. Then we can find an  $A\mbox{-}$indiscernible sequence $\<{\gr c_i}:i<\kappa\>$ such that ${\gr c_0}\in\grD\niff {\gr c_1}\in\grD$. Define
  ...
  We prove \ssf{5}$\IMP$\ssf{1}. Assume \ssf{5} and fix any ${\gr a}\equiv_M{\gr b}$ for some model $M$ containing $A$. It suffices to prove that ${\gr a}\in\grD\iff{\gr b}\in\grD$. Let $p({\gr z})\in S(\U)$ be a global coheir of $\tp({\gr a}/M)=\tp({\gr b}/M)$. Let $c=\<{\gr c_i}:i<\omega\>$ be a Morley sequence of $p({\gr z})$ over $M,{\gr a},{\gr b}$. Then both ${\gr a},c$ and ${\gr b},c$ are $A\mbox{-}$indiscernible sequences. So from \ssf{5} we obtain ${\gr a}\in\grD\iff{\gr c_0}\in\grD\iff{\gr b}\in\grD$. 
\end{proof}
...
\begin{proof}
  ...
  \ssf{2}$\IMP$\ssf{1}\quad If $p({\mr x})$ is not Lascar invariant over $A$ then ${\gr c_0}\in{\gr\D_{p,\phi}}\niff {\gr c_1}\in{\gr\D_{p,\phi}}$ for some $A$-indiscernible sequence $c=\<{\gr c_i}:i<\omega\>$ and some $\phi({\mr x}\,;{\gr z})\in L$. Then $p({\mr x})$ contains the formula $\phi({\mr x}\,;{\gr c_0})\niff \phi({\mr x}\,;{\gr c_1})$. Hence, $c$ is not indiscernible over any realization of $p({\mr x})_{\restriction{\gr c_0},{\gr c_1}}$ 

  \ssf{1}$\IMP$\ssf{2}\quad Assume \ssf{1} and fix an $A$-indiscernible sequence $c=\<{\gr c_i}:i<\omega\>$ and some ${\mr a}\models p_{\restriction c}$. We need to prove that for every formula $\phi({\mr x}\,;z')\in L$, where $z'={\gr z_1},\dots,{\gr z_n}$, 
...
We used finite satisfiability and coheirs to obtain invariant types. Here we mention a natural generalization which produces Lascar invariant types. 

We say that $\phi({\mr x})\in L(\U)$ is \emph{almost satisfied\/} in $A$ if it is satisfied in every model containing $A$. A type $p({\mr x})\subseteq L(\U)$ is \emph{almost (finitely) satisfied\/} in $A$ if each of its finite subsets is almost satisfied in $A$. 

A global type  $p({\mr x})\in S(\U)$ that is almost satisfiable in $A$, is called a \emph{global coheir\/} of $p_{\restriction A}({\mr x})$. Note that when $A$ is a model we obtain the same notion defined a the end of Section~\hyperref[coheirs]{\ref*{invariant}.\ref*{coheirs}}. 

The following is immediate.

\begin{proposition}
Let $p({\mr x})\in S(\U)$ be a global coheir of $p_{\restriction A}({\mr x})$. Then $p({\mr x})$ is Lascar invariant over $A$.\QED
\end{proposition}

We say that $A$ is a \emph{coheir extension base\/} if whenever $\phi({\mr x})\vee\psi({\mr x})$ is almost satisfied in $A$, at least one of $\phi({\mr x})$ or $\psi({\mr x})$ is almost satisfied in $A$. 

\noindent\llap{\textcolor{red}{\Large\danger}\kern1.5ex}The terminology is not standard. It is related with the notion of extensions bases for forking, see e.g.~\cite{Simon}.  

\begin{example}
Trivially, every model is a coheir extension base. When $T=T_{\rm dlo}$ only the models are coheir extension bases. The general proof is left to the reader. Here we consider the case $A\cap(a,c)=\0$ for some $a,c\in A$. Let $b\in(a,c)$. Then every model containing $A$ intersects $(a,b)\cup (b,c)$, but some models may omit $(a,b)$, others $(b,c)$. In Chapter~\ref{external} we will see that in some important class of theories, \textit{stable theories}, all sets are coheir extension bases.\QED
\end{example}
...
%We say that ${\gr a}$ and ${\gr b}$ have the same \emph{Kim-Pillay strong type over $A$\/} if $\K({\gr a}/A)=\K({\gr b}/A)$ or, in other words, when ${\gr a}\in\grD\iff{\gr b}\in\grD$ for every type-definable set $\grD$ that is Lascar invariant over $A$. We write \emph{$a\stackrel{\smash{\scalebox{.5}{\rm\bf KP}}}{\equiv}_Ab$}.
...
\ceq{\hfill e'({\gr a}\,;{\gr b})}{\deq}{\E x'\in\grC\ \E  x''\in\grC \quad {\gr a},x'\equiv_A{\gr b},x''}
...
The original proof of Newelski theorem is rather long and complex. A simplified proof appears in Rodrigo Pel\'aez's thesis~\cite[Section 3.3]{Pelaez}. The proof here is a streamlined version the latter.

We are grateful to Volodya Shavrukov for fixing a wrong proof of Lemma~\ref{lem_newelski}.
...
\bib{Pelaez}{book}{
   ...
   series={PhD Thesis}
   publisher={Universitat de Barcelona, Departament de
L\'ogica, Hist\'oria i Filosofia de la Ci\'encia},
   date={2008},
}
...
A set $\grD$ is called \emph{externally definable\/} if there are a global type $p({\mr x})\in S(\U)$ and a formula $\phi({\mr x}\,;{\gr z})$ such that $\grD=\grD_{p,\phi}$. Equivalently, a set $\grD$ is externally definable if it is the trace on $\U^{|{\gr z}|}$ of a set which is definable in some elementary extension of $\U$. This explains the terminology.

We prefer to deal with external definability in a different, though equivalent, way. 

\begin{definition}\label{def_epprox}
We say that $\grD$ is \emph{approximable\/} by the formula $\phi({\mr x}\,;{\gr z})$ if for all finite $B\subseteq\U^{|{\gr z}|}$ there is a ${\mr b}\in\U^{|{\mr x}|}$ such that $\phi({\mr b}\,;B)=\grD\cap B$. We may call the formula $\phi({\mr x}\,;{\gr z})$ the \emph{sort} of $\grD$. If in addition we have that $\phi({\mr b}\,;{\gr\U})\subseteq\grD$, we say that  $\grD$ is \emph{approximable from below}. If  $\grD\subseteq\phi({\mr b}\,;{\gr\U})$ we say that  $\grD$ is \emph{approximable from above}.\QED
\end{definition} 
...
\begin{example}
Let $T$ be the theory a dense linear orders without endpoints and let $\grD\subseteq\U$ be an interval. Then $\grD$ is approximable both from below and from above by the formula \mbox{$x_1<z<x_2$}.  Now let $T$ be the theory of the random graph. Then every $\grD\subseteq\U$ is approximable and, when $\grD$ has small cardinality, it is approximable from above but not from below.\QED
\end{example}
...
\begin{proof}
To prove \ssf{2}$\IMP$\ssf{1}, for a contradiction assume \ssf{2} and $\neg$\ssf{1}. For each formula $\psi({\mr x}\,;{\gr z})\in L$ choose a finite set $B$ such that $\psi({\mr b}\,;B)\neq\grD\cap B$ for every ${\mr b}\in\U^{|{\mr x}|}$. Let $C$ be the union of all these finite sets. Clearly $|C|\le|T|$.  By \ssf{2} there are a formula $\phi({\mr x}\,;{\gr z})$ and a tuple ${\mr c}$ such that $\phi({\mr c}\,;C)=\grD\cap C$, contradicting the definition of $C$.
...
If $\grD$ is approximable of sort $\phi({\mr x}\,;{\gr z})$ then so is any $\grC$ such that $\grC\equiv\grD$. The same holds for approximability from below and from above.
\end{proposition}
...
So the same holds for any $\grC\equiv\grD$. As for approximability from below, add the conjunct $\A {\gr z}\,\big[\phi({\mr x}\,;{\gr z})\imp {\gr z}\in\grD\big]$ to the formula above, and similarly for approximability from above.
\end{proof}
...
Given $A\subseteq\U$ and $\grD\subseteq\U^{|z|}$ we call $D=\grD\cap A^{|z|}$ the \emph{trace\/} of $\grD$ on $A$. For $\psi(z)\in L(\U)$ we write \emph{$\psi(A)$\/} for $\psi(\U)\cap A^{|z|}$, the trace of $\psi(\U)$ on $A$.

Let $p(x)\in S(\U)$ be a global type and let $\phi(x;z)$ be a pure formula. Sets of the form $\grD_{p,\phi}$ are called \emph{externally definable sets\/} as they may be seen as the trace on $\U$ of a set definable in an elementary extension of $\U$. We now give an equivalent description of externally definable sets.

We say that $\grD\subseteq\U^{|z|}$ is an \emph{approximable set\/} if for some formula  $\phi(x;z)$, for every finite set $B\subseteq\U^{|z|}$ there exists a $b\in\U^{|x|}$ such that $\phi(b;B)=\grD\cap B$. We may say that $\phi(x;z)$ approximates $\grD$. We say that $\grD$ is approximable \emph{in\/} $A$ if we can further require that $b\in A^{|x|}$. Terminology in this paragraph is not standard.
...
In the definition of approximable we fixed a sort $\phi(x;z)$ otherwise every set would be trivially approximable. We leave to the reader to check that, if we require $\grD$ to be approximable on sufficiently large sets, uniformity follows.
...
\begin{proof}
Suppose $\grD$ is not invariant, say $a\in\grD$ and $fa\notin\grD$ for some $f\in\Aut(\U/A)$. Let $\phi(x;z)$ be the formula that approximates $\grD$ in $A$. Then there is a $b\in A^{|x|}$ such that  $\phi(b;a)\wedge\neg\phi(b;f\!a)$ a contradiction.
\end{proof}
