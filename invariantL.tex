...
\begin{theorem}\label{thm_Lascar_indiscernibles}
  ...
  \begin{itemize}
    ...
    \item[3.] $\oorbit(\grD/A)$ has cardinality $\le 2^{2^\lambda}$
...
\begin{definition}\label{def_Lascar_type}\strut We write \emph{\strut${\gr a}\equivL_A{\gr b}$\/} and say that ${\gr a}$ and ${\gr b}$ have the same \emph{Lascar strong type\/} over $A$ if the equivalence ${\gr a}\in\grD\iff{\gr b}\in\grD$ holds for every set $\grD$ that is Lascar invariant over $A$ or, in other words, if $\Ll({\gr a}/A)=\Ll({\gr b}/A)$.
...
\noindent\llap{\textcolor{red}{\Large\warning}\kern1.5ex}%
A global type $p({\mr x})\in S(\U)$ that is finitely satisfiable in every model containing $A$, is called a \emph{Lascar coheir\/} of $p_{\restriction A}({\mr x})$. 
...
\begin{proof}
  ...
  Then $p({\mr x})\proves\phi({\mr x}\,;{\gr c_0})\niff\phi({\mr x}\,;{\gr c_1})$.
...
We say that $A$ is a \emph{extension base for Lascar coheirs\/} if every type that is finitely satisfiable in every $M\supseteq A$ has an extension to a global type with the same property.
...
\begin{proposition}\label{prop_exisntence_coheirs_over_sets}
  The following are equivalent
  \begin{itemize}
    \item[1.] $A$ is a extension base for Lascar coheirs
...
Trivially, every model is an extension base for Lascar coheirs.
The following example shows that some theories may have no other extension bases.

\begin{example}
  Consider $T_{\rm dlo}$.
  Suppose $A$ is not a model.
  We show that $A$ is not an extension base for Lascar coheirs.
...
In Chapter~\ref{stability} we will see that for some important class of theories, \textit{stable theories}, all sets are extension bases for Lascar coheirs.
...
\begin{exercise}
  Let $A$ be a extension base for Lascar coheirs.
...
\begin{exercise}
  Let $A$ be a extension base for Lascar coheirs.
  Prove that for every $a\in\U^x$ there is a structure $\V\preceq\U$ isomorphic to $\U$ over $A$ such that $a\cnonfork_M\V$ for every $A\subseteq M\preceq\V$.
\end{exercise}
...
\begin{exercise}
  ...
  Prove that if $\D_{p,\phi}$ is saturated, see definition~\ref{def_ins_sat}, then it is definable and belongs to $\acl^\eq\!A$.
\end{exercise}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{proposition}\label{prop_trans_action_KP}
  ...
  \ceq{\hfill Q}{=}{\big\{\K({\gr a}/A)\ :\  {\gr a}\models p({\gr z})\big\}}.
