...
\begin{proposition}
Every type $p({\mr x})\subseteq L(\U)$ that is finitely satisfiable in every $Msupseteq A$ is Lascar invariant over $A$.\QED
\end{proposition}

\begin{proof}
  Let $\<{\gr c_i}:i<\omega\>$ be an $A\mbox{-}$indiscernible sequence.
  Suppose for a contradiction that ${\gr c_0}\in\gr\D_{p,\phi}\niff{\gr c_1}\in\gr\D_{p,\phi}$ for some $\phi({\mr x}\,;{\gr z})\in L$.
  ...
  This contradicts indiscernibility ovet $M$.
\end{proof}


\noindent\llap{\textcolor{red}{\Large\danger}\kern1.5ex}We say that $A$ is a \emph{coheir extension base\/} if every type that is finitely satisfiable in every $M\supseteq A$ has an extension to a global onewith the same property.
