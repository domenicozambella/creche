...
\ceq{\hfill q({\mr x})}{=}{\{\theta({\mr x})\in L(A)\ :\ \theta({\mr x})\textrm{ strongly generic}\}.}

By (the proof of) Corollary~\ref{corol_str_gen}, this type is finitely consistent in $\mrX$, strongly generic, and invariant.


If there is a weakly persistent set with finite diameter then $\mrX$ has finite diamenter.
