...
\begin{exercise}\label{ex_Lstp_indiscernibles}
  Pcerove that the equivalence relation ${\gr a}\stackrel{\smash{\scalebox{.5}{\rm L}}}{\equiv}_A{\gr b}$ is the transitive closure of the relation: there is a sequence $\<{\gr c_i}:i<\omega\>$ indiscernible over $A$ such that ${\gr c_0}={\gr a}$ and ${\gr c_1}={\gr b}$. Hint: apply Theorem~\ref{thm_Lascar_indiscernibles}.\QED
\end{exercise}
...
It is a natural generalization to a set of the notion of finite satisfiability over a model introduced in Section~\hyperref[coheirs]{\ref*{invariant}.\ref*{coheirs}}.
...
% Ideally, we would like to prove a similar existence property for Lascar invariance over a set but this is not possible in general.
% In fact, in Example~\ref{ex_circ_order}, we preset a theory that has no global type that is Lascar invariant over $\0$.
...
\begin{proposition}\label{prop_exisntence_coheirs_over_sets}
  ...
  \begin{itemize}
    ...
    \item[2.] for all $\phi({\mr x}), \psi({\mr x})\in L(\U)$ if $\phi({\mr x})\vee\psi({\mr x})$ is almost satisfiable in $A$, then $\phi({\mr x})$ or $\psi({\mr x})$ is almost satisfiable in $A$. 
  \end{itemize}  
\end{proposition}
  
\begin{proof} 
  \ssf2$\IMP$\ssf1 Assume \ssf2. If $\phi({\mr x})\vee\psi({\mr x})$ is almost satisfiable in $A$, then it is contained in a global type $p({\mr x})$ the type almost satisfiable in $A$. 
  Then \ssf1 follows from the completeness of $p({\mr x})$.
  
  \ssf1$\IMP$\ssf2 Assume \ssf2 and repeat the proof of Proposition~\ref{prop_exisntence_coheirs}.
\end{proof}
  
Trivially, every model is an extension base for coheirs.
The following example shows that for some theories may have no other extension bases.
...
%   Let $U$ be the usual unit circle in the plane and define a ternary relation $r(x,y,z)$ on $U$ that holds if either $x$ and $z$ are diametrically opposite, or $y$ lies in the (closed) shortest arc between $x$ and $z$.\QED
% \end{example}
