...
\begin{proof} 
  \ssf2$\IMP$\ssf1 Assume \ssf2. If $\phi({\mr x})\vee\psi({\mr x})$ is almost satisfiable in $A$, then it is contained in a global type $p({\mr x})$ that is almost satisfiable in $A$. 
...
\begin{example}\label{ex_circ_order}
  ...
  We will implicitly use in what follows that $T$ has elimination of quantifiers.
  Let $p(x)\in S(\U)$.
  We claim that $p(x)$ is not Lascar invariant over the emty set.
