...
Let $\X\subseteq\U^x$ be a set that is type-definable (over some small set of parameters) and invariant.
We write $x\in\X$ for the type that defines $\X$.
A definable set $\D$ is \emph{generic\/} relative to $\X$ if finitely many translations of $\D$ cover $\X$; we say \emph{$n$-generic\/} if $n$ translates suffices.
This notions depend on the $G$ and $\X$ that are inferred from the context.

A collection of sets has the \emph{finite intersection property\/} if every finite subcollection has nonempty intersection.
We say that a set $\D$ is \emph{fip\/} if its orbit has the finite intersection property.
The following fact is immediate 

\begin{fact}\label{fact_fip}
  Let $G$ and $\X$ as above be given.
  The following are equivalent
  \begin{itemize}
    \item[1.] $\D$ is not generic;
    \item[2.] $\neg\D$ is fip.\QED
  \end{itemize}
\end{fact}

\begin{theorem}\label{thm_generic_invariant}
  For every global type $p(x)$ that contains $x\in\X$ the following are equivalent 
  \begin{itemize}
    \item[1.] $p(x)$ is invariant;
    \item[2.] $p(x)\proves x\in\D$ for every generic set $\D$;
    \item[3.] $p(x)\proves x\in\D$ for every 2-generic set $\D$.
  \end{itemize}
\end{theorem}

\begin{proof}
  \ssf1$\IMP$\ssf2 
  If $\D$ is 2-generic, \ssf2 follows from completeness and invariance.

  \ssf2$\IMP$\ssf3 Trivial.
  
  \ssf3$\IMP$\ssf1 
  If $p(x)$ is not invariant then, by completeness, $p(x)\proves x\in\D\cap\neg f\D$ for some $f\in G$.
  Then $p(x)\proves x\notin\neg\D\cup f\D$.
  But note that the set $\neg\D\cup f\D$ is $2$-generic.
  In fact $\neg\D\cup f\D$ and its $f$-translation cover $\U^x$.
\end{proof}

\begin{remark}
  Let $\phi(x\,;z)\in L(\U)$.
  The local version of Theorem~\ref{thm_generic_invariant} reads as follows.
  If $p(x)\in S_\phi(\U)$ if is finitely consistent with $\X$ then the 1-3 are equivalent when $\D$ ranges over the Boolean combinations of the sets $\phi(\U\,;b)$ with $b\in\U^z$.
  The proof is virtually identical.\QED
\end{remark}

Theorem~\ref{thm_generic_invariant} gives a condition for a global type to be invariant.
The following theorem gives a condition for a formula to be contained in an invariant type.

\begin{theorem}\label{thm_generic_invariant2}
  For every definable set $\D\subseteq\U^x$ the following are equivalent 
  \begin{itemize}
    \item[1.] $p(x)\proves x\in\D$ for some global invariant type $p(x)$ finitely consistent with $\X$;
    \item[2.] every finite cover of $\D$ by definable sets contains a 2-fip set;
    \item[3.] every finite cover of $\D$ by definable sets contains a fip set.
  \end{itemize}
\end{theorem}

\begin{proof}
  We tacitily assume that $\D$ is consistent with $\X$ othervise the equivalence is vacuously true.

  \ssf1$\IMP$\ssf2 
  Suppose $\C_1,\dots,\C_n$ cover $\D$. 
  By completeness, $p(x)\proves x\in\C_i$ for some $i$.
  Then $\neg\C_i$ is not 2-generic. 
  Then $\C_i$ is 2-fip by Fact~\ref{fact_fip}
  Then \ssf2 follows from completeness and invariance.

  \ssf2$\IMP$\ssf3 Trivial.
  
  \ssf3$\IMP$\ssf1 Let $p(x)$ be maximal among the types that are finitely consistent with $\X$, contain $x\in\EuScript D$ and are such that $\theta(\U)$ satisfies \ssf3 for every $\theta(x)\in p$.
  We claim that $p$ is a complete type.
  Suppose for a contradiction that $\theta(x),\neg\theta(x)\notin p$.
  By maximality there is some $\psi(x)\in p$ and some $\EuScript C_1,\dots,\EuScript C_n$ that cove both $\psi(\EuScript U)\cap\theta(\U)$ and $\psi(\EuScript U)\smallsetminus\theta(\U)$ and such that no $\EuScript C_i$ has a fip.
  This is a contradiction because $\EuScript C_1,\dots,\EuScript C_n$ cover $\psi(\EuScript U)$.
  It is only left to show that $p(x)$ is invariant.
  If not, by Theorem~\ref{thm_generic_invariant} and completeness $p(x)\proves x\in\neg\B$ for some generic set $\B$.
  By Fact~\ref{fact_fip} $\B$ has is not fip contraticing \ssf3.
\end{proof}

Let $\mu(x)\subseteq L(\U)$ be a type finitely consistent with $x\in\X$ and $G$-invariant.
We assume that $\mu(x)$ is closed under conjunction and logical consequences.
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Expansions}
...
\begin{proof}
  ...
  \ssf{2}$\IMP$\ssf{3}.\quad When $|M|\le|L(A)|$ the implication follows from the bounds discussed in Section~\hyperref[invariant_sets]{\ref*{invariant}.\ref*{invariant_sets}}. We temporary add this assumption on $M$. Once the proof of the proposition is completed, is easily seen to be redundant (by \ssf{4} and Proposition~\ref{prop_numero_quasi_invarianti}). 
  ...
  \ssf{5}$\IMP$\ssf{1}.\quad Fix any ${\gr a}\equiv_N{\gr b}$ where $A\subseteq N$. It suffices to prove that ${\gr a}\in\grD\iff{\gr b}\in\grD$. Let $p({\gr z})\in S(\U)$ be a global coheir of $\tp({\gr a}/N)=\tp({\gr b}/N)$. Let ${\gr\bar c}=\<{\gr c_i}:i<\omega\>$ be a Morley sequence of $p({\gr z})$ over $N,{\gr a},{\gr b}$. Then both ${\gr a},{\gr\bar c}$ and ${\gr b},{\gr\bar c}$ are $A\mbox{-}$indiscernible sequences. Therefore, from \ssf{5} we obtain ${\gr a}\in\grD\iff{\gr c_0}\in\grD\iff{\gr b}\in\grD$. 
\end{proof}
...
\end{proposition}
\begin{proof} We prove \ssf{1}$\IFF$\ssf{3}. Equivalence  \ssf{1}$\IFF$\ssf{2} can be proved similarly.
