...
We say that $\grD$ is \emph{externally definable\/} if $\grD={\gr\D_{p,\phi}}$ for some type $p({\mr x})\in S_\phi(\U)$.
...
In Definition~\ref{def_approx}, the sort $\phi({\mr x}\,;{\gr z})$ is fixed (otherwise any set would be approximable) but we can drop this requirement if we allow $B$ to have infinite cardinality.

\begin{proposition}\label{lem_approx_nonunif}
For every $\grD$ the following are equivalent
\begin{itemize}
\item[1.] $\grD$ is approximable
\item[2.] if ${\gr C}\subseteq\U^{\gr z}$ has cardinality $\le|L|$, then $\psi({\gr C})=\grD\cap{\gr C}$ for some $\psi({\gr z})\in L(\U)$.
\end{itemize}
\end{proposition}
...
We say that $\pi({\mr x}\,;{\gr z})$ is \emph{unstable\/} if it admits ladder sequences of arbitrary finite length; \emph{stable\/} otherwise.
If $\pi({\mr x}\,;{\gr z})$  is unstable we may also sat that it has the \emph{order property.}

% In the following \emph{$\mrX$\/} $\subseteq\U^{\mr x}$ and \emph{$\grZ$\/} $\subseteq\U^{\gr z}$ are two given sets that are type-definable over $A$.
% If the condition above holds when we require the ladder sequence to contain only elements of $\mrX\times\grZ$, we say that $\pi({\mr x}\,;{\gr z})$ is stable \emph{restricted to\/} $\mrX$ and $\grZ$.

We will mainly deal with relations that are definable but it noteworthy that a few results in this section do not need this assumption.
...
\begin{fact}\label{fact_infiniteladders}
...
Sometimes \ssf2 in the above fact is taken as definition of stability.
The following is another useful characterization.
...
\begin{proof}
  \ssf2$\IMP$\ssf1. \ Suppose that the ladder $\<{\mr a_i}\,;{\gr b_i} : i<\omega\>$ witnessee the failure of \ssf2 in Fact~\ref{fact_infiniteladders}.
  ...
  \ssf2$\IMP$\ssf1. Immediate by indiscernibility.
\end{proof}
...
\begin{remark}\label{rem_sability_no_compactness}
  ...
  We say that the relation $\pi({\mr x}\,;{\gr z})$ is stable between the sets $\mrX$ and $\grZ$ if for some $n$ no ladder of length $n$ exists with ${\mr a_i}\in\mrX$ and ${\gr b_i}\in\grZ$.

  Let $\grD$ be approximable by $\pi({\mr x}\,;{\gr z})\wedge {\mr x}\in\mrX\wedge {\gr z}\in\grZ$.
  Then there are ${\mr a_{1,1}},\dots,{\mr a_{n,m}}\in \mrX$ such that for every ${\gr b}\in\grZ$

  \ceq{\hfill {\gr b}\in\grD}{\iff}{\bigvee^n_{i=1}\bigwedge^m_{j=1}\pi({\mr a_{i,j}}\,;{\gr b}).}

  Note also that, when $\mrX$ and $\grZ$ are type-definable, Fact~\ref{fact_infiniteladders}, Theorem~\ref{thm_sability_indiscernibility}, and Lemma~\ref{lem_stab_Boole} generalize in the obvious way.
\end{remark} 

\bigskip
\begin{exercise}
  Prove that a relation $\pi(x\,;z)$ is instable if and only if for every $n<\omega$ there is a set $B$ and a sequence $\<a_i:i<n\>$ such that $\pi(a_i\,;B)\subset\pi(a_{i+1}\,;B)$ for all $i<n$.
\end{exercise}

\begin{exercise}
  Find some unstable $p(x\,;z)\subseteq L(A)$ that admits no ladder of infinite length. 
  Hint: work in a dense linear order, and consider the type containing the formulas $x<y\ \vee\  y\notin\{a_0,\dots,a_n\}$ for some suitable $\<a_i:i<n\>$.
\end{exercise}
...
\begin{exercise}
  Prove that the following are equivalent for every relation $\pi(x\,;z)$
  \begin{itemize}
    \item [1.] $\pi(x\,;z)$ is instable
    \item [2.] there are arbitrary long finite sequences $\<a_i\,;b_i : i<n\>$ such that for every $i,j<n$

    \ceq{\hfill i<j}{\IFF}{\pi(a_i\,;b_j)}

    \item [3.] there are arbitrary long finite sequences $\<a_i\,;b_i : i<n\>$ such that for every $i,j<n$

    \ceq{\hfill j\le i}{\IFF}{\pi(a_i\,;b_j)}
  \end{itemize}
\end{exercise}
...
\begin{corollary}\label{corol_stable_coheir_over_models}
  ...
  Then there is a type $p({\mr x})\in S_\phi(\U)$ such that $q({\mr x})\cup p({\mr x})$ is finitely satisfiable in every $M\supseteq A$.
\end{corollary}
...
\begin{proof}
  ...
  Suppose not than there are a conjunction $\alpha({\mr x})$ of formulas in $q({\mr x})$, a conjunction $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ of formulas in $p({\mr x})$, and a formula $\phi({\mr x}\,;{\gr b})$ such that 
...
Note that, when all formulas in $L_{{\mr x}\,{\gr z}}(M)$ are stable, the theorem says
...
\begin{corollary}\label{corol_harrington1}
  Let $\phi({\mr x}\,;{\gr z})\in L(\U)$ be stable.
  Let $p({\mr x})\in S_\phi(\U)$ and $q({\gr z})\in S_{\phi^{\rm op}}(\U^{\mr x})$ be invariant over $M$.
...
\begin{proof}
  ...
  \ceq{\hfill\phi({\mr a};{\gr b})}{\iff}{{\mr a}\in{\mr\D_{q,\phi^{\rm op}}}.}

  In fact, as ${\mr\D_{q,\phi^{\rm op}}}$ is definable over $M$, the contrary would contradict ${\gr b}\models q({\gr z}){\restriction}M$.
  The corollary follows that we can prove 

  \ceq{1.\hfill\phi({\mr a}\,;{\gr b})}{\iff}{{\gr b}\in{\gr\D_{p,\phi}}}

  Note that \ssf1 is a stable formula. 
  Hence from its negation and Theorem~\ref{thm_symmetry} we obtain that $\phi({\mr a}\,;{\gr b'})\niff{\gr b'}\in{\gr\D_{p,\phi}}$ holds for some ${\gr b'}\in M^{\gr z}$.
  This would contradict ${\mr a}\models p({\mr x}){\restriction}M$.
\end{proof}
...
\begin{proposition}\label{prop_type_over_acl2}
  Let $\phi({\mr x}\,;{\gr z})\in L(\U)$ be stable.
  Then for every $p({\mr x})\in S_\phi(\U)$ the following are equivalent
  \begin{itemize}
...
\begin{proof}
  ...
  Let $\psi({\mr x})$ be a conjunction of formulas in $p({\mr x})$, we prove that $\psi({\mr x})$ is satisfied in $M$.
...
A type $q({\mr x})\subseteq L(\U)$ is \emph{stationary\/} if there is a unique global type $p({\mr x})\in S_\phi(\U)$ that is Lascar invariant over $A$.
The formula $\phi({\mr x}\,;{\gr z})$ and the set $A$ are to be inferred from the context.
...
\begin{theorem}[ (Stationarity)]\label{thm_stationarity}
  Let $\phi({\mr x}\,;{\gr z})\in L(\U)$ be stable.
  Assume $q({\mr x})$ decides all Boolean combinations of formulas of the form $\phi({\mr x}\,;{\gr b})$ for any ${\gr b}\in\U^{\gr z}$ that are invariant over $\acl^\eq\!A$.
  Then $q({\mr x})$ is stationary.
\end{theorem}

\begin{proof}
  Existence follows from Corollary~\ref{corol_stable_coheir_over_models} and Proposition~\ref{prop_type_over_acl2}.
  To prove uniqueness, pick two types $p_i({\mr x})\in S_\phi(\U)$, for $i=1,2$, that are finitely consistent with $q({\mr x})$ and invariant over $\acl^\eq\!A$ (equivalently, Lascar invariant over $A$).
  It suffices to prove that ${\gr\D_{p_1,\phi}}={\gr\D_{p_2,\phi}}$.
  Let ${\gr b}\in\U^{\gr z}$ be given.

  There exists a type $t({\gr z})\in S_{\phi^{\rm op}}(\U)$ that is finitely satisfied in every model $M\supseteq A$ and is consistent with $\tp({\gr b}/\acl^\eq\!A)$.
  By invariance ${\gr\D_{p_i,\phi}}$ and ${\mr\D_{t,\phi^{\rm op}}}$ are both in $\acl^\eq\!A$.
  Let $M\supseteq A$.
  Let ${\mr a_i}\models p_i({\mr x}){\restriction}M$ and ${\gr b'}\models t({\gr z}){\restriction}M$.
  ...
  As ${\gr\D_{p_i,\phi}}$ are both in $\acl^\eq\!A$, we also have  ${\gr b}\in{\gr\D_{p_1,\phi}}\iff{\gr b}\in{\gr\D_{p_2,\phi}}$.
\end{proof}
...
\begin{theorem}\label{thm_stability_definable_rovescio1}
  ...
  Let ${\gr b}\in\U^{\gr z}$ be arbitrary.
  Then, for some ${\gr b_i}\equiv_M{\gr b}$, where $i=1,\dots,n$, there is a positive Boolean combination of the formulas $\phi({\mr x};{\gr b_i})$ which is equivalent to a formula in $L(M)$.
\end{theorem}

\begin{proof}
  Let $\V\preceq\U$ be isomorphic to $\U$ over $M$ and such that ${\gr b}\cnonfork_M\V$.
  Such a $\V$ exists by Proposition~\ref{prop_saturate_heir}.
  ...
  Let $\theta({\mr x};{\gr\bar b})$, where ${\gr\bar b} = {\gr b_1}, \dots, {\gr b_n}$, be the formula that defines such a Boolean combination.
  ...
  Finally, as ${\gr\bar b}$ is in $\V$, by elementarity we obtain that $\psi(\U^{\mr x})=\theta(\U^{\mr x};{\gr\bar b})$.
\end{proof}
...
\begin{theorem}\label{thm_stability_definable_rovescio2}
  Let $\phi({\mr x}\,;{\gr z})\in L(A)$ be stable.
  Let ${\gr b}\in\U^{\gr z}$ be arbitrary.
  Then, for some ${\gr b_i}\equiv_{\acl^\eq\!A}{\gr b}$, where $i=1,\dots,n$, there is a positive Boolean combination of the formulas $\phi({\mr x};{\gr b_i})$ which is equivalent to a formula in $L(A)$.
\end{theorem}
...
\begin{proof}
  ...
  Let $\theta({\mr x};{\gr b_1},\dots,{\gr b_n})$ be the formula that defines such a Boolean combination.
...
We are interested in the action of the groups $\Aut(\U/\!A)$ and $\Aut(\U/\acl^\eq\!A)$, which we abbreviate to \emph{$G$} and \emph{$H$,} respectively.
Clearly $H\trianglelefteq G$.
To any given formula $\phi({\mr x}\,;{\gr z})\in L(A)$ we associate the set $\Delta$ containing Boolean combinations of the formulas $\phi({\mr x}\,;{\gr z_i})$, where ${\gr z_i}$ are copies of ${\gr z}$.
In this section $\grZ=\U^{\gr z}$, while $\mrX$ will be specified in the context.

% \ceq{\hfill\Psi}{=}{\big\{\theta({\gr z})\in\BDelta(\mrX)\ : \textrm{ with finite $H$-orbit}\big\}}
...
%   Let $\Delta$ be stable.
%   Reading Proposition~\ref{prop_type_over_acl2} in the language $\LDelta$, we obtain that $p({\mr x})$ is $H$-invariant if and only if for every $\phi({\mr x}\,;{\gr z})\in\Delta$ the sets ${\gr\D_{p,\phi}}$ have finite $H$-orbit.
...
The following theorem shows that when $\phi({\mr x}\,;{\gr z})$ is stable Theorem~\ref{thm_coalesce} applies for suitable set $\mrX$.

\begin{theorem}\label{thm_thick_finsat}
  Let $\phi({\mr x}\,;{\gr z})\in L(A)$ be stable.
  Let $\mrX=q(\U^{\mr x})$, where $q({\mr x})\subseteq L(\U)$ is a small type that is finitely satisfiable in every $M\supseteq A$.
  Then for every $\Delta(\grZ)$-definable set $\mrD$ the following are equivalent 
  \begin{itemize}
...
\end{theorem}
\begin{proof}
  We only need to prove \ssf1$\IMP$\ssf2 because the converse is trivial.
  As Boolean combinations of stable formulas are stable, for simplicity we assume that $\mrD$ is defined by the formula $\phi({\mr x}\,;{\gr b})$.

  It suffices to prove that $q({\mr x})\cup\{\phi({\mr x}\,;{\gr b})\}$ is finitely satisfied in every $M\supseteq A$.
  Then from Corollary~\ref{corol_stable_coheir_over_models} we obtain a type $p({\mr x})\in S_\phi(\U)$ containing $\phi({\mr x}\,;{\gr b})$ such that $q({\mr x})\cup p({\mr x})$ is finitely  satisfied in every model $M\supseteq A$. 
  Then $p({\mr x})$ is $H$-invariant, and thertefore $\mrD$ is $H$-wide by Theorems~\ref{thm_syndetic_invariant} and~\ref{thm_syndetic_invariant2}.
  
  Let $M\supseteq A$ be given.
  By invariance we can assume that $q({\mr x})\subseteq L(M)$.
  Let $\sigma({\mr x})\in q$, we claim that $\sigma({\mr x})\wedge\phi({\mr x}\,;{\gr b})$ is satisfied in $M$.
  Let $\theta({\mr x}\,;{\gr\bar b})$ be the formula in the proof of Theorem~\ref{thm_stability_definable_rovescio1} which is equivalent to a formula $\psi({\mr x})\in L(M)$.
  As $\theta({\mr x}\,;{\gr\bar b})$ is a positive Boolean combination of translations of $\mrD$ and $\mrD$ is $H$-thick, $\theta({\mr x}\,;{\gr\bar b})$ is satisfied in $\mrX\subseteq\sigma(\U^{\mr x})$.

  Then also $\psi({\mr x})$ is satisfied in $\sigma(\U^{\mr x})$ and, by elementarity in $\sigma(M^{\mr x})$.
  Uning again the positivity of $\theta({\mr x}\,;{\gr\bar b})$ we conclude that $\sigma({\mr x})\wedge\phi({\mr x}\,;{\gr b_i})$ is satisfied in $M$ for some $i$.
  The claim follows because ${\gr b_i}\equiv_M{\gr b}$.
\end{proof}

Now, putting together the above theorem, Theorems~\ref{thm_stationarity}, and Fact~\ref{fact_stationarity} we obtain the following 

\begin{corollary}
  With the same notation as Theorem~\ref{thm_thick_finsat}, assume also that $q({\mr x})$ decides all Boolean combinations of formulas of the form $\phi({\mr x}\,;{\gr b})$ for any ${\gr b}\in\U^{\gr z}$ that are invariant over $\acl^\eq\!A$.
  Then for every $\mrD$, the following are equivalent
  \begin{itemize}
    \item[1.] $\mrD$ is $H$-syndetic
    \item[2.] $\mrD$ is $H$-thick.\smallskip
  \end{itemize}
  In other words, either $\mrD$ or $\neg\mrD$ is $H$-syndetic.
\end{corollary}

Stationarity and definability of stable types allows to strengthen Theorem~\ref{thm_Shelah_strong_types}.
This is known as Shelah's \textit{finite equivalence relation theorem.}
(A major difference is that below in \ssf5 we can use a single finite equivalence relation for the relation $\equivSh$.)

\begin{theorem}\label{thm_FER}
  \def\medrel#1{\parbox[t]{5ex}{$\displaystyle\hfil #1$}}
  \def\ceq#1#2#3{\parbox[t]{3ex}{$\displaystyle #1$}\medrel{#2}$\displaystyle  #3$}
  Let $\phi({\mr x}\,;{\gr z})\in L(A)$ be stable.
  Let $\mrX=\pi(\U^{\mr x})$, for some $\pi({\mr x})\in S(A)$.
  Define \smallskip

  \ceq{\hfill P}{=}{\big\{p({\mr x})\in S_\phi(\U)\ :\ \textrm{ consistent with }\pi({\mr x})\textrm{ and }H\text{-invariant}\big\}}.\medskip

  \ceq{\hfill Q}{=}{\big\{q({\mr x})\in S(\acl^\eq\!A)\ :\  q({\mr x})\supseteq \pi({\mr x})\big\}}.\medskip
  
  Then the following hold
  \begin{itemize}

    \item [1.] concistency is a one-to-one correspondence between $P$ and $Q$
    \item [2.] $G$ acts transitively on $P$
    \item [3.] $P$ is finite
    \item [4.] the types in $P$ are $G$-syndetic
    \item [5.] there is a finite equivalence relation $\epsilon({\mr x}\,;{\mr y})\in L(A)$ such that for every $c\in\U^{\mr x}$ there is a unique $p({\mr x})\in P$ consistent with $\epsilon({\mr x}\,;{\mr c})$.
  \end{itemize}
\end{theorem}

\begin{proof}
  \ssf1.
  Clear, as the types in $Q$ are stationary by Theorem~\ref{thm_stationarity}.
  
  \ssf2.
  By Proposition~\ref{prop_trans_action}, $\Aut(\U/\!A)$ acts transitively on $Q$.
  Then by \ssf1 it also acts transitively on $P$.

  \ssf3.
  We may identify a type $p({\mr x})\in S_\phi(\U)$ with the set ${\gr\D_{p,\phi}}$.
  The orbit over $A$ of $p({\mr x})\in P$ corresponds to the orbit over $A$ of ${\gr\D_{p,\phi}}$.
  As $p({\mr x})\in P$ is invariant over $\acl^\eq\!A$, the sets ${\gr\D_{p,\phi}}$ have a finite orbit over $A$.
  Therefore by \ssf2, $P$ is finite.

  \ssf4.
  By \ssf2 and \ssf3.

  \ssf5.
  By Theorem~\ref{thm_Shelah_strong_types} and \ssf3.
\end{proof}

\begin{proposition}\label{prop_Gsyndetic_thick}
  Let $\phi({\mr x}\,;{\gr z})\in L(A)$ be stable.
  Let $\mrX=\pi(\U^{\mr x})$, where $\pi({\mr x})\in S(A)$.
  Then for every $\mrD$ the following are equivalent
  \begin{itemize}
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  By stability, there exist a type $p({\mr x})\in S_\phi(\U)$ that is $H$-invariant.
  Then $\Sigma_H({\mr x})$ is consistent and Proposition~\ref{prop_Gsyndetic_Hthick1} applies.
  
  \ssf2$\IMP$\ssf1.
  By \ssf2, there is an $H$-invariant type $p({\mr x})\in S_\phi(\U)$ consistent with ${\mr x}\in\mrD$.
  Then $p({\mr x})\in P$ of Theorem~\ref{thm_FER} and, therefore it is  $G$-syndetic.
\end{proof}

\begin{theorem}\label{thm_P=Q}
  Let $\phi({\mr x}\,;{\gr z})\in L(A)$ be stable.
  Let $Q_G$ be as in Definition~\ref{def_Q} and $P$ be as in Theorem~\ref{thm_FER}.
  Let $\mrX=\pi(\U^{\mr x})$, for some $\pi({\mr x})\in S(A)$.
  Then $Q_G=P$.
\end{theorem}
...
\begin{proof}
  $\supseteq$. 
  Let $p({\mr x})\in P$.
  All formulas in $p({\mr x})$ are $H$-wide, and therefore $G$-syndetic by Proposition~\ref{prop_Gsyndetic_thick}.
  As $p({\mr x})$ complete, $p({\mr x})\in Q_G$.

  $\subseteq$. 
  Let $q({\mr x})\in Q_G$.
  By Theorem~\ref{prop_Gsyndetic_thick}, $q({\mr x})$ is $H$-wide, then $q({\mr x})$ is consistent with some $p({\mr x})\in P$.
  Then, by the converse inclusion, $q({\mr x})=p({\mr x})$.
\end{proof}
...
\begin{proof}
  Let  $\phi(x,y\,;z)\in L$ not stable.
  Then there are indiscernibles $\<a_i,b_i\,;c_i\ :\ i\in\QQ\>$ such that 
\end{proof}
