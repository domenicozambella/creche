...
\begin{proposition}\label{prop_type_over_acl2}
  Let $\phi({\mr x}\,;{\gr z})\in L(\U)$ be stable.
  Then for every $p({\mr x})\in S_\phi(\U^{\gr z})$ the following are equivalent
  \begin{itemize}
...
The sets $\phi({\mr x}\,;{\gr z})$ and $A$ are to be inferred from the context.
...
\begin{theorem}[ (Stationarity)]\label{thm_stationarity}
  Let $\phi({\mr x}\,;{\gr z})\in L(\U)$ be stable.
  Let $q({\mr x})\subseteq L(\U)$ be a type that is finitely satisfiable in every $M\supseteq A$.
  ...
  Then there is a unique type $p({\mr x})\in S_\phi(\U)$ finitely consistent with $q({\mr x})$ that is invariant over $\acl^\eq\!A$.
\end{theorem}

\begin{proof}
  Existence follows from Corollary~\ref{corol_stable_coheir_over_models} and Proposition~\ref{prop_type_over_acl2}.
  To prove uniqueness, pick two types $p_i({\mr x})\in S_\phi(\U)$, for $i=1,2$, that are finitely consistent with $q({\mr x})$ and invariant over $\acl^\eq\!A$.
  It suffices to prove that ${\gr\D_{p_1,\phi}}={\gr\D_{p_2,\phi}}$.
  Let ${\gr b}\in\U^{\gr z}$ be given.

  There exists a type $t({\gr z})\in S_\phi(\U^{\mr x})$ that is finitely satisfied in every model $M\supseteq A$ and is consistent with $\tp({\gr b}/\acl^\eq\!A)$.
