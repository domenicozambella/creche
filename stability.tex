...
Let $p({\mr x})\in S_\phi(\grZ)$.
For every formula $\phi({\mr x}\,;{\gr z})\in L$ we define (this the same definition in Section~\ref{invariant_sets}, only restricted to $\grZ$)

\ceq{\hfill\emph{$\gr\D_{p,\phi}$}}{=}{\Big\{{\gr a}\in\grZ\ :\ \phi({\mr x}\,;{\gr a})\in p\Big\}.}

We say that $\grD\subseteq\grZ$ is \emph{externally definable\/} if it is of the form ${\gr\D_{p,\phi}}$ .
Equivalently, a set $\grD$ is externally definable if it is the trace on $\grZ$ of a set which is definable in some elementary extension of $\U$.
More precisely, let ${}^{*\kern-.2ex}\U$ be an elementary extension of $\U$.
% Let ${\mr{}^{*\kern-.2ex}\X}$ and ${\mr{}^{*\kern-.2ex}\Z}$ 
Then $\grD=\phi({\mr{}^{*\kern-.3ex}b}\,;{}^{*\kern-.2ex}\grZ)\cap\U^{\gr z}$ for some ${\mr  ^{*\kern-.3ex}b}\,{\in}{\mr{}^{*\kern-.2ex}\X}$.
This interpretation explains the terminology.
...
Let $\pi\subseteq\mrX\times\grZ$ be a binary relation (which need not be definable in any sense).
We denote by $\pi({\mr x}\,;{\gr z})$ the predicate associated to $\pi$.

\begin{definition}\label{def_approx}
We say that $\grD\subseteq\grZ$ is \emph{approximated\/} by the relation $\pi({\mr x}\,;{\gr z})$ if for every finite ${\gr B}\subseteq\grZ$ there is a tuple ${\mr a}\in\mrX$ such that $\pi({\mr a}\,;{\gr B})=\grD\cap{\gr B}$.
If in addition $\pi({\mr a}\,;\grZ)\subseteq\grD$, we say that  $\grD$ is approximated \emph{from below.}
Equivalently, we say that $\grD$ is approximated from below if for every finite ${\gr B}\subseteq\grD$ there is a tuple ${\mr a}\in\mrX$ such that  ${\gr B}\subseteq\pi({\mr a}\,;\grZ)\subseteq\grD$.
The dual notion of approximation \emph{from above\/} is defined as expected (and coincides with $\grZ\sm\grD$ being approximated by $\neg\pi({\mr x}\,;{\gr z})$ from below).
\end{definition} 
...
\begin{proposition}\label{prop_approx=external}
  ...
  Let $\grD\subseteq\grZ$ and $\phi({\mr x}\,;{\gr z})\in L(\U)$ be given.
...
Now, let $T=T_{\rm rg}$.
Then every $\D\subseteq\U$ is approximable be the formula $r(x\,;y)$.
When $\D$ has small infinite cardinality, it is approximable from above but not from below.
\end{example}

In Definition~\ref{def_approx}, the approximating formula/relation is fixed (otherwise any set would be approximable) but this requirement of uniformity may be dropped if we allow $B$ to have larger cardinality.

\begin{proposition}\label{lem_approx_nonunif}
For every $\grD\subseteq\grZ$ the following are equivalent
\begin{itemize}
\item[1.] $\grD$ is approximable by some $\phi({\mr x}\,;{\gr z})\in L$
\item[2.] for all ${\gr C}\subseteq\grZ$ of cardinality $\le|L|$ there is $\psi({\gr z})\in L(\U)$ such that $\psi({\gr C})=\grD\cap{\gr C}$.\nobreak
\end{itemize}
\end{proposition}

\begin{proof}
To prove \ssf{2}$\IMP$\ssf{1} negate \ssf{1}
For each formula $\phi({\mr x}\,;{\gr z})\in L$ choose a finite set ${\gr B}$ such that $\phi({\mr b}\,;{\gr B})\neq\grD\cap{\gr B}$ for every ${\mr b}\in\U^{\mr x}$.
...
If $\grD\subseteq\U^{\gr z}$ is approximated by $\phi({\mr x}\,;{\gr z})\in L$ then so is any $\grC$ such that $\grC\equiv\grD$, see Section~\ref{expansions} for the notation.
...
We say that $\<{\mr a_i}\,;{\gr b_i} : i<\alpha\>$, where ${\mr a_i}\in\mrX$ and ${\gr b_i}\in\grZ$ is a \emph{ladder\/}  of length $\alpha$ (between $\mrX$ and $\grZ$) for $\pi({\mr x}\,;{\gr z})$ if for every $i<j<\alpha$

\ceq{\hfill\pi({\mr a_i}\,;{\gr b_j})}{\wedge}{\neg\pi({\mr a_j}\,;{\gr b_i}).}

We say that $\pi({\mr x}\,;{\gr z})$ is \emph{stable\/} (between $\mrX$ and $\grZ$) if for some $n$ there is no ladder of length $n$.
...
Then we say that a formula or a type is stable accordingly.
...
\begin{fact}\label{}
  ...
  \begin{itemize}
    ...
    \item [2.] for some finite $n$, there is no ladder of length $n$ for $p({\mr x}\,;{\gr z})$
    \item [3.] there is $q({\mr x}\,;{\gr z})\subseteq L(A)$ equivalent to $p({\mr x}\,;{\gr z})$ containing only stable formulas.
  \end{itemize}
\end{fact}
...
% In this section there is no assumption on $\pi({\mr x}\,;{\gr z})$.
Note that invariance (over some small set of parameters) would have simplified the above proof as we could have required the ladder sequence to be indiscernible.

\begin{lemma}\label{thm_sability_indiscernibility}
  Let $\pi({\mr x}\,;{\gr z})$ be invariant over $A$.
  Then the following are equivalent
  \begin{itemize}
    \item[1.] $\pi({\mr x}\,;{\gr z})$ has a ladder sequence of length $\omega$
    \item[2.] $\pi({\mr a_0}\,;{\gr b_1})\wedge\neg\pi({\mr a_1}\,;{\gr b_0})$ for some $A$-indiscernible sequence $\<{\mr a_i}\,;{\gr b_i} : i<\omega\>$.
  \end{itemize}
\end{lemma}

\begin{proof}
  (1$\IMP$2) \ Let $\<{\mr a_i}\,;{\gr b_i} : i<\omega\>$ be a ladder sequence.
...
The following theorem claims what is arguably one of the most important properties of stable relations: any set that is externally definable by a stable formula is definable (by a related formula).

\begin{theorem}\label{thm_def_stable_formula}
Any $\grD\subseteq\grZ$ approximated by a stable relation $\pi({\mr x}\,;{\gr z})$ is definable in the following sense.
There are ${\mr a_{i,j}}\in\mrX$ such that
...
\begin{lemma}
If $\grD\subseteq\grZ$ is approximated from below by a stable relation $\pi({\mr x}\,;{\gr z})$ then

\ceq{\hfill {\gr z}\in\grD}{\iff}{\bigvee^n_{i=0}\pi({\mr a_i}\,;{\gr z})}

for some ${\mr a_0},\dots,{\mr a_n}\in\mrX$.
\end{lemma}
...
Choose ${\mr a_n}\in\mrX$ such that ${\gr b_0},\dots,{\gr b_{n-1}}\in\pi({\mr a_n}\,;\grZ)\subseteq\grD$.
This is possible because $\grD$ is approximated from below.
Now, if possible, choose ${\gr b_n}$ such that

\ceq{\hfill{\gr b_n}}{\in}{\grD\sm\bigcup^n_{i=0}\pi({\mr a_i}\,;\grZ)}.
...
If $\grD\subseteq\grZ$ is approximated by a stable relation $\pi({\mr x}\,;{\gr z})$.
Then, for some $m$, the relation
...
We prove that there are some ${\mr a_0},\dots,{\mr a_m}$ such that ${\gr C}\subseteq\psi({\mr a_0},\dots,{\mr a_m}\,;\grZ)\subseteq\grD$.
As in the proof above, we define by recursion a ladder sequence for $\pi({\mr x}\,;{\gr z})$.
Suppose that ${\mr a_0},\dots,{\mr a_{n-1}}$ and ${\gr b_0},\dots,{\gr b_{n-1}}\in\grZ\sm\grD$ have been defined.
We first define ${\mr a_n}$, then ${\gr b_n}$. 
Choose ${\mr a_n}\in\U^{\mr x}$ such that 

\hfil${\gr C}\ \subseteq\ \pi({\mr a_n}\,;\grZ)\ \subseteq\ \grZ\sm\{{\gr b_0},\dots,{\gr b_{n-1}}\}$.
...
\ceq{\hfill{\gr b_n}}{\in}{\bigcap^n_{i=0}\pi({\mr a_i},\grZ)\sm\grD}
...
The following proposition highlights the connection between the stability of the formula $\phi({\mr x}\,;{\gr z})$ and the cardinality of $S_\phi(\grZ)$, equivalently the number of sets externally definable by $\phi({\mr x}\,;{\gr z})$. 

\begin{theorem}\label{thm_def_stable_formula2}
  ($\mrX,\grZ$ type-definable)
  The following are equivalent
  \begin{itemize}
  \item[1.] $\phi({\mr x}\,;{\gr z})$ is stable
  \item[2.] every subset of $\grZ$ that is externally definable by $\phi({\mr x}\,;{\gr z})$ is definable
  \item[3.] there are $\le\kappa$ subsets of $\grZ$ that are externally definable by $\phi({\mr x}\,;{\gr z})$
  \item[4.] there are $<2^\kappa$ subsets of $\grZ$ that are externally definable by $\phi({\mr x}\,;{\gr z})$.
  \end{itemize}
 \end{theorem}
...
Namely, $\<\psi_s:s\in 2^{<\omega}\>$  where $\psi_\varnothing=\top$ and, for $s\in 2^{<\omega}$ and $i\in 2$, we have $\psi_{s^\frown 0}({\mr x})=\neg\phi({\mr x}\,;{\gr b_s})$ and $\psi_{s^\frown 1}({\mr x})=\phi({\mr x}\,;{\gr b_s})$ for some ${\gr b_s}\in\grZ$.
We also require that the branches are consistent in $\mrX$.
...
\node[bag0] {{$\top$}}
    child {
        node[bag] {\llap{$\neg$}$\phi({\mr x}\,;{\gr b_{\varnothing}})$}      
            child {
                node[bag] {\llap{$\neg$}$\phi({\mr x}\,;{\gr b_0})$}
                    child {
                       node[bag] {\footnotesize\llap{$\neg$}$\phi({\mr x}\,;{\gr b_{00}})$\rlap{$\ \cdots$}}
                       edge from parent
                    }    
                    child {
                       node[bag] {\footnotesize$\phi({\mr x}\,;{\gr b_{00}})$\rlap{$\ \cdots$}}
                       edge from parent
                    }  
                 edge from parent
            }
            child {
                node[bag] {$\phi({\mr x}\,;{\gr b_0})$}
                edge from parent
                    child {
                       node[bag] {\footnotesize\llap{$\neg$}$\phi({\mr x}\,;{\gr b_{01}})$\rlap{$\ \cdots$}}
                       edge from parent
                    }    
                    child {
                       node[bag] {\footnotesize$\phi({\mr x}\,;{\gr b_{01}})$\rlap{$\ \cdots$}}
                       edge from parent
                    }  
                 edge from parent
            }
    ...
    child {
        node[bag] {$\phi({\mr x}\,;{\gr b_{\varnothing}})$}         
            child {
                node[bag] {\llap{$\neg$}$\phi({\mr x}\,;{\gr b_1})$}
                    child {
                       node[bag] {\footnotesize\llap{$\neg$}$\phi({\mr x}\,;{\gr b_{10}})$\rlap{$\ \cdots$}}
                       edge from parent
                    }    
                    child {
                       node[bag] {\footnotesize$\phi({\mr x}\,;{\gr b_{10}})$\rlap{$\ \cdots$}}
                       edge from parent
                    }  
                 edge from parent
            }
            child {
                node[bag] {$\phi({\mr x}\,;{\gr b_1})$}
                edge from parent
                    child {
                       node[bag] {\footnotesize\llap{$\neg$}$\phi({\mr x}\,;{\gr b_{11}})$\rlap{$\ \cdots$}}
                       edge from parent
                    }    
                    child {
                       node[bag] {\footnotesize$\phi({\mr x}\,;{\gr b_{11}})$\rlap{$\ \cdots$}}
                       edge from parent
                    }  
                 edge from parent
            } 
...
The assumption of type-definability is redundant and $\phi({\mr x}\,;{\gr z})$ can be replaced with an arbitrary relation $\pi({\mr x}\,;{\gr z})$.
But the proof would be more involved.
...
\begin{proof}
  ...
  Then there is ${\gr b_\varnothing}\in\grZ$ such that there are $2^\kappa$ sets ${\gr\D}$ externally definable by $\phi({\mr x}\,;{\gr z})$ and such that ${\gr b_\varnothing}\in{\gr\D}$ and $2^\kappa$ sets such that ${\gr b_\varnothing}\notin{\gr\D}$.
...
\begin{lemma}\label{lem_ziegler}
  ...
  Assume that $\phi({\mr x}\,;{\gr z})$ is stable.
  Let ${\gr b}\in\grZ$.
  Suppose that $\phi({\mr x}\,;{\gr b})\vee\psi({\mr x}\,;{\gr b})$ is satisfied in $\mrX\cap M^{\mr x}$ for every sufficiently saturated $M\supseteq A$.
  Then $\phi({\mr x}\,;{\gr b})$ or $\psi({\mr x}\,;{\gr b})$ is satisfied in  $\mrX\cap M^{\mr x}$ for every sufficiently saturated $M\supseteq A$.
\end{lemma}

\begin{proof}
  Negate the theorem.
  Then there are two models containing $A$, the first omitting $\phi({\mr x}\,;{\gr b})$, the second omitting $\psi({\mr x}\,;{\gr b})$.
  It is easy to see that we can expand these models to two substructures $\V_0$ and $\V_1$ that are $A$-isomorphic to $\U$.
  Then $\phi({\mr\X_0}\,;{\gr b})=\psi({\mr\X_1}\,;{\gr b})=\varnothing$ where ${\mr\X_i}=\mrX\cap\V_i^{{\mr x}}$.
  ...
  We write $\V_s=f_s[\U]$ and ${\gr b_s}=f_s\,{\gr b}$.
  Then, we obtain

  \#\hfill$\phi({\mr\X_{s\frown0}}\,;{\gr b_s})\ =\ \psi({\mr\X_{s\frown1}}\,;{\gr b_s})\ =\ \varnothing$\hfill where ${\mr\X_{s}}=\mrX\cap\V_{s}^{{\mr x}}$.
  
  We prove that the branches of the tree $\<\phi({\mr x}\,;{\gr b_s})\, :\,  s\in2^{<\omega}\>$ are consistent in $\mrX$. 
  ...
  Let ${\mr a}\in\theta(\mrX)$ be arbitrary.
  Let $s\in2^n$ be given.
  We prove that ${\mr a_s}=f_s{\mr a}$ witnesses the consistency of $\{\phi^{s_i}({\mr x}\,;{\gr b_{s\restriction i}})\, :\,i<n\}$.
  Clearly ${\mr a_s}\in\theta({\mr\X_s})$, then, as $\V_s\preceq\V_{s\restriction i}$, we have that ${\mr a_s}\in\theta({\mr\X_{s\restriction i}})$.
  We prove that $\phi^{s_i}({\mr a_s}\,;{\gr b_{s\restriction i}})$.
  When $s_i=0$, from \# we obtain ${\mr\X_{s\restriction(i+1)}}\subseteq\neg\phi({\mr\X_{s\restriction(i+1)}}\,;{\gr b_{s\restriction i}})$ and $\phi^0({\mr a}\,;{\gr b_{s\restriction i}})$ follows.
  When $s_i=1$, from the second inclusion in \# we obtain ${\mr\X_{s\restriction(i+1)}}\subseteq\neg\psi({\mr\X_{s\restriction(i+1)}}\,;{\gr b_{s\restriction i}})$.
  Therefore, from $\theta({\mr a})$ and \#\#, we obtain  ${\mr\X_{s\restriction(i+1)}}\subseteq\phi({\mr\X_{s\restriction(i+1)}}\,;{\gr b_{s\restriction i}})$ and $\phi^1({\mr a_s}\,;{\gr b_{s\restriction i}})$ follows.
  ...
  Note that $\phi'({\mr\bar x}\,;{\gr z})$ is a stable formula (between ${\mr\X^n}$ and $\grZ$).
  Therefore we can apply what proved above to the formulas $\phi'({\mr\bar x}\,;{\gr b})$ and $\psi'({\mr\bar x}\,;{\gr b})$ and note that these formulas are satisfied in ${\mr\X^n}\cap M^{{\mr x_1}\dots{\mr x_n}}$ if and only if $\phi({\mr x}\,;{\gr b})$, respectively $\psi({\mr x}\,;{\gr b})$, are satisfied in ${\mrX}\cap M^{\mr x}$.
\end{proof}
...
Most theorems in this section have a formulation that only requires the stability of a given formula $\phi({\mr x}\,;{\gr z})\in L$ or a set of formulas $\Delta\subseteq L_{{\mr x}\,{\gr z}}$.
...
\begin{corollary}\label{corol_stable_coheir_over_models}
  [$\mrX,\grZ$ type-definable over $A$; $\Delta\subseteq L_{{\mr x}\,{\gr z}}(A)$ stable.]
  Let $q({\mr x})\subseteq L(\grZ)$ be finitely satisfiable in $\mrX\cap M^{\mr x}$ for every $M\supseteq A$.
  Then there is a type $p({\mr x})\in S_\Delta(\grZ)$ such that $q({\mr x})\cup p({\mr x})$ is finitely satisfiable in $\mrX\cap M^{\mr x}$ for every $M\supseteq A$.
\end{corollary}

\begin{proof}
  Let $p({\mr x})\subseteq \pmDelta(\grZ)$ be maximal such that $q({\mr x})\cup p({\mr x})$ is finitely satisfiable in $\mrX\cap M^{\mr x}$ for every $M\supseteq A$.
  We prove that $p({\mr x})$ is complete.
  Suppose not, then there are a conjunction $\alpha({\mr x})$ of formulas in $q({\mr x})$, a conjunction $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ of formulas in $p({\mr x})$, and a formula $\phi({\mr x}\,;{\gr b})$ for some $\phi({\mr x}\,;{\gr z})\in\Delta$ such that 
  ...
  are not satisfied in $\mrX\cap M^{\mr x}$ for every $M\supseteq A$.
  The disjunction of the two formulas is satisfied in $\mrX\cap M^{\mr x}$ for every $M\supseteq A$ by the definition of $p({\mr x})$.
...
In fact, when $T$ is stable (i.e., when all formulas are stable), it implies that

\ceq{\hfill{\mr a}\cnonfork_M{\gr b}}{\IMP}{{\gr b}\cnonfork_M{\mr a}.}

\begin{theorem}[ (Symmetry)]\label{thm_symmetry}
  [$\mrX,\grZ$ type-definable over $A\subseteq M$]
  Let $\phi({\mr x}\,;{\gr z})\in L(M)$ be stable.
  Let ${\mr a}\in\mrX$ and ${\gr b}\in\grZ$ be such that ${\mr a}\cnonfork_M{\gr b}$.
  Then

  \ceq{\hfill\phi({\mr a}\,;{\gr b})}{\IMP}{\phi({\mr a}\,;M^{\gr z})\neq\varnothing.}

  When $M$ is sufficiently saturated we can also claim that $\phi({\mr a}\,;\grZ\cap M^{\gr z})\neq\varnothing$. 
\end{theorem}
...
\begin{proof}
  ...
  By stability, there is a formula $\psi({\gr z})\in L(\mrX\cap\V^{\mr x})$ such that $\psi(\grZ\cap\V^{\gr z})=\phi({\mr a}\,;\grZ\cap\V^{\gr z})$.
  Recall that by Lemma~\ref{lem_coheir_independence} (non-splitting) if ${\gr b'}\equiv_M{\gr b''}$ are in $\V^{\gr z}$ then ${\gr b'}\equiv_{M,\kern.2ex{\mr a}}{\gr b''}$.
  Then $\phi({\mr a}\,;\grZ\cap\V^{\gr z})$ is invariant under $\Aut(\V/M)$, and so is $\psi(\grZ\cap\V^{\gr z})$.
  Therefore we can assume that $\psi({\gr z})\in L(M)$, see Exercise~\ref{ex_inv_def}.
  Then $\psi({\gr z})$ is satisfied in $M$ and so is $\phi({\mr a},{\gr z})$.
\end{proof}
...
\begin{corollary}\label{corol_harrington0}
  ...
  Let ${\mr a}\in\mrX$ and ${\gr b}\in\grZ$.
...
\begin{proof}
  ...
  Assuming $\neg\phi({\mr a}\,;{\gr b})$ we obtain $\neg\theta({\mr a})$ and $\neg\psi({\gr b})$ by a similar argument.
...
Let $\Delta\subseteq L_{{\mr x}\,{\gr z}}$ and $A\subseteq\U$ be given.e
...
Finally, define \emph{$\GDelta(A)$\/} to be the set of formulas $\phi({\mr x})\in\qfDelta(\U)$ that are invariant over $A$.
...
\begin{theorem}[ (Stationarity)]\label{thm_stationarity}
  ...
  Let $q({\mr x})\subseteq L(\U)$ be a type that is invariant over $\acl^\eq\!A$ decides all formulas in $\Delta(\U^{\gr z})$ that are invariant over $\acl^\eq\!A$.
  Then there is a unique type $p({\mr x})\in S_\Delta(\U^{ge z})$ that is consistent with $q({\mr x})$ and is invariant over $\acl^\eq A$.
\end{theorem}

\begin{proof}
  Existence follows from Corollary~\ref{corol_stable_coheir_over_models} and Proposition~\ref{prop_type_over_acl2}.
  Suppose that $p_i({\mr x})\in S_\Delta(\U)$, for $i=1,2$, are two types that are consistent with $q({\mr x})$ and invariant over $\acl^\eq\!A$.
  Let $\phi({\mr x}\,;{\gr z})\in\Delta$ and ${\gr b}\in\U^{\gr z}$ be given.
  Pick ${\mr a_i}\models p_i({\mr x})\restriction \acl^\eq\!A,{\gr b}$.
  It suffices to show that $\phi({\mr a_1}\,;{\gr b})\iff\phi({\mr a_2}\,;{\gr b})$.
  Note that we can choose the ${\mr a_i}$ so that ${\mr a_1},{\mr a_2}\cnonfork_M{\gr b}$.
  As $p_i({\mr x})$ are finitely satisfied in $M$, by symmetry 

  It suffices to prove that ${\gr\D_{p_1,\phi}}={\gr\D_{p_2,\phi}}$ for every $\phi({\mr x}\,;{\gr z})\in\Delta$.

  Let $M\subseteq A$.
  Let $\psi_i({\mr x_1},\dots,{\mr x_n}\,;{\gr z})$ be such that ${\gr\D_{p_i,\phi}}=\psi_i({\mr a_1},\dots,{\mr a_n}\,;\grZ)$ where ${\mr a_1},\dots,{\mr a_n}\in\mrX\cap M^{\mr x}$.

  
  By Proposition~\ref{prop_type_over_acl2}, the types $p_i({\mr x})$ are finitely satisfied in $\mrX\cap M^{\mr x}$.
  We claim that $\psi_1({\gr b})\iff\psi_2({\gr b})$, for all ${\gr b}\in\grZ$.

  These formulas exist by stability.
  By invariance we can assume that they have parameters in $\acl^\eq\!A$.
...
It has an application in the proof of Theorem~\ref{thm_persistent_finsat}.
...
The set $\Delta\subseteq L_{{\mr x}\,{\gr z}}$ is fixed and, generally, assumed to be stable.
In this section we always work with the language $\LDelta$.
In particular the algebraic closure in always in $\LDelta$.

We consider the action of two groups $\Aut(\UDelta)$ and $\Autf(\UDelta)$.
%, which by Corollary~\ref{corol_Ltype_Shtype} coincides with the automorphism group $\UDelta$ over $\LDelta\mbox{-}\acl^\eq\varnothing$.
The set $\grZ$ is fixed but arbitrary.
The set $\mrX$ is to be inferred from the context.
We say \emph{\DLascar\/} generic (over $\varnothing$) for generic under the action of $\Autf(\UDelta)$.
Similarly for persistent/invariant/wide.
%We say \emph{generic/persistent/wide\/} over $\acl^\eq\varnothing$ when the action is of 

\begin{theorem}\label{thm_persistent_finsat}
  Let $\Delta$ be stable.
  Assume that $\mrX$ and $\grZ$ are type-definable and invariant over  $\Aut(\UDelta/\acl^\eq\varnothing)$.
  Let $\mrD$ be a $\qfDelta(\grZ)$-definable set.
  Then the following are equivalent 
  \begin{itemize}
    \item[1.] $\mrD$ is \DLascar\ persistent
    \item[2.] $\mrD$ is \DLascar\ wide.
    \item[3.] $\mrD$ is persistent under the action of $\Aut(\UDelta/\acl^\eq\varnothing)$ 
    \item[4.] $\mrD$ is wide under the action of $\Aut(\UDelta/\acl^\eq\varnothing)$.
  \end{itemize}
\end{theorem}
\begin{proof}
  Implications \ssf4$\IMP$\ssf3 and \ssf2$\IMP$\ssf1 are trivial.
  Implications \ssf4$\IMP$\ssf3 and \ssf3$\IMP$\ssf1 hold because $\Autf(\UDelta)\le\Aut(\UDelta/\acl^\eq\varnothing)$.
  ...
  It suffices to prove that $\psi({\mr x})\wedge\phi({\mr x}\,;{\gr b})$ is satisfied in every $\LDelta$-model $M$.
  Then from Corollary~\ref{corol_stable_coheir_over_models} it follows that $q({\mr x})\cup\{\phi({\mr x}\,;{\gr b})\}$ is contained in a type $p({\mr x})\in S_\Delta(\grZ)$ that is Lascar invariant over $A$.
  Then $\mrD$ is Lascar wide by Theorem~\ref{thm_generic_invariant2}.
  
  Let $M$ be an $\LDelta$-model.
  Then $q({\mr x})\subseteq\LDelta(M)$.
  Let $\psi({\mr x})$ be a conjunction of formulas in $q({\mr x})$. 
  Let $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ be the formula in the proof of Theorem~\ref{thm_stability_definable_rovescio1}.
  As $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ is equivalent to a formula in $\LDelta(M)$, by elementarity it satisfiable in $M$.
  Hence so is $\psi({\mr x})\wedge\phi({\mr x}\,;{\gr b_i})$ for some $i$.
  As ${\gr b_i}\equiv_M{\gr b}$, also $\phi({\mr x}\,;{\gr b})$ is satisfiable.

  \ssf2$\IMP$\ssf4.
  Assume \ssf2.
  Then there is a type $p({\mr x})\in S_\Delta(\grZ)$ that is Lascar persistent and $p({\mr x})\proves{\mr x}\in\mrD$.
  By Theorem~\ref{prop_type_over_acl2} $p({\mr x})$ is invariant (or persistent) over $\acl^\eq\varnothing$.
  Then $\mrD$ is wide under the action of $\Aut(\UDelta/\acl^\eq\varnothing)$.
\end{proof}
...
% \begin{proof}
%   Let $\mrD$ be a $\qfDelta(\U)$-definable set that is weakly Lascar persistent.
%   We prove that $\mrD$ is Lascar persistent.
%   Pick a finite $H\subseteq\Autf(\U/A)$ such that $\cup\,H\,\mrD$ is Lascar persistent.
%   By Theorem~\ref{thm_persistent_finsat}, the type $\{{\mr x}\in\cup\,H\,\mrD\}\cup q({\mr x})$ is finitely satisfiable in every $M\supseteq A$.
...
\begin{remark}
  Let $\Delta$ be stable.
  Let $G=\Aut(\UDelta)$ and $H=\Autf(\UDelta)$.
  Assume that $\mrX$ and $\grZ$ are type-definable and invariant over $G$.
  From Theorem~\ref{thm_persistent_finsat} and Remark~\ref{rem_coalesce} it follows that $\gamma_H({\mr x})$ is consistent.
  Therefore, by Proposition~\ref{prop_Ggeneric_Hpersistent}, G-generic sets are $H$-wide.
\end{remark}

We want a version of Theorem~\ref{thm_Shelah_strong_types} and Proposition~\ref{prop_trans_action} that includes requirements on the language.
The proof is essentially the same (but it would be rude to leave it as an exercise).

Theorem~\ref{thm_persistent_finsat} shows that when $\Delta$ is stable Theorem~\ref{thm_coalesce} applies with $\Autf(\U)$ for $G$.
Therefore, in particular, \DLascar\ generic sets are \DLascar\ wide.
The following remark strengthens this result.

\begin{fact}
  Let $\mrD\in\acl^\eq\varnothing$.
  Then $\mrD$ is generic under the action of $\Aut(\UDelta)$.
\end{fact}
...
\begin{lemma}\label{lem_trans_action}
  ...
  Let $\Delta$ be stable.
  Define, for convenience, $G=\Aut(\UDelta)$ and $H=\Autf(\UDelta)$.
  Assume that $\mrX$ and $\grZ$ are type-definable and invariant over $G$.
  Also, assume that $G$ acts transitively on $\mrX$.
  Finally, let 
  
  \ceq{\hfill P}{=}{\big\{p({\mr x})\in S_\Delta(\grZ)\ :\ p({\mr x})\textrm{ is $H$-invariant}\big\}}.
  
  Then $G$ acts transitively on $P$.
  Moreover, for any ${\mr a}\models p({\mr x})$ there is a unique $q({\mr x})\in Q$ such that

  \ceq{\hfill q({\mr x})}{\iff}{\big\{\epsilon({\mr x}\,;{\mr a})\ :\ \epsilon({\mr x}\,;{\mr y})\in\GDelta(A)\textrm{ finite equivalence relation}\big\}}.

\end{lemma}

\begin{proof}
  We prove that any two types $p_1,p_2\in P$ are realized be some $a_1,a_2$ that are .
  ...
  Then $p({\mr x})\imp\neg\psi({\mr x})$.
  But $\psi({\mr x})\in\qfDelta(\acl^\eq\!A)$ and is invariant over $A$.
  ...
  As this is definable and invariant over $A$, it belongs to $\GDelta(A)$.
...
\begin{proof} 
  ...
  \ssf2$\IMP$\ssf3. Clear, because $\Autf(\U/A)\le\Autf(\UDelta)$.

  \ssf3$\IMP$\ssf1.
  For ${\mr a}\in\mrX$ let $q({\mr x})=\tp({\mr a}/\acl^\eq\!A)$.
  Assume \ssf2 then, by Proposition~\ref{prop_stable_genericiffpersistent}, $\mrD$ is Lascar generic relative to $q(\U^{\mr x})$, i.e.\@ with $q(\U^{\mr x})$ for $\mrX$.
...
\begin{proposition}
  Let $\Delta$ be stable.
  Assume that the action of $\grZ$ is transitive.
  Then every $\grZ$-generic type $q({\mr x})\in S_\Delta(\grZ)$ is fixed by any ${\gr g}\in\gr\Z^0$.
\end{proposition}
...
% \begin{proof}
%   By Theorem~\ref{thm_Ggeneric_persistent}  

%   \ceq{\hfill{\gr\Z^0}}{\subseteq}{\big\{{\gr g}\in\grZ\ :\ \mrD\cap{\gr g}{\cdot}\mrD\neq\varnothing\textrm{ for every }\qfDelta(\grZ)\textrm{-definable }H\textrm{-persistent set }\mrD\big\}}
