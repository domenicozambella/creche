...
\begin{proof}
  ...
  If $\mrD$ is not $H$-generic then $\neg\mrD$ is $H$-persistent and, by Proposition~\ref{prop_stable_genericiffpersistent}, $H$-generic.
\end{proof}
...
\begin{proof} 
  ...
  Then $\gamma_H({\mr x})$ is consistent and Proposition~\ref{prop_Ggeneric_Hpersistent1} applies.
  ...
  Then $\mrD$ is $H$-wide relative to $q(\U^{\mr x})$ and, by Proposition~\ref{prop_stable_genericiffpersistent}, $H$-generic relative to $q(\U^{\mr x})$.
  For every $g\in G$ we also have that $g{\cdot}\mrD$ is $H$-generic relative to $g{\cdot}q(\U^{\mr x})$.
  Therefore $\mrD$ is also $H$-generic relative to $g{\cdot}q(\U^{\mr x})$.
...
\begin{proof} 
  \ssf1$\IMP$\ssf2.
  Because $\grZ$-generic implies $G$-generic.
  
  \ssf2$\IMP$\ssf3.
  By Theorem~\ref{thm_Ggeneric_persistent}, which applies because, by transitivity, the elements of $\mrX$ have the same $\LDelta$-type.

  \ssf3$\IMP$\ssf1.
  After replacing $G$ by $\grZ$, we may repeat the proof of \ssf2$\IMP$\ssf1 of Theorem~\ref{thm_Ggeneric_persistent}.
  %
  % \ssf1$\IMP$\ssf2.
  % Let ${\gr g}$ be given.
  % By stability, there is a type $p({\mr x})\in L'\mbox{-}S(\grZ)$ that is invariant, and therefore definable, over $L'\mbox{-}\acl^\eq\varnothing$.
...
Corollary~\ref{corol_stable_generic} becomes the following.,
