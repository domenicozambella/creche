...
We say that $\grD$ is \emph{externally definable\/} if it is of the form ${\gr\D_{p,\phi}}$ for some type $p({\mr x})\in S_\phi(\U^{\gr z})$.
...
\begin{corollary}\label{corol_harrington0}
  ...
  Suppose there exist $\mrA\subseteq\U^{\mr x}$ and $\grB\subseteq\U^{\gr z}$ that are $\BDelta(M^{\gr z})$, respectively $\BDelta(M^{\mr x})$-definable and 

  \ceq{1.\hfill\phi({\mr a}\,;M^{\gr z})}{=}{\grB\cap M^{\gr z}}

  \ceq{2.\hfill\phi(M^{\mr x};{\gr b})}{=}{\mrA\cap M^{\mr x}},
...
\begin{corollary}\label{corol_harrington1}
  ...
  Let ${\mr a}\in\U^{\mr x}$ and ${\gr b}\in\U^{\gr z}$ be such that 

  \ceq{\ssf1.\hfill\phi({\mr a}\,;M^{\gr z})}{=}{M^{\gr z}\cap {\gr\D_{p,\phi}}}

  \ceq{\ssf2.\hfill\phi(M^{\mr x};{\gr b})}{=}{M^{\mr x}\cap {\mr\D_{q,\phi^{\rm op}}}}.

  Then ${\mr a}\in{\mr\D_{q,\phi^{\rm op}}}\ \iff\ {\gr b}\in{\gr\D_{p,\phi}}$
\end{corollary}
...
\begin{theorem}[ (Stationarity)]\label{thm_stationarity}
  Let $\Delta$ be stable.
  Let $q({\mr x})\subseteq L(\U)$ be a type that is invariant over $\acl^\eq\!A$ and decides all $\BDelta(\U^{\gr z})$-formulas that are invariant over $\acl^\eq\!A$.
  Then there is a unique type $p({\mr x})\in S_\Delta(\U^{\gr z})$ consistent with $q({\mr x})$ that is invariant over $\acl^\eq\!A$.
\end{theorem}

The assumptions on $q({\mr x})$ are a mouthful.
If we replace $\BDelta$ with $\LDelta$ we could simply require $q({\mr x})$ to be a complete type over $\LDelta\mbox{-}\acl^\eq\varnothing$.

\begin{proof}
  Existence follows from Corollary~\ref{corol_stable_coheir_over_models} and Proposition~\ref{prop_type_over_acl2}.
  To prove uniqueness, pick two types $p_i({\mr x})\in S_\Delta(\U^{\gr z})$, for $i=1,2$, that are consistent with $q({\mr x})$ and invariant over $\acl^\eq\!A$.
  ...
  By stability there exists a type $t({\gr z})\in S_\phi(\U^{\mr x})$ that is invariant over $\acl^\eq\!A$ and consistent with $\tp({\gr b}/\acl^\eq\!A)$.
  By stability and invariance ${\gr\D_{p_i,\phi}}$ and ${\mr\D_{t,\phi^{\rm op}}}$ are in $\acl^\eq\!A$.
  Let $M\supseteq A$.
  Let ${\gr b'}$ be such that ${\gr b}\equiv_{\acl^\eq\!A}{\gr b'}\models t({\gr z}){\restriction}M$.
  Let ${\mr a_i}\models\big(p_i({\mr x})\cup q({\mr x})\big){\restriction}M$.
  From Corollary~\ref{corol_harrington1} we obtain ${\mr a_i}\in{\mr\D_{t,\phi^{\rm op}}}\iff{\gr b'}\in{\gr\D_{p_i,\phi}}$.
...
In this section we always work with the language $\LDelta$, somewhat pedantically, the notation will keep reminding this.

Unless otherwise stated $\mrX=\U^{\mr x}$ and $\grZ=\U^{\gr z}$.
...
\begin{theorem}\label{thm_persistent_finsat}
  ...
  Then the following are equivalent 
  \begin{itemize}
    \item[1.] $\mrD$ is \DLascar\ persistent/invariant
    \item[2.] $\mrD$ is \DLascar\ wide.
    \item[3.] $\mrD$ is persistent under the action of $\Aut(\UDelta/\LDelta\mbox{-}\acl^\eq\varnothing)$ 
    \item[4.] $\mrD$ is wide under the action of $\Aut(\UDelta/\LDelta\mbox{-}\acl^\eq\varnothing)$.
  \end{itemize}
\end{theorem}
\begin{proof}
  Implications \ssf4$\IMP$\ssf3 and \ssf2$\IMP$\ssf1 are trivial.
  Implications \ssf4$\IMP$\ssf3 and \ssf3$\IMP$\ssf1 hold because $\Autf(\UDelta)\le\Aut(\UDelta/\LDelta\mbox{-}\acl^\eq\varnothing)$.
  ...
  It suffices to prove that $\phi({\mr x}\,;{\gr b})$ is satisfied in every $\LDelta$-model $M$.
  Then from Corollary~\ref{corol_stable_coheir_over_models} it follows that $\phi({\mr x}\,;{\gr b})$ is contained in a type $p({\mr x})\in S_\Delta(\grZ)$ that is \DLascar\ invariant.
  Then $\mrD$ is \DLascar\ wide by Theorem~\ref{thm_generic_invariant2}.
  ...
  \ssf2$\IMP$\ssf4.
  Assume \ssf2.
  Then there is a type $p({\mr x})\in S_\Delta(\grZ)$ that is \DLascar\  persistent and $p({\mr x})\proves{\mr x}\in\mrD$.
  By Theorem~\ref{prop_type_over_acl2} $p({\mr x})$ is invariant over $\LDelta\mbox{-}\acl^\eq\varnothing$.
  Then $\mrD$ is wide under the action of $\Aut(\UDelta/\LDelta\mbox{-}\acl^\eq\varnothing)$.
\end{proof}

The above theorem shows that when $\Delta$ is stable Theorem~\ref{thm_coalesce} applies with $\Autf(\UDelta)$ for $G$.
...
% \begin{proof}
%   Let $\mrD$ be a $\BDelta(\U)$-definable set that is weakly \DLascar\  persistent.
%   We prove that $\mrD$ is \DLascar\  persistent.
%   Pick a finite $H\subseteq\Autf(\U/A)$ such that $\cup\,H\,\mrD$ is \DLascar\  persistent.
%   By Theorem~\ref{thm_persistent_finsat}, the type $\{{\mr x}\in\cup\,H\,\mrD\}\cup q({\mr x})$ is finitely satisfiable in every $M\supseteq A$.
%   Then, by Corollary~\ref{corol_stable_coheir_over_models} there is a \DLascar\  invariant type $p({\mr x})\in S_\Delta(\U)$ containing ${\mr x}\in\cup\,H\,\mrD$ and finitely consistent with $q({\mr x})$.
...
\begin{proposition}\label{prop_stable_genericiffpersistent}
  Let $\Delta$ be stable.
  Let $\mrX=q(\U^{\mr x})$ where $q({\mr x})\subseteq\BDelta(\grZ)$  is invariant over $\LDelta\mbox{-}\acl^\eq\varnothing$ and decides all $\BDelta(\grZ)$-formulas that are invariant over $\LDelta\mbox{-}\acl^\eq\varnothing$.
  Then for every $\BDelta(\grZ)$-definable set $\mrD$, the following are equivalent
  \begin{itemize}
    \item[1.] $\mrD$ is \DLascar\  generic
    \item[2.] $\mrD$ is \DLascar\  persistent.
  \end{itemize}
\end{proposition}
...
\begin{proof}
  ...
  \ssf2$\IMP$\ssf1.
  Assume $\mrD$ is not \DLascar\  generic.
  Then $\neg\mrD$ is \DLascar\  persistent.
  Assume for a contradiction that also $\mrD$ is \DLascar\ persistent.
  By Theorem~\ref{thm_persistent_finsat} and Exercise~\ref{ex_X_vs_U} both $\mrD$ and $\neg\mrD$ are \DLascar\ wide.
  Then by Theorem~\ref{thm_generic_invariant2} both $q({\mr x})\cup\{{\mr x}\in\mrD\}$ and $q({\mr x})\cup\{{\mr x}\notin\mrD\}$ extend to \DLascar\ invariant global types.
...
\begin{corollary}\label{corol_stable_generic}
  Let $\Delta$ be stable.
  Let $\mrX=q(\U^{\mr x})$ where $q({\mr x})\subseteq\BDelta(\grZ)$ is a type that is invariant over $\LDelta\mbox{-}\acl^\eq\varnothing$ and decides all $\BDelta(\grZ)$-formulas that are invariant over $\LDelta\mbox{-}\acl^\eq\varnothing$.
  Then for every $\BDelta(\grZ)$-definable set $\mrD$, either $\mrD$ or $\neg\mrD$ is \DLascar\ generic.
\end{corollary}

\begin{proof}
  By Theorem~\ref{thm_stationarity} there is a type $p({\mr x})\in S_\Delta(\grZ)$ finitely consistent with $q({\mr x})$ that is \DLascar\ invariant and therefore persistent.
...
\begin{theorem}\label{thm_FER}
  Let $\Delta$ be stable.
  Let $\mrX=q(\U^{\mr x})$, for $q({\mr x})\subseteq\BDelta(\grZ)$ be a type that is invariant (over $\varnothing$) and decides all invariant $\BDelta(\grZ)$-formulas.
  Finally, let \smallskip
  
  \ceq{\hfill P}{=}{\big\{p({\mr x})\in S_\Delta(\grZ)\ :\ p({\mr x})\textrm{ is \DLascar\ invariant}\big\}}. \smallskip
  
  Then the following hold
  \begin{itemize}
    \item [1.] $\Aut(\UDelta)$ acts transitively on $P$
    ...
    % 
    \item [3.] Let $E$ be the set of finite equivalence relations on $\mrX$ that are $\BDelta(\grZ)$-definable and invariant over $\varnothing$.%under the action of $\Aut(\UDelta)$.
    For every ${\mr a}\in\mrX$  there is a unique $p({\mr x})$ in $P$ that is consistent with $\big\{\epsilon({\mr x}\,;{\mr a})\ :\ \epsilon({\mr x}\,;{\mr y})\in E\big\}$.
...
\begin{proof}
  ...
  Let $q_i({\mr x})$ be the set of $\BDelta(\grZ)$-formulas that are consequence of $p_i({\mr x})$.
  By Lemma~\ref{lem_trans_action} $f\,q_1=q_2$ for some $f\in\Aut(\UDelta)$.
  ...
  The orbit of $p({\mr x})$ under the action of $\Aut(\UDelta)$ corresponds to the orbit of $\<{\gr\D_{p,\phi}}:\phi({\mr x}\,;{\gr z})\in\Delta\>$.
  As $p\in P$ is \DLascar\ invariant, by Theorem~\ref{thm_persistent_finsat} the sets ${\gr\D_{p,\phi}}$ are in $\acl^\eq\varnothing$.
  ...
  \ssf3. 
  Let $p_i:i<\lambda$ be an enumeration without repetitions of $P$.
  Let $q_1({\mr x})$ be some complete $\BDelta(\acl^\eq\!A)$-type consistent with $p_1$.
  It is easy to see that the orbit of $q_1({\mr x})$ under the action of $\Aut(\U/A)$ consists of $\BDelta(\acl^\eq\!A)$-types $q_1,\dots,q_n$ that are complete and consistent with $p_1,\dots,p_n$.
  ...
  From the definition of $\epsilon({\mr x},{\mr y})$ we know that it belongs to $\BDelta(\acl^\eq\!A)$, but, as $\epsilon({\mr x},{\mr y})$ is invariant over $A$, it is equivalent to a formula in $L(A)$.
  Hence \ssf3 follows.
\end{proof}

\begin{exercise}\label{ex_X_vs_U}
  Let $q({\mr x})\subseteq\BDelta(\U^{gr z})$.
  Prove that if $\mrD$ is wide relative to $\mrX=q(\U^{\mr x})$, than it is wide relative to $\U^{\mr x}$.
\end{exercise}
...
\begin{theorem}\label{thm_Ggeneric_persistent}
  ...
  \begin{itemize}
    ...
    \item [2.] $\mrD$ is \DLascar\ invariant.
  \end{itemize}\smallskip

  In particular it follows that, if these properties hold for $\mrD$, they hold also for ${\gr g}{\cdot}\mrD$ for any ${\gr g}\in\grZ$. 
\end{theorem}

\begin{proof} 
  \ssf1$\IMP$\ssf2.
  By stability, there exist a type $p({\mr x})\in S_\Delta(\grZ)$ that is invariant under the action of $\Autf(\UDelta)$.
  As $\mrD$ is generic under the action of $\Aut(\UDelta)$, we can apply Corollary~\ref{prop_Ggeneric_persistent} with $H=\Autf(\UDelta)$ to obtain that $\mrD$ is persistent/invariant under the action of $\Autf(\UDelta)$.

  \ssf2$\IMP$\ssf1.
  For ${\mr a}\in\mrX$ let $q({\mr x})$ be the set of $\BDelta(\grZ)$-formulas that hold in ${\mr a}$ and are invariant over $\LDelta\mbox{-}\acl^\eq\varnothing$.
  Assume \ssf2 then, by Proposition~\ref{prop_stable_genericiffpersistent}, $\mrD$ is \DLascar\ generic relative to $q(\U^{\mr x})$, i.e.\@ with $q(\U^{\mr x})$ for $\mrX$.
  As the same reasoning applies to all ${\mr a}\in\mrX$, there is a set ${\gr Z}\subseteq\grZ$ of cardinality $<2^{|L(A)|}$ such that $\bigcup_{{\gr h}\in{\gr Z}}{\gr h}{\cdot}\mrD$ covers $\mrX$.
  By compactness, we can assume ${\gr Z}$ finite, then the $\grZ$-genericity of $\mrD$ follows.
  % \ssf1$\IMP$\ssf2.
  % Let ${\gr g}$ be given.
  ...
  % This shows that there is \DLascar\ invariant type containing $\phi({\mr x}\,;{\gr g})$.
  % As the finite satisfiability in $\mrX$ is evident, \ssf1 follows.
  % \ssf3$\IMP$\ssf1.
  % Let ${\gr g}$ be such that $\phi({\mr x}\,;{\gr g})$ is not \DLascar\ persistent.
  % Then $\neg\phi({\mr x}\,;{\gr g})$ is \DLascar\ generic.
...
\begin{corollary}\label{corol_feq_thm}
  ...
  \begin{itemize}
    ...
    \item [2.] $Q$ is finite
    \item [3.] there is a $\grZ$-invariant finite equivalence relation $\epsilon({\mr x}\,;{\mr y})\in\BDelta(A)$ such that for every ${\mr a}\in\mrX$ there is a unique $q({\mr x})$ in $Q$ that is consistent with $\epsilon({\mr x}\,;{\mr a})$.
  \end{itemize}
\end{corollary}
...
%   \ceq{\hfill{\gr\Z^0}}{\subseteq}{\big\{{\gr g}\in\grZ\ :\ \mrD\cap{\gr g}{\cdot}\mrD\neq\varnothing\textrm{ for every }\BDelta(\grZ)\textrm{-definable }H\textrm{-persistent set }\mrD\big\}}
