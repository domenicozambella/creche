...
\begin{theorem}\label{thm_FER}
  ...
  \begin{itemize}
    ...
    % 
    \item [3.] let $E$ be as in Lemma~\ref{lem_trans_action0}~--~then for every ${\mr a}\in\mrX$ there is a unique $p({\mr x})\in P$ that is consistent with $\big\{\epsilon({\mr x}\,;{\mr a})\ :\ \epsilon({\mr x}\,;{\mr y})\in E\big\}$.
  \end{itemize}
\end{theorem}
...
\begin{proof} 
  \ssf1$\IMP$\ssf2$\IMP$\ssf3. Because $\grZ$-generic implies $G$-generic, and by Theorem~\ref{thm_Ggeneric_persistent}.

  \ssf3$\IMP$\ssf1.
  After replacing $G$ by $\grZ$, the proof of \ssf2$\IMP$\ssf1 in Theorem~\ref{thm_Ggeneric_persistent} applies (by transitivity, the $\grZ$-orbit of $q(\U^{\mr x})$ covers $t(\U^{\mr x})=\mrX$).
  %
  % \ssf1$\IMP$\ssf2.
  % Let ${\gr g}$ be given.
  % By stability, there is a type $p({\mr x})\in L'\mbox{-}S(\grZ)$ that is invariant, and therefore definable, over $L'\mbox{-}\acl^\eq\varnothing$.
...
\begin{corollary}\label{corol_genericornotgeneric}
  Let $\Delta$ be stable.
  Assume that $\grZ$ acts transitively.
  Let $\mrD$ be any $\BDelta(\grZ)$-definable set.
