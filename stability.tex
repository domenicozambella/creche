...
\begin{corollary}\label{corol_stable_coheir_over_models}
  ...
  Let $q({\mr x})\subseteq L(\U)$ be finitely satisfiable in every $M\subseteq A$.
  Then there is a global $\phi$-type $p({\mr x})\in S_{\phi}(\U)$ such that $q({\mr x})\cup p({\mr x})$ is finitely satisfiable in every $M\supseteq A$, in particular invariant over $\acl^\eq\!A$.
\end{corollary}
...
\begin{proof}
  ...
  Let $q_i({\mr x})\in S_\phi(\U^\eq)$, for $i=1,2$, be two types that extend $q({\mr x})$ and are invariant over $\acl^\eq\!A$.
...
\begin{corollary}\label{corol_persistent_finsat}
  ...
  Then the following are equivalent
  \begin{itemize}
    \item[1.] $\phi({\mr a}\,;{\gr z})$ is persistent in $\U^{\gr z}$ under the action of $\Autf(\U/A)$
...
\begin{proof}
  ...
  \ssf2$\IMP$\ssf1.
  From \ssf2 it follows that $\phi({\mr a}\,;{\gr z})$ is contained in a global type $p({\gr z})$ that is Lascar invariant over $A$.
