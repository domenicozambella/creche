...
\begin{proof}
  ...
  \ssf2$\IMP$\ssf1.
  From \ssf2 and Corollary~\ref{corol_stable_coheir_over_models} it follows that $\phi({\mr a}\,;{\gr z})$ is contained in a global type $p({\gr z})$ that is finitely satisfied in every $M\subseteq A$.
...
\begin{proposition}\label{prop_stable_genericiffpersistent}
  ...
  \begin{itemize}
    \item[1.] $\mrD$ is generic in $q(\U^{\mr x})$.
    \item[2.] $\mrD$ is persistent in $q(\U^{\mr x})$.
  \end{itemize}
\end{proposition}
...
\begin{proof}
  ...
  By Theorem~\ref{thm_generic_invariant} $p({\mr x})\proves{\mr x}\in\mrD$.
\end{proof}
