...
\begin{corollary}\label{corol_harrington0}
  ...
  % Recall that by stability ${\gr\D_{p,\phi}}$ and ${\mr\D_{q,\phi^{\rm op}}}$ are definable.
  Let ${\mr a}\in\U^{\mr x}$ and ${\gr b}\in\U^{\gr z}$ be such that 

  \ceq{1.\hfill\phi({\mr a}\,;M^{\gr z})}{=}{M^{\gr z}\cap {\gr\D_{p,\phi}}}

  \ceq{2.\hfill\phi(M^{\mr x};{\gr b})}{=}{M^{\mr x}\cap {\mr\D_{q,\phi^{\rm op}}}}.
...
\begin{proof}
  By stability ${\gr\D_{p,\phi}}$ and ${\mr\D_{q,\phi^{\rm op}}}$ are definable.
  By invariance they are definable over $M$.
  Note that we can assume ${\mr a}\cnonfork_M{\gr b}$.
  Then, from \ssf2 we obtain immediately

  \ceq{2$'$.\hfill\phi({\mr a};{\gr b})}{\iff}{{\mr a}\in {\mr\D_{q,\phi^{\rm op}}}}.

  The corollary follows if we can also show that 

  \ceq{1$'$.\hfill\phi({\mr a}\,;{\gr b})}{\iff}{{\gr b}\in {\gr\D_{p,\phi}}}

  Negate \ssf1$'$ for a contradiction.
  The formula $\phi({\mr x}\,;{\gr z})\niff{\gr z}\in {\gr\D_{p,\phi}}$ is stable, therefore by Theorem~\ref{thm_symmetry} $\neg\ssf1'$ entails that $\phi({\mr a}\,;{\gr b'})\niff{\gr b'}\in {\gr\D_{p,\phi}}$ for some ${\gr b'}\in M^{\gr z}$.
  This contradicts \ssf1.
\end{proof}
...
\begin{theorem}[ (Stationarity)]\label{thm_stationarity}
  ...
  Let $q({\mr x})\subseteq L(\U)$ be a type that is invariant over $\acl^\eq\!A$ and decides all $\qfDelta(\U^{\mr x})$-formulas that are invariant over $\acl^\eq\!A$.
...
\begin{proof}
  ...
  From Corollary~\ref{corol_harrington0} we obtain ${\mr a_i}\in{\mr\D_{t,\phi^{\rm op}}}\iff{\gr b'}\in{\gr\D_{p_i,\phi}}$.
...
We consider the action of the group $G=\Aut(\UDelta)$ and its normal subgroup $H=\Autf(\UDelta)$, which by Corollary~\ref{corol_Ltype_Shtype} coincides with the automorphism group $\UDelta$ over $\LDelta\mbox{-}\acl^\eq\!A$.
The set $\grZ\subseteq\U^{\gr z}$ is fixed but arbitrary.
The set $\mrX$ is to be inferred from the context: we may use $\U^{\mr x}$, a $G$-orbit or an $H$-orbit.
We say \emph{generic\/} over $A$ and \emph{Lascar generic\/} over $A$ for generic under the action of $\Aut(\UDelta/A)$, respectively $\Autf(\UDelta/A)$.
We use \emph{persistent/wide\/} similarly.

\begin{theorem}\label{thm_persistent_finsat}
  Let $\Delta\subseteq L_{{\mr x}\,{\gr z}}$ be stable.
  Let $\mrD$ be a $\qfDelta(\U^{\gr z})$-definable set.
  Then the following are equivalent 
  \begin{itemize}
...
\begin{proof}
  ...
  It suffices to prove that $\phi({\mr x}\,;{\gr b})$ is satisfied in every $M\supseteq A$.
  Then, by Corollary~\ref{corol_stable_coheir_over_models}, it follows that $\{\phi({\mr x}\,;{\gr b})\}$ is contained in a type $p({\mr x})\in S_\Delta(\U^{\gr z})$ that is Lascar invariant over $A$.
  Then $\mrD$ is Lascar wide by Theorem~\ref{thm_generic_invariant2}.
  
  Let $M\supseteq A$ be arbitrary.
  Let $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ be the formula in the proof of Theorem~\ref{thm_stability_definable_rovescio2}.
...
The above theorem shows that when $\Delta$ is stable Theorem~\ref{thm_coalesce} applies with $\Autf(\U/A)$ for $G$.
Therefore, in particular, Lascar generic sets are Lascar wide.
