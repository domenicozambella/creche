...
\begin{corollary}
  ...
  \begin{itemize}
    ...
    \item [2.] $Q$ is finite
    \item [3.] there is a $\grZ$-invariant finite equivalence relation $\epsilon({\mr x}\,;{\mr y})\in\GDelta(A)$ such that
...
\begin{proof}
  ...
  From this \ssf1 and \ssf2 follow immediately.
  ...
  Let $q_1,\dots,q_n$ be as defined in the proof of \ssf3 of Theorem~\ref{thm_FER}.
  Then each $q'\in Q$ is consistent with exactly one $q_i$.
  Then \ssf3 is clear because the types $q_i({\mr x})$ define the equivalence classes of $\epsilon({\mr x}\,;{\mr y})$.
\end{proof}

Define 

\ceq{\hfill\Stab_G(\grD)}{=}{\big\{g\in G\ :\ g\cdot\grD=\grD\big\}}

\ceq{\hfill\Stab_{G,\Delta}(p)}{=}{\bigcap_{\phi\in\Delta}\Stab_G(\gr\D_{p,\phi})}
