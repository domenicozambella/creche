...
(A major difference is that below in \ssf4 we can use a single finite equivalence relation for the relation $\equivSh$.)
...
\begin{theorem}\label{thm_FER}
  ...
  \begin{itemize}
    ...
    \item [3.] $P$ is finite
    \item [4.] there is a finite equivalence relation $\epsilon({\mr x}\,;{\mr y})\in L(A)$ such that for every $c\in\U^{\mr x}$ there is a unique $p({\mr x})\in P$ consistent with $\epsilon({\mr x}\,;{\mr c})$.
    \item[5.] the types in $P$ are $G$-syndetic.
  \end{itemize}
\end{theorem}
...
\begin{proof}
  ...
  Then, the first inclusion, $q({\mr x})=p({\mr x})$.
\end{proof}
