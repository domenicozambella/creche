...
\begin{exercise}
  Prove that a relation $\pi({\mr x}\,;{\gr z})$ is instable if and only if for every $n<\omega$ there is a set ${\gr A}$ and a sequence $\<{\mr a_i}:i<n\>$ such that $\pi({\mr a_i}\,;{\gr A})\subset\pi({\mr a_{i+1}}\,;{\gr A})$ for all $i<n$.
\end{exercise}
