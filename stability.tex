...
\begin{theorem}[ (Stationarity)]\label{thm_stationarity}
  ...
  Then there is a unique type $p({\mr x})\in S_\Delta(\U^{\gr z})$ consistent with $q({\mr x})$ that is Larcar invariant over $A$.
\end{theorem}

\begin{proof}
  Existence follows from Corollary~\ref{corol_stable_coheir_over_models} and Proposition~\ref{prop_type_over_acl2}.
  To prove uniqueness, pick two types $p_i({\mr x})\in S_\Delta(\U^{\gr z})$, for $i=1,2$, that are consistent with $q({\mr x})$ and Lascar invariant over $A$.
  ...
  Let $M\supseteq A$.
  By stability there exists a type $t({\gr z})\in S_\phi(\U^{\mr x})$ that is invariant over $M$ and is consistent with $\tp({\gr b}/M)$.
...
The set $\grZ\subseteq^{\gr z}$ is arbitrary, while $\mrX$ will be specified in the context.
As in Chapter~\ref{actions}, we abbreviate $\Aut(\UDelta)$ with \emph{$G$.}
...
The following theorem shows that when $\Delta$ is stable Theorem~\ref{thm_coalesce} applies.
% The second part is shows that in some cases $H$ can be replaced by $K$.

\begin{theorem}\label{thm_persistent_finsat}
  Let $\Delta$ be stable.
  Let $\mrX=q(\U^{\mr x})$, where $q({\mr x})\subseteq\LDelta(\mrX,\grZ)$ be a type that is finitely satisfiable in every $\LDelta$-model.
...
\begin{proof}
  ...
  As $p({\mr x})$ is $H$-persistent, $\mrD$ is $H$-wide by Theorem~\ref{thm_generic_invariant2}.
...
\begin{proposition}\label{prop_stable_genericiffpersistent}
  Let $\Delta$ be stable.
  Let $\mrX=q(\U^{\mr x})$, where $q({\mr x})\subseteq\LDelta(\mrX,\grZ)$ is a type that is finitely satisfiable in every $\LDelta$-model.
...
\begin{theorem}\label{thm_FER}
  ...
  \begin{itemize}
    ...
    % 
    \item [3.] Let $E$ be as in Lemma~\ref{lem_trans_action0}.
    Then for every ${\mr a}\in\mrX$ there is a unique $p({\mr x})\in P$ that is consistent with $\big\{\epsilon({\mr x}\,;{\mr a})\ :\ \epsilon({\mr x}\,;{\mr y})\in E\big\}$.
  \end{itemize}
\end{theorem}
...
\begin{proof}
  ...
  Let $\Phi$ be as in Lemma~\ref{lem_trans_action0}.
  ...
  \ssf3.
  Follows immediately from Lemma~\ref{lem_trans_action0}.
\end{proof}
...
\begin{proof} 
  ...
  Then $\gamma_H({\mr x})$ is consistent and Proposition~\ref{prop_Ggeneric_Hpersistent} applies.
  
  \ssf2$\IMP$\ssf1.
  % Negate \ssf1.
  % Then $\neg\mrD$ is $G$-persistent.
  By \ssf2, there is an $H$-invariant type $p({\mr x})\in S_\Delta(\grZ)$ consistent with ${\mr x}\in\mrD$.
  Let $q({\mr x})$ be the set of $\Phi$-formulas that are consequence of $p({\mr x})$, where $\Phi$ is as in Lemma~\ref{lem_trans_action0}.
  Then $\mrD$ is $H$-wide relative to $q(\U^{\mr x})$ and, by Proposition~\ref{prop_stable_genericiffpersistent}, $H$-generic.
  For every $g\in G$ we also have that $g{\cdot}\mrD$ is $H$-generic relative to $g{\cdot}q(\U^{\mr x})$.
  The $G$-orbit of $q(\U^{\mr x})$ has small cardinality and, by Lemma~\ref{lem_trans_action0}, covers $\mrX$.
...
Finally we prove that $Q=P$, where $Q$ is as defined in Definition~\ref{def_Q} and $P$ is as in Theorem~\ref{thm_Ggeneric_persistent}.
...
\begin{proof}
  Let $p({\mr x})\in P$.
  All formulas in $p({\mr x})$ are $H$-wide, and therefore $G$-generic.
...
\begin{proof} 
  \ssf1$\IMP$\ssf2.
  By Theorem~\ref{thm_Ggeneric_persistent} as $\grZ$-generic implies $G$-generic.
