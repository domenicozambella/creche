...
\def\medrel#1{\parbox[t]{6ex}{$\displaystyle\hfil #1$}}
\def\ceq#1#2#3{\parbox{8ex}{$\displaystyle #1$}\medrel{#2}$\displaystyle  #3$}
...
\begin{theorem}
  ...
  Let $p({\mr x})$ be complete $\GDelta(A)$-type.
  Let 
  
  \ceq{\hfill Q}{=}{\big\{q({\mr x})\subseteq\GDelta(\acl^\eq\!A)\ :\ q({\mr x})\supseteq p({\mr x})\textrm{ complete}\big\}}.
  
  Then
  \begin{itemize}
    \item [1.] $\Aut(\U/A)$ acts on $Q$ transistively, i.e.\@ any two types in $Q$ are conjugated over $A$
    \item [2.] $Q$ is finite
    \item [3.] for some finite equivalence relation $\varepsilon({\mr x}\,;{\mr y})\in\GDelta(A)$
  \end{itemize}
\end{theorem}
...
\begin{proof}
  1. 
  (The hypotheses on $\Delta$ are not necessary.) 
  We prove that any two types $q_1,q_2\in Q$ are realized be some $a_1\equiv_Aa_2$.
  The claim follows beause any automorphism in $\Aut(\U/A)$ that takes $a_1$ to $a_2$ takes also $q_1$ in $q_2$.
  ...
  Then $p'({\mr x})\imp\neg\psi({\mr x})$.
  But $\psi({\mr x})\in\BDelta(A)$ and is invariant over $A$.
  Therefore $\psi({\mr x})\in\GDelta(A)$.
  Then $\psi({\mr x})\in p$, a contradiction.
  
  2. Define 
  
  \ceq{\hfill P}{=}{\big\{p'({\mr x})\in S_\Delta(\U)\ :\ p'({\mr x})\cup p({\mr x})\textrm{ is consistent}\big\}}.

  By Theorem~\ref{thm_stationarity} for every $q\in Q$ there is a unique $p'\in P$ consistent with $q$.
  Therefore it suffices to prove that $P$ is finite.
  Note that $\Aut(\U/A)$ acts transitively on $P$.
...
\begin{proof}
  ...
  Indeed, it suffices to note that ${\gr\D_{{h^{-1}}p',r'}}={\gr h^{-1}}\,{\gr\D_{p',r'}}$ and that if 
  ${\gr\D_{p',r'}}\in L'\mbox{-}\acl^\eq\!\0$ then also ${\gr h^{-1}}\,{\gr\D_{p,r'}}\in L'\mbox{-}\acl^\eq\!\0$.
