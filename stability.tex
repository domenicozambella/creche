...
In particular, $\Delta$ is the set of formulas defined there.
...
\begin{theorem}\label{thm_Ggeneric_persistent}
  ...
  \begin{itemize}
    ...
    \item [2.] $\phi({\mr x}\,;{\gr 1})$ is $\grZ$-generic.
  \end{itemize}
\end{theorem}

% The imaginaries and the eq-expansion.
% Finite equivalence relations and Shelah strong types.
% Elimination of imaginaries.
% Lascar invariance, Lascar strong types, strong automorphisms.
% Group actions on a model, generic formulas and types.
% Stable formulas.
% The definability of externally definable sets.
% The binary rank and the number of types.
...
\begin{proof}
  \ssf2$\IMP$\ssf1.
  By Proposition~\ref{prop_generic_finsat}.
  ...
  % Now let $p({\mr x})\in S_\phi(\U)$ a type finitely satisfiable in ${\mr x}\in\mrX$ and containing ${h^{-1}}p'({\mr x})$ with $r'$ replaced by $\phi$.
