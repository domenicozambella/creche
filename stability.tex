...
\begin{corollary}\label{corol_persistent_finsat}
  ...
  Finally, let $\mrD$ be a $\qfDelta(\U)$-definable set.
  Then the following are equivalent 
  \begin{itemize}
...
\begin{proof}
  ...
  Let $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ be the $\qfDelta(\grZ)$-formula that defines $\mrD$.
  As $\theta({\mr x}\,;{\gr z_1},\dots,{\gr z_n})$ is stable, by Proposition~\ref{prop_type_over_acl2} both $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ and its negation are finitely satisfied in every model $M\supseteq A$.
...
\begin{proof}
  ...
  For the converse inclusion we prove that $\gamma_{\Z^0}({\mr x})$ is finitely consistent and apply Proposition~\ref{prop_Z0_inclusione1}.
  By Proposition~\ref{corol_def_mu} it suffices to prove that there exists a $\gr\Z^0$-invariant type $p({\mr x})\in S_\Delta(\grZ)$.
  Let $Q$ be as in Corollary~\ref{corol_feq_thm}.
  Let $q({\mr x})\in Q$ be such that $q({\mr x})\proves{\mr x}\in\mrD$.
  As $Q$ is finite, the stabilizer of any $q({\mr x})\in Q$ is a subgroup $\grW\le\grZ$ of finite index.
  Therefore all $q({\mr x})\in Q$ are $\gr\Z^0$-invariant.
  % pick ${\gr g}\in\grZ$ such that $\mrD\cap{\gr g}{\cdot}\mrD=\varnothing$ for some $\qfDelta(\grZ)$-definable $\grZ$-generic set $\mrD$.
  % We can assume that $\Delta$ contains only the formulas occurring in the definition of $\mrD$ therefore, that it is finite.
  % Let $Q$ be as in Corollary~\ref{corol_feq_thm}.
  % Let $q({\mr x})\in Q$ be such that $q({\mr x})\proves{\mr x}\in\mrD$.
