...
\begin{remark}
  ...
  From Theorem~\ref{thm_persistent_finsat} and Remark~\ref{rem_coalesce} it follows that $\gamma_H(x)$ is consistent.
...
\begin{lemma}\label{lem_trans_action}
  ...
  Let $G=\Aut(\UDelta)$ and $H=\Autf(\UDelta)$.
  Assume that $\mrX$ and $\grZ$ are type-definable and invariant over $G$.
  Moreover $G$ acts transitively on $\mrX$.
  Finally, let 
  
  \ceq{\hfill P}{=}{\big\{p({\mr x})\in S_\Delta(\grZ)\ :\ p({\mr x})\textrm{ $H$-invariant}\big\}}.
