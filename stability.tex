...
\begin{theorem}[ (Shelah's Finite Equivalence Relation Theorem)]\label{thm_FER}
...
\begin{proof}
  ...
  It is easy to see that the orbit of $q_1({\mr x})$ under the action of $\Aut(\U/A)$ comprise complete $\GDelta(\acl^\eq\!A)$-types $q_1,\dots,q_n$ that are complete and consistent with $p_1,\dots,p_n$.
...
\begin{proof} 
  \ssf1$\IMP$\ssf2.
  By stability, there exist a type $p({\mr x})\in L'\mbox{-}S_\phi(\grZ)$ that is invariant under the action of $L'\mbox{-}\Autf(\U)$.
  ...
  Assume \ssf2 then, by Proposition~\ref{prop_stable_genericiffpersistent}, $\phi({\mr x}\,;{\gr g})$ is Lascar generic relative to $q(\U^{\mr x})$, i.e.\@ with $q(\U^{\mr x})$ for $\mrX$.
  ...
  % \ssf1$\IMP$\ssf2.
  % Let ${\gr g}$ be given.
  % By stability, there is a type $p({\mr x})\in L'\mbox{-}S(\grZ)$ that is invariant, and therefore definable, over $L'\mbox{-}\acl^\eq\0$.
  % If $\phi({\mr x}\,;{\gr g})$ is $\grZ$-generic, $p({\mr x})\proves\phi({\mr x}\,;{\gr h}\,{\gr g})$ for some ${\gr h}\in\grZ$.
  % Then $\phi({\mr x}\,;{\gr g})\in{\gr h^{-1}}p$.
  % We claim that ${\gr h^{-1}}p({\mr x})$ is also definable over $L'\mbox{-}\acl^\eq\0$.
  % Indeed, it suffices to note that ${\gr\D_{{h^{-1}}p,\phi}}={\gr h^{-1}}\,{\gr\D_{p,\phi}}$ and that if ${\gr\D_{p,\phi}}\in L'\mbox{-}\acl^\eq\0$ then also ${\gr h^{-1}}\,{\gr\D_{p,\phi}}\in L'\mbox{-}\acl^\eq\0$.
  % Now read ${\gr h^{-1}}p({\mr x})$ as a type $q({\mr x})\in S_\Delta(\grZ)$ in the natural way.
  % Note that $\phi({\mr x}\,;{\gr g})\in q$ and that $q({\mr x})$ is definable over $\acl^\eq\0$ since this is a superset of $L'\mbox{-}\acl^\eq\0$.
...
\begin{corollary}
  ...
  \ceq{\hfill Q}{=}{\big\{q({\mr x})\in S_\Delta(\grZ)\ :\ q({\mr x})\textrm{ is }\grZ\textrm{-generic}\big\}}.
  
  Then the following hold
  \begin{itemize}
    \item [1.] $\grZ$ acts on $Q$ transitively
    \item [2.] $Q$ is finite
    \item [3.] there is a finite equivalence relation $\epsilon({\mr x}\,;{\mr y})\in\GDelta(A)$ that if $\grZ$-invariant and such that for all $q_i,q_j\in Q$ 
    
    \ceq{\hfill q_i=q_j}{\IFF}{q_i({\mr x})\cup q_j({\mr y}) \textrm{ is consistent with }\epsilon({\mr x},{\mr y})}
    \item [4.] $\phi({\mr x}\,;{\gr g})$ is generic if and only if $\phi({\mr x}\,;{\gr g})\in q$
for some $q\in Q$.    
  \end{itemize}
\end{corollary}

\begin{proof}
  Apply Theorem~\ref{thm_FER} in $\U'$ with ${\mr x}={\mr x}$ for $p_0({\mr x})$.
  Let $\epsilon({\mr x},{\mr y})\in L'$ and $P$ be as defined in the proof of that theorem.
  As $\epsilon({\mr x},{\mr y})$ is invariant under the action of $L'\mbox{-}\Aut(\U')$ it is a fortiori $\grZ$-invariant.
...
Define 
\def\Stab{{\rm Stab}}
\ceq{\hfill\Stab_G(\mrD)}{=}{\{g\in G\ :\ g\cdot\mrD=\mrD\}}

\begin{proposition}
  The following are equivalent
  \begin{itemize}
    \item [1.] $\Stab_G(\mrD)$ has finite index in $G$
    \item [2.] $\mrD$ has a finite $G$-orbit.
  \end{itemize}
\end{proposition}
