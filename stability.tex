...
\begin{corollary}\label{corol_harrington0}
  ...
  \ceq{1.\hfill\phi({\mr a}\,;M^{\gr z})}{=}{\grB\cap M^{\gr z}}\smallskip

  \ceq{2.\hfill\phi(M^{\mr x};{\gr b})}{=}{\mrA\cap M^{\mr x}},\smallskip
...
\begin{corollary}\label{corol_harrington1}
  ...
  Let ${\mr a}\in\U^{\mr x}$ and ${\gr b}\in\U^{\gr z}$ be such that\smallskip

  \ceq{\ssf1.\hfill\phi({\mr a}\,;M^{\gr z})}{=}{M^{\gr z}\cap {\gr\D_{p,\phi}}}\smallskip

  \ceq{\ssf2.\hfill\phi(M^{\mr x};{\gr b})}{=}{M^{\mr x}\cap {\mr\D_{q,\phi^{\rm op}}}}.\smallskip
...
In this section, unless otherwise stated, $\mrX=\U^{\mr x}$ and $\grZ=\U^{\gr z}$.
As in Chapter~\ref{actions}, we write \emph{$G$\/} for $\Aut(\UDelta)$.
We also set $H=\Autf(\UDelta)$.
...
% Similarly for persistent/invariant/wide.\phi(\mrX\,;{\gr b})
...
The first part shows that when $\Delta$ is stable Theorem~\ref{thm_coalesce} applies.
...
\begin{theorem}\label{thm_persistent_finsat}
  ...
  Then the following are equivalent 
  \begin{itemize}
    \item[1.] $\mrD$ is $H$-persistent
    \item[2.] $\mrD$ is $H$-wide
    \item[3.] $\mrD$ is $K$-persistent 
    \item[4.] $\mrD$ is $K$-wide.\smallskip
  \end{itemize}
\end{theorem}
\begin{proof}
  ...
  Then from Corollary~\ref{corol_stable_coheir_over_models} it follows that $\phi({\mr x}\,;{\gr b})$ extends to a type $p({\mr x})\in S_\Delta(\grZ)$ that is $H$-invariant.
  Then $\mrD$ is $H$-wide by Theorem~\ref{thm_generic_invariant2}.
...
% \begin{proof}
%   Let $\mrD$ be a $\BDelta(\U)$-definable set that is weakly $H$- persistent.
%   We prove that $\mrD$ is $H$- persistent.
%   Pick a finite $H\subseteq\Autf(\U/A)$ such that $\cup\,H\,\mrD$ is $H$- persistent.
...
\begin{proposition}\label{prop_stable_genericiffpersistent}
  ...
  %Let $\mrX=q(\U^{\mr x})$ where $q({\mr x})\subseteq\BDelta(\grZ)$ is an $H$-invariant type that decides all $\BDelta(\grZ)$-formulas that are $H$-invariant.
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  By Theorems~\ref{thm_coalesce} and~\ref{thm_persistent_finsat}.

  \ssf2$\IMP$\ssf1.
  Assume $\mrD$ is not $H$-generic.
  Then $\neg\mrD$ is $H$-persistent.
  Assume for a contradiction that also $\mrD$ is $H$-persistent.
  By Theorem~\ref{thm_persistent_finsat} both $\mrD$ and $\neg\mrD$ are $H$-wide.
  Then by Theorem~\ref{thm_generic_invariant2} both ${\mr x}\in\mrD$ and ${\mr x}\notin\mrD$ extend to $H$-invariant global types.
  This contradicts stationarity (of the type ${\mr x}\in\mrX$), see Proposition~\ref{prop_type_over_acl2} and Theorem~\ref{thm_stationarity}.
\end{proof}
...
\begin{proof}
  By Theorem~\ref{thm_stationarity} there is a type $p({\mr x})\in S_\Delta(\grZ)$ consistent with $q({\mr x})$ that is $H$-invariant and therefore persistent.
...
\begin{proof}
  ...
  As $p({\mr x})\in P$ is $H$-invariant, by Remark~\ref{rem_type_over_acl2} the sets ${\gr\D_{p,\phi}}$ are in $\Psi$.
  Hence they have a finite $G$-orbit.
  If $\Delta$ is finite the orbit of $\<{\gr\D_{p,\phi}}:\phi({\mr x}\,;{\gr z})\in\Delta\>$ is finite, and so is the orbit of $p({\mr x})$.
  Therefore by \ssf1, $P$ is finite.

  \ssf3. 
  Let $p_i:i<\lambda$ be an enumeration without repetitions of $P$.
  Let $q_i({\mr x})$ be as in \ssf1.
  Under the action of $G$ consists of $\BDelta(\acl^\eq\!A)$-types $q_1,\dots,q_n$ that are complete and consistent with $p_1,\dots,p_n$.
...
\begin{exercise}\label{ex_X_vs_U}
  Let $q(x)\subseteq\BDelta(\Z)$.
  Let $\X=q(\U^x)$ where $q(x)\subseteq\BDelta(\Z)$ is a small $H$-invariant types that decides all $\BDelta(\Z)$-formulas that are $H$-invariant.
  Prove that if $\D$ is $H$-wide relative to $\X=q(\U^x)$, then it is wide relative to $\U^x$.
\end{exercise}
...
\begin{proof} 
  ...
  Assume \ssf2 then, by Proposition~\ref{prop_stable_genericiffpersistent}, $\mrD$ is $H$-generic relative to $q(\U^{\mr x})$, i.e.\@ with $q(\U^{\mr x})$ for $\mrX$.
  ...
  % \ssf3$\IMP$\ssf1.
  % Let ${\gr g}$ be such that $\phi({\mr x}\,;{\gr g})$ is not $H$-persistent.
  % Then $\neg\phi({\mr x}\,;{\gr g})$ is $H$-generic.
