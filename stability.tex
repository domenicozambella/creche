...
\hfil${\gr C}\ \subseteq\ \phi({\mr a_n}\,;\U^{\gr z})\ \subseteq\ \U^{\gr z}\sm\{{\gr b_0},\dots,{\gr b_{n-1}}\}$.
...
\begin{corollary}\label{corol_harrington0}
  ...
  Let $\theta({\mr x}),\psi({\gr z})\in L(M)$ be such that $\theta(M^{\gr z})=\phi({\mr a}\,;M^{\gr z})$ and $\psi(M^{\mr x})=\phi(M^{\mr x}\,;{\gr b})$ (these exist by stability).
...
\begin{proof}
  ...
  As $\phi({\mr x}\,;{\gr z})\wedge\neg\theta({\gr z})$ is stable, Theorem~\ref{thm_symmetry} yelds $\phi({\mr a}\,;M^{\gr z})\sm\theta(M^{\gr z})\neq\varnothing$ which contradicts  $\theta(M^{\gr z})=\phi({\mr a}\,;{\gr M)}$.
...
We first prove a special case of the theorem which has a simpler proof.
...
\begin{proof}
    ...
    By Theorem~\ref{thm_def_stable_formula} there are some ${\mr a_1},\dots,{\mr a_n}\models q({\mr x})$ such that ${\gr\D_{p,\phi}}$ is a positive Bolean combination of the sets $\phi({\mr a_i}\,;\U^{\gr z})$.
    Let $\theta({\mr a_1},\dots,{\mr a_n}\,;\U^{\gr z})$ be such a Boolean combination.
    Finally, from Proposition~\ref{prop_type_over_acl2} we obtain ${\gr\D_{p,\phi}}\in\acl^\eq\!A$.
    ...
    \ceq{\hfill\bigcup_{i=1}^n f_i{\gr\D_{p,\phi}}}{=}{\bigvee_{i=1}^n\theta(f_i{\mr a_1},\dots,f_i{\mr a_n}\,;\U^{\gr z}).}

   As the set on the l.h.s.\@ is definable over $A$, the formula on the r.h.s.\@ is the required positive Boolean combination.
\end{proof}

\begin{remark}
    If $\phi({\mr a}\,;z)$ is persistent over $A$ then $\theta({\mr a_1},\dots,{\mr a_n}\,;{\gr z})$ is consistent and therefore it is satisfiable in every model $M\supseteq A$.
\end{remark}
