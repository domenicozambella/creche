...
\begin{corollary}\label{corol_harrington0}
  ...
  Suppose there exist $\mrA\subseteq\U^{\mr x}$ and $\grB\subseteq\U^{\gr z}$ that are $\qfDelta(M^{\gr z})$, respectively $\qfDelta(M^{\mr x})$-definable and 
...
\begin{theorem}[ (Stationarity)]\label{thm_stationarity}
  Let $\Delta$ be stable.
  Let $q({\mr x})\subseteq L(\U)$ be a type that is invariant over $\acl^\eq\!A$ and decides all $\qfDelta(\U^{\gr z})$-formulas that are invariant over $\acl^\eq\!A$.
...
\begin{proof}
  ...
  Then from Corollary~\ref{corol_stable_coheir_over_models} it follows that $\phi({\mr x}\,;{\gr b})$ is contained in a type $p({\mr x})\in S_\Delta(\U^{\gr z})$ that is \DLascar\ invariant.
  ...
  \ssf2$\IMP$\ssf4.
  Assume \ssf2.
  Then there is a type $p({\mr x})\in S_\Delta(\U^{\gr z})$ that is \DLascar\  persistent and $p({\mr x})\proves{\mr x}\in\mrD$.
...
% \begin{proof}
%   Let $\mrD$ be a $\qfDelta(\U)$-definable set that is weakly \DLascar\  persistent.
...
\begin{proposition}\label{prop_stable_genericiffpersistent}
  Let $\Delta$ be stable.
  Let $\mrX=q(\U^{\mr x})$ where $q({\mr x})\subseteq\qfDelta(\U^{\gr z})$  is invariant over $\acl^\eq\!A$ and decides all $\qfDelta(\U^{\gr z})$-formulas that are invariant over $\LDelta\mbox{-}\acl^\eq\varnothing$.
  Then for every $\qfDelta(\U^{\gr z})$-definable set $\mrD$, the following are equivalent
  \begin{itemize}
...
\begin{corollary}\label{corol_stable_generic}
  Let $\Delta$ be stable.
  Let $\mrX=q(\U^{\mr x})$ where $q({\mr x})\subseteq\qfDelta(\U^{\gr z})$ is a type that is invariant over $\LDelta\mbox{-}\acl^\eq\varnothing$ and decides all $\qfDelta(\U^{\gr z})$-formulas that are invariant over $\LDelta\mbox{-}\acl^\eq\varnothing$.
  Then for every $\qfDelta(\U^{\gr z})$-definable set $\mrD$, either $\mrD$ or $\neg\mrD$ is \DLascar\ generic.
\end{corollary}

\begin{proof}
  By Theorem~\ref{thm_stationarity} there is a type $p({\mr x})\in S_\Delta(\U)$ finitely consistent with $q({\mr x})$ that is \DLascar\ invariant and therefore persistent.
...
\begin{theorem}\label{thm_FER}
  Let $\Delta$ be stable.
  Let $\mrX=q(\U^{\mr x})$, where $q({\mr x})\subseteq\qfDelta(\U^{\gr z})$ is a type that is invariant (over $\varnothing$) and decides all invariant $\qfDelta(\U^{\gr z})$-formulas.
  Finally, let \smallskip
  
  \ceq{\hfill P}{=}{\big\{p({\mr x})\in S_\Delta(\U^{\gr z})\ :\ \textrm{consistent with }\mrX\textrm{ and is \DLascar\ invariant}\big\}}. \smallskip
...
\begin{proof}
  ...
  As $p\in P$ is \DLascar\ invariant over $A$, the sets ${\gr\D_{p,\phi}}$ are in $\acl^\eq\!A$.
...
\begin{exercise}\label{ex_X_vs_U}
  Let $q({\mr x})\subseteq\qfDelta(\U^{gr z})$.
...
\begin{theorem}\label{thm_Ggeneric_persistent}
  ...
  Then for every $\qfDelta(\U^{\gr z})$-definable set $\mrD$ the following are equivalent
  \begin{itemize}
    \item [1.] $\mrD$ is generic under the action of $\U^{\gr z}$
    \item [2.] $\mrD$ is \DLascar\ persistent.
  \end{itemize}\smallskip

  In particular, it follows the if these properties hold for $\mrD$, they hold also for ${\gr g}{\cdot}\mrD$ for any ${\gr g}\in\U^{\gr z}$. 
\end{theorem}

It is worthwhile to recall that, under the assumption of stability, \DLascar\ wide is equivalent to \DLascar\ persistent, see Theorem~\ref{thm_persistent_finsat}.

\begin{proof} 
  \ssf1$\IMP$\ssf2.
  By stability, there exist a type $p({\mr x})\in S_\Delta(\U^{\gr z})$ that is invariant under the action of $\Autf(\UDelta)$.
  ...
  \ssf2$\IMP$\ssf1.
  For ${\mr a}\in\U^{\mr x}$ let $q({\mr x})=\tp({\mr a}/\acl^\eq\!A)$.
