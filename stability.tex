...
Note that from the theorem it follows that, when $T$ is stable
...
\begin{proof}
  ...
  By Theorem~\ref{thm_def_stable_formula}, with the role of the two tuples of variables reversed, there are some ${\gr b_1},\dots,{\gr b_n}\models q({\gr z})$ such that ${\mr\D_{p,\phi^{\rm op}}}$ is equivalent to a positive Bolean combination of the sets $\phi(\U^{\mr x}\,;{\gr b_i})$.
  Let $\theta(\U^{\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ be such a Boolean combination.
  When $A=\acl^\eq\!A$, this is the required formula as ${\mr\D_{p,\phi^{\rm op}}}\in\acl^\eq\!A$ by Proposition~\ref{prop_type_over_acl2}.
...
We adopt the notation and terminology of Chapter~\ref{actions} with $G=\Autf(\U/A)$, $\grZ=\U^{\gr z}$, and $\Delta=\{\phi({\mr x}\,;{\gr z})\}$.
...
\begin{corollary}\label{corol_persistent_finsat}
  Let $\phi({\mr x}\,;{\gr z})\in L(\U)$ be stable.
  Let $\mrX=\U^{\mr x}$.
  Let $\mrD$ be a $B_\phi(\U)$-definable set.
  Then the following are equivalent 
  \begin{itemize}
    \item[1.] $\mrD$ is Lascar persistent over $A$
    \item[2.] $\mrD$ intersects every $M\supseteq A$.
  \end{itemize}
\end{corollary}
\begin{proof}
  As Boolean combinations of stable formulas are stable, we can assume that $\mrD$ is defined by the formula $\phi({\mr x}\,;{\gr b})$.

  \ssf1$\IMP$\ssf2.
  If $\mrD$ is Lascar persistent, the formula $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ in the proof of Theorem~\ref{thm_stability_definable_rovescio} is consistent.
  By elementarity $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ is satisfiable in every model $M\supseteq A$, hence so is $\phi({\mr x}\,;{\gr b})$.
  
  \ssf2$\IMP$\ssf1.
  From \ssf2 and Corollary~\ref{corol_stable_coheir_over_models} it follows that $\phi({\mr x}\,;{\gr b})$ is contained in a global type $p({\mr x})$ that is finitely satisfied in every $M\supseteq A$.
...
\begin{proof}
  Let $\mrD$ be a $B_\phi(\U)$-definable set that is weakly Lascar persistent.
  ...
  Then, by Corollary~\ref{corol_stable_coheir_over_models} there is a Lascar invariant type $p({\mr x})\in S_\phi(\U)$ containing the formula that defines $\cup\,H\,\mrD$.
...
\begin{proposition}\label{prop_stable_genericiffpersistent}
  ...
  Let $\mrX=q(\U^{\mr x})$.
  Then for every $B_\phi(\U)$-definable set $\mrD$, the following are equivalent
  \begin{itemize}
...
\begin{proof}
  \ssf1$\IMP$\ssf2.
  By Theorem~\ref{thm_stationarity} there is a type $p({\mr x})\in S_\phi(\U)$ consistent with $q({\mr x})$ that is Lascar invariant.
  By Theorem~\ref{thm_generic_invariant} $p({\mr x})$ Lascar persistent.
  Therefore, by Corollary~\ref{corol_def_mu}, it is Lascar generic.

  \ssf2$\IMP$\ssf1.
  Assume $\mrD$ is not Lascar generic.
  Then $\neg\mrD$ is Lascar persistent.
  Assume for a contradiction that also $\mrD$ is Lascar persistent.
  Let $\theta({\mr x}\,;{\gr b_1},\dots,{\gr b_n})$ be the $B_\phi(\U)$-formula that defines $\mrD$.
  ...
  Then there are two distinct types $p_i({\mr x})\in S_\phi(\U)$ finitely consistent with $q({\mr x})$ and finitely satisfied in every $M\supseteq A$.
...
\begin{corollary}\label{corol_stable_generic}
  ...
  Let $q({\mr x})\in S(\acl^\eq\!A)$.
  Let $\mrX=q(\U^{\mr x})$.
  Then for every $B_\phi(\U)$-definable set $\mrD$, either $\mrD$ or $\neg\mrD$ is Lascar generic over $A$.
\end{corollary}

\begin{proof}
  By Theorem~\ref{thm_stationarity} there is a type $p({\mr x})\in S_\phi(\U)$ finitely consistent with $q({\mr x})$ that is Lascar invariant and therefore persistent.
