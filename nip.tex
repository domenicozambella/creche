...
\begin{proof}
The property we want to prove is elementary in $L(\grD)$. By  Remark~\ref{rem_el_sat} we can assume that $\grD$ is saturated. By Corollary~\ref{corol_altrank}, there is no infinite sequence $\<{\gr b_i}:i<\omega\>$
...
\begin{theorem}
Let $\grD$ be a set approximable by a formula with finite VC-dimension. Then $\grD$ is approximable from above and from below.
\end{theorem}

\begin{proof}
Let $B$ be a small set. For every ${\gr b}\in B^{|{\gr z}|}\cap\grD$ there is a type
\end{proof}
