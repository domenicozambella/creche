...
We introduce \textit{Fra\"iss\'e limits}, also known as \textit{homogeneous-universal\/} or \textit{syndetic\/} structures, which here we call \textit{rich\/} models, after Poizat.
...
\begin{proof}
When $\lambda=\omega$, we can take the proof of Theorem~\ref{thm_zigzagcantor} and replace \textit{partial embedding\/} by \textit{morphism\/} and the references to Lemma~\ref{lem_ordinericco} by references to Proposition~\ref{prop_6.1}.
