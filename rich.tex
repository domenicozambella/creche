...
\begin{remark}
    ...
    (This is a conseguence of Corollary~\ref{coroll_riccozigzag})\QED
\end{remark}
...
%
For the time being we do not need the syntactic version of elimination of quantifiers, so when saying that \textit{$T$ has quantifier elimination\/} we mean \ssf{2} of the theorem above.
...
\begin{exercise}
    ...
    \end{itemize}
    Prove that if $L$ is countable and there is a back-and-forth system between $M$ and $N$ then there are $M'\preceq M$ and $N'\preceq N$ such that $M'\simeq N'$.\QED
\end{exercise}
...
Namely, for every morphism $k:M\to N$ there are $N'\supseteq N$ and a morphism  $g:M\to N'$ such that the following diagram commutes
...
\begin{itemize}
\item[c8.] if $\<M_i:i<\lambda\>$ is a $\le$-chain,  then $\displaystyle M_i\le\bigcup_{j<\lambda}M_j$ \ for all $i<\lambda$.\QED
\end{itemize}
\end{definition}
