...
In Section~\ref{dlo} we implicitly used $\M_{\rm ob}=\Mod(T_{\rm lo})$ and partial isomorphisms as $\M_{\rm hom}$.
In Section~\ref{randomgraph} we used $\M_{\rm ob}=\Mod(T_{\rm gph})$ and again partial isomorphisms as $\M_{\rm hom}$.
...
Show that $N$ is homogeneous if morphisms are elementary maps but it is not if morphisms are simply partial isomorphisms.
\end{exercise}

\begin{exercise}\label{ex_DLO_RG_QE}
Every partial isomorphism $k:M\to N$ between models of $T_{\rm dlo}$ (or models of  $T_{\rm rg}$) is an elementary map.
Hint: use downward Löwenheim-Skolem and Theorem~\ref{thm_zigzagcantor}.
\end{exercise}
...
We interweave the usual back-and-forth-argument with the construction in Löwenheim-Skolem Theorem~\ref{thm_lowenheimskolemallingiu} in order to obtain $M'\preceq M$ and  $N'\preceq N$.
...
\begin{remark}
    ...
    (This is a conseguence of Corollary~\ref{coroll_riccozigzag})
\end{remark}
...
Partial isomorphisms between models of $T_{\rm dlo}$ and between models of $T_{\rm rg}$ are elementary maps.
\end{corollary}
...
\begin{itemize}
    \item[1.] $T$ has elimination of quantifiers
    \item[2.] every \textit{finite\/} partial isomorphism between models $T$ is an elementary map.
\end{itemize}
\end{exercise}
