...
\begin{definition}
Assume that $\M$ satisfies \ssf{c0}-\ssf{c3} of Definition~\ref{def_com_c}. We say that a model $N$ is \emph{$\lambda\jj$rich\/} if for every model $M$, every $b\in M$ and every morphism $k:M\imp N$ of cardinality $<\lambda$ there is a $c\in N$ such that $k\cup\{\<b,c\>\}:M\to N$ is a morphism. We say that $N$ is \emph{rich\/} tout court  if it is $\lambda\jj$rich for $\lambda=|N|$. When $\M_{\rm ob}=\Mod(T_0)$ for some theory $T_0$ and $\M_{\rm ar}$ is clear from the context, we say \emph{rich model of $T_0$}.\QED
\end{definition}
