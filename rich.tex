...
Assume that $\M$ satisfies \ssf{c1}-\ssf{c3} of Definition~\ref{def_com_c}.
We say that a model $N$ is \emph{$\lambda$-universal\/} if for every model $M$ of cardinality $\le\lambda$ in the same connected component of $N$ there is an embedding $k:M\hookrightarrow N$.
We say that a model $N$ is \emph{$\lambda$-homogeneous\/} if every $k:N\to N$ of cardinality $<\lambda$ extends to a bijective morphism $h:N\isomap N$ (an automorphism when \ssf{c4} below holds).

Note that the larger $\M_{\rm hom}$, the stronger notion of homogeneity.
When  $\M_{\rm hom}$ contains all partial isomorphisms between models (the largest class of morphisms considered here), it is common to say \emph{$\lambda$-ultra\-homo\-geneous\/} for $\lambda$-homogeneous.
...
It is obvious that $\lambda$-rich models are $\lambda$-universal and, by Theorem~\ref{thm_riccozigzag}, they are $\lambda$-ho\-mo\-ge\-ne\-ous.
...
Let $\lambda\ge|L|$ be given. The \emph{theory of the rich models\/} of $\M$ is the set $T_1$ of sentences that hold in all $\lambda$ rich models. The theorem below proves that $T_1$ does not depends on $\lambda$ (and we could also restrict to rich models when they exist).

It also follows from the theorem that $T_1$ is complete as soon as $\M$ is connected.
...
In particular, $\lambda$-rich models in the same connected component are elementary equivalent.
\end{theorem}
...
The answer is affirmative when $\lambda=\omega$ and $T_1$ is either $T_{\rm dlo}$ or $T_{\rm rg}$, but this is not always the case, see Example~\ref{ex_infiniti_colori} for an easy to grasp counterexample.
But in fact, any non $\lambda$-categorical theory counterexamples.
...
\begin{remark}
    ...
    (This is a conseguence of Corollary~\ref{corol_QE_back_and_forth})\QED
\end{remark}

A very interesting interesting variant of the question asked above is considered in Theorem~\ref{thm_ricchezza_saturazione_QE} where it is related to an important phenomenon that we now indroduce.
...
\noindent\llap{\textcolor{red}{\Large\danger}\kern1.5ex}This theorem will be proved only in Chapter~\ref{elimination}, see Exercise~\ref{ex_prove_thm_EQ} or Corollary~\ref{corol_QE}.
%
For the time being we do not need the syntactic version of elimination of quantifiers, so when saying that \textit{$T$ has quantifier elimination\/} we intend \ssf{2} of the theorem above.
For instance we rephrase Corollary~\ref{coroll_tutteleimmersionisonoelementari_QE} above by saying that $T_{\rm dlo}$ and $T_{\rm rg}$ have elimination of quantifiers.
...
Do $\omega$-rich models of $T_0$ exist? Is their theory $\omega$-categorical? What if we add to $T_0$ the axiom $r_0(x)\vee\dots\vee r_n(x)$~?\QED
\end{exercise}
...
\begin{itemize}
    \item[1.] $T$ has elimination of quantifiers;
    \item[2.] every \textit{finite\/} partial isomorphism between models $T$ is an elementary map.\QED
\end{itemize}
\end{exercise}
...
For that we need to weaken the notions of $\lambda$-universality and  $\lambda$-homogeneity.
...
(Assume~\ref{ass_c1-6}) \ Prove that any weakly $\lambda$-ho\-mo\-ge\-ne\-ous structure of cardinality $\lambda$ is homogeneous.\QED
\end{exercise}
...
\item[2.] $N$ is  weakly $\lambda$-universal and weakly $\lambda$-homogeneous.
\end{itemize}
\end{theorem}
...
By weak $\lambda$-universality there is a morphism $f:M\imp N$ with domain of definition $\dom k\cup\big\{b\big\}$.
