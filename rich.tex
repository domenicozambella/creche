...
\begin{definition}
Assume that $\M$ satisfies \ssf{c0}-\ssf{c3} of Definition~\ref{def_com_c}. We say that a model $N$ is \emph{$\lambda\jj$rich\/} if for every model $M$, every $b\in M$ and every morphism $k:M\imp N$ of cardinality $<\lambda$ there is a $c\in N$ such that $k\cup\{\<b,c\>\}:M\to N$ is a morphism. We say that $N$ is \emph{rich\/} if it is $\lambda\jj$rich for $\lambda=|N|$. When $\M_{\rm ob}=\Mod(T_0)$ for some theory $T_0$ and $\M_{\rm hom}$ is clear from the context, we say \emph{rich model of $T_0$}.\QED
\end{definition}
...
\end{theorem}
\begin{proof} 
Implication \ssf{1}$\IMP$\ssf{2} is clear as noted above, so we prove \ssf{2}$\IMP$\ssf{1}. Let $k:M\imp N$ be a morphism of cardinality $<|N|$ and let $b\in M$. As $N$ is universal, there is a morphism \hbox{$f:M\imp N$} defined on $\dom k\cup\big\{b\big\}$. By \ssf{c3} the map $k\circ f^{-1}:N\imp N$ is a morphism of cardinality $<|N|$. By homogeneity it has an extension to a bijective morphism $h:N\hookdoubleheadrightarrow N$. It is immediate that $h\circ f:M\imp N$ is the required extension of $k$.
\end{proof}
