...
\begin{exercise}ø
  Let ${\mr x}$ be a finite tuple.
  A set $\mrN\subseteq\U^{\mr x}$ is nowhere dense in the $A$-topology if for every consistent $\phi({\mr x})\in L(A)$ there is some consistent $\psi({\mr x})\in L(A)$ such that $\psi({\mr x})\imp\phi({\mr x})$ and $\psi(\mrN)=\varnothing$.
  A set $\mrM\subseteq\U^{\mr x}$ is meager if it is union of countably many nowhere dense sets.
  
  Prove that for every countable model $M$ the following are equivalent
  \begin{itemize}
    \item [1.] $\mrM$ is meager in the $M$-topology
    \item [2.] $\mrM\cap M=\varnothing$.
  \end{itemize}
\end{exercise}
