...
\begin{itemize}
   ...
   \item otherwise, let $\displaystyle\psi_{i+1}({\gr z})=\psi_i({\gr z}) \wedge \E{\mr x}\,\big[\xi_i({\mr x}\,;{\gr z})\wedge\neg\phi({\mr x})\big]$ for some (any) $\phi({\mr x})\in p$ that makes $\psi_{i+1}({\gr z})$ is consistent.
\end{itemize}
Note that if ${\gr a}\models\psi_{i+1}({\gr z})$ then $\xi_i({\mr x}\,;{\gr a})$ does not isolate $p({\mr x})$.
Therefore the proof is complete if we can show that it is always possible to find the formula $\phi({\mr x})$ required above.
...
Then $p({\mr x},{\gr a})=\tp({\mr b}/A,{\gr a})$ and $\E{\mr x}\,p({\mr x},{\gr z})\iff\tp({\gr a}/A)$.
...
\begin{proposition}
If $T$ is small (over $\0$) then it is small over any finite $A$.
\end{proposition}
...
The union of the chain is the requires saturated model.
