...
\end{itemize}
\end{theorem}
The set $A$ is introduced for convenience. By \ssf{3} of Proposition~\ref{prop_ERNS_equivalents} below, no theory is $\omega$-categorical over an infinite set. The reader can easily prove that that categoricity over some finite $A$ is equivalent to categoricity over $\0$ (see Exercise~\ref{ex_omega_cat_overA}).
...
Let $T$ be, as always in this chapter, a complete theory without finite models. We say that $T$ is \emph{small over $A$\/} if $S_{{\mr x}}(A)$ is countable for every ${\mr x}$ of finite length. When $A$ is empty, we simply say that $T$ is \emph{small}. The set $A$ is introduced for convenience, $A=\0$ is the most interesting case. 
...
Let $\Delta$ be a set of formulas (we mainly use $\Delta=L_{\mr x}(A)$ in this section). A \emph{binary tree of formulas in $\Delta$\/} is a sequence $\<\phi_s\ :\ s\in2^{<\omega}\>$ of formulas such that\nobreak
\begin{itemize}
...
\ssf{3}$\IMP$\ssf{1}\quad We prove the contrapositive: we assume that $\U^{|{\mr x}|}/\mathord\equiv_A$ is uncountable for some finite ${\mr x}$ and define a tree of formulas in $L_{\mr x}(A)$ by induction. Begin with $\phi_\0({\mr x})=\top$. For $s\in 2^{<\omega}$ be the conjunction of the formulas $\phi_{s\restriction n}$ for all $n<|s|$. Assume as induction hypothesis that $p_s({\mr\U})$ is uncountable (here and below, we count modulo $\equiv_A$). This will guarantee the consistency of the branches.%  For convinience we also assume that $\phi_s({\mr x})\imp\phi_{s\restriction n}({\mr x})$ for every $n<|s|$.


We claim that there is a formula $\psi({\mr x})\in L(A)$ such that both sets $p_s({\mr\U})\cap\psi({\mr\U})$ and $p_s({\mr\U})\cap\neg\psi({\mr\U})$ are uncountable. If $\psi({\mr x})$ is such a formula, we define $\phi_{s0}({\mr x})=\psi({\mr x})$ and $\phi_{s1}({\mr x})=\neg\psi({\mr x})$ as this preserves the induction hypothesis.

First we prove that the following type is consistent

\ceq{\hfill q({\mr x})}{=}{\Big\{\xi({\mr x})\in L(A)\ :\ \phi_s({\mr\U})\cap\neg\xi({\mr\U}) \textrm{ is countable }\Big\}.}

Suppose not, then $\neg\xi_1({\mr x})\vee\dots\vee\neg\xi_n({\mr x})$ holds for some $\xi_i({\mr x})\in q$. Hence, as $p_s({\mr\U})$ is uncountable, at least one of the sets $p_s({\mr\U})\cap\neg\xi_i({\mr\U})$ is uncountable, contradicting the definition of $p({\mr x})$.
...
An uncountable, non-isolated, complete type that cannot be omitted was produced by Gebhard Fuhrken in 1962. I am not aware of any modern exposition of this (or other) example but for a post of Alex Kruckman to StackExhange~\cite{AK}. This is the source of Example~\ref{ex_AK}.
