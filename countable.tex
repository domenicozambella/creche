...
With the notation in the second proof of the L\"owenheim-Skolem Theorem~\ref{thm_lowenheimskolemallingiu}, at stage $i=\pi(j,k)$ apply Lemma~\ref{lem_kuratowskiUlam} to find a solution $a$ of $\phi_k({\mr x})$ such that $A_{i+1}=A_i,a$ does not isolates $p({\mr x})$.
\end{proof}
...
\ceq{\hfill p(x)}{\imp}{\smash{\bigvee_{n<\omega}p_n(x)}}
...
\begin{proposition}
If $T$ is small (over $\0$) then it is small over any finite set $A$.
\end{proposition}
