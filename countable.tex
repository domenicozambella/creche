...
\begin{exercise}(To be rew-ritten)
  Let $x$ be a finite tuple.
  A set $N\subseteq\U^x$ is nowhere dense in the $A$-topology if for every consistent $\phi(x)\in L(A)$ there is some consistent $\psi(x)\in L(A)$ such that $\psi(x)\imp\phi(x)$ and $\psi(N)=\varnothing$.
  A set $M\subseteq\U^x$ is meager if it is union of countably many nowhere dense sets.
  
  Prove that for every countable model $M$ the following are equivalent
  \begin{itemize}
    \item [1.] $M$ is meager in the $M$-topology
    \item [2.] $M\cap M=\varnothing$.
  \end{itemize}
\end{exercise}
