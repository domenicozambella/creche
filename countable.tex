...
The language has a ternary relation symbol for $f(x)=y$ and unary relations for $F$, $X$ and $Y$.
Let $\U$ be a saturated elementary extension of $M$.
Then $\U$ is partitioned in three definable sets $\U_F$, $\U_X$ and $\U_Y$.
...
Now, fix some countable $Y_1\subseteq\U_Y$.
Let $c\in\U_Y\sm Y_1$ and set $p(y)=\tp(c/X,Y_1)$.
We claim that $p(y)$ is realized in every model containing $X,Y_1$.
In fact, by the remark above $p(\U)=\U_Y\sm Y_1$.
But every model containing $X$ contains also uncountably many elements of $\U_Y$, hence it contains an element conjugated to $c$ which therefore realizes $p(y)$.
We claim also that $p(y)$ is not isolated.
If for a contradiction there is a consistent formula $\phi(y)$ such that $\phi(y)\imp p(y)$, then $\phi(y)$ has a solution in $\U_Y\sm Y_1$.
By the remark above, this implies that $\phi(\U)$ is a cofinite subset of $\U_Y$ and this contradicts $\phi(\U)\subseteq p(\U)$\QED
\end{example}

\begin{exercise}
Let $p(x)\subseteq L(B)$ and $p_n(x)\subseteq L(A)$ be consistent types such that\smallskip

\ceq{\hfill p(x)}{\imp}{\bigvee_{n<\omega}p_n(x)}

Prove that there is an $n<\omega$ and a formula $\phi(x)\in L(A)$ consistent with $p(x)$ such that $p(x)\wedge\phi(x) \imp p_n(x)$.\QED
\end{exercise}
\begin{comment}
\begin{exercise}
...
The material in Section~\ref{toy} is freely taken from~\cite[Section~2.6]{Pillay} 
