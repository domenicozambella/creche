...
\ssf{3}$\IMP$\ssf{1}\quad We prove the contrapositive: we assume that $\U^{|{\mr x}|}/\mathord\equiv_A$ is uncountable for some finite ${\mr x}$ and define a tree of formulas in $L_{\mr x}(A)$ by induction. Let $\phi_\0({\mr x})=\top$ and assume as induction hypothesis that $\phi_s({\mr\U})/\mathord\equiv_A$ is uncountable. This will guarantee the consistency of the branches.  For convinience we also assume that $\phi_s({\mr x})\imp\phi_{s\restriction n}({\mr x})$ for every $n<|s|$.
...
\ceq{\hfill p({\mr x})}{=}{\Big\{\xi({\mr x})\in L(A)\ :\ \phi_s({\mr\U})\cap\neg\xi({\mr\U})\,/\equiv_A \textrm{ is countable }\Big\}.}

Suppose not, then $\neg\xi_1({\mr x})\vee\dots\vee\neg\xi_n({\mr x})$ holds for some $\xi_i({\mr x})\in p$. Hence, as $\phi_s({\mr\U})/\mathord\equiv_A$ is uncountable, at least one of the sets $\phi_s({\mr\U})\cap\neg\xi_i({\mr\U})\,/\equiv_A$ is uncountable, contradicting the definition of $p({\mr x})$.
