...
\begin{exercise}
  ...
  A set $\mrS\subseteq\U^{\mr x}$ is nowhere dense in the $A$-topology if for every consistent $\phi({\mr x})\in L(A)$ there is some consistent $\psi({\mr x})\in L(A)$ such that $\psi({\mr x})\imp\phi({\mr x})$ and $\psi(\mrS)=\varnothing$.
