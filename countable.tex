...
Take $\psi_0({\gr z})= \psi({\gr z})$.
At stage $i+1$:
\begin{itemize}
   \item if $\xi_i({\mr x}\,;{\gr z})\wedge\psi_i({\gr z})$ is inconsistent, let $\psi_{i+1}({\gr z})=\psi_i({\gr z})$;
   \item otherwise, pick some $\phi({\mr x})\in p$ such that $\displaystyle\psi_i({\gr z}) \wedge \E{\mr x}\,\big[\xi_i({\mr x}\,;{\gr z})\wedge\neg\phi({\mr x})\big]$ is consistent and let this conjunction be $\psi_{i+1}({\gr z})$.
\end{itemize}
This guarantees that $\xi_i({\mr x}\,;{\gr a})$ for any ${\gr a}\models\psi_{i+1}({\gr z})$ does not isolate $p({\mr x})$.
The proof is complete if we can show that it is always possible to find the formula $\phi({\mr x})$ required above.
...
\begin{void_thm}[Omitting Types Theorem] Assume $L(A)$ is countable.
...
Since $M$ is a model, this this is equivalent to $M$ omitting $p({\mr x})$.

We proceed as in the proof of the downward L\"owenheim-Skolem theorem.
Assume that $A_i$ does not isolate $p({\mr x})$.
With the notation of Proof~\ref{diym_II_lowenheimskolemallingiu}, at stage $i=\pi(j,k)$ apply Lemma~\ref{lem_kuratowskiUlam} to find a solution $a$ of $\phi_k({\mr x})$ such that $A_{i+1}=A_i,a$ does not isolates $p({\mr x})$.
\end{proof}
...
Now, let $Y_1\subseteq\U_Y$ be countable.
Let $c\in\U_Y\sm Y_1$ and let $p(y)=\tp(c/X,Y_1)$.
We claim that $p(y)$ is realized in every model containing $X,Y_1$.
In fact, by the remark above $p(\U)=\U_Y\sm Y_1$.
...
Then $\phi(y)$ has a solution in $\U_Y\sm Y_1$.
By the remark above, this implies that $\phi(\U)$ is a cofinite subset of $\U_Y$ and this contradicts $\phi(\U)\subseteq p(\U)$.\QED
\end{example}
...
\begin{exercise} 
  ...
  Prove that $p(x)$ is isolated over $A$.\QED
  % 
  % \def\ceq#1#2#3{\parbox[b]{15ex}{$\displaystyle #1$}\parbox[b]{6ex}{\hfil$#2$}$\displaystyle #3$}
  % Hint. 
  % In general when $B\subseteq A$
  %
  % \ceq{\hfill p(x)}{\iff}{\bigvee\Big\{q(x)\in S(A)\ :\ q(x){\rm\ consistent\ with\ } p(x)\Big\}}
  %
  % If all these types $q(x)$ are isolated, by compactness there are finitely many of them.
  % Let $\phi_1(x)\dots,\phi_n(x)\in L(A)$ be the the formulas that isolate these types.
  % Then 
  % 
...
Note that this equivalent to claiming that ${\mr a}$ is an isolated point in $\U^{|{\mr x}|}$ with respect to the $A\jj$topology defined in Section~\hyperref[saturation]{\ref*{saturation}.\ref*{monster}}.
...
\end{proposition}
\begin{proof}
Let $p({\mr x},{\gr z})=\tp ({\mr b},{\gr a}/A)$.
Then  $p({\mr x},{\gr a})=\tp({\mr b}/A,{\gr a})$.
Note also that $\E{\mr x}\,p({\mr x},{\gr z})=\tp({\gr a}/A)$.
...
\begin{proof}
\ssf{1}$\IMP$\ssf{2}\quad This holds by elementarity.

\ssf{2}$\IMP$\ssf{1}\quad We construct by induction a sequence $\<{a_i}:i<\omega\>$.
...
\begin{proof} \ssf{1}$\IMP$\ssf{2}\quad This is an immediate consequence of the omitting types theorem.
...
\ssf{2}$\IMP$\ssf{1}\quad Observe that \ssf{2} implies that every countable model containing $A$ is atomic over $A$.
...
\begin{exercise}\label{ex_oligomorphic}
   ...
   \item[2.] for every countable model $M$ and every finite $|{\mr x}$, in $M^{|{\mr x}|}$ there is a finite number of orbits under $\Aut(M)$.\QED
   \end{itemize}
\end{exercise}
...
\begin{itemize}   
\item[1.] $T$ is $\omega$-categorical;
\item[2.] there is a countable model that is both saturated and atomic.\QED 
\end{itemize}
\end{exercise}
...
The set $A$ is introduced for convenience; in most application 
$A$ is the empty set.
...
\ceq{\hfill p_s}{=}{\big\{\phi_{s\restriction n}\ :\ n\le|s|\big\}.}
...
\begin{exercise}
Prove that if $T$ is small over $A$, then $T$ is small over $A,a$ for every finite tuple $a$.\QED
\end{exercise}
...
If $T$ is $\omega\jj$categorical and every finite set is contained in a finite homogeneous substructure, then $T$ has the finite model property.
\end{lemma}
\begin{proof}
