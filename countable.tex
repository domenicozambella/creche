...
Let $T$ be, as always in this chapter, a complete theory without finite models. We say that $T$ is \emph{small over $A$\/} if $S_{{\mr x}}(A)$ is countable for every ${\mr x}$ of finite length. When $A$ is empty, we simply say that $T$ is \emph{small}. The set $A$ is only introduced for convenience, $A=\0$ is the only interesting case. 

\begin{proposition}
No $T$ is small over an uncountable set and if $T$ is small (over $\0$) then it is small over any countable $A$.
\end{proposition}
...
\ceq{\hfill p({\mr x})}{=}{\Big\{\psi({\mr x})\in L(A)\ :\ \phi_s({\mr\U})\cap\neg\psi({\mr\U})\,/\equiv_A \textrm{ is countable }\Big\}.}
