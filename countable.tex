...
Let $\U$ be a saturated elementary extension of $M$. Then $\U$ is partitioned in threes definable sets $\U_F$, $\U_X$ and $\U_Y$. Each element of $\U_F$ defines a bijection between $\U_X$ and $\U_Y$.

Note that for any two elements $a,b\in\U_Y$, there is an automorphism of $\U$ that fixes $\U_X\cup\U_Y\sm\{a,b\}$ and flips $a$ and $b$.
...
\begin{proof} \ssf{1}$\IMP$\ssf{2}\quad This is an immediate consequence of the omitting types theorem. In fact, if $p({\mr x})$ is a non-isolated $A\jj$type, then there are two countable models $M$ and $N$ containing $A$ such that $M$ realizes $p({\mr x})\subseteq L(A)$ while $N$ omits it.
