...
Our purpose is to illustrade the method of proof of Theorem~\ref{thm_Hindman}.
...
Pick any type $p({\mr x})\in S({\mr\U})$ finitely satisfied in $M$ but not realized in $M$ and let ${\mr\bar c}=\<{\mr c_i}:i<\omega\>$ be a coheir sequence of $p({\mr x})$ over $M$.
...
the orbit of ${\mr a}$ under $\Aut(\U/A)$, 
with \emph{${\mr a}_A$}.
...
\begin{proof}
  Let $p({\mr y})=\tp({\mr b}/A)$.
  Then $\mrA\cdot_A{\mr b}$ is defined by the type $q({\mr z})$, where

  \ceq{\hfill q({\mr z})}{=}{\E {\mr x},{\mr y}\ \big[ {\mr z}={\mr x}{\cdot}{\mr y}\ \wedge\ {\mr x}\in\mrA\ \wedge\ {\mr y}\cnonfork_A p\big]}.
\end{proof}
...
The quotient map $\U\to\U/{\equiv_A}$ is almost a homeomorphism.


\begin{proposition}\label{prop_orbits_main}
Assume $\cnonfork_{\!A}$ is stationary,
...
Let ${\mr a'}\in{\mr a}_A$ such that ${\mr a'}\cnonfork_{\!A}{\mr b}$.
...
we obtain an element ${\mr v}\in\mrV\sm{\mr\U}$ with idempotent orbit 
...
% 
% \noindent\llap{\textcolor{red}{\Large\warning}\kern1.5ex}The following terminology in not standard.We say that the set $\mrB\subseteq\U^{|{\mr x}|}$, typically a definable set, is \emph{strongly quasi-invariant over $A$\/} if for every definable set $\mrD$ either $\mrB\cap\mrD$ or $\mrB\cap\neg\mrD$ is quasi-invariant.
% 
% \begin{proposition}If $\mrB$ is strongly quasi-invariant over $A$ and $\<{\mr\D_i}:i<n\>$ is a finite sequence of definable sets that covers $\mrB$, then $\mrB\cap{\mr\D_i}$ is strongly quasi-invariant over $A$ for some $i$.
% \end{proposition}
% 
...
\ceq{\hfill\mbox{\emph{$q(z)\otimes p({\mr x})$}}}{\deq}{\bigcup_{A\subseteq B}\ q_{\restriction B}(z)\otimes_B p({\mr x})}.
\end{definition}
