...
\begin{lemma}
If $\mrA$ is type definable over $A$ then so is $\mrA\cdot_A{\mr b}$ for any $b$.
\end{lemma}
\begin{proof}
...
\ceq{\hfill p({\mr x}, {\mr y})}
{=}
{{\mr y}\cnonfork_{\!A}{\mr c}\ \ \wedge\ \ {\mr y}\cdot_A{\mr c}\equiv_A{\mr x}}
...
By Corollary~\ref{corol_min_idempotent},
the set $\mrA\cdot_{\!M}{\mr b}$ is idempotent.
It is also type-definable and contained in $\mrA$, 
therefore by minimality $\mrA\cdot_{\!M}{\mr b}=\mrA$.
Let ${\mr\Aa'}\subseteq\mrA$ contain those ${\mr a}\cnonfork_M{\mr b}$ such that ${\mr a}{\cdot}{\mr b}\equiv_M{\mr b}$.
As ${\mr b}\in\mrA\cdot_{\!M}{\mr b}$, some ${\mr a}\in{\mr\Aa'}$ exists.
It is easy to verify that ${\mr\Aa'}$ is type-definable and invariant over $M$.
By minimality, ${\mr\Aa'}=\mrA$ and we can assume that ${\mr\Aa'}$ 
is the minimal set satisfying the property above.
For any ${\mr a}\in{\mr\Aa'}$, 
it is easy to verify that ${\mr a}\cdot_{\!M}{\mr a}\in{\mr\Aa'}$.
Then, by minimality, ${\mr a}_M={\mr\Aa'}=\mrA={\mr b}_M$.
\end{proof}

\begin{corollary}\label{corol_idempotent}
Under the same assumptions of the lemma above, every type-definable idempotent set contains an element 
with an idempotent orbit.\QED
\end{corollary}
