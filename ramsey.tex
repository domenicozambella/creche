...
\begin{proposition}
Let $\sigma:\mrU\to\mrU$ be a definable semigroup homomorphism. Then 
\begin{itemize}
...
The set on r.h.s. is easily seen to be equal to \smallskip

\hfil$\Big\{\sigma({\mr a'}{\cdot}{\mr b'})\ :\ \sigma({\mr a'})\nonforkc_M\sigma({\mr b'}),\ {\mr a'}\equiv_M{\mr a},\  {\mr b'}\equiv_M{\mr b} \Big\}$.\smallskip

Hence it suffices to check that for every ${\mr a'}$ and ${\mr b'}$\smallskip

\ceq{\hfill{\mr a'}\nonforkc_M{\mr b'}}
{\IFF}
{\sigma({\mr a'})\nonforkc_M\sigma({\mr b'})}.\smallskip

The proof of this equivalence is left to the reader (here is where we use that $M$ is a model).
\end{itemize}\vskip-1.2\baselineskip
\end{proof}

\begin{theorem}
Let ${\gr C}\subseteq{\mr G}$ be semigroups. Let ${\gr\sigma_1},\dots,{\gr\sigma_n}$ be  \emph{retractions\/} of ${\mr G}$ to ${\gr C}$, that is,
homomorphisms ${\gr\sigma_i}:{\mr G}\to{\gr C}$ such that ${\gr\sigma_i}{\restriction}{\gr C}={\rm id}_{\gr C}$.
Then, for every finite coloring of ${\gr C}$,
there is an ${\mr g}\in{\mr G}$ such that the set $\{{\gr\sigma_i}({\mr g}):i=1,\dots,n\}$ is monochromatic.
\end{theorem}

\begin{proof}
Let ${\mr G}\preceq\mrU$ and ${\gr C}\preceq\grV$,
where  $\mrU$ and $\grV$ are the domains in a many-sorted monster model.
The language contains a symbol for multiplication and for an embedding of $\grV$ in $\mrU$. 
For a lighter notation, we assume $\grV\subseteq\mrU$.
The language also contains a symbol for the retractions ${\gr\sigma_i}:\mrU\to\grV$.

Let ${\gr a}_{\mr G}$ be $\le_{\mr G}$-minimal in $\grV$.
Let ${\mr g}_{\mr G}$ be $\le_{\mr G}$-minimal in $\mrU$
and such that ${\mr g}\le_{\mr G}{\gr a}$.
% Define ${\mr c_i}={\gr\sigma_i}({\mr g})$.
% We claim that ${\gr c_i}\le_{\mr G}{\gr a}$.
% So by the $\le_{\mr G}$-minimality of ${\gr a}$, the claim implies $({\mr c_i})_{\mr G}={\gr a}_{\mr G}$ for every $i$.
% Hence the theorem follows by the claim. 

Recall that ${\mr g}\le_{\mr G}{\gr a}$ means that 
${\mr g}\cdot_{\mr G}{\gr a}={\gr a}\cdot_{\mr G}{\mr g}={\mr g}_{\mr G}$.
Applying ${\gr\sigma_i}$ to these equalities we obtain 
${\gr\sigma_i}({\mr g})\le_{\mr G}{\gr a}$ and, by minimality, 
${\gr\sigma_i}({\mr g})\equiv_M{\gr a}$.
In particular the set $\{{\gr\sigma_i}({\mr g}):i=1,\dots,n\}$ is monochromatic. 
Though this very ${\mr g}$ need not belong to ${\mr G}$,
