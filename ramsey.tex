...
If $\mrA$ is minimal among the  $M$-invariant,
idempotent,
type-definable sets then $\mrA=\O({\mr a}/M)$ for some (any) element ${\mr a}\in\mrA$.
\end{lemma}
\begin{proof}
By Corollary~\ref{corol_min_idempotent},
...
Under the same assumptions as the lemma, every type-definable idempotent set contains an element with an idmepotent orbit.
\end{corollary}
...
\ceq{\hfill\emph{$\fp_n({\mr\bar a})$}}{=}{\Big\{{\mr a_{i_1}}\kern-0.5ex\cdot\dots\cdot{\mr a_{i_n}}\ :\quad i_0<\dots<i_{n-1}<\omega\Big\}}.
...
\begin{comment}
We know to combine the ideas used in the proofs 
...
\begin{Carlson}\label{thm_HalesJewett}
Let $C$ be an infinite semigroup.
Fix a finite tuple of variables $x$.
Let $G$ be the set of terms $t(x)$ with parameters in $C$  
which we consider as a semigroup under the natural operation.

For every coloring of $G$ there are two infinite sequences
$\bar t=\<t_i:i\in\omega\>$ and $\bar s=\<s_i:i\in\omega\>$ 
of terms in $G$ such that 
\begin{itemize}
\item[1.] None of the terms in $\bar t$ is constant.

\item[2.] The set 
$\displaystyle\bigg\{t(x/s_{\restriction I})\ :\ t\in\fp(\bar t)\textrm{ and }I\in{\omega\choose |x|}\bigg\}$
is monochromatic.
\end{itemize}
\end{Carlson}
