...
% We arte interested in left ideal that are type-definable (over $M$).
...
It is interesting to note that we could define, in the obvious way, \textit{right ideals\/} and even \textit{two-sided ideals.}
But the above fact does not hold for these ideals because, in general, ${\mr b}*\mrC$ is not type-definable.

It is also useful to remark that when $\mrM$ is a minimal left ideal and ${\mr a}\in\mrM$ then $\mrM*{\mr a}=\mrM$, by minimality.
...
\begin{proof}
Let $S\preceq\mrS$.
Here $\mrS$ is a monster model in a language that expands the language of semigroups with a symbol for all subsets of $S$.
...
From the second claim of Proposition~\ref{prop_HJ_tecnical} we obtain that 
$\sigma({\mr v}*{\mr u}*{\mr v})\in{\mr v}*\sigma[\mrM]*{\mr v}\subseteq\mrC*{\mr v}={\mr v}*\mrN$ for all $\sigma\in\Sigma$.
...
Note that, by nicety, ${\mr v}*{\mr u}*{\mr v}\notin\mrC$.
