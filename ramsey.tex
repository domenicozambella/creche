...
Let $\mrA\subseteq\mrS$.
When $\mrS*\mrA\subseteq\mrA$, we say that $\mrA$ is a \emph{left-ideal.}
...
\begin{fact}
  Every left-ideal contains a minimal left-ideal and this is type-definable.
\end{fact}

\begin{proof}
  First note that, if $\mrA$ is an left-ideal and ${\mr b}\in\mrA$, then $\mrS*{\mr b}$ is a left-ideal contained in $\mrA$ that is type-definable.
  Given any left-ideal $\mrA$ ideal we construct a maximal descending chain of type-definable letft-ideals contained in $\mrA$.
  By compactness the intersection of this chain is nonempty.
  By what remarked above, it is a minimal left-ideal.
\end{proof}

Note that every left-ideal is an idempotent set and therefore contains idempotent elements.

\begin{lemma}
  Let $\mrM$ be a minimal left-ideal.
  ...
  \begin{itemize}
    ...
    \item [5.] $\mrM$ is covered by sets of the form ${\mr u}*\mrM$ as ${\mr u}\in\mrM$ ranges over the idempotents.\smallskip
  \end{itemize}
\end{lemma}

\begin{proof}
  (1) \ By minimality $\mrM*{\mr u}=\mrM$.
  Therefore ${\mr a'}*{\mr u}={\mr a}$ for some ${\mr a'}\in\mrM$.
  Then ${\mr a}*{\mr u}={\mr a'}*{\mr u}*{\mr u}={\mr a}$.

  (2) \ By (1) and idempotency, ${\mr u}$ is an identity of ${\mr u}*\mrM$.
  It suffices to prove the existence of a left inverse.
  Let ${\mr a}\in{\mr u}*\mrM$.
  By minimality $\mrM*{\mr a}=\mrM$.
  Therefore ${\mr u}*\mrM*{\mr a}={\mr u}*\mrM$.
  Then ${\mr u}*{\mr a'}*{\mr a}={\mr u}$ for some ${\mr a'}\in\mrM$.
  Then ${\mr u}*{\mr a'}$ is the left inverse of ${\mr a}$.

  (3) \ By (1), the map ${\mr a}\mapsto{\mr v}*{\mr a}$ is a semigroup homomorphism.
   By (1) and (2), its inverse is ${\mr a}\mapsto{\mr u}*{\mr a}$.
   So, ${\mr a}\mapsto{\mr v}*{\mr a}$ is a group isomomorphism.

  (4) \ Suppose there is an ${\mr a}\in{\mr u}*\mrM\cap{\mr v}*\mrM$.
...
\begin{proof}
Let $S\preceq\U$.
Here $\U$ is a monster model in a language that expands the natural one with a symbol for all subsets of $S$.
As observed in Remark~\ref{rk_coheir_stationary}, 
this makes $\cnonfork_S$ stationary.
Let $\mrC$ be the definable set such that $C=S\cap\mrC$.
...
By nicety $\U\sm\mrC$ is an idempotent such that ${\mr c}\cdot_S(\U\sm\mrC)\subseteq(\U\sm\mrC)$.
Therefore we can apply the first claim in Proposition~\ref{prop_minimal_existence2} with $\U\sm\mrC$ for $\mrA$.
This yields the existence of some  ${\mr g}\in{\mr c}\cdot_S(\U\sm\mrC)\cdot_S{\mr c}$ with idempotent orbit.
%
...
$\sigma\,{\mr g}\in{\mr c}\cdot_S\mrC\cdot_S{\mr c}$ 
...
$\sigma\,{\mr g}\equiv_S{\mr c}$ for every $\sigma\in\Sigma$.
%
In particular the set $\{\sigma\,{\mr g}:\sigma\in\Sigma\}$ is monochromatic.

Though ${\mr g}$ above need not belong to $S\sm C$, by elementarity $S\sm C$ contains some $a$ with the same property.
\end{proof}
