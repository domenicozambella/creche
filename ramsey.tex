...
Then $\mrA\cdot_{\!A}{\mr a}$ contains an element that is left-minimal.
\end{proposition}
\begin{proof}
...
We can always assume that the ${\mr c}$ above has indempotent orbit. In fact, by Corollary~\ref{corol_idempotent}, there is some ${\mr c'}\in\mrA\cdot_{\!A}{\mr c}$ with indempotent orbit. Such a ${\mr c'}$ is also left-minimal.
...
such that ${\mr c}\cdot_M\mrA,\ \mrA\cdot_M{\mr c}\subseteq\mrA$.
Then
\begin{itemize}
\item[1.] $\big({\mr c}\cdot_M\mrA\big)\cap\big(\mrA\cdot_M{\mr c}\big)$ is non empty
\item[2.] if ${\mr c}$ has idempotent orbit, then \ssf{1} contains some ${\mr g}$ with idempotent orbit 
\item[3.] if moreover ${\mr c}$ is left-minimal, then ${\mr c}\equiv_M{\mr g}$ for every ${\mr g}$ as in \ssf{2}.
\end{itemize} 
\end{proposition}
...
\begin{proof} \ssf{1.} \  
Fix any ${\mr a}\in \mrA$. By the assumptions of the proposition, ${\mr c}\cdot_M{\mr a},\ {\mr a}\cdot_M{\mr c}\in\mrA$. Then ${\mr c}\cdot_M{\mr a}\cdot_M{\mr c}$ is contained in the set in \ssf{1}.
...
Then ${\mr b}\cdot_M{\mr c}={\mr b}$, from which we obtain that ${\mr c}\cdot_M{\mr b}$ is idempotent and is contained in the set in \ssf{1}.

\ssf{3.} \ From ${\mr g}\in{\mr c}\cdot_M\mrA$ and the idempotency of ${\mr c}_M$ 
we obtain ${\mr g}_M={\mr c}\cdot_M{\mr g}$. 
From ${\mr g}\in\mrA\cdot_M{\mr c}$ and the left-minimality of ${\mr c}_M$ we obtain ${\mr c}\in\mrA\cdot_M{\mr g}$. Hence ${\mr c}_M={\mr c}\cdot_M{\mr g}$, by the idempotency of ${\mr g}_M$. Therefore ${\mr c}_M={\mr g}_M$, which proves \ssf{2}.
\end{proof}
...
Hence there is some idempotent ${\mr g}\in \big({\mr c}\cdot_G\mrA\big)\cap\big(\mrA\cdot_G{\mr c}\big)$. By  nicety, ${\mr g}\in\mrA$. From Proposition~\ref{prop_HJ_tecnical} we obtain
