...
\def\mrI{{\mr\I}} 
Assume the notation of Section~\ref{semigroups}.
Assume also that $\nonforkc_{\!M}$ is $1$-stationary and let $M$ be a model.

We write ${\mr a}\le_M{\mr b}$ if ${\mr a}\in\big({\mr b}\cdot_M\mrU\big)\cap\big(\mrU\cdot_M{\mr b}\big)$.
This relation is easily seen to be transitive 
Contrary to what the symbol may suggest, when $\mrU$ is not a monoid, ${\mr a}\le_M{\mr a}$ may not hold.

\begin{proposition}\label{prop_antisimmetria}
If ${\mr a}\le_M{\mr b}\le_M{\mr a}$ then  ${\mr a}\equiv_M{\mr b}$.
\end{proposition}
\begin{proof}
By definition, ${\mr a}\le_M{\mr b}$ implies that 
${\mr a}_M={\mr b}\cdot_M{\mr r}={\mr l}\cdot_M{\mr b}$
for some ${\mr r}, {\mr l}\in \mrU$.
It follows that ${\mr b}_M={\mr b}\cdot_M{\mr a}={\mr a}\cdot_M{\mr b}$.
The symmetric argument shows that ${\mr b}\le_M{\mr a}$ implies 
${\mr a}_M={\mr a}\cdot_M{\mr b}={\mr b}\cdot_M{\mr a}$.
Hence ${\mr a}\equiv_M{\mr b}$ follows.
\end{proof}

\begin{proposition}\label{prop_minimal_existence}
For every idempotent ${\mr a}_M$, there is a $\le_M$-minimal ${\mr b}\le_M{\mr a}$ with idempotent orbit.
\end{proposition}

\begin{proof}
Construct a decreasing $\le_M$-chain of elements with idempotent orbit.
Let ${\mr b_0}={\mr a}$.
At stage $i$, if ${\mr b_i}$ is $\le_M$-minimal we are finished.
Otherwise, there is some ${\mr b_{i+1}}<_M{\mr b_i}$.
At a limit stage $\lambda$ note that the intersection of the sets ${\mr b_i}\cdot_M\mrU\,\cap\,\mrU\cdot_M{\mr b_i}$ for $i<\lambda$
is type definable (???).
It is also idempotent and therefore, 
by Lemma~\ref{lem_Hindman}, it contains an element ${\mr b_\lambda}$ with idempotent orbit.
Then ${\mr b_\lambda}\le_M{\mr b_i}$  for every $i<\lambda$.
...
The following is an abstract version of the Hales-Jewett Theorem
which is slightly easier to prove.
...
homomorphisms such that $\sigma_{i\restriction C}={\rm id}_C$.
...
\begin{proof}
Let $G\preceq\mrV$.
Here $\mrV$ is a monster model in a language contains 
...
In $\mrU$ we define the order relation $\le_G$ as above.
By Proposition~\ref{prop_minimal_existence},
there is an ${\mr a}\in\mrU$ such that ${\mr a}\equiv_M{\mr b}$
for every ${\mr b}\in\big({\mr a}\cdot_G\mrU\big)\cap\big(\mrU\cdot_G{\mr a}\big)$.

Pick any ${\mr u}\in\mrV\sm\mrU$ and let ${\mr g}={\mr a}{\cdot}{\mr u}{\cdot}{\mr a}$.
As $\mrU$ is a nice subsemigroup, ${\mr g}\notin\mrU$.
Clearly ${\mr g}\in\big({\mr a}\cdot_G\mrV\big)\cap\big(\mrV\cdot_G{\mr a}\big)$.
By Proposition~\ref{prop_HJ_tecnical},
$\sigma_i({\mr g})\in\big({\mr a}\cdot_G\mrU\big)\cap\big(\mrU\cdot_G{\mr a}\big)$.
Hence, by Proposition~\ref{prop_antisimmetria}, $\sigma_i({\mr g})\equiv_G{\mr a}$.
In particular the set $\{\sigma_i({\mr g}):i=1,\dots,n\}$ is monochromatic. 
This very ${\mr g}$ need not belong to $G\sm C$ but,
by elementarity, $G\sm C$ contains some ${\mr g'}$ with the same property proving the theorem.
\end{proof}
