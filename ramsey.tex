...
then the partition is also called a \emph{coloring},
or \emph{$k$-coloring},
of $X$. We say that $Y\subseteq X$ is \emph{monochromatic\/} if $f\restriction Y$ is constant on $Y$. 

Let $M$ be an arbitrary infinite set. Fix $n,k<\omega$ and fix a coloring $f$ of the set of all \emph{$n$-subsets} of $M$,
...
not necessarily all of the same sort. For any two sets $\mrA,\mrB\subseteq\mrU$ we define
...
% If $\mrA\cdot_{\!A}\mrU\subseteq\mrA$ we say that it is a \emph{right-ideal\/} (over $A$).

\begin{corollary}\label{corol_min_idempotent}
Assume $\mrB\subseteq\mrA$ are both $A$-invariant. Then if $\mrA$ is idempotent,
...
\section{Ideals and idempotens}

If $\mrB\cdot_{\!A}\mrA=\mrA\cdot_{\!A}\mrB=\mrA$, 
...
A \emph{nice subsemigroup\/} of $G$ is a subsemigroup $C$ with the property that if $a{\cdot}b\in C$ then both $a,b\in C$.

\begin{theorem}
Let $\sigma_1,\dots,\sigma_n$ be  \emph{retractions\/} of $G$ to $C$, that is,
a homomorphisms $\sigma_i:G\to C$ such that $\sigma_i{\restriction}C=\id_C$.
Then for every finite coloring of $C$ there is an $a\in G$ such that the set $\{\sigma_i(a):i=1,\dots,n\}$ is monocromatic.
\end{theorem}

\begin{proof}
We think $G$ as a structure in a language $L$ that extends the language of multiplicative groups with unary relation symbols for every subset of $G$.
Let $\chi({\mr x})$ be the formula that defines the subsemigroup $C$.
