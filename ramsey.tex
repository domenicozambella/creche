...
In this and the following sections we focus on semigroups definable in a first-order structure. For lighter notation, we assume that $\mrU$ is the domain of a sort in a many-sorted monster model. The language contains, among others, the symbol \emph{$\ \cdot\ $} which is interpreted as binary associative operation on $\mrU$. 
...
As $\nonforkc_{\!A}$ is invariant over $A$, the set $\mrA\cdot_{\!A}\mrB$ is also invariant over $A$. We write \emph{${\mr a}\cdot_{\!A}\mrB$} for $\O({\mr a}/A)\cdot_{\!A}\mrB$. The expressions \emph{${\mr a}\cdot_{\!A}{\mr b}$} and \emph{$\mrA\cdot_{\!A}{\mr b}$} have a similar meaning.

\begin{lemma}
For every invariant sets $\mrA$, $\mrB$, and $\mrC$
...
We say that $\mrA$ is \emph{idempotent\/} if $\mrA\cdot_{\!A}\mrA\subseteq\mrA$.
...
Then, if $\mrA$ is minimal type-definable indempotent, $\mrA=\O({\mr a}/A)$ for any ${\mr a}\in\mrA$.
\end{lemma}
