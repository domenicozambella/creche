...
${\mr a}\equiv_{S,\,{\mr b}{\cdot}{\mr c}}{\mr a'}\cnonfork_S{\mr b}{\cdot}{\mr c},\,{\mr a},\,{\mr b},\,{\mr c}$.
...
type-definable over $S$, then $\mrA=({\mr b})_S$ for some (any) ${\mr b}\in\mrA$.
\end{lemma}
...
Let ${\mr g}$ be an element of ${\mr\S'}$ with idempotent orbit 
as given by Corollary~\ref{corol_idempotent}.
%
Let $p({\mr x})\in S(\U)$ be a global coheir of $\tp({\mr g}/S)$.
%
Let ${\mr\bar g}$ a coheir sequence of $p({\mr x})$, that is,

\ceq{\hfill {\mr g_i}}{\models}{p_{\restriction S,\,{\mr g_{\restriction i} }  }({\mr x}).}

We write ${\mr \cev{g}_{\restriction i}}$ for the tuple ${\mr g_{i-1}},\dots,{\mr g_{0}}$.
%
By the idempotency of ${\mr g}_S$ and Proposition~\ref{prop_orbits_main}, 
${\mr h}\equiv_S{\mr g}$ for all ${\mr h}\in \fp\,{\mr \cev{g}_{\restriction i}}$ and all $i$.
It follows in particular that $\fp\,{\mr \cev{g}_{\restriction i}}$ is monochromatic, 
say all its elements have color $1$.
%
Now, we use the sequence ${\mr\bar g}$ to define $\bar a\in S^\omega$
such that all elements of $\fp\,\bar a$ have color $1$.

Assume as induction hypothesis that we have $a_{\restriction i}\in S^i$ 
...
%
As ${\mr h}\equiv_S{\mr g}$, we conclude that $b\cdot{\mr h}\equiv_Sb\cdot {\mr g_0}$, which proves the claim.
...
If $\mrA\subseteq\mrC$ is such that $\mrC*\mrA\subseteq\mrA$, we say that $\mrA$ is a \emph{left ideal\/} of $\mrC$.
...
\begin{fact}
  ...
  Every left ideal of $\mrC$ contains a minimal left ideal and this is type-definable.
\end{fact}

\begin{proof}
  First note that, if $\mrA\subseteq\mrC$ is an left ideal (of $\mrC$) and ${\mr b}\in\mrA$, then $\mrC*{\mr b}$ is a left ideal contained in $\mrA$ that is type-definable.
  Therefore, given any left ideal $\mrA$ we construct a maximal descending chain of type-definable left ideals contained in $\mrA$.
  By compactness the intersection of this chain is nonempty.
  By what remarked above, this is a minimal left ideal
\end{proof}

It is useful to remark that when $\mrM$ is a minimal left ideal and ${\mr a}\in\mrM$ then $\mrM*{\mr a}=\mrM$, by minimality.
On the other hand, though ${\mr a}*\mrM\subseteq\mrM$ for every ${\mr a}\in\mrC$, we cannot infer the converse inclusion.
...
However for two-sided ideals we have the following remarkable fact.
...
\begin{proof}
  ...
  To prove that $\mrK$ is also a right ideal note that, by what remarked above

  \ceq{\hfill\mrK*{\mr a}}{=}{\bigcup\big\{\mrM*{\mr a}\ :\ \mrM\text{ a minimal left ideal of }\mrC\big\}}\medrel{=}$\mrK$.\smallskip
...
\begin{lemma}\label{lem_leftideal}
  ...
  \begin{itemize}
    ...
    \item [6.] if ${\mr a}\in\mrC$ is an idempotent, ${\mr u}*{\mr a}={\mr u}$ and ${\mr a}*{\mr u}$ is idempotent.\smallskip
  \end{itemize}
\end{lemma}

\begin{proof}
  \ssf1. By minimality $\mrM*{\mr u}=\mrM$.
  Therefore ${\mr a'}*{\mr u}={\mr a}$ for some ${\mr a'}\in\mrM$.
  Then ${\mr a}*{\mr u}={\mr a'}*{\mr u}*{\mr u}={\mr a}$.

  \ssf2. \ By \ssf1 and idempotency, ${\mr u}$ is an identity of ${\mr u}*\mrM$.
  ...
  \ssf3. By \ssf1, the map ${\mr a}\mapsto{\mr v}*{\mr a}$ is a semigroup homomorphism.
   By \ssf1 and \ssf2, its inverse is ${\mr a}\mapsto{\mr u}*{\mr a}$.
   So, ${\mr a}\mapsto{\mr v}*{\mr a}$ is a group isomomorphism.

  \ssf4. Suppose there is an ${\mr a}\in{\mr u}*\mrM\cap{\mr v}*\mrM$.
  ...
  \ssf5. Let ${\mr a}\in\mrM$.
  As $\mrM*{\mr a}=\mrM$, the set $\{ {\mr c}\,:\, {\mr c}* {\mr a} =  {\mr a}\}$ is nonempty.
  As this is a type-definable idempotent set, it contains an idempotent element ${\mr u}$.
  Then ${\mr a}\in{\mr u}*\mrM$.

  \ssf6. As $\mrM*{\mr a}=\mrM$, there is ${\mr u'}\in\mrM$ such that ${\mr u'}*{\mr a}={\mr u}$.
  Then, as ${\mr a}$ is idempotent, ${\mr u}*{\mr a}={\mr u}$ follows.
  From this equality, we easily obtain that ${\mr a}*{\mr u}$ is idempotent.
\end{proof}
...
\end{proposition}
\begin{proof}\ \ssf{1.}  
As ${\mr a}\equiv_M{\mr a'}$ implies $\sigma\,{\mr a}\equiv_S\sigma\,{\mr a'}$,
...
Let $\mrM\subseteq \mrS\sm\mrC$ be minimal a left ideal and pick an idempotent ${\mr u}\in\mrM$.
Let $\mrN\subseteq\mrC$ be a minimal left ideal of $\mrC$.
Let ${\mr v}\in\mrN$ be an idempotent.

As ${\mr v}*{\mr u}*{\mr v}\in{\mr v}*\mrM*{\mr v}$, from \ssf2 in Proposition~\ref{prop_HJ_tecnical} we obtain that for every $\sigma\in\Sigma$

\ceq{\hfill \sigma({\mr v}*{\mr u}*{\mr v})}{\in}{{\mr v}*\sigma[\mrM]*{\mr v}}
\medrel{\subseteq}${\mr v}*\mrC*{\mr v}$
\medrel{=}${\mr v}*\mrN$

By \ssf6 of Lemma~\ref{lem_leftideal}, ${\mr v}*{\mr u}*{\mr v}$ is an idempotent.
Then $\sigma({\mr v}*{\mr u}*{\mr v})$ is idempotent for all $\sigma\in\Sigma$.
As ${\mr v}*\mrN$ contains only one idempotent (orbit), all these $\sigma({\mr v}*{\mr u}*{\mr v})$ are elementarily equivalent over $S$.
In particular the set $\{\sigma({\mr v}*{\mr u}*{\mr v})\,:\,\sigma\in\Sigma\}$ is monochromatic.
Note that, by nicety, ${\mr v}*{\mr u}*{\mr v}\in\mrS\sm\mrC$.
Finally, the theorem follows by elementarity.
\end{proof}
...
\begin{exercise}
  Assume $\cnonfork_S$ is stationary.
  Let $\mrA$ be idempotent and type-definable over $S$.
  Let ${\mr c}$ is anidempotent such that ${\mr c}\in\mrA*{\mr g}$ for every ${\mr g}\in\mrA*{\mr c}$, with idempotent orbit, and such that ${\mr c}*\mrA\subseteq\mrA$.
  Prove that ${\mr c}*\mrA*{\mr c}$ is a group with identity ${\mr c}$.
\end{exercise}
