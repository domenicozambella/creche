...
The quotient map $\mrU\to\mrU/{\equiv_A}$ comes close to a homeomorphism (but not quite).
...
Every type definable idempotent set contains an element with an idmepotent orbit.
\end{corollary}
...
As observed in Remark~\ref{rk_coheir_stationary}, this language makes $\nonforkc_G$ trivially $1$-stationary.
...
\begin{theorem}
Let $G$ be a semigroups and let $C\subseteq G$ be a nice subsemigroup.
...
Let ${\mr a}\in\mrU$ be right-minimal with idempotent orbit.
...
By nicety, $\big({\mr a}\cdot_G\mrV\big)\cap\big(\mrV\cdot_G{\mr a}\big)$ 
has non empty intersection with $\mrV\sm\mrU$.
This intersection is easily seen to be an idempotent set.
Hence, by Corollary~\ref{corol_idempotent} it contains some ${\mr g}$ with idempotent orbit. Now, from ${\mr g}\in\big({\mr a}\cdot_G\mrV\big)\cap\big(\mrV\cdot_G{\mr a}\big)$
and Proposition~\ref{prop_HJ_tecnical}, we obtain 
$\sigma({\mr g})\in\big({\mr a}\cdot_G\mrU\big)\cap\big(\mrU\cdot_G{\mr a}\big)$
for all $\sigma\in\Sigma$.
As $\sigma({\mr g})$ is also idempotent, 
we apply Proposition~\ref{prop_minimal_existence2} to conclude that 
$\sigma({\mr g})\equiv_G{\mr a}$.
...
Fix a tuple of variable $x$ and let $\Sigma\subseteq C^{|x|}$ be and arbitrary finite set.
...
Now we can just apply the theorem above.
\end{proof}

\section{Carlson Theorem}

We know to combine the ideas in the proofs 
of Hindman Theorem and Hales-Jewett Theorem 
to obtain Carlson Theorem below. 

\begin{theorem}
Let $G$ be a semigroups and let $C\subseteq G$ be a nice subsemigroup.
Let $\Sigma$ be a set of retractions of $G$ to $C$.
Then, for every finite coloring of $G$,
there is a sequence of retractions $\bar\sigma=\<\sigma_i:i<\omega\>$ in $\Sigma$
and a sequence of elements of ${\mr\bar g}=\<{\mr g_i}:i<\omega\>$ of elements of $G\sm C$
such that 
