...
type-definable over $M$, then $\mrA=\O({\mr a}/M)$ for some (any) element ${\mr a}\in\mrA$.
\end{lemma}
\begin{proof}
By Corollary~\ref{corol_min_idempotent},
...
Under the same assumptions of the lemma above, every type-definable idempotent set contains an element with an idmepotent orbit.
\end{corollary}
...
and let ${\mr\bar a}\in G^\omega$.
We write $\fp_n({\mr\bar a})$ for the sets of products of $n$ elements of ${\mr\bar a}$ taken in increasing order.
Namely,

\ceq{\hfill\emph{$\fp_n({\mr\bar a})$}}{=}{\Big\{{\mr a_{i_1}}\kern-0.5ex\cdot\dots\cdot{\mr a_{i_n}}\ :\quad i_1<\dots<i_n<\omega\Big\}}.
...
there is an ${\mr\bar a}\in G^\omega$ 
such that $\fp({\mr\bar a})$ is monochromatic.
\end{Hindman}
...
with a unary relation symbol for each subset of $G$.
...
Let ${\mr b}$ be an element of $\mrU$ with idempotent orbit as given by Corollary~\ref{corol_idempotent}.
Let $p({\mr x})\in S(\U)$ be a global coheir of $\tp({\mr b}/G)$.
Let ${\mr\bar c}$ be a coheir sequence such that

\ceq{\hfill {\mr c_i}}{\models}{p_{\restriction G,\,\fp({\mr c_{\restriction i} } ) }({\mr x}),}

By the idempotency of ${\mr b}_G$ and Proposition~\ref{prop_orbits_main}, ${\mr a}\equiv_G{\mr b}$ for all ${\mr a}\in \fp({\mr\bar c})$.
It follows in particular that $\fp({\mr\bar c})$ is monochromatic.
For definiteness, say the color \textit{red}.
Now, we use the sequence ${\mr\bar c}$ to define ${\mr\bar a}\in G^\omega$ such that $\fp({\mr\bar a})$ is red.

Assume as induction hypothesis that $\fp({\mr a_{\restriction i}},\,{\mr\bar c})$ is red.
Our goal is to find ${\mr a_i}$ such that the same property holds for ${\mr a_{\restriction i}},\,{\mr a_i},\,{\mr\bar c}$.

Let $\phi({\mr a_{\restriction i}},\,{\mr c_i},\,{\mr c_{\restriction i}})$ 
be the sentence that says that 
$\fp_{i+1}({\mr a_{\restriction i}},\,{\mr c_i},\,{\mr c_{\restriction i}})$ is red.
As ${\mr\bar c}$ is a coheir sequence we can find  ${\mr a_i}\in M$ 
such that $\phi({\mr a_{\restriction i}},\,{\mr a_i},\,{\mr c_{\restriction i}})$.
Hence $\fp_{i+1}({\mr a_{\restriction i}},\,{\mr a_i},\,{\mr c_{\restriction i}})$ is red.
As ${\mr\bar c}$ is indiscernible over $G$, 
this is equivalent to $\fp({\mr a_{\restriction i}},\,{\mr a_i},\,{\mr\bar c})$.
\end{proof}

\section{Hales-Jewett Theorem}

Hales-Juvett's theorem is a purely combinatorial statement that implies van der Waerden's theorem.  The original proof Alfred Hales and Robert Jewett is combinatorial. An alternative proof, also combinatorial, has been given by Sheron Shelah. Our proof uses similar tools as Andreas Blass' proof. But we use saturation where he uses Stone-Check compactification. We present two versions of the main theorem. First we prove an abstract algebraic version due to Sabine Koppelberg which is easier to state and to prove. Finally the classical version is derived from the algebraic one.

We need a few definitions. 
We say that a set $\mrA$ is a \emph{right-ideal\/} if $\mrA\cdot_{\!A}\mrU=\mrA$.
We say that an element ${\mr a}$ is \emph{right-minimal\/} if 
${\mr a}\in{\mr b}\cdot_{\!A}\mrU$ for every ${\mr b}\in{\mr a}\cdot_{\!A}\mrU$.
In other words, ${\mr a}$ is right-minimal 
if ${\mr a}\cdot_{\!A}\mrU$, i.e.\@ the right-ideal generated by the orbit of ${\mr a}$, is minimal among the type-definable right-ideals.
The ambient semigroup $\mrU$ and the set of parameters $A$ will be clear from the contex.

\begin{proposition}\label{prop_minimal_existence1}
Every invariant right-ideal contains a right-minimal element with idempotent orbit.
\end{proposition}
\begin{proof}
Let $\mrA$ be right-ideal invariant over $A$.
For every ${\mr b}\in\mrA$ the set ${\mr b}\cdot_{\!A}\mrU\subseteq\mrA$ is type-definable over $A$. 
By compacteness we can require that it is minimal. 
As ${\mr b}\cdot_{\!A}\mrU$ is clearly idempotent, by Corollary~\ref{corol_idempotent} it contains an element ${\mr a}$ with idempotent orbit. Therefore ${\mr a}$ is the required right-minimal element.
\end{proof}
...
Then 
\begin{itemize}
\item[1.] $\big({\mr a}\cdot_M\mrU\big)\cap\big(\mrU\cdot_M{\mr a}\big)$ 
contains some ${\mr b}$ with idempotent orbit;
\item[2.] moreover, if ${\mr a}$ is right-minimal, then ${\mr a}\equiv_M{\mr b}$ for every ${\mr b}$ as in \ssf{1}.
\end{itemize}
\end{proposition}
\begin{proof} 
...
\begin{itemize}
\item[1.] Pick ${\mr c}\in{\mr a}\cdot_M\mrU$ with idempotent orbit.
Such ${\mr c}$ exists by Corollary~\ref{corol_idempotent}.
From the idempotency of ${\mr a}_M$ we obtain ${\mr c}_M={\mr a}\cdot_M{\mr c}$. 
% From the right-minimality of ${\mr a}$ we infer that ${\mr a}\in{\mr c}\cdot_M\mrU$ 
...
\item[2.] From ${\mr b}\in\mrU\cdot_M{\mr a}$ and idempotency of ${\mr a}_M$ we obtain ${\mr b}_M={\mr b}\cdot_M{\mr a}$. From the right-minimality of ${\mr a}_M$ we infer that ${\mr a}\in{\mr b}\cdot_M\mrU$. Hence ${\mr a}_M={\mr b}\cdot_M{\mr a}$, by the idempotency of ${\mr b}$. Therefore ${\mr a}_M={\mr b}_M$, which proves \ssf{2}.
.
\end{itemize}\vskip-1.2\baselineskip
\end{proof}
...
Let ${\mr a}\in\mrU$ be right-minimal with idempotent orbit over $G$.
Such ${\mr a}$ exists by  Proposition~\ref{prop_minimal_existence1}.%Therefore, by Proposition~\ref{prop_minimal_existence2}, ${\mr a}\equiv_G{\mr b}$
for every idempotent ${\mr b}\in\big({\mr a}\cdot_G\mrU\big)\cap\big(\mrU\cdot_G{\mr a}\big)$.

We move to $\mrV$ as ambient semigroup.
Inspection of the proof of claim \ssf{1} in Proposition~\ref{prop_minimal_existence2} and nicety, 
we know that there is a ${\mr v}\in\big({\mr a}\cdot_G\mrV\big)\cap\big(\mrV\cdot_G{\mr a}\big)$ 
with idempotent orbit.
Now, from ${\mr v}\in\big({\mr a}\cdot_G\mrV\big)\cap\big(\mrV\cdot_G{\mr a}\big)$
and Proposition~\ref{prop_HJ_tecnical}, we obtain 
$\sigma\,{\mr v}\in\big({\mr a}\cdot_G\mrU\big)\cap\big(\mrU\cdot_G{\mr a}\big)$
for all $\sigma\in\Sigma$.
As $\sigma\,{\mr v}$ is also idempotent, 
we apply Proposition~\ref{prop_minimal_existence2} to conclude that 
$\sigma\,{\mr v}\equiv_G{\mr a}$.
In particular the set $\{\sigma\,{\mr v}:\sigma\in\Sigma\}$ is monochromatic. 

Though the element ${\mr v}$ above need not belong to $G\sm C$,
by elementarity $G\sm C$ contains some ${\mr g}$ with the same property and this proves the theorem.
\end{proof}

We conclude by showing how the classical Hales-Jewett's theorem follows from theorem above.

\begin{HalesJewett}\label{thm_HalesJewett}
Let $C$ be an infinite semigroup.
Fix a tuple of variable $x$ and let $\Sigma\subseteq C^{|x|}$ be a finite set.
Fix also a finite coloring of $C$.
Then there is a non constant term $t(x)$ 
of the language of semigroups with parameters in $C$
such that $\{ t(\sigma): \sigma\in\Sigma\}$ is monochromatic.
\end{HalesJewett}

\begin{proof}
Let $G$ be the set of terms in the language of semigroups with parameters in $C$.
Then $G$ is a semigroup under the natural operation.
For every $\sigma\in\Sigma$ the map $t(x)\mapsto t(x/\sigma)$ is clearly a retraction.
Recall that $t(x/\sigma)$ denotes the substitution of $\sigma$ for $x$ in $t$.
Now we apply the theorem above.
\end{proof}

It is a simple exercise to combine the ideas used in the proofs 
of Hindman's theorem and Hales-Jewett's theorem 
to obtain the following.


\begin{proposition}
Let $G$ be a semigroup and let $C\subset G$ be a nice subsemigroup.
Let $\sigma:G\to C$ be a retraction.
Then for every finite coloring of $C$ there is a ${\mr\bar g}\in (G\sm C)^\omega$ 
such that the sets 
$\fp(\sigma\,{\mr\bar g}\big)$ is monochromatic.\QED
\end{proposition}

As a matter of fact, 
combining the two theorems mentioned above we obtain a seemingly stronger proposition (required for the following). 
To state it, we need to extend the scope of $\fp(\mbox{-})$ to sets of tuples of equal length.

\ceq{\hfill\emph{$\fp_n\{{\mr\bar g^j}\ :\ j\in J\}$} }{=}{\Big\{{\mr g^{j_1}_{i_1}}\kern-0.5ex\cdot\dots\cdot{\mr g^{j_n}_{i_n}}\ :\quad i_1<\dots<i_n<\alpha, \ \  j_1,\dots,j_n\in J\Big\}}.
...
\begin{proposition}
Let $C$ be a semigroup and let $C\subset G$ be a nice subsemigroup.
Let $\Sigma$ be a countable set of retractions of $G$ to $C$.
Then for every finite coloring of $C$ there is a ${\mr\bar g}\in (G\sm C)^\omega$ 
such that the sets 
$\fp \{\sigma\,{\mr g_{\restriction\,\omega\sm n}}\,:\,\sigma\in\Sigma' \}$ are all monochromatic, 
where $\Sigma'\subseteq\Sigma$ are arbitrary finite sets 
and $n=n_{\Sigma'}$.\QED
\end{proposition}

\excludecomment{commentproof}
\begin{commentproof}[Proof (sketch)]
...
$A_i=G\, \cup\, \fp\{{\mr v_{\restriction i}}:\sigma\in\Sigma\}$.
By the idempotency of ${\mr v}_G$ and Proposition~\ref{prop_orbits_main}, 
${\mr a}\equiv_G{\mr v}$ for all ${\mr a}\in \fp\{{\mr v_{\restriction i}}:\sigma\in\Sigma\}$.
It follows in particular that $\fp\{\sigma\,{\mr v_{\restriction i}}:\sigma\in\Sigma\}$ is monochromatic.
For definiteness, call its color \textit{red}.

Now we use ${\mr\bar v}$ to define the sequence ${\mr\bar g}$ required by the theorem.
Assume $\Sigma$ is the union of $\Sigma_n$, a sequence of finite sets. 
Assume as induction hypothesis that 
$\fp\{\sigma({\mr g_{\restriction n\sm i}},\,{\mr\bar c}) : \sigma\in\Sigma_i\}$ is red for all $i\le n$.
We want ${\mr g_n}$ such that the same property holds
for ${\mr g_{\restriction n+1}},\,{\mr\bar c}$ and for all $i\le n+1$.
It suffices to verify that this is expressible by a first order formula, and then reason as in the proof of Theorem~\ref{thm_Hindman}.
\end{commentproof}


\section{Carlson Theorem}

\def\dotgt{$\rlap{\kern0.7ex$\cdot$}>$}

Let $<$ be a transitive relation on the elements of a semigroup $\mrU$.

\begin{itemize}
\item[1.] upward directed: for every ${\mr a},{\mr b}$ there is ${\mr c}$ such that ${\mr a},{\mr b}<{\mr c}$;
\item[2.] ${\mr a}<{\mr b},{\mr c}$ implies ${\mr a}<{\mr b}{\cdot}{\mr c}$.
\end{itemize}

The set $\mrA\subseteq\mrU$ is \emph{\dotgt-closed\/} 
if ${\mr a}{\cdot}{\mr b}\in\mrA$ 
for every ${\mr a},{\mr b}\in\mrA$ such that ${\mr a}>{\mr b}$.
An \emph{ascending path\/} is a tuple ${\mr\bar a}$ of length $\le\omega$ such that ${\mr a_i}<{\mr a_{i+1}}$. Similarly we define \emph{descending paths}.

\begin{theorem}\label{thm_abstract_C}
Let $G$ be a semigroup and let $C\subset G$ be a nice subsemigroup.
Let $<$ be a relation on $G$ satisfying \ssf{1-4} above.
Let $A\subseteq G$ be \dotgt-closed subset.
Fix also retraction $\sigma:G\to C$.
Then, for every finite coloring of $C$,
there is an ascending ${\mr\bar g}\in A^\omega$ such that $\fp(\sigma\,{\mr\bar g})$ is monochromatic.
\end{theorem}

\begin{proof}
Reason as in the proof of Theorem~\ref{thm_abstract_HJ} and assume the same notation.
Now the language contains also a symbol $<$ for the transitive relation in the proposition.
Let $\mrA\subseteq\mrV$ be the unique definable set such that $A=G\cap\mrA$.
Let 
...
Let $p({\mr x})\in S(\mrV)$ be a global coheir of $\tp({\mr v}/G)$ and let ${\mr\bar v}$ 
be a coheir sequence such that ${\mr v_i}\models p_{\restriction A_i}({\mr x})$ where 
$A_i=G\, \cup\, \fp\, {\mr v_{\restriction i} }$.
By the idempotency of ${\mr v}_G$ and Proposition~\ref{prop_orbits_main}, 
${\mr a}\equiv_G{\mr v}$ for all ${\mr a}\in \fp\,{\mr\bar v}$.
It follows that also $\sigma\,{\mr a}\equiv_G\sigma\,{\mr v}$ and in particular that $\fp(\sigma\,{\mr\bar v})$ is monochromatic.
For definiteness, call its color \textit{red}.

Now we use ${\mr\bar v}$ to define the sequence required by the theorem.
Assume as induction hypothesis that 
$\fp\,\sigma({\mr g_{\restriction i}},{\mr\bar c})$ is red.
We want ${\mr g_i}$ such that the same holds
for ${\mr g_{\restriction i+1}},\,{\mr\bar c}$.
It suffices to verify that this is expressible by a first order formula, and then reason as in the proof of Theorem~\ref{thm_Hindman}.

We check that ${\mr\bar g}$ is ascending. Clearly ${\mr v}\in\mrV\sm G$, so it suffices to prove that $G<\mrV\sm G$
\end{proof}

Let $x=\<x_i:i<\omega\>$ be a tuple of variables. 
If $s(x)$ and $t(x)$ are terms in the language of semigroups, 
we write $s(x)<t(x)$ if the indices the variables actually occurring in $s(x)$ 
are all smaller than the indices of those occurring in $t(x)$.
We call a term $t(x)$ \textit{descending\/} if it has the form 
$x_{i_1}\cdot\cdot\cdot\, x_{i_n}$ for some $\omega>i_1>\dots>i_n$.
Clearly, the set of descending terms is \dotgt-closed.

\theoremstyle{mio}
\newtheorem{Carlson}[thm]{Carlson Theorem}
\begin{Carlson}\label{thm_Carlson}
Let $C$ be a semigroup.
Fix a finite coloring of $C$ and some $a\in C^\omega$.
Then there is an ascending tuple of descending terms $\<t_i(x):i<\omega\>$
such that $\fp\<t_i(a):i<\omega\>$ is monochromatic.
\end{Carlson}

\begin{proof}
Let $G$ be the set of terms with free variables among $x$ and parameters in $C$. 
Let $\sigma:G\to C$ be the reduction that maps $t(x)\mapsto t(x/a)$.
\end{proof}
