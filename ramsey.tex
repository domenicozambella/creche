...
It is immediate that if ${\mr c}$ is left-minimal then any ${\mr b}\in\mrA\cdot_M{\mr c}$ is also left-minimal.% and $\mrA\cdot_M{\mr c}=\mrA\cdot_M{\mr b}$.
...
When $\mrA\cap(\mrA\cdot_M{\mr a})$ is nonempty, we can also require that ${\mr c}$ has idempotent orbit.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{prop_minimal_existence2}
  ...
  \item[2.] moreover if ${\mr c}$ is left-minimal, then ${\mr c}\equiv_M{\mr g}$ for every ${\mr g}$ as in \ssf{1}.
  \end{itemize} 
\end{proposition}
...
Then $({\mr g})_M={\mr c}\cdot_M{\mr b}$ is as required by \ssf1.

\ssf{2}. From ${\mr g}\in{\mr c}\cdot_M\mrA\cdot_M{\mr c}$ and ${\mr c}\cdot_M\mrA\subseteq\mrA$ we obtain that ${\mr g}\in\mrA\cdot_M{\mr c}$.
