...
\begin{exercise}
  Let $\bar a=\<a_i:i\in I\>$ be an $A$-indiscernible sequence and let $J$ with $|J|\le \kappa$ be a linear order extending $I$.
...
Let ${\mr g_0}$ be an element of ${\mr\S'}$ with idempotent orbit 
...
\begin{proof}
  ...
  By Fact~\ref{fact_mli}, it sufficies to prove that $\mrM*{\mr b}*{\mr a}*{\mr b}\subseteq\mrM*{\mr b}$.
  This follows because ${\mr b}*{\mr a}\in\mrM$, hence $\mrM*{\mr b}*{\mr a}\subseteq\mrM$, by Fact~\ref{fact_mli} and the minimality of $\mrM$.
\end{proof}
...
\begin{lemma}\label{lem_leftideal}
  ...
  \begin{itemize}
    ...
    \item [6.] if ${\mr b}\in\mrC$ is an idempotent, then ${\mr b}*\mrM*{\mr b}$ icontains an idempotent.\smallskip
  \end{itemize}
\end{lemma}
...
As ${\mr v}*\mrN$ contains only one idempotent, $\sigma({\mr u})={\mr v}$ for every $\sigma$.
