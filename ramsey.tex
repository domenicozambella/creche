...
Let $M$ be a model and assume $\nonforkc_{\!M}$ is $1$-stationary.
Let ${\mr a}$ be right-minimal with idempotent orbit. 
Then 
\begin{itemize}
\item[1.] $\big({\mr a}\cdot_M\mrU\big)\cap\big(\mrU\cdot_M{\mr a}\big)$ 
contains some element ${\mr b}$ with idempotent orbit;
\item[2.] ${\mr a}\equiv_M{\mr b}$ for every ${\mr b}$ as in \ssf{1}.
\end{itemize}
\end{proposition}
\begin{proof} 
The set in \ssf{1} is not type-definable, 
therefore Corollary~\ref{corol_idempotent} does not apply 
directly and we need a more involved argument.
\begin{itemize}
...
\item[2.] From ${\mr b}\in\mrU\cdot_M{\mr a}$ and idempotency of ${\mr a}_M$ we obtain ${\mr b}_M={\mr b}\cdot_M{\mr a}$. From the right-minimality of ${\mr a}_M$ we infer that ${\mr a}\in{\mr b}\cdot_M\mrU$. Hence, ${\mr a}_M={\mr b}\cdot_M{\mr a}$, by the idempotency of ${\mr b}$. Therefore ${\mr a}\equiv_M{\mr b}$.
\end{itemize}\vskip-1.2\baselineskip
\end{proof}
...
The following theorem is an abstract version of the Hales-Jewett theorem due to Sabine Koppelberg
that is easier to state and to prove.
...
\begin{HalesJewett}[(abstract version)]\label{thm_abstract_HJ}
...
Therefore, by Proposition~\ref{prop_minimal_existence2}, ${\mr a}\equiv_G{\mr b}$
for every idempotent ${\mr b}\in\big({\mr a}\cdot_G\mrU\big)\cap\big(\mrU\cdot_G{\mr a}\big)$.

We move to $\mrV$ as ambient semigroup.
The intersection of $\big({\mr a}\cdot_G\mrV\big)\cap\big(\mrV\cdot_G{\mr a}\big)$ 
with $\mrV\sm\mrU$ is non empty, because by nicety both sets contain 
${\mr a}\cdot_G\big(\mrV\sm\mrU\big)\cdot_G{\mr a}$.
This intersection is easily seen to be an idempotent set.
Hence, by Corollary~\ref{corol_idempotent} it contains some ${\mr v}$ with idempotent orbit. Now, from ${\mr v}\in\big({\mr a}\cdot_G\mrV\big)\cap\big(\mrV\cdot_G{\mr a}\big)$
...
\item[1.] irreflexive 
\item[2.] transitive
\item[3.] upward directed: for every $a,b$ there is $c$ such that $a,b<c$;
\item[4.] $a<b,c$ implies $a<b{\cdot}c$.
\end{itemize}
