...
there is an infinite monochromatic \dotlt-closed set $A\subseteq G$.
...
\item[2.] moreover, if ${\mr a}$ is left-minimal, ${\mr a}\equiv_M{\mr c}$ for every ${\mr c}$ as in \ssf{1}.
\end{itemize}
\end{proposition}
...
\begin{proof} \ssf{1.} \ Pick ${\mr b}\in\mrW\cdot_M{\mr a}$ with idempotent orbit.
Such a ${\mr b}$ exists by Corollary~\ref{corol_idempotent}.
From the idempotency of ${\mr a}_M$ we obtain ${\mr b}\in{\mr b}\cdot_M{\mr a}$. 
% From the left-minimality of ${\mr a}$ we infer that ${\mr a}\in{\mr b}\cdot_M\mrW$ 
% and, by the idempotency of ${\mr b}_M$, that ${\mr a}_M={\mr b}\cdot_M{\mr a}$.
Now it is immediate to verify that any ${\mr c}\in{\mr a}\cdot_M{\mr b}\cdot_M{\mr a}$ is as required by \ssf{1}.

\ssf{2.} \ From ${\mr c}\in{\mr a}\cdot_M\mrW$ and the idempotency of ${\mr a}_M$ 
we obtain ${\mr c}_M={\mr a}\cdot_M{\mr c}$. 
From the left-minimality of ${\mr a}_M$ we infer that ${\mr a}\in\mrW\cdot_M{\mr c}$. Hence ${\mr a}_M={\mr a}\cdot_M{\mr c}$, by the idempotency of ${\mr c}$. Therefore ${\mr a}_M={\mr c}_M$, which proves \ssf{2}.
\end{proof}
...
In fact, by nicety, in the proof of claim \ssf{1} we can take ${\mr b}\in\mrV\sm\mrU$.
