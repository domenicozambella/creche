...
Let $M$ be a model and assume $\nonforkc_{\!M}$ is $1$-stationary.
If ${\mr a}$ is right-minimal and has idempotent orbit, 
then ${\mr a}\equiv_M{\mr b}$ 
for every idempotent ${\mr b}\in\big({\mr a}\cdot_M\mrU\big)\cap\big(\mrU\cdot_M{\mr a}\big)$.
\end{proposition}
\begin{proof}
Fix ${\mr b}$ be as in the proposition.
From ${\mr b}\in\mrU\cdot_M{\mr a}$ and the idempotency of ${\mr a}_M$ we obtain ${\mr b}_M={\mr b}\cdot_M{\mr a}$.
From ${\mr b}\in{\mr a}\cdot_M\mrU$ and right-minimality we have ${\mr a}\in{\mr b}\cdot_M\mrU$.
Therefore, from the idempotency of ${\mr b}_M$ we obtain ${\mr a}_M={\mr b}\cdot_M{\mr a}$. Hence ${\mr a}_M={\mr b}_M$.
\end{proof}
...
\begin{theorem}\label{thm_abstract_HJ}
...
As a matter of fact, 
combining the two theorems mentioned above we obtain a seemingly stronger proposition (which is not required for the following). 
To state it, we need to extend the scope of $\fp(\mbox{-})$ so that we may apply it to sets of tuples of equal length.
...
It follows in particular that $\fp({\mr\bar v})$ is monochromatic.
For definiteness, call its color \textit{red}.

Now we use ${\mr\bar v}$ to define the sequence ${\mr\bar g}$ required by the theorem.
Assume $\Sigma$ is the union of some sequence of finite sets $\Sigma_n$. 
...
Let $G$ be a semigroup and let $C\subset G$ be a nice subsemigroup.
Let $<$ be a transitive relation on $G$ 
with infinite ascending but no infinite descending path.
Let $A\subseteq G$ be \dotgt-closed subset.
Fix also retraction $\sigma:G\to C$.
Then, for every finite coloring of $C$,
there is an increasing ${\mr g}\in A^\omega$ such that $\{\sigma({\mr g}):\sigma\in\Sigma\}$ is monochromatic.
\end{theorem}
...
Clearly, the set of descending terms is $>$-closed.
...
Then there is an increasing tuple of descending terms $\<t_i(x):i<\omega\>$
