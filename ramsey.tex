...
if ${\mr a}\cdot_{\!A}\mrU$, the right-ideal generated by the orbit of ${\mr a}$, is minimal among the type-definable right-ideals.
...
\end{proposition}
\begin{proof}
Let $\mrA$ be invariant over $A$. Then for every ${\mr b}\in\mrA$ the set ${\mr b}\cdot_{\!A}\mrU\subseteq\mrA$ is type-definable over $A$. By compacteness we can require that it is minimal. As it is clearly idempotent, by Corollary~\ref{corol_idempotent}, it contains an element ${\mr a}$ with idempotent orbit. Therefore ${\mr a}$ is the required right-minimal element.
\end{proof}
...
Let $\sigma_i:G\to C$ for $i=1,\dots,n$ be \emph{retractions}, that is,
homomorphisms such that $\sigma_i{\restriction}C={\rm id}_C$.
Then, for every finite coloring of $C$,
there is a ${\mr g}\in G\sm C$ such that $\{\sigma_i({\mr g}):i=1,\dots,n\}$ is monochromatic.
\end{theorem}
...
the retractions $\sigma_i:\mrV\to\mrU$.

Let ${\mr a}\in\mrU$ be right-minimal with idempotent orbit. Therefore by Proposition~\ref{prop_minimal_existence2}, ${\mr a}\equiv_M{\mr b}$
...
$\sigma_i({\mr g})\in\big({\mr a}\cdot_G\mrU\big)\cap\big(\mrU\cdot_G{\mr a}\big)$.
As these $\sigma_i({\mr g})$ are also idempotent, 
we apply Proposition~\ref{prop_minimal_existence2} to conclude that $\sigma_i({\mr g})\equiv_G{\mr a}$ for all $i$.
In particular the set $\{\sigma_i({\mr g}):i=1,\dots,n\}$ is monochromatic. 
...
Fix a tuple of variable $x$ and let $F\subseteq C^{|x|}$ be and arbitrary finite set.
...
\begin{proof}
Let $G$ be the set of terms quotiented by equality modulo the theory of semigroups.
Then to every tuple $a\in F$ the map $t(x)\mapsto t(x/a)$ is a retraction.
Then we can apply the theorem above. 
\end{proof}
