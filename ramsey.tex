...
Similarly for \emph{$\mrA\cdot_{\!A}{\mr b}$} and \emph{${\mr a}\cdot_{\!A}{\mr b}$}.
In this and the next section we abbreviate $\O({\mr a}/A)$ with ${\mr a}_A$
...
The essential difference between the two theorems is contained in Lemma~\ref{lem_Hindman}.
...
Let $\mrI$ be an idempotent.
A set $\mrA$ such that $\mrA\cdot_{\!A}\mrI\subseteq\mrI\cdot_{\!A}\mrA=\mrA$ is called a (two-sided) \emph{ideal\/} of $\mrI$.
Let us fix $\mrI$ for the time being (in most of the applications $\mrI$ is the whole semigroup $\mrU$).
We order the ideals by defining \emph{$\mrA\le_{\!A}\mrB$\/} if $\mrA\cdot_{\!A}\mrB\subseteq\mrB\cdot_{\!A}\mrA=\mrA$.
Clearly, if $\mrA\subseteq\mrB$ are both ideals, then also $\mrA\le_{\!A}\mrB$.
We are interested in ideals that are $\le_{\!A}$-minimal.

\begin{proposition}
Fix some type-definable idempotent $\mrI$.
Then $\le_{\!A}$-minimal ideals exist.
\end{proposition}
...
we say that $\mrA$ absorbs $\mrB$ and write $\mrA\le_A\mrB$.
When $A=M$, a model, and $\nonforkc_{\!M}$ is $1$-stationary, 
using associativity the relation $\le_M$ is easily proved to be transitive.
In general it is not a reflexive relation.
In fact, $\mrA\le_M\mrA$ holds exactly when $\mrA$ is idempotent.

If $\O()$ is idempotent then there is a $b$ such that  such that .


Let ${\gr G}$ be a semigroup.
A \emph{nice subsemigroup\/} of ${\gr G}$ is a subsemigroup ${\mr C}$ with the property that if ${\mr a}{\cdot}{\mr b}\in{\mr C}$ then both ${\mr a},{\mr b}\in{\mr C}$.
\clearpage
\begin{theorem}
Let $\sigma_1,\dots,\sigma_n$ be  \emph{retractions\/} of ${\gr G}$ to ${\mr C}$, that is,
a homomorphisms $\sigma_i:{\gr G}\to{\mr C}$ such that $\sigma_i{\restriction}{\mr C}={\rm id}_{\mr C}$.
Then, for every finite coloring of ${\mr C}$,
there is an ${\gr g}\in{\gr G}$ such that the set $\{\sigma_i({\gr g}):i=1,\dots,n\}$ is monochromatic.
\end{theorem}

\begin{proof}
Let ${\gr G}\preceq\grV$ and ${\mr C}\preceq\mrU$,
where  $\grV$ and $\mrU$ are the domains in a many-sorted monster model.
The language contains a symbol for multiplication and for a semigroup embedding of $\mrU$ in $\grV$, 
therefore, for lighter notation, we can assume $\mrU\subseteq\grV$.
The language also contains a symbol for the retractions $\sigma_i:\grV\to\mrU$.
Then we may use freely that 
${\gr g}\equiv_{\gr G}{\gr g'}$ implies $\sigma_i({\gr g})\equiv_{\gr G}\sigma_i({\gr g'})$.
Also 

Let ${\mr a}$ be $\le_{\gr G}$-minimal in $\mrU$.
Let ${\gr g}\in\grV$ be such that ${\gr g}\le_{\gr G}{\mr a}$ and 
${\gr g}_{\gr G}$ is $\le_{\gr G}$-minimal in $\grV$.
% Define ${\mr c_i}=\sigma_i({\gr g})$.
% We claim that ${\gr c_i}\le_{\gr G}{\mr a}$.
% So by the $\le_{\gr G}$-minimality of ${\mr a}$, the claim implies $({\mr c_i})_{\gr G}={\mr a}_{\gr G}$ for every $i$.
% Hence the theorem follows by the claim. 

Recall that ${\gr g}\le_{\gr G}{\mr a}$ means that 
${\gr g}\cdot_{\gr G}{\mr a}={\mr a}\cdot_{\gr G}{\gr g}={\gr g}$.
Applying $\sigma_i$ to each member we obtain 
$\sigma_i({\gr g})\cdot_{\gr G}{\mr a}={\mr a}\cdot_{\gr G}\sigma_i({\gr g})=\sigma_i({\gr g})$.
Hence $\sigma_i({\gr g})\le_{\gr G}{\mr a}$ and, by minimality, 
$\sigma_i({\gr g})\equiv_{\gr G}{\mr a}$.
In particular the set $\{\sigma_i({\gr g}):i=1,\dots,n\}$ is monochromatic. 
Though this very ${\gr g}$ need not be in ${\gr G}$,
by elementarity some ${\gr g'}\in{\gr G}$ is as required by the theorem.
\end{proof}
