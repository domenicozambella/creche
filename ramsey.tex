...
% Let ${\mr\bar a}$ be a tuple of elements of $\mrG$ of length $\le\omega$.
% Let $\Sigma$ be a set of semigroup homomorphisms $\mrG\to\mrC$.
% Then for any $n\le|{\mr\bar a}|$ we define
% 
% \ceq{\hfill\fp^{\Sigma}_n\,{\mr\bar a} }{=}{\Big\{\sigma_{i_0}\,{\mr a_{i_0}}\kern-0.5ex\cdot\dots\cdot\sigma_{i_k}\,{\mr a_{i_k}}\ :\quad i_0<\dots<i_k<|{\mr\bar a}|, \ \  \bar\sigma\in\Sigma^{|{\mr\bar a}|}, \ k< n\Big\}}
% 
% When $n=|{\mr\bar a}|$ we write $\fp^{\Sigma}{\mr\bar a}$.
% 
Let ${\mr\bar a}$ be a tuple of elements of $\mrG$ of length $\le\omega$.
...
When $n=|{\mr\bar a}|$ we write \emph{$\fp^{\bar\Sigma}{\mr\bar a}$}. %Also, if $\bar\Sigma$ is such that $\Sigma_n=\Sigma$ for some fixed $\Sigma$, we write \emph{$\fp^{\Sigma}{\mr\bar a}$} for $\fp^{\bar\Sigma}{\mr\bar a}$.

\begin{proposition}[(Hindman + Hales-Jewett)]\label{prop_H+HJ}
