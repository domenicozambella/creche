...
We may identify the partition of a set $X$ into $k$ subsets with a map $f:X\to [k]$.
The elements of $[k]=\{1,\dots,k\}$ are called colors and the partition is also called a \emph{coloring}, or \emph{$k$-coloring}, of $X$. We say that $Y\subseteq X$ is \emph{monochromatic\/} if $f\restriction Y$ is constant on $Y$. 
...
For every infinite set $M$, every $n,k<\omega$, every $k$-coloring of ${M\choose n}$ has an infinite monochromatic subgraph $H\subseteq M$.
\end{Ramsey}
\begin{proof}
...
There is a first-order sentence saying that the formulas $r_i({\mr x_0},\dots,{\mr x_{n-1}})$ form a coloring of ${M\choose n}$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Ehrenfeucht-Mostowski construction of indiscernibles}
...
We write \emph{$p({\mr\bar x})\;=\;\textrm{{\small EM}-tp}({\mr\bar a}/A)$} and say that $p({\mr\bar x})$ is the \emph{Ehren\-feucht-Mostowski type\/} of ${\mr\bar a}$ over $A$ if $p({\mr\bar x})$ is the set of formulas $\phi({\mr x_{\restriction n}})\in L(A)$ such that $\phi({\mr a_{\restriction I_0}})$ holds for every $I_0\in{I\choose n}$.
...
\section{Indempotent types in semigroups (crap)}\label{semigroups}
...
Symmetrically, say that $q({\mr x})$ defines a \emph{left-ideal\/} if  ${\mr a}{\cdot}{\mr b}\models q({\mr x})$ for every ${\mr b}\models q({\mr x})$ and ${\mr a}\in\mrU$ such that ${\mr a}\cnonfork_A{\mr b}$.
