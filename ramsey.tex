...
In this and in the subsequent sections we focus on semigroups definable in first-order structures.
For ease of notation, we identify our semigroup with $\mrU$,
which here denotes the domain of a sort in a many-sorted monster model.
...
In this and the next section we denote $\O({\mr a}/A)$, 
...
The quotient map $\mrU\to\mrU/{\equiv_A}$ is almost (though not quite) a homeomorphism.
...
Such a ${\mr c}$ exists by Corollary~\ref{corol_idempotent}.
...
We can further require that ${\mr b}\in\mrV\sm\mrU$: in the proof of claim \ssf{1}, with $\mrV$ for $\mrW$, 
...
but we use saturated models where he uses Stone-\v{C}ech compactification.
