...
of the main result of this section, the Lyndon-Robinson Lemma. 
In fact, under the additional assumption of completeness, Lemma~\ref{lem_qfdefinability} 
becomes essentially identical to the claim in Exercise~\ref{ex_qe_in_U}.

However, we are interested in criteria for quantifier
elimination, e.g.\@ Corollary~\ref{corol_QE} below.
Often we first need to prove quantifier elimination 
in order to be able to prove completeness.
Therefore the assumption of completeness would make criteria 
of quantifier elimination less applicable.
...
If $\ssf{C}\subseteq\{\A,\E,\neg,\vee,\wedge\}$ is a set of connectives, we write \emph{$\ssf{C}\Delta$} for the closure of $\Delta$ with respect to all connectives in $\ssf{C}$. We may write $\pmDelta$ for $\{\neg\}\Delta$.\QED%Note that $\ssf{C}_1\ssf{C}_2\Delta$ does not always coincide with $\big(\ssf{C}_1\cup\ssf{C}_2)\Delta$.
\end{definition}
...
\ssf{3}$\IMP$\ssf{1}\quad Let ${\gr a}$ enumerate $\dom k$ and let $|{\gr z}|=|{\gr a}|$. Formulas in $\{\E\}\Delta$ with free variables among ${\gr z}$ are of the form  $\E {\mr\bar x}\, \phi ({\mr\bar x}\,;{\gr z})$ where $\phi({\mr\bar x}\,;{\gr z})$ is in $\Delta$ and ${\mr\bar x}$ is some fixed tuple of length $\omega$. Assume $M\models\E{\mr\bar x}\,\phi({\mr\bar x}\,;{\gr a})$ and let ${\mr\bar b}$ be such that $M\models\phi({\mr\bar b}\,;{\gr a})$. By \ssf{3}, we can extend $k$ to some $\Delta$-morphism $h:M\to N$ defined in ${\mr\bar b}$. Then $N\models\phi(h{\mr\bar b}\,;h{\gr a})$ and therefore $N\models\E {\mr\bar x}\,\phi({\mr\bar x}\,;k{\gr a})$.
\end{proof}
...
Let $L$ be the language of strict orders.  Let $T$ be the theory of \emph{discrete linear orders}. Namely, $T$ extends $T_{\rm dlo}$  (see Section~\hyperref[dlo]{\ref*{relational}.\ref*{dlo}}) with the following two of axioms with the following two of axioms
\begin{itemize}
\item[dis$\uparrow$.] $\E z\ \big[x<z\ \wedge\ \neg\E y\ x<y<z\big]$;
\item[dis$\downarrow$.] $\E z\ \big[ z<x\ \wedge\ \neg\E y\ z<y<x\big]$.
\end{itemize}
Let $\Delta$ be the set of formulas that contains (all alphabetic variants of) the formulas $x<_ny:=\E^{\ge n}z\ (x\mathord<z\mathord< y)$ and their negations (read $<_0$ as $<$). Prove that the theory of discrete linear orders has $\Delta$-elimination of quantifiers. Prove that the structure $\QQ\times\ZZ$ ordered with the lexicographic order 
...
\begin{exercise}
Prove that $\M'$ satisfies finite character of morphisms, \ssf{c2} of Definition~\ref{def_com_c}.\QED
\end{exercise}
