...
In general we say that $T$ is \emph{$\Delta$-model-complete\/} if every $\Delta$-embedding $h:M\hookrightarrow N$ between models of $T$ is a $\{\A\E\}\Delta$-embedding. 

The notion (and the somewhat weird terminology) was introduced by Abraham Robinson. Model-completeness is equivalent to elimination of quantifiers up to one.

\begin{proposition}
The following are equivalent
\begin{itemize}
\item[1.] $T$ is $\Delta$-model-complete;
\item[2.] every formula in $\{\A\E\mathord\wedge\mathord\vee\}\Delta$ is equivalent over $T$ to a formula in $\{\E\mathord\wedge\mathord\vee\}\Delta$.
\end{itemize}
\end{proposition}

\begin{proof}
\ssf{1}$\IMP$\ssf{2}\quad By \ssf{1}, every formula $\{\A\E\mathord\wedge\mathord\vee\}\Delta$ is preserved by $\Delta$-embeddings. Then \ssf{2} follows from Theorem~\ref{thm_embeddings_preservation_E}.

\ssf{2}$\IMP$\ssf{1}\quad Clear, because $\Delta$-embeddings preserve formulas in $\{\E\mathord\wedge\mathord\vee\}\Delta$.
\end{proof}

\begin{exercise}
Prove that the theory of discrete linear orders defined in Exercise~\ref{ex_discrete_lo} is model-complete.\QED
\end{exercise}
