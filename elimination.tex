...
\ssf{3}$\IMP$\ssf{1}\quad Let ${\gr a}$ enumerate $\dom k$ and let $|{\gr z}|=|{\gr a}|$. Formulas in $\{\E\wedge\}\Delta$ with free variables among ${\gr z}$ are of the form  $\E {\mr\bar x}\, \phi ({\mr\bar x}\,;{\gr z})$ where $\phi({\mr\bar x}\,;{\gr z})$ is in $\Delta$ and ${\mr\bar x}$ is some fixed tuple of length $\omega$. Assume $M\models\E{\mr\bar x}\,\phi({\mr\bar x}\,;{\gr a})$ and let ${\mr\bar b}$ be such that $M\models\phi({\mr\bar b}\,;{\gr a})$. By \ssf{3}, we can extend $k$ to some $\Delta$-morphism $h:M\to N$ defined in ${\mr\bar b}$. Then $N\models\phi(h{\mr\bar b}\,;h{\gr a})$ and therefore $N\models\E {\mr\bar x}\,\phi({\mr\bar x}\,;k{\gr a})$.
\end{proof}
...
\item[3.] $k:M\imp N$ extends to an $\Delta$-embedding.
\end{itemize}
\end{corollary}
...
\begin{proof} Left as an exercise for the reader. Hint: to prove implication \ssf{1}$\IMP$\ssf{2} define $p(x,y)=\neg\{\A\vee\}\Deltatp_N (ka,c)$, where $a$ is a tuple that enumerates $\dom k$. From \ssf{1} obtain that $p(a,y)$ is finitely consistent in $M$. Then proceed as in the proof of Proposition~\ref{prop_EDelta_estensione}.
\end{proof}
...
\item[3.] $k:M\imp N$ extends to an $\Delta$-epimorphism.
\end{itemize}
\end{corollary}
...
\item[2.] every $\Delta$-epimorphism between models of $T$ preserves $\phi(x)$.
\end{itemize}
\end{theorem}
...
\end{itemize}
Hence a theory with quantifier elimination is complete if it decides the characteristic, see Definition~\ref{rmk_characteristic}).
\end{remark}
...
\begin{itemize}
  \item[a.] for every $b\in M$ some $\Delta$-morphism $h:M\imp N$ extends $k$ and $b\in\dom h$;                                                       \item[b.] for every $c\in N$ some $\Delta$-morphism $h:M\imp N$ extends $k$ and $c\in\range h$.
\end{itemize}
\end{itemize}
\end{corollary}
...
\begin{exercise}
Consider $\RR$ as a structure in the language of real vector spaces expanded with the usual order relation. Prove that $\Th(\RR)$ has elimination of quantifiers.
\end{exercise}
...
Suppose that all completions of $T$ are of the form 
$T\cup S$ for some set $S$ of quantifier-free sentences.
