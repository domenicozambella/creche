...
\begin{proof} Left as an exercise for the reader. To prove implication \ssf{1}$\IMP$\ssf{2} define $p(x,y)=\neg\{\A\vee\}\Deltatp_N (ka,c)$, where $a$ is a tuple that enumerates $\dom k$. From \ssf{1} we obtain that $p(a,y)$ is finitely consistent in $M$. Then we proceed as in the proof of Proposition~\ref{prop_EDelta_estensione}.
\end{proof}
...
\ssf{2}$\IMP$\ssf{1}\quad We prove by induction of syntax that $\Delta$-morphism preserve the truth of all formulas in $\{\A,\E,\mathord\wedge,\mathord\vee\}\Delta$, this suffices by Lemma~\ref{lem_qfdefinability}. Induction for the connectives $\vee$ and $\wedge$ is trivial.  So assume as induction hypothesis that the truth of  $\phi(x,y)$ is preserved.  By Lemma~\ref{lem_qfdefinability} $\phi(x,y)$ is equivalent tot a formula in $\Delta$, hence by \ssf{2} the truth of $\E y\, \phi (x,y)$ and $\A y \,\phi (x,y)$ is preserved.
\end{proof}

Condition \ssf{2} of the corollary above may be difficult to verify directly. Condition \ssf{2} of the following corollary of Proposition~\ref{prop_EDelta_estensione} and~\ref{prop_ADelta_estensione} may be easier.
