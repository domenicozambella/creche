...
If $p_{=}(x)$ is trivial, as defined in Notation~\ref{notation1}, then we say that $p(x)$ is \emph{equationally trivial.}
In this case $\proves p(x)\iff p_<(x)$.
If $\attp({\mr b}/A)$ equationally trivial, we say that $b$ is \emph{transcendental\/} over $A$.
%
...
\begin{proof}
  ...
  We need to prove that for every ${\gr a}\in A^{|{\gr z}|}$ and every polynomial $t({\mr x}\,;{\gr z})$ one of the following holds
  \begin{itemize}
  \item[a.] $\proves\   \phi({\mr x})\imp t({\mr x}\,;{\gr z})>0$;
  \item[b.] $\proves\   \phi({\mr x})\imp t({\mr x}\,;{\gr a})<0$;
  \item[c.] $\proves\   \phi({\mr x})\imp t({\mr x}\,;{\gr a})=0$.
  \end{itemize}
  
  Suppose not for a contradiction. 
  %
  Then there are three models $\<A\>_M\subseteq N_i\models T_{\rm oid}$, some ${\mr b_i}\in N_i$ such that 
  \begin{itemize}
  \item[a$'$.] $N_1\models \phi({\mr b_1})$ \ and \ $N_1\models t({\mr b_1}\,;{\gr a})>0$;
  \item[b$'$.] $N_2\models \phi({\mr b_2})$ \ and \ $N_2\models t({\mr b_2}\,;{\gr a}) < 0$;
  \item[c$'$.] $N_3\models \phi({\mr b_3})$ \ and \ $N_3\models t({\mr b_3}\,;{\gr a}) = 0$.
  \end{itemize} 
\end{proof}
