...
If $\grD$ is approximated by $\phi({\mr x}\,;{\gr z})$ then so is any $\grC$ such that $\grC\equiv\grD$, see Section~\hyperref[expansions]{\ref*{invariant}.\ref*{expansions}} for the definition.
...
The proof is exactly the same, in fact elementarity and saturation are never used (to a great extent, no model theory is used either, see Section~\hyperref[stability_combinatorics]{\ref*{combinatorics}.\ref*{stability_combinatorics}}).\QED
\end{remark}
...
The following proposition highlight the connection between stability and the cardinality of types.  For the definition of tree of formulas see Definition~\ref{def_tree_formulas}.

\begin{proposition}The following are equivalent
\begin{itemize}
\item[1.] $\phi({\mr x}\,;{\gr z})$ is a stable formula;
\item[2.] there are $2^\kappa$ many types in $S_{\phi}(\U)$;
\item[3.] there are uncountably many types in $S_{\phi}(\U)$; 
\item[4.] there is $\big\<{\gr b_s}:s\in 2^{<\omega}\big\>$ such that $\big\<\phi({\mr x}\,;{\gr b_s}):s\in 2^{<\omega}\big\>$ is a tree of formulas.
\end{itemize}
\end{proposition}
...
\ceq{\hfill i< j}{\IFF}{\phantom{\neg}\phi({\mr a_i}\,;{\gr b_j})}\hfill for all $i,j\in\I$
...
\ssf{2}$\IMP$\ssf{3}\quad Is trivial.

\ssf{3}$\IMP$\ssf{4}\quad Similar to the proof of implication \ssf{4}$\IMP$\ssf{1} in~\ref{prop_small_equivalents}. Fix $s\in 2^{\omega}$ and assume that ${\gr b_{s\restriction i}}$ has been defined for all $i\le |s|$ so that there are uncountably many types $p({\mr x})\in S_{\phi}(\U)$ such that $\phi({\mr x}\,;{\gr b})\iff s(i)=1$ for all $i<|s|$. Note that, by \ssf{3}, ${\gr b_\0}$ can be chosen arbitrarily. For clarity we define

\ceq{\hfill \phi_s({\mr x})}{=}{\bigwedge_{i<|s|}\phi({\mr x}\,;{\gr b_{s\restriction i}})\iff s(i)=1.}

So, our induction hypothesis says that there are uncountably many types containing $\phi_s({\mr x})$. We now claim that the following type is consistent


\ceq{\hfill q({\mr x})}{=}{\Big\{\phi({\mr x}\,;{\gr b})\ :\ {\gr b}\in\U^{|{\gr z}|} \textrm{ countably many }p\textrm{ contain } \phi_s({\mr x})\wedge\neg\phi({\mr x}\,;{\gr b})\Big\}.}


Suppose not, then for some finitely many formulas $\phi({\mr x}\,;{\gr b_i})\in q$

\ceq{\hfill \phi_s({\mr x})}{\imp}{\bigvee^n_{i=1}\neg\phi({\mr x}\,;{\gr b_i}).}

By the induction hypothesis at least one of the formulas $\phi_s({\mr x})\wedge\phi({\mr x}\,;{\gr b_i})$ is contained in uncountably many $\phi$-types, contradicting the definition of $q$. This proves the claim and we resume the proof of the theorem.

We need to prove that for some ${\gr b}$ both $\phi_s({\mr x})\wedge\phi({\mr x}\,;{\gr b})$ and $\phi_s({\mr x})\wedge\neg\phi({\mr x}\,;{\gr b})$ are contained in uncountably many $\phi$-types. If not then the type $q$ above is a complete $\phi$-type: a unique $q({\mr x})\in S_{\phi}(\U)$ containing $\phi_s({\mr x})$.  

%Assuming \ssf{3} we construct a tree of formulas $\big\<\phi({\mr x}\,;{\gr b_s}):s\in 2^{<\omega}\big\>$ as in Definition~\ref{def_tree_formulas}. Hence to every $s\in s^\omega$ there corresponds a distinct type $p_s$. 


\ssf{4}$\IMP$\ssf{1} Fix an arbitrary $n$. For $i=0,\dots, n$ abbreviate $b_{1^i}$ by $b_i$.

\ceq{\hfill a_j}{\models}{\displaystyle\bigwedge^{j-1}_{i=0}\phi({\mr x}\,;{\gr b_i})\wedge \neg\phi({\mr x}\,;{\gr b_j})}
