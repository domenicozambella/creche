...
The following proposition highlights the connection between the stability of and  $\phi({\mr x}\,;{\gr z})$ and the cardinality of $S_\phi(\U)$, equivalently the number of subsets externally definable by $\\phi({\mr x}\,;{\gr z})$. 
...
We use binary trees to prove the following theorem.
We say that a formula $\phi(x)\in L(\U)$ is strongly almost satisfied in $A$ if the conjunction of finitely many $A$-conjugates of $\phi(x)$ is almost satisfied in $A$.

\begin{theorem}\label{thm_ziegler}
   Let $\phi({\mr x}\,;{\gr z})\in L$ be stable.
   Let $\psi({\mr x})\in L(\U)$ be strongly almost satisfied in $A$. 
   Then for every ${\gr b}$ either $\psi({\mr x})\wedge\phi({\mr x}\,;{\gr b})$ or $\psi({\mr x})\wedge\neg\phi({\mr x}\,;{\gr b})$ is almost satisfied in $A$.
\end{theorem} 

\begin{proof}
Suppose not. Then there are two models containing $A$, the first omitting $\psi({\mr x})\wedge\phi({\mr x}\,;{\gr b})$, the second omitting $\psi({\mr x})\wedge\neg\phi({\mr x}\,;{\gr b})$.
It is easy to see that we can expand these models to two substructures $\V_0$ and $\V_1$ that are $A$-isomorphic to $\U$ and such that $\psi(\V_0)\subseteq\neg\phi(\V_0\,;{\gr b})$ and $\psi(\V_1)\subseteq\phi(\V_1\,;{\gr b})$.

Let $f_i:\U\to\V_i$ be the $A$-isomorphisms mentioned above. 
Let $\V_\0=\U$ and ${\gr b_\0}={\gr b}$.
Also, write $\psi({\mr x},c_\0)$ for $\psi({\mr x})$, where $c_\0$ are all the parameters in the formula.
Define inductively for every $s\in 2^{<\omega}$ 

\hfil$\V_{s^\frown i}=f_i[\V_s]$\hfil ${\gr b_{s\frown i}}=f_i({\gr b_s})$ \hfil $c_{s\frown i}=f_i (c_s)$.

Then from the inclusion above we obtain

\hfil$\psi(\V_{s\frown0},c_s)\subseteq\neg\phi(\V_{s\frown0}\,;{\gr b_s})$ \ \ and \ \  $\psi(\V_{s\frown1},c_s)\subseteq\phi(\V_{s\frown1}\,;{\gr b_s})$.

Note that  $\psi({\mr x},c_s)$ is almost satisfiable, $\psi(\V_{s\frown i},c_s)$ is nonempty.
Any element of $\psi(\V_{s\frown i},c_s)$ is a solution of $\phi(x\,;{\gr b_r})$ for any $r\subseteq s$.
This proves that $\<\phi(x\,;{\gr b_r})\ :\  s\in2^{<\omega}\>$ is a binary tree, contradicting the stability of  $\phi(x\,;{\gr bz})$.
\end{proof}

\begin{corollary}\label{corol_stable_coheir_over_models}
  Let $\phi({\mr x}\,;{\gr z})\in L$ be stable.
  Every $q({\mr x})\subseteq L_{\phi}(A)$ has a global extension $p({\mr x})\in S_{\phi}(\U)$ that is almost finitely satisfied in $A$.
  In particular $p({\mr x})$ is Lascar invariant over $A$.
\end{corollary}

\begin{proof}
  By Proposition~\ref{prop_exisntence_coheirs_over_sets} it suffices to prove that if $\psi({\mr x})\vee\vartheta({\mr x})\in L(\U)$ is finitely satisfied over every model $M$ containing $A$ then so is either $\psi({\mr x})$ or $\vartheta({\mr x})$.
  This follows easily from Theorem~\ref{thm_ziegler}.
\end{proof}
...
Most theorem in this section have a formulation that only requires the stability of a fixed formula $\phi({\mr x},{\gr z})\in L$.
This is sometimes called \textit{local\/} stability.
For simplicity, here we assume that all formulas are stable.

\begin{theorem}\label{thm_symmetry}
  Let $\phi({\mr x}\,;{\gr z})\in L(\U)$ be stable.
  For every $M,{\mr a},{\gr b}$ 

  \ceq{\hfill\phi(M^{\mr x}\,;{\gr b})\neq\varnothing}{\IFF}{\phi({\mr a}\,;M^{\gr z})\neq\varnothing}
\end{theorem}

Note that this proves that, when $T$ is stable  

\ceq{\hfill{\mr a}\cnonfork_M{\gr b}}{\IFF}{{\gr b}\cnonfork_M{\mr a}}

\begin{proof}
  Assume $\phi(M^{\mr x}\,;{\gr b})\neq\varnothing$.
  Let ${\gr\V}$ be as in Proposition~\ref{prop_saturate_heir}.
  By stability, there is $\psi({\gr z})\in L({\gr\V})$ such that $\psi({\gr\V})=\phi({\mr a},{\gr\V})$.
  Recall that by Lemma~\ref{lem_coheir_independence} (non-splitting) if ${\gr b'}\equiv_M{\gr b''}$ are in ${\gr\V}$ then ${\gr b'}\equiv_{M,a}{\gr b''}$.
  Then $\psi({\gr\V})$ is invariant under $\Aut({\gr\V}/M)$.
  Then, up to equivalence, $\psi({\gr z})\in L(M)$.
  Therefore $\psi({\gr z})$ is satisfied in $M$
  and so is $\phi({\mr a},{\gr z})$.
  This prove one direction and, by symmetry, the theorem.
\end{proof}

We deduce a version of Harrington's mysterious Lemma cfr.~\cite[Lemma 8.3.4]{TZ}.

\begin{corollary}\label{corol_harrington}
  ($T$ stable) 
  Let $\phi({\mr x}\,;{\gr z})\in L(\U)$.
  Let $\V$ be an elementary extension of $\U$.
  Let ${\mr a}\in{\mr\V^x}$ and ${\gr b}\in{\gr\V^z}$.
  By stability the sets ${\grB}=\phi({\mr a}\,;{\gr\U^z})$ and ${\mrA}=\phi({\mr\U^x}\,;{\gr b})$ are definable.
  We write ${\mr x}\in{\mrA}$ and ${\gr z}\in{\grB}$ for the respective defining formulas.
  Then 
  
  \ceq{\hfill\V}{\models}{\phantom{\neg}\phi({\mr a}\,;{\gr b})\ \imp\ {\mr a}\in{\mrA}\wedge{\gr b}\in{\grB}}\quad  and

  \ceq{\hfill\V}{\models}{\neg\phi({\mr a}\,;{\gr b})\ \imp\ {\mr a}\notin{\mrA}\wedge{\gr b}\notin{\grB}.}

  In particular $\V\models{\mr a}\in{\mrA}\iff{\gr b}\in{\grB}$.
\end{corollary}

It suffices to assume $\phi({\mr x}\,;{\gr z})$ stable, see Exercise~\ref{ex_harrington}.

\begin{proof}
  We can assume that ${\mr a}\cnonfork_\U{\gr b}$.
  Negate the theorem.
  There are four ways for the theorem to fail.
  Suppose first that $\V\models\phi({\mr a}\,;{\gr b})$ and ${\gr b}\notin{\grB}$.
  By symmetry,  ${\gr b}\cnonfork_\U{\mr a}$.
  Therefore $\V\models\phi({\mr a}\,;{\gr b_0})$ for some ${\gr b_0}\in{\gr\U^z}\sm{\grB}$ which contradicts the definition of ${\grB}$.
  In the other three cases the reasoning is similar.
\end{proof}
...
\begin{proposition}\label{prop_type_over_acl2}
  ($T$ stable) 
  Let $\phi({\mr x}\,;{\gr z})\in L(\U)$. Let $p({\mr x})\in S(\U)$. Then the following are equivalent
\begin{itemize}
\item[1.] $p({\mr x})$ is Lascar invariant over $A$;
\item[2.] ${\gr\D_{p,\phi}}\in \acl^\eq\!A$ for all $\phi({\mr x}\,;{\gr z})\in L$.
\end{itemize}
\end{proposition}
\begin{proof}
 \ssf{2}$\IMP$\ssf{1} Clear (stability is not required).
 
 \ssf{1}$\IMP$\ssf{2} The sets ${\gr\D_{p,\phi}}$ are externally definable therefore, by Theorem~\ref{thm_def_stable_formula2}, they are definable (over $\U$).
...
\begin{proposition}\label{prop_type_over_acl_stationary} 
  ($T$ stable) Every type $q({\mr x})\in S\big(\acl^\eq\!A\big)$ is stationary.
\end{proposition}

\begin{proof}
Let $q_i({\mr x})\in S(\U^\eq)$, for $i=1,2$, be two global types that extend $q({\mr x})$ and are invariant over $\acl^\eq\!A$.
Let $\V$ be a sufficiently saturated elementary extension of $\U$. 
Let ${\mr a_i}\in{\mr\V^x}$ realize $q_i({\mr x})$.
We prove that $q_1({\mr x}) =q_2({\mr x})$ by showing that for every $\phi({\mr x}\,;{\gr z})\in L$ the two sets ${\gr\B_i}=\phi({\mr a_i}\,;{\gr\U^z})$ are equal.


Let ${\gr b}\in{\gr\U^z}$ be arbitrary.
Let $q({\gr z})$ be a global extension of $\tp({\gr b}/\acl^\eq\!A)$ that is Lascar invariant over $A$.
This exists by Corollary~\ref{corol_stable_coheir_over_models}.
Let ${\gr b'}\in{\gr\V^z}$ realize $q({\gr z})$.
Let ${\mrA}=\phi({\mr\U^x}\,;{\gr b'})$. 
By stability and invariance, $\mrA$ is definable over $\acl^\eq\!A$.
Then ${\mr a_1}\in\mrA\iff{\mr a_2}\in\mrA$.
By Corollary~\ref{corol_harrington}, ${\mr a_i}\in\mrA\iff{\gr b'}\in\gr\B_i$.
These three equivalences yield ${\gr b'}\in{\gr\B_1}\iff{\gr b'}\in\gr\B_2$.
By stability and invariance the sets $\gr\B_i$ are definable over $\acl^\eq\!A$.
Therefore ${\gr b}\in{\gr\B_1}\iff{\gr b}\in\gr\B_2$.
\end{proof}
...
\begin{proof}\ssf{1}$\IMP$\ssf{2}.
  ...
  Then ${\mr a},{\mr\bar c}$ and ${\mr b},{\mr\bar c}$ are $A$-indiscernible sequences, which proves \ssf{1}, see Exercise~\ref{ex_Lntp_indiscernibles}.
\end{proof}
...
The following theorem deals with a property of definability that go in the dual direction.
...
\begin{proof}
  ...
  By Theorem~\ref{thm_def_stable_formula} with $\V$ for $\U$, there are ${\mr a_1},\dots,{\mr a_n}$ as above such that $\phi({\mr a}\,;\V^{\gr z})$ is a positive Bolean combination of the sets $\phi({\mr a_i},;\V^{\gr z})$.
  Let $\theta({\mr a_1},\dots,{\mr a_n}\,;\V^{\gr z})=\phi({\mr a}\,;\V^{\gr z})$ be such a Boolean combination.
...
The stability $\phi({\mr x}\,;{\gr z})$ is sufficient for the theorem to hold, but the proof is simpler when the whole theory is stable.

\begin{proof}
  Let $\V\preceq\U$ be isomorphic to $\U$.
  There are ${\mr a_1},\dots,{\mr a_n}$ as above such that $\phi({\mr a}\,;\V^{\gr z})$ is a positive Boolean combination of the sets $\phi({\mr a_i},;\V^{\gr z})$.
  Let $\theta({\mr a_1},\dots,{\mr a_n}\,;\V^{\gr z})$ be such a Boolean combination.
  By Corollary~\ref{corol_stable_coheir_over_models}, we can choose $\V$ containing $A$ and such that every formula $\phi({\mr x})\in\tp({\mr a}/\V)$ is satisfied in every model containing $A$.
  It follows that for every $M$ containing $A$ and ${\gr b}\equiv_M{\gr b'}$ in $\V$ we have ${\gr b}\equiv_{M,a}{\gr b'}$.
  Then $\theta({\mr a_1},\dots,{\mr a_n}\,;\V^{\gr z})$ is definable over every model containing $A$.
  We conclude that there is a formula $\psi(z)\in L(\acl^\eq A)$ equivalent to $\theta({\mr a_1},\dots,{\mr a_n}\,;{\gr z})$ in $\V$ and, by elementarity, in $\U$.
\end{proof}

\begin{exercise}\label{ex_harrington}
  This is a (local) version of Corollary~\ref{corol_harrington}. 
  Prove that in Theorem~\ref{corol_harrington} it suffices to require that $\phi(x\,;z)$ is stable.\QED
\end{exercise}
...
\begin{exercise}\label{ex_Cb}
  ...
  \begin{exercise}
    Prove that if ${\mr a}\cnonfork_M{\gr b}\IFF{\gr b}\cnonfork_M{\mr a}$ holds for every $M,{\mr a},{\gr b}$ then $T$ is stable.\QED
  \end{exercise}
...
\begin{proposition}
  If every formula $\phi(x\,;y)\in L$, where $|y|=1$, is stable then $T$ is stable.
\end{proposition}
...
In this section $\U$ is a group; the group operations are in the language.

A set $\X\subseteq\U$ is generic if there are some finitely many $g_1,\dots,g_n\in\U$ such that the sets $g_i\X$ cover $\U$.
\end{document}
