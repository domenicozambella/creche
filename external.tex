...
Now let $N$ be a model that contains $A$ and such that $\<N,\grC\>\preceq\<\U,\grC\>$.
Assume also that $\<N,\grC\>$ is $|A|^+$-saturated, we can further require that all ${\gr b_i}\in N^{|{\gr z}|}$. Hence $\psi({\gr z})\in L(N)$.
\end{proof}
...
\begin{proof}
  Let $c$ be tuple that enumerates a $|M|^+$-saturated model containing $M$ and define $q({\gr z})={\gr z}\cnonfork_Mc$, see Definition~\ref{def_coheir_idepencence}.
  For every ${\gr b}\models q({\gr z})$ the type $\tp({\gr b}/c)$ extends to a global coheir, in particular, a type invariant over $M$. 
  ...
  By compactness, the required formula $\psi({\gr z})$ is the disjunction $\psi_{\gr b_1}({\gr z})\vee\dots\vee\psi_{\gr b_n}({\gr z})$ for some ${\gr b_1},\dots,{\gr b_n}\in q({\gr\U})\cap\grC$.
