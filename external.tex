...
\item[4.] there is no binary tree of formulas in $\big\{\phi({\mr x}\,;{\gr b}):{\gr b}\in\U^{|{\gr z}|}\big\}^\pm$, see Definition~\ref{def_tree_formulas}.
\end{itemize}
\end{proposition}
...
We say that $T$ is a \emph{stable theory\/} if every formula is stable. When $T$ is stable, Lascar invariance is equivalent to something much more manageable.

\begin{proposition}\label{prop_type_over_acl2} Let $T$ be stable and let $p({\mr x})\in S(\U)$ be Lascar invariant over $A$. Then for all formulas $\phi({\mr x}\,;{\gr z})\in L$ the sets ${\gr\D_{p,\phi}}$ are definable and, as such, belong to $\acl^\eq\!A$.
\end{proposition}
\begin{proof}
 The sets ${\gr\D_{p,\phi}}$ are externally definable therefore, by Theorem~\ref{thm_def_stable_formula}, definable (over $\U$). As $p({\mr x})$ is Lascar invariant over $A$, so are the sets ${\gr\D_{p,\phi}}$. Hence they are in $\acl^\eq\!A$. 
\end{proof}
% 
...
A type over some set $A$ is \emph{stationary\/} if it has a unique global extension that is Lascar invariant over $A$ (strictly speaking: \textit{non-forking over} $A$). The following proposition says that in a stable theory with elimination of imaginaries types over algebraically closed sets are stationary.

\begin{proposition}\label{prop_type_over_acl_stationary} Let $T$ be stable and let $q({\mr x})\in S\big(\acl^\eq\!A\big)$. Then there is a unique global type $p({\mr x})\in S(\U)$ that is consistent with $q({\mr x})$ and Lascar invariant over $A$.
\end{proposition}

\begin{proof}
Let $p_i({\mr x})\in S(\U)$, for $i=1,2$, be two global types Lascar invariant over $A$ and consistent with $q({\mr x})$. To prove that $p_1({\mr x}) =p_2({\mr x})$, it suffices to show that for every formula $\phi({\mr x}\,;{\gr z})\in L$

\ceq{\#\hfill {\gr\D_{p_1,\phi}}}{=}{{\gr\D_{p_2,\phi}}}

Note that, by Proposition~\ref{prop_type_over_acl2} both sets belong to  $\acl^\eq\!A$.

 Let $p'_i({\mr x})\in S(\U^\eq)$ denote the unique extension of $p_i({\mr x})$. Then $q({\mr x})\subseteq p'_i({\mr x})$. Clearly the formula $\A{\gr z}\,\big[\phi({\mr x}\,;{\gr z})\iff {\gr z}\in {\gr\D_{p_i,\phi}}\big]$ belong to $p'_i({\mr x})$. 

As $p_i({\mr x})$ are Lascar invariant over $A$, so are the sets ${\gr\D_{p_i,\phi}}$. As they are definable sets, they belong to $\acl^\eq(A)$. Then both formulas $\A{\gr z}\,\big[\phi({\mr x}\,;{\gr z})\iff {\gr z}\in {\gr\D_{p_i,\phi}}\big]$ belong to $q({\mr x})$ and $\#$ follows.
\end{proof}
...
\end{itemize}
\end{corollary}
\begin{proof}\ssf{1}$\IMP$\ssf{2}. This holds for every theory $T$ and it is left as an exercise to the reader.

\ssf{2}$\IMP$\ssf{1}. Assume $a\equivSh_A b$. By Proposition~\ref{prop_Shelah_strong_types} we have $a\equiv_{\acl^\eq\!A} b$. Let $q(x)=\tp(a/\acl^\eq\!A)=\tp(b/\acl^\eq\!A)$. Let $p(x)\in S(\U^\eq)$ be the unique global type that is Lascar invariant over $A$ and extends $q(x)$ which we obtain from by Proposition~\ref{prop_type_over_acl_stationary}. Fix some model $M$ containing $A$. Let $c=\<c_i:i<\omega\>$ be such that $c_i\models p_{M,a,b,c_{\restriction i}}$. Then $a,c$ and $b,c$ are $A$-indiscernible sequences, which proves \ssf{1}.
\end{proof}
...
\ssf{1}$\IMP$\ssf{2}. Assume$\neg$\ssf{2} and let $\<a_i : i<\omega\>$ be $A$-indiscernible, but not totally $A$-in\-dis\-cern\-ible. Then there is a formula $\phi(x, y)\in L(A)$ and some $i<j$ such that $\phi(a_i , a_j )\wedge\neg\phi(a_j , a_i) $. By indiscernibility  $\phi(a_i, a_j)$ holds if and only if $i \le j$. Hence $\phi(x, y)$ is not stable.
\end{proof}

\begin{exercise}
Prove that if every formula $\phi({\mr x}\,;{\gr z})\in L$ with $|{\mr x}|=1$ is stable then $T$ is stable.

Hint: by compactness, if all sets approximable by $\phi({\mr x}\,;y,{\gr z})$ are definable, so are the sets approximable by $\phi({\mr x},y\,;{\gr z})$.\QED
\end{exercise}
