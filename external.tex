...
Namely, $\<\psi_s:s\in 2^{<\omega}\>$  where $\psi_\0=\top$ and for $s\in 2^{<\omega}$ and $i\in 2$ we have $\psi_{s^\frown i}({\mr x})=\phi({\mr x}\,;{\gr b_s})^i$.
...
\begin{proof}
  ...
  Assume inductively that ${\gr b}:2^n\to{\gr\U^z}$ is such that for all $s\in2^n$ and $r\in2^{n+1}$ there are $2^\kappa$ sets ${\gr\D}$ externally definable by $\phi({\mr x}\,;{\gr z})$ and such that ${\gr b_{s\restriction i}}\in{\ge\D}\iff r(i)$.
...
\begin{theorem}\label{thm_ziegler}
   ...
   Then for every $A$ and ${\gr b}$ either $\phi({\mr x}\,;{\gr b})$ or $\neg\phi({\mr x}\,;{\gr b})$ is almost satisfied in $A$.
\end{theorem} 

\begin{proof}
Suppose not. Then there are two models containing $A$, the first omitting $\phi({\mr x}\,;{\gr b})$, the second omitting $\neg\phi({\mr x}\,;{\gr b})$.
It is easy to see that we can expand these models to two substructures $\V_0$ and $\V_1$ that are $A$-isomorphic to $\U$ and such that $\phi(\V_0^{\mr x}\,;{\gr b})=\neg\phi(\V_1^{\mr x}\,;{\gr b})=\0$.
In other words, $\phi(\V_i^{\mr x}\,;{\gr b})^i=\V_i^{\mr x}$, for $i=0,1$.

Rename $\V_\0=\U$ and ${\gr b_\0}={\gr b}$.
Inductively, given $s\in 2^n$ and ${\gr b_s}\in\V_s^{\mr x}$, pick two structures $\V_{s^\frown i}\preceq\V_s$ that are $A$-isomorphic copies of $V_s$ such that $\phi(\V_{s^\frown 0}^{\mr x}\,;{\gr b_s})=\neg\phi(\V_{s^\frown 1}^{\mr x}\,;{\gr b_s})=\0$.
Finally, let ${\gr b_{s^\frown i}}$ be the images of ${\gr b_s}$ under the isomorphisms that map $\V_s\to\V_{s^\frown i}$.

In this way we obtain a binary tree as above.
We only have to show that every branch is consistent.
This because $\phi(\V_{s^\frown i}\,;b_s)^i=\V_{s^\frown i}$ and $\V_s\preceq\V_r$ for every $r\subseteq s$.
\end{proof}
