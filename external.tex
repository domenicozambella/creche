...
\begin{proof}
  ...
  In fact, $\phi({\mr a_{r\restriction (n+1)}}\,;{\gr b_s})$ holds for all $s\in2^{<n}$.
  Then concistency follows by compactness.

  
  Let $\psi_\0({\mr x})=\top$.
  Then, for $s\in 2^{<\omega}$ and $i\in 2$, define $\psi_{s^\frown i}({\mr x})=\neg^i\phi({\mr x}\,;{\gr b_{s^\frown1}})$, where $\neg^i$ has been defined in Section~\ref{honest}.
  We claim that $\<\psi_s({\mr x}):s\in 2^{<\omega}\>$ is a binary tree. 
  We need to prove is that the branches are consistent.
  Then it suffices to show that $\psi_{r\restriction (n+1)}({\mr x})\in p_r$.

  First, suppose that $r(n)=0$. 
  Then $\psi_{r\restriction (n+1)}({\mr x})=\phi({\mr x}\,;{\gr b_{r\restriction n^\frown1}})$.
  This formula belongs to $p_r({\mr x})$ because $r<r{\restriction} n^\frown1$.
  Otherwise $r(n)=1$. 
  Then $\psi_{r\restriction (n+1)}({\mr x})=\neg\phi({\mr x}\,;{\gr b_{r\restriction n^\frown1}})$.
  This formula belongs to $p_r({\mr x})$ because $r{\restriction} n^\frown1<r$.
