...
We say that the formula $\phi({\mr x}\,;{\gr z})\in L$ has \emph{Vapknik-Chevronenkis dimension $n$\/} if this is the largest finite cardinatity of a set ${\mr A}\subseteq\U^{|{\mr x}|}$ such that $|S_\phi({\mr A})|=2^n$. If such $n$ exists, we say that we say that  $\phi({\mr x}\,;{\gr z})$ has finite VC-dimension. 
