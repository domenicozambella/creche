...
\begin{exercise}
   ...
   \item[2.] there a set ${\gr B}$ such that $\phi({\mr a_0}\,;{\gr B})\supset\dots\supset\phi({\mr a_n}\,;{\gr B})$ for some $\<{\mr a_i} : i<n\>$.\QED
   \end{itemize}
\end{exercise}
...
\begin{proof}
The theorem follows immediately from the three lemmas below.
\end{proof}

Below, in Theorem~\ref{thm_def_stable_formula2}, we proves the converse of the theorem above:  if every set approxiamted by $\phi({\mr x}\,;{\gr z})$ is definable then  $\phi({\mr x}\,;{\gr z})$ is stable.
...
We use binary trees to prove the following theorem. Its corollary is a major result for 

\begin{theorem}
   Let $\phi(x\,;z)\in L$ be stable.
   Then for every $A$ and $b$ either $\phi(x\,;b)$ or $\neg\phi(x\,;b)$ is almost satisfied in $A$.
\end{theorem} 

\begin{proof}
Suppose not. Then there are two models containing $A$, the first omitting $\phi(x\,;b)$, the second omitting $\neg\phi(x\,;b)$.
It is easy to see that we can expand these models to two substructures $\V_0$ and $\V_1$ that are $A$-isomorphic to $\U$ and such that $\phi(\V_0\,;b)=\neg\phi(\V_1\,;b)=\0$.
In other words, $\phi(\V_i\,;b)^i=\V_i$, for $i=0,1$.

Rename $\V_\0=\U$ and $b_\0=b$.
Inductively, given $s\in 2^n$ and $b_s\in\V_s$, pick two structures $\V_{s^\frown i}\subseteq\V_s$ that are $A$-isomorphic copies of $V_s$ such that $\phi(\V_{s^\frown 0}\,;b_s)=\neg\phi(\V_{s^\frown 1}\,;b_s)=\0$.
Finally, let $b_{s^\frown i}$ be the images of $b_s$ under the isomorphisms $\V_s\to\V_{s^\frown i}$.
...
\begin{corollary}
   Every consistent $q(x)\subseteq L(A)$ has a global extension $p(x)\in S(\U)$ that is Lascar invariant over $A$. 
\end{corollary}
...
\end{proposition}
\begin{proof}
 \ssf{2}$\IMP$\ssf{1} are clear (stability is not required).
...
\begin{theorem}
  If $T$ is stable then for every $M,{\mr a},{\gr b}$ 
...
\begin{theorem}[ (Pierre Simon)]
  If every $\phi(x\,;z)\in L(\U)$, where $|x|=|y|=1$ is stable then $T$ is stable.
\end{theorem} 
