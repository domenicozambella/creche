...
\end{theorem}
\begin{proof}
\ssf{2}$\IMP$\ssf{1}. Suppose the formula $\phi({\mr x}\,;{\gr z})$ is unstable. Let $\<{\gr z_s}:s\in2^{<\lambda}\>$ be a sequence variables of length $|{\gr z}|$. Let $p({\gr z_s}:s\in2^{<\lambda})$ be the type that says that $\big\<\phi({\mr x}\,;{\gr z_s}):i<2^{<\lambda}\big\>$ is a binary tree. As  $\phi({\mr x}\,;{\gr z})$ is unstable, $p$ is finitely consistent. As $\lambda$ is an arbitrary infinite cardinal, this contradicts \ssf{2}.
