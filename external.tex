...
We say that the formula $\phi({\mr x}\,;{\gr z})\in L$ has \emph{Vapknik-Chervonenkis dimension $n$\/} if this is the largest finite cardinatity of a set ${\gr B}\subseteq\U^{|{\gr z}|}$ such that $|S_\phi({\gr B})|=2^n$. If such $n$ does not exist, we say that we say that  $\phi({\mr x}\,;{\gr z})$ has \emph{infinite VC-dimension}. 
...
Let $\grC$ be a set approximable by a formula with finite VC-dimension and let $A$ be any (small) set of parameters. Then every global $A$-invariant type $p({\gr z})$ contains a formula $\psi({\gr z})$ such that either $\psi({\gr\U})\subseteq\grC$ or $\psi({\gr\U})\subseteq\neg\grC$. Moreover, for every $|A|^+$-saturated model $N$ containing $A$, we can require that $\psi({\gr z})\in L(N)$.
\end{lemma}
...
Now let $N$ be a model that contains $A$ and such that $\<N,\grC\>\preceq\<\U,\grC\>$. Assume also that $\<N,\grC\>$ is $|A|^+$-saturated, we can further require that all ${\gr b_i}\in N^{|{\gr z}|}$. Hence $\psi({\gr z})\in L(N)$.
\end{proof}

\begin{corollary}\label{corol_honest}
  Let $\grC$ be a set approximable by a formula with finite VC-dimension.
  Then every model $M$ there are a type $q({\gr z})\subseteq L(M)$ such that $M^{|{\gr z}|}\subseteq q({\gr\U})$ and a formula $\psi({\gr z})\in L(\U)$ such that $q({\gr\U})\cap\grC\subseteq\psi({\gr\U})\subseteq\grC$.
  In particular $\grC$ is approximable from below.
\end{corollary}

\begin{proof}
  Let $c$ be tuple that enumerates a $|M|^+$-saturated model containing $M$ and define $q({\gr z})={\gr z}\cnonfork_Mc$.
  By the lemma above, for every ${\gr b}\models q({\gr z})$ there is a formula $\psi_{\gr b}({\gr z})\in\tp({\gr b}/c)$ such that either $\psi_{\gr b}({\gr\U})\subseteq\grC$ or $\psi_{\gr b}({\gr\U})\subseteq\neg\grC$.
  Which one obtains depends on whether ${\gr b}\in\grC$ or ${\gr b}\notin\grC$.
  Hence

  \ceq{\hfill q({\gr\U})}{=}{\bigcup_{{\gr b}\models q({\gr z})}\psi_{\gr b}({\gr\U}).}

  Compactness, we obtain the required formula $\psi({\gr z})=\psi_{\gr b_1}({\gr z})\vee\dots\vee\psi_{\gr b_n}({\gr z})$ for some ${\gr b_1},\dots,{\gr b_n}\in q({\gr\U})\cap\grC$.

  Finally, note that $\grC$ is approximable from below by Lemma~\ref{lem_approx_nonunif}.
\end{proof}
...
Choose a positive integer $q_\psi$ for each formula $\psi\in L$. Let $\phi()\in L$ be given.
Then there are finitely may formulas $\psi_j()$, for $j<n$, such that for every set $\D$ that is approximable by $\phi()$ there is a $j<n$ such that for every $B\subseteq\D$ of cardinality $<q_{\psi_j}$ there is a $d\in\D$ such that $B\subseteq\psi_j()\subseteq\D$.
...
Binary trees of formulas have been introduced in Definition~\ref{def_tree_formulas}. Here we restrict to trees of a particular form. Namely, $\<\psi_s:s\in 2^{<\omega}\>$  where $\psi_\0=\top$ and for $s\in 2^{<\omega}$ and $i\in 2$ we have $\psi_{s^\frown i}({\mr x})=\neg^i\phi({\mr x}\,;{\gr b_s})$. The notation $\neg^i$ has been defined in Section~\ref{honest}. When a binary tree of this form exists, we say that $\phi({\mr x}\,;{\gr z})$ has the \emph{binary tree property}.
