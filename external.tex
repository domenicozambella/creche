...
% If $p({\mr x})\in S(\U)$ is a definable global type, the set 
% 
% \ceq{\hfill\emph{\Cb(p)}}{=}{{\dcl}^\eq\Big(\big\{{\gr\D_{p,\phi}}\ :\ \phi({\mr x}\,;{\gr z})\in L\big\}\Big)}
% 
% is called the \emph{canonical base\/} of $p({\mr x})$. Clearly (almost by definition), any $f\in\Aut(\U)$ fixes $p({\mr x})$ if an only if it fixes the set $\Cb(p)$ pointwise. 

When $T$ is stable, Lascar invariance is equivalent to a more manageable kind of invariance.
