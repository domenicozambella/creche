...
\begin{proof}
  ...
  \#\hfil$\V_{s\frown0}\subseteq\neg\phi(\V_{s\frown0}\,;{\gr b_s})$ \ \ and \ \  $\V_{s\frown1}\subseteq\neg\psi(\V_{s\frown1}\,;{\gr b_s})$.
  ...
  Let ${\mr a}\in\theta(\V_s)$ be arbitrary. We claim that ${\mr a}$ witnesses the concistency of  $\{\phi^{s_i}({\mr x}\,;{\gr b_{s\restriction i}})\ :\ i<n\}$.
  As $\V_s\preceq\V_{s\restriction i}$, we have that ${\mr a}\in\theta(\V_{s\restriction i})$.
  We prove that $\phi^{s_i}({\mr a}\,;{\gr b_{s\restriction i}})$.
  If $s_i=0$ then from \# we obtain $\V_{s\restriction(i+1)}\subseteq\phi^0(\V_{s\restriction(i+1)}\,;{\gr b_{s\restriction i}})$ and $\phi^0({\mr a}\,;{\gr b_{s\restriction i}})$ follows by elementarity.
  If $s_i=1$, from the second inclusion in \# we obtain $\V_{s\restriction(i+1)}\subseteq\neg\psi(\V_{s\restriction(i+1)}\,;{\gr b_{s\restriction i}})$.
  Therefore, from $\theta({\mr a})$ and \#\#, we obtain that $\V_{s\restriction(i+1)}\subseteq\phi(\V_{s\restriction(i+1)}\,;{\gr b_{s\restriction i}})$ and $\phi^1({\mr a}\,;{\gr b_{s\restriction i}})$ follows.
...
\begin{theorem}[ (Symmetry)]\label{thm_symmetry}
  ...
  \ceq{\hfill\phi({\mr M}\,;{\gr b})\neq\varnothing}{\IMP}{\phi({\mr a}\,;{\gr M})\neq\varnothing}
\end{theorem}
...
\begin{proof}
  Assume $\phi({\mr M}\,;{\gr b})\neq\varnothing$.
  Let $\V$ be as in Proposition~\ref{prop_saturate_heir}.
  By stability, there is $\psi({\gr z})\in L(\V)$ such that $\psi({\gr\V})=\phi({\mr a},{\gr\V})$.
  Recall that by Lemma~\ref{lem_coheir_independence} (non-splitting) if ${\gr b'}\equiv_M{\gr b''}$ are in $\V$ then ${\gr b'}\equiv_{M, {\mr a}}{\gr b''}$.
...
\end{proposition}
\begin{proof}
  \ssf{2}$\IMP$\ssf{3}$\IMP$\ssf{4} (and even \ssf{1}$\IMP$\ssf{3}) are clear -- stability is not required for these implications.

  \ssf{1}$\IMP$\ssf{2} First, for simplicity, we prove that any $\phi({\mr x}\,;{\gr b})\in p$ is satisfied in any $M\supseteq A$.
  Let $\psi({\gr z})\in L(\acl^\eq\!A)$ be a formula defining ${\gr\D_{p,\phi}}$.
  Let $M\supseteq A$ be given.
  Pick any ${\mr a}\models p({\mr x}){\restriction} M$ such that ${\gr b}\cnonfork{\mr a}$.
  As $\phi({\mr a}\,;{\gr M})=\psi({\gr M})$ and $\psi({\gr M})\neq\0$, from Theorem~\ref{thm_symmetry} we obtain $\phi({\mr M}\,;{\gr b})\neq\0$.
  The same argument extends easily to the conjunction of formulas of the form $\phi({\mr x}\,;{\gr b})$ and negation thereof (use that the complement and the Cartesian products of sets in $\acl^\eq\!A$ is again $\acl^\eq\!A$).
 
  \ssf{3}$\IMP$\ssf{1} By Theorem~\ref{thm_def_stable_formula2} the sets ${\gr\D_{p,\phi}}$ are externally definable, therefore they are definable (over $\U$).
...
\begin{proof}
  ...
  Suppose not.
  Then $\phi({\mr M}\,;{\gr b})\wedge\neg\theta({\gr b})\neq\varnothing$.
  As $\phi({\mr x}\,;{\gr z})\wedge\neg\theta({\gr z})$ is stable, from Theorem~\ref{thm_symmetry} we obtain  $\phi({\mr a}\,;{\gr M})\wedge\neg\theta({\gr M})\neq\varnothing$ which contradicts  $\theta({\gr M})=\phi({\mr a}\,;{\gr M)}$.
  Assuming $\neg\phi({\mr a}\,;{\gr b})$ with a symilar argument we obtain $\neg\psi({\mr a})$ and $\neg\theta({\gr b})$.
...
The actual statement is slightly more involved as it is a local version.

\begin{theorem}[ (Stationarity)]\label{thm_stationarity}
  Let $\phi({\mr x}\,;{\gr z})\in L$ be stable.
  Let $p_i({\mr x})\in S_\phi(\U)$, for $i=1,2$, be Lascar invariant over $A$.
  Let $q_i({\mr x})$ be the types containing the logical consequences in of $p_i({\mr x})$ that are in $L(\acl^\eq\!A)$.
  Then $q_1=q_2$ implies $p_1=p_2$.
\end{theorem}

\begin{proof}
  We prove that ${\gr\D_{p_1,\phi}}={\gr\D_{p_2,\phi}}$.
  Let $\psi_i({\gr z})\in L(\acl^\eq\!A)$ be the formulas that say ${\gr z}\in{\gr\D_{p_i,\phi}}$.
  Let ${\gr b}\in{\gr\U^z}$ be arbitrary.
  Let $M$ be any model containing $A$.
  Let ${\mr a_i}\models p_i({\mr x}){\restriction} M$ be such that ${\mr a_i}\cnonfork_M{\gr b}$.
  Then $\psi_i({\gr M})=\phi({\mr a_i}\,;{\gr M})$.
  Apply Theorem~\ref{thm_symmetry} to the formulas $\psi_i({\gr z})\iff\phi({\mr x}\,;{\gr z})$ and conclude that the formulas $\psi_i({\gr b})\iff\phi({\mr x}\,;{\gr b})$ holds for every tuple in $M^{\mr x}$.
  It follows that $\psi_1({\gr b})\iff\psi_2({\gr b})$.
\end{proof}
...
\begin{proof}
  ...
  We conclude that there is a formula $\psi({\mr z})\in L(\acl^\eq A)$ equivalent to $\theta({\mr a_1},\dots,{\mr a_n}\,;{\gr z})$ in $\V$ and, by elementarity, in $\U$.
\end{proof}
