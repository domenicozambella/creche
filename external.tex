...
Let  $\<{\mr a_i}:i<\omega\>$ be an indiscernible sequence with the same EM-type as $\<{\mr c_i}:i<\omega\>$. Then $\<{\mr a_i}:i<\omega\>$ satisfies $\E{\gr z}\,\psi_{I\restriction n}({\mr x_0},\dots,{\mr x_{n-1}}\,;{\gr z})$ for all $n$. By compactness there is a $b$ such that
...
\begin{proof}
  ...
  Again by compactness, for every $r\in2^\omega$ the type $\big\{\phi({\mr x}\,;{\gr b_s})\iff r<s : s\in2^{<\omega}\big\}$ is consistent.
  This shows that the infinite branches of the tree $\<\phi({\mr x}\,;{\gr b_s}):s\in2^{<\omega}\>$ are consistent.
  Hence $\phi({\mr x}\,;{\gr z})$ has the binary tree property.
...
\item[2.] $\big|S_{\phi}(A)\big|=|A|$ for all countable sets $A$;
