...
\begin{proof}\ssf{2}$\IFF$\ssf{3}$\IFF$\ssf{4}\quad By Lemma~\ref{lem_bin_tree}.

\ssf{3}$\IMP$\ssf{1}\quad
...
is a finitely consistent and can be extended to some $p({\mr x})\in S_{\phi}(\U)$. Any two such types are mutually inconsistent. As there are $2^\omega$ Dedekind's cut $\neg$\ssf{2}  follows.


\ssf{1}$\IMP$\ssf{4} Assume $\neg$\ssf{4}, then there is a binary tree which without loss of generality has the form $\<\phi_s({\mr x}\,;{\gr b_s}):s\in 2^{<\omega}\>$, where $\phi_s$ stands for $\phi$ if $s$ ends in $1$, for $\neg\phi$ if $s$ ends in $0$, for $\top$ when $s=\0$. 
...
The following is arguably one of the most important results that holds when the whole theory is stable. A type over some set $A$ is \emph{stationary\/} if it has a unique global extension that is Lascar invariant over $A$ (strictly speaking: \textit{non-forking over} $A$). The following proposition says that in a stable theory with elimination of imaginaries types over algebraically closed sets are stationary.
...
\end{itemize}
\end{corollary}
\begin{proof}\ssf{2}$\IMP$\ssf{1}\quad This clearly holds for every theory $T$.

\ssf{1}$\IMP$\ssf{2}\quad Exercise.%It suffices to show that if $a\equivSh_A b$ then $a\equiv_M b$ for some model $M$ containing $A$.
\end{proof}
