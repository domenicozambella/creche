...
\begin{proof}
  \ssf{1}$\IMP$\ssf{2}.
  By $\ssf{1}$ and compactness, there is a ladder sequence  $\<{\mr a_s}\,;{\gr b_s} : s\in2^{<\omega}\>$, where $2^{<\omega}$ is ordered lexicografically.
...
\begin{theorem}
   ...
   Then for every $A$ and $b$ either $\phi(x\,;b)$ or $\neg\phi(x\,;b)$ is almost satified in $A$.
\end{theorem} 

\begin{proof}
Suppose not. Then there are two models containing $A$, the first omitting $\phi(x\,;b)$, the second omitting $\neg\phi(x\,;b)$.
It is easy to see that we can expand these models to two subtructures $\V_0$ and $\V_1$ that are $A$-isomorphic to $\U$ and such that $\phi(\V_0\,;b)=\neg\phi(\V_1\,;b)=\0$.
In other words, $\phi(\V_i\,;b)^i=\V_i$, for $i=0,1$.

Rename $\V_\0=\U$ and $b_\0=b$.
Inductively, given $s\in 2^n$ and $b_s\in\V_s$, pick two strucures $\V_{s^\frown i}\subseteq\V_s$ that are $A$-isomorphic copies of $V_s$ such that $\phi(\V_{s^\frown 0}\,;b_s)=\neg\phi(\V_{s^\frown 1}\,;b_s)=\0$.
