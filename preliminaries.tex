...
\end{itemize}
Assume $M$ is a graph, what required in \ssf{b} is equivalent to saying that $M$ is a \textit{bipartite garaph}, or equivalently that it has \textit{chromatic number $2$\/} i.e., we can color the vertices with $2$ colors so that no two adjacent vertices share the same color.\QED
\end{exercise}
