...
\item a function that assign to every $f\in L_{\rm fun}$ and $r\in L_{\rm rel}$ non negative integers $n_f$ and $n_r$ that we call \emph{ariety\/}\index{arity} of the function, respectively relation, symbol. In words we say that $f$ is a \emph{$n_f$-ary} function symbol, similarly for $r$. We may say \emph{constant\/}\index{constant} for $0$-ary function symbol.
\end{itemize}
...
\begin{itemize}
\item[o.] if $r\in L_{\rm rel}\cup \{=\}$ and $t$ is a tuple obtained concatenating $n_r$ terms then $r\,t$ is a formula. Formulas of this form are called \emph{atomic}\index{formula!atomic};
...
Let $M$ be a structure in the signature of graphs as defined in Example~\ref{expl_graph1} (but not necessarily a graph). Write a sentence $\psi$ such that, 
\begin{itemize} 
\item[a.] $M\models\psi$ if and only if there is an $A\subseteq M$ such that $r^M\ \subseteq\ A\times\neg A$.
\end{itemize}
Remark: the property in \ssf{a} is an asymmetric version of that in \ssf{b} below. Assume $M$ is a graph and 
\begin{itemize} 
\item[b.] $M\models\psi$ if and only if there is an $A\subseteq M$ such that $r^M\ \subseteq\ (A\times \neg A)\;\cup\;(\neg A\times A)$.
\end{itemize}
What required in \ssf{b} is equivalent to saying that $M$ is a \textit{bipartite garaph}, or equivalently that it has \textit{chromatic number $2$\/} i.e., we can color the vertices with $2$ colors so that no two adjacent vertices share the same color.\QED
\end{exercise}
